...
def genesis_system(kinds: list, key_defaults: dict, cat_lines: str) -> str:
    ...
    return (
        ...
        "stage the agent declares (its declare_stages tool), run BEFORE the scorer. WHAT the agent may "
        "edit is SEPARATE (edit_surface minus protect; the file cmd runs is NOT auto-protected). So: if "
...
COMMAND_SYSTEM = (
    ...
    "architectures'); inject(operator one of draft/improve/merge, params, rationale) for ONE "
    "concrete experiment — there is NO `debug` operator: a failed node is repaired IN PLACE, so "
    "use reset(node_id, stage) to re-run it rather than asking for a new node to retry it; "
    "emit several inject steps for several experiments; deep_research to "
...
CHAT_SYSTEM = (
    ...
    "actually recommend an experiment, name the operator (improve/draft/merge — there is no `debug`, "
    "a failed node is repaired in place), give the exact "
