...
# The paid CONCEPT-LENS subsystem is `serve/concept_lens_service.py` (doc 25 SR-04): 21 helpers,
# two ledger folds, the provider worker and the four command bodies. What stays here is the four
# route decorators with the docstrings that are their OpenAPI descriptions, and the ConceptFrame
# materializer they hand in — that one is genuinely read-model machinery, since the unpaid
# `/concepts` GET is its other consumer. The three identity-shape constants come from there too:
# one definition, imported by the router rather than restated beside it.
from looplab.serve.concept_lens_service import (
    _RUN_GENERATION_RE, _SHA256_RE,
    durable_abandon_concept_lens,
    durable_abandon_recovered_concept_lens,
    durable_derive_concept_lens,
    durable_recover_concept_lens_receipt,
)
from looplab.serve.concept_frame import (build_core as _build_concept_core, lens_request as _concept_lens_request, project_frame as _project_concept_frame)
...
def _conversation_etag(run_id: str, node_id: int, run_generation: Optional[str],
    ...
    different selected representation and must never validate a cached payload from this one — and
    so is the same window ANCHORED somewhere else, which is why `before` is material here as well as
    inside the index's own window revision.
    """
...
def build_router(srv) -> APIRouter:
    ...
    async def derive_concept_lens(run_id: str, request: Request, response: Response):
        ...
        return await durable_derive_concept_lens(
            srv, run_id, request, response, materialize_core=_materialize_concept_core)
    ...
    def recover_concept_lens_receipt(run_id: str, response: Response,
        ...
        """
        return durable_recover_concept_lens_receipt(
            srv, run_id, response, expected_generation,
            materialize_core=_materialize_concept_core)
    ...
    async def abandon_recovered_concept_lens(run_id: str, request: Request,
        ...
        return await durable_abandon_recovered_concept_lens(
            srv, run_id, request, response, materialize_core=_materialize_concept_core)
    ...
    async def abandon_concept_lens(run_id: str, request: Request, response: Response):
        ...
        """
        return await durable_abandon_concept_lens(
            srv, run_id, request, response, materialize_core=_materialize_concept_core)
    ...
    def _node_trace(rd: Path, nid: int, cap: Optional[int] = None, *,
                    attempt: int = 0, before: Optional[str] = None) -> dict:
        try:
            ...
            tv = srv.node_trace_view(rd, nid, cap=cap, generation=attempt, before=before)
            return {"node_id": nid, "attempt": attempt, "before": before,
                    "schema": tv.get("schema"),
    ...
    def node_trace(run_id: str, nid: int, limit: int = Query(default=0, ge=0),
        ...
        `before` (a span id from `/nodes/{nid}/episodes`) MOVES that window instead of growing it: the
        same `limit` spans, ending at the anchored step. It is the half `limit` cannot do — a bigger
        limit is the same tail — and it is what makes an early repair on a long node reachable at
    ...
    def node_episodes_route(run_id: str, nid: int,
        ...
        This is the surface F6 was missing. The two per-node trace reads are bounded windows over the
        TAIL of `(node_id, generation)`, so on a heavily-repaired node most of the history is not
        reachable by asking for more — measured on `runs/rubert-dr-0804` node 1 (14,507 spans, 2,345
        inline repairs, 3 h 50 m): the 512-span default window shows its last 7.6 minutes and the
        4,096-span ceiling its last 59.3. The attempt picker does not help either, because inline
        repair does not open a lifecycle generation — every one of those spans is generation 0. So
        ...
        # Same restored contract as the two window routes: an EARLIER attempt is a historical read
    ...
    def node_conversation(run_id: str, nid: int, request: Request, response: Response,
        ...
        answer two different questions and neither substitutes for the other — `attempt` picks a
        lifecycle GENERATION (what `node_reset` creates), `before` a position INSIDE one, which is
        where the inline repairs live (2,345 of them in generation 0 of rubert-dr-0804 node 1).
        ...
        def source_snapshot():
            ...
            revision, count, conditional_ok = observed.node_window_snapshot(
                nid, span_cap, generation=read_attempt, before=anchor)
        ...
        candidate_etag = (_conversation_etag(
            ...
            trace_run_id=trace_scalars.run_id, task_id=trace_scalars.task_id, before=anchor)
        ...
        if not (conversation.get("projection") or {}).get("unavailable"):
            try:
                ...
                after_etag = _conversation_etag(
                    ...
                    task_id=after_trace_scalars.task_id, before=anchor)
    ...
    # The two agents a provenance graph has to be able to tell apart, and the roles that say WHICH
    # of them did what.  Until 2026-08-19 there was only the software one, and every activity was
    # associated with it unconditionally — so a node whose idea an OPERATOR wrote by branching from a
    # snapshot (`Node.forked_from`) was exported as the engine's own proposal, with the parent's
    # inherited `ll:rationale` sitting on it as if the Researcher had written it for this experiment.
    # That is the exact misreading `forked_from` exists to prevent, in the one surface whose entire
    # job is provenance.  Roles are attached only where there are two agents to distinguish, so an
    # ordinary node's association is byte-identical to what it always was.
    _PROV_OPERATOR_AGENT = "agent:operator"
    _PROV_ROLE_IDEA = "ll:idea-author"
    _PROV_ROLE_IMPLEMENTER = "ll:implementer"

    def _fork_attribution(n) -> Optional[dict]:
        """Split a branched node's idea into what the OPERATOR authored and what the PARENT gave it.

        Reads only the stamped receipt (`serve/control_validation.py::_normalize_fork_receipt`) plus
        the node's own idea, and refuses to guess past either:

          * `authored_fields` / `not_carried_fields` are the server's split of the diff and are what
            "the operator changed" means.  A receipt written before they existed carries neither, and
            this returns `ll:attribution: "legacy"` with the raw `changed_fields` and NO split rather
            than re-deriving one the parent's idea is needed for (and the parent may since have been
            reset out of the folded state).
          * `carried_over` — the fields the parent DID contribute — is the one part a reader can
            derive soundly: an idea field this node substantively carries that the receipt does not
            list as different from its base is, by construction, the base's own value.  It is the
            complement of `changed_fields`, not of `authored_fields`, so a field the branch left
            behind never reads as inherited.
        """
        receipt = n.forked_from
        if not isinstance(receipt, dict):
            return None
        changed = receipt.get("changed_fields")
        if not isinstance(changed, list) or not all(isinstance(f, str) for f in changed):
            # A hand-authored or corrupt receipt proves the branch happened and nothing else. Say the
            # lineage, claim no attribution — an empty split would read as "the operator changed
            # nothing", which is a stronger statement than "it was not recorded".
            return {"ll:attribution": "unrecorded"}
        carried_over = sorted(
            field for field in Idea.model_fields
            if field not in changed and idea_field_carried(getattr(n.idea, field, None)))
        authored = receipt.get("authored_fields")
        not_carried = receipt.get("not_carried_fields")
        out = {"ll:changed_fields": list(changed), "ll:carried_over_fields": carried_over}
        if isinstance(authored, list) and isinstance(not_carried, list):
            out["ll:attribution"] = "stamped"
            out["ll:authored_fields"] = list(authored)
            out["ll:not_carried_fields"] = list(not_carried)
        else:
    ...
    def prov(run_id: str):
        ...
        agents = {agent: {"prov:type": "prov:SoftwareAgent"}}
        for n in st.nodes.values():
            ...
            attribution = _fork_attribution(n)
            if attribution is not None:
                receipt = n.forked_from
                # The idea was authored by a person. Say so on the ACTIVITY (the authoring act), keep
                # the software agent on it as the IMPLEMENTER — the Developer really did write this
                # node's code — and give both associations a role, so neither reads as the whole
                # story. The vantage point travels too: `observed_seq` is the projection of the run
                # the operator was reading when they formed the branch.
                agents.setdefault(_PROV_OPERATOR_AGENT,
                                  {"prov:type": "prov:Person", "prov:label": "run operator"})
                waw[f"waw:{n.id}:{generation}"]["prov:role"] = _PROV_ROLE_IMPLEMENTER
                waw[f"waw:{n.id}:{generation}:operator"] = {
                    "prov:activity": a, "prov:agent": _PROV_OPERATOR_AGENT,
                    "prov:role": _PROV_ROLE_IDEA}
                act[a].update(attribution)
                act[a]["ll:idea_author"] = "operator"
                act[a]["ll:branched_from_node"] = receipt.get("node_id")
                act[a]["ll:branched_from_generation"] = receipt.get("generation")
                act[a]["ll:observed_seq"] = receipt.get("observed_seq")
                act[a]["ll:base_idea_digest"] = receipt.get("base_idea_digest")
