...
# 214 -> 215 on 2026-09-23: `repair_context_record` (review 2026-09-22, ENG2-14 / doc 50 ES2-05),
# beside `deep_repair` — whether the repair Developer's context states who stopped the stage, how a
# silent process ended, the key a metric-less run should have printed, and the node's repair
# history. A row on the same ground as the one above: it changes a PROMPT. Re-derived by
# INTERSECTION: the 214 previous keys plus exactly that one, no duplicate and none removed.
# 215 -> 216 on 2026-09-23: `node_budget_cue` (Q-3, the Researcher's context audit), beside
# `budget_aware` — whether each proposal is told the run's remaining node budget and where its
# plan's endgame reserve begins. A row on the same ground as the two above: it changes a PROMPT.
# Re-derived by INTERSECTION: the 215 previous keys plus exactly that one, no duplicate and none
# removed.
# 216 -> 217 on 2026-09-23: `propose_brief_fit` (Q-3, the Researcher's context audit), beside
# `node_budget_cue` — whether the proposal's working set keeps whole rows under its budget with a
# receipt, states each fact once and shows an untriaged failure's own last error line. A row on
...
# 219 -> 220 on 2026-09-23: `developer_phase_context` (review 2026-09-23, Q-2), beside
...
# 223 -> 224 on 2026-09-26: `card_verdict_support` (doc 67 67.1), beside `propose_brief_fit` —
# whether a supported card's row on the proposal board says what its verdict rests on (replicated,
# one measurement, inside the measured eval noise, or not held by its confirmation seeds). A row on
# the same PROMPT ground as the entries above. Re-derived by INTERSECTION: the 223 previous keys
# plus exactly that one, none removed.
# 224 -> 225 on 2026-09-26: `ablation_probe_hint` (doc 67 67.4), beside `ablate_code_blocks` —
...
# 226 -> 227 on 2026-09-26: `brief_node_frontier` (doc 67 67.9), beside `brief_mixed_comparability`
...
SETTINGS_UI_SCHEMA_KEYSET_REVISION = "5849f8e84516c830ce92f3b60b580567c01bb738770c72e9e14155c5e0e271d7"
