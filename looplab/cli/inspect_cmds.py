...
from looplab.cli.run_report import (echo_card_and_build_tables, echo_containments,
                                    echo_edit_types, echo_reconciliation, echo_section, minutes,
...
def timings(run_dir: Path = typer.Argument(...),
    ...
    for sp in spans:
        ...
        if isinstance(parent, str) and parent:   # a non-string id is damage, not a parent link
            child_sum[parent] += span_seconds(sp.get("duration_s"))
    ...
    no_start = 0
    for sp in spans:
        raw = span_seconds(sp.get("duration_s"))
        dur = raw
        ...
        start = span_seconds(sp.get("start"))
        if start:
        ...
        if nid is None:
            if node is None:
                cell = run_level[span_category(sp)]
                cell[0] += dur
                cell[1] += 1
            continue
        if node is not None and nid != node:
            continue
        cell = per_node[nid][span_category(sp)]
        cell[0] += dur
        cell[1] += 1
...
def edit_types(run_dir: Path = typer.Argument(..., help=_RUN_DIR_HINT)):
    ...
    nine types, while ~30 % of ADDED lines were lines the same lineage had already deleted — a share
    that ROSE over the run in 118 of the 121. LoopLab could not tell either fact about itself: the
    diff was there (`tools/node_diff.py`), the classification was not.

    Read-only, no model, no write. Every pair is a node against its FIRST parent (the lineage a
    re-introduction is defined over), and the improvement column is direction-aware, taken from the
    run's own `run_started`.
    """
    store = _require_run_dir(run_dir)
    _echo_log_integrity(store, run_dir)
