...
| `data:` / `references:` mount SOURCES | read (readwrite when `edit: true`) | the sanctioned read channel — a mount source may legally live inside the editable tree, and `edit:true` is the operator saying in-place writes are allowed. A FILE source stays that file: the kernel rule carries only the rights a file can hold (`landlock.py::rule_access`), never widened to its directory, whose siblings — and, inside the editable tree, the tree itself — were not declared (review 2026-09-22, RTA-08) |
