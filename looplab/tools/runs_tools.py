    ...
    def specs(self) -> list[dict]:
        return [
            fn_spec("propose_run",
                ...
                "The agent BUILDS whatever code the experiment needs (e.g. the training pipeline) INSIDE "
                "`repo`; the operator's `cmd` is the AUTHORITATIVE, non-rewritable way it's scored. Say in "
                "the goal that each node must actually TRAIN a fresh model and score THAT model — never "
