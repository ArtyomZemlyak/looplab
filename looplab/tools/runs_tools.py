    ...
    def specs(self) -> list[dict]:
        return [
            fn_spec("propose_run",
                ...
                "([\"python\",\"test.py\"]) or an object {command:[...], metric:{reader,key}, timeout}. "
                "`metric.reader` is one of stdout_json / stdout_regex / file_json / file_regex — HOW to "
                "read the printed metric (NOT the direction; the objective name is `key`). Set "
                ...
                "TRAINING vs SCORING — the important distinction: `cmd` is the SCORING step (it reads the "
                "metric), NOT the trainer. Training is a SEPARATE preceding stage the AGENT declares at run "
                "time (via its `declare_stages` tool). Three cases:\n"
                "  (a) NO `cmd` given → the agent is free to build the whole train+score entrypoint itself; "
                "just say in the goal how the metric is printed and it's parsed normally.\n"
                "  (b) `cmd` is an EXISTING scorer in the repo (e.g. the framework's `test.py`) → give that "
                "command, PROTECT it (`protect:[\"test.py\"]`) so the agent can't tamper with scoring, and "
                "say in the goal that the agent must add a TRAIN step before it (the engine runs the agent's "
                "train stage, then your protected `cmd` scores the freshly-trained model).\n"
                "  (c) `cmd` names an entrypoint the agent must BUILD (doesn't exist yet) → give it but do "
                "NOT protect it (a protected file can't be created), and tell the agent it must TRAIN a fresh "
                "model then score it.\n"
