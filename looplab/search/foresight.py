...
class ForesightPanelResearcher(WrapsResearcher):
    ...
    def _prioritize_board(self, state, parent) -> None:
        """FOREAGENT prioritization of distinct OPEN research beliefs. Untested beliefs arrive in batches
        from many sources — deep-research `recommended_directions`, a human "+ Add", the strategist —
        and all land as `open` work-item cards. ``open_research_beliefs`` collapses cards sharing one
        ``belief_id`` before `_state_brief` shows the top five; without ranking, their order is arbitrary
        insertion order, so which gets tested (and which is silently
        ...
        # Rank the open Card board as distinct BELIEFS (peer review): `open_research_beliefs` collapses
