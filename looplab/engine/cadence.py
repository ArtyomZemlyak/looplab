...
def at_creation_boundary(pending: int, *, while_evaluating: bool) -> bool:
    ...
    ALL FIVE now call this, and the fifth calls it CONDITIONALLY — which is a refusal in one
    configuration and a fix in the other, so say so here rather than let the next reader assume
    either. The 2026-08-19 corpus audit is what closed the middle two: `lessons_distilled` and
    `report_generated (trigger=cadence)` are 19/1/0/1/0/0 and 26/1/0/1/0/0 over dense-retrieval /
    v6 / v7 / v8 / v9 / the live `e5small-dr-unified-v2` — zero in exactly the three runs with no
    quiescent prefix, on the same configuration as the runs where they fire.
    ...
    bought to reach work already being done. Under `concurrent_research=false` that half does not
    exist at all (`_spawn_research` returns at its first line), the serial gate is the run's ONLY
    research path, and the same table's zero-quiescent-prefix column says what that meant: no
    scheduled think, ever. One path needs no agreement between two, so the gate reaches this
    boundary there — `docs/BACKLOG.md` F1i-b, closed 2026-09-08.
