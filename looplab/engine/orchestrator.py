...
class Engine(ConfirmPhaseMixin, AblationMixin, NoveltyGateMixin, StrategyCadenceMixin,
    ...
    async def _run_with_llm_broker(self) -> RunState:
        ...
        while True:
            ...
            if creates:
                ...
                _signal, state, _created_no_terminal = await self._handle_create_actions(
    ...
    async def _handle_create_actions(self, creates, state, *, created_no_terminal,
                                     decision_seq, max_es, max_s, start):
        ...
        Returns `(signal, state, created_no_terminal)` where signal is `"break"` or `"continue"`.
        `state` is returned because this branch re-folds it and the loop reads the newer value.

        `created_no_terminal` round-trips for a sharper reason: it is the runaway counter, seeded
        before the loop and reset only when a node reaches terminal, and this branch both increments
        and decrements it. Passed by value it would lose every mutation, the
        "node creation not converging" guard would never trip, and a spinning run would loop forever
        instead of finishing. It is rebound to the original local name below so the lifted lines stay
        byte-identical.
        """
        ...
        if _created_no_terminal > max(self.policy.max_nodes, 4) * 3 + 50:
            if self._finish_with_report_if_quiescent(state, {
                ...
                return "break", state, _created_no_terminal
            return "continue", state, _created_no_terminal
