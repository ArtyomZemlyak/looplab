...
def make_app(run_root: str | os.PathLike) -> "FastAPI":
    ...
    async def genesis(request: Request):
        ...
        sys_prompt = (
            ...
            "- reply: a substantive 2-3 sentence message (NEVER a single word or bare fragment) that "
            "states the plan in plain language — name the task you chose, the model and node budget if "
            "the goal implied them, and what the user should check or do next (e.g. confirm the data "
            "path, or just hit Start). Be concrete and warm, not terse.\n"
            "- rationale: one terse line on why.\n"
            "PREFER A COMPLETE, LAUNCHABLE PLAN: author a concrete `task` (or pick a task_file) with all "
            "the fields its kind needs whenever the goal gives you enough to make a sensible choice — a "
            "reasonable best guess the user can edit beats a blank card they can't launch. Only when the "
            "goal is genuinely too ambiguous to pick a KIND at all, still invent a sensible run_id, leave "
            "task / task_file empty, and ask ONE specific clarifying question in `reply` (name the options "
