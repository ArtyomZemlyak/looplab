...
(`adapters/reference_score.py`) — the engine pins it on `run_started`, and the run row carries
`headroom(best, reference, direction)`.
...
`target_not_better` says so), or when there is no best.
...
def headroom(best, reference, direction: str) -> Optional[dict]:
    ...
    out = {"baseline": reference["baseline"], "target": target_mark,
           "gain": sign * (best - baseline), "gap_closed": None}
    if target_mark is not None:
        span = target_mark["value"] - baseline
        if sign * span > 0:
            out["gap_closed"] = (best - baseline) / span
        else:
...
def headroom_line(room: dict) -> str:
    ...
    line = (f"headroom: {room['gain']:+.6g} over the baseline {baseline['value']:.6g} "
            f"({baseline['source']})")
