...
class Settings(BaseSettings):
    ...
    # the Researcher (and the Developer, when `developer_run_tools` is on) get list_all_runs +
    # read_run_code + read_run_experiment so they can read the code + result of ANY past experiment
    # anywhere and reuse an approach. Broader than `cross_run_tools` (same-task only); the agent decides
    # when a foreign run is relevant. Advisory; never changes best-selection. Needs the run's own dir
    # wired through (no-op for unit-built roles).
