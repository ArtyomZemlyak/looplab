...
class Settings(BaseSettings):
    ...
    # run's runs root; the web start route admits only a run of its own runs root) and optionally a
    # node other than its champion. On a FRESH run directory only, `looplab run` (the web start route
    # spawns it too) imports that node — code, files, idea — as the run's first experiment: an
    ...
    # launch fact), so no EngineOptions field.
    seed_from_run: str = ""
