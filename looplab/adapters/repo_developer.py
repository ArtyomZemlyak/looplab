...
_REPO_DEV_SYSTEM_BODY = (
    ...
    "with the required key). CRITICAL: the eval command runs `<entrypoint>.py`, so THAT FILE MUST EXIST "
    "in the workspace after your edits — a fresh node starts WITHOUT it. If your idea only tweaks the "
    "training script (e.g. train.py / loss.py), you STILL must (re)write the entrypoint that orchestrates "
    "train→test and prints the metric — editing only train.py leaves the eval with 'no such file: "
    ...
    "an undertrained / parent model and freezes the metric. Train UNCONDITIONALLY into a fresh/cleared "
    "checkpoint dir every run, and ensure the FULL schedule completes (all requested epochs — the best-"
    "val checkpoint of a full run, not an epoch-0/1 checkpoint from a training that never finished). ALSO include any related "
    ...
    "MULTI-STAGE PIPELINE (recommended for train-then-eval work): instead of one opaque entrypoint, you "
    "MAY declare the eval as ordered stages by writing `looplab_stages.json` in the workdir root: "
    "{\"stages\": [{\"name\": \"data_prep\", \"command\": [\"python\", \"prep.py\"]}, "
    "{\"name\": \"train\", \"command\": [\"python\", \"train.py\"], \"timeout\": 7200, \"check\": true}, "
    "{\"name\": \"eval\", \"command\": [\"python\", \"test.py\"]}]}. Stages run in order in the SAME "
    "workdir (artifacts persist: train writes a checkpoint, eval reads it); each shows its own pass/fail, "
    "the LAST stage must print the metric JSON line, and a crash is pinpointed to its stage — so a failed "
    "`eval` can be re-run WITHOUT re-`train`. Add \"check\": true to a stage to have it verified before "
    "the next runs. Prefer this whenever the experiment TRAINS (so a run that skips training is obvious). "
    "Without the file, your single entrypoint is used exactly as before.\n\n"
