    ...
    def _full(sid) -> list:
        ...
        while True:
            ...
            if "input_carry" not in a or not isinstance(cur, list):
                base = cur if isinstance(cur, list) else []; break   # old log / non-list → input IS full
            frm = a.get("input_from")
            if frm is None:                            # self-contained base: its `input` is the full ctx
                memo[cur_sid] = list(cur); partial[cur_sid] = False; base = memo[cur_sid]; break
        ...
        for csid, carry, delta in reversed(chain):     # apply deltas base→leaf, memoizing every level
            ...
            memo[csid] = full; partial[csid] = broke
        if sid not in memo:
            memo[sid] = full; partial[sid] = broke
        return memo[sid]
