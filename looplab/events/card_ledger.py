...
def _apply_card_drops(st: RunState, ledger: _CardLedger, aliases: _CardAliases) -> dict[str, dict]:
    ...
    for d in st.cards_dropped:
        ...
        # REVIEW (mega-review 2026-08-13): `canon_at` makes the drop's resolution depend on the
        ...
        # position?") this reader turned that answer from no to yes. The types.py comment on that
        # registry was not updated, and the splice-neutrality proof does not model a racing drop —
        # both should move with any further use of `canon_at`.
