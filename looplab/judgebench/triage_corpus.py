...
Six preserved runs of ONE task family (ESCI dense retrieval, two backbones), ONE operator, one box.
...
TORCH_OOM_MARKERS = (
    "torch.OutOfMemoryError", "torch.cuda.OutOfMemoryError", "OutOfMemoryError",
    "CUDA out of memory", "HIP out of memory", "XPU out of memory",
)
...
CORPUS_LIMITS = (
    "SCOPE: six preserved runs of ONE task family (ESCI dense retrieval, two backbones), ONE "
...
def _stage_logs(run_dir: Path) -> dict:
    """`(node_id, stage) -> (path, mtime, size)` for every stage log left in the workspaces."""
    found: dict = {}
    nodes = run_dir / "nodes"
    if not nodes.is_dir():
        return found
    for node in nodes.iterdir():
        ...
        for log in node.glob("*.log"):
            try:
                ...
                continue
            found[(node_id, log.stem)] = (str(log), stat.st_mtime, stat.st_size)
    return found
...
def extract_run(run_dir) -> list:
    ...
    for failure in failures:
        ...
        if stage and (node, stage) in logs:
            path, mtime, size = logs[(node, stage)]
            paired = any(abs((ts or 0.0) - mtime) <= LOG_PAIR_TOLERANCE_S
