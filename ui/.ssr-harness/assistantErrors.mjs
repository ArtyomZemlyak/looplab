...
function infoFor(kind, status = null) {
	if (kind === "rate_limit") return {
		...
		retryable: true,
		technical: `HTTP ${status || 429} · rate_limit`
	};
	if (kind === "credentials") return {
		...
		retryable: false,
		technical: status ? `HTTP ${status} · credentials` : "credentials"
	};
	if (kind === "unavailable") return {
		...
		retryable: true,
		technical: status ? `HTTP ${status} · unavailable` : "provider_unavailable"
	};
	return {
		kind: "provider_error",
		status,
		...
		retryable: true,
		technical: status ? `HTTP ${status} · provider_error` : "provider_error"
	};
}
