...
export function virtualRange(layout, scrollTop, viewportHeight, overscan = DEFAULT_TIMELINE_OVERSCAN) {
	...
	return {
		...
		end: Math.min(layout.count, last + pad + 1)
	};
}
...
export const tailScrollTop = (totalHeight, viewportHeight) => Math.max(0, (Number(totalHeight) || 0) - Math.max(0, Number(viewportHeight) || 0));
export function timelineBottomGap(scrollHeight, scrollTop, viewportHeight) {
	return Math.max(0, (Number(scrollHeight) || 0) - Math.max(0, Number(scrollTop) || 0) - Math.max(0, Number(viewportHeight) || 0));
}
export function timelineViewportAtTail(scrollHeight, scrollTop, viewportHeight, tolerance = 2) {
	return timelineBottomGap(scrollHeight, scrollTop, viewportHeight) <= Math.max(0, Number(tolerance) || 0);
}
