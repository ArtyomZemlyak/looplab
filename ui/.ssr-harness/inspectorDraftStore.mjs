...
export function useInspectorDraftField(store, scope, field, initial, { disposable = false } = {}) {
	...
	const subscribe = useCallback((listener) => store.subscribe(scope, listener), [scope, store]);
	const snapshot = useCallback(() => store.readField(scope, field, fallback), [
		fallback,
		field,
		scope,
		store
	...
	const setValue = useCallback((update) => store.updateField(scope, field, update, fallback, { disposable }), [
		disposable,
		fallback,
		field,
		scope,
