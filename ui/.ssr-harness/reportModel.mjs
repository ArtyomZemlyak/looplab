...
const { totalChars: TOTAL_CHARS, headlineChars: HEADLINE_CHARS, paragraphChars: PARAGRAPH_CHARS, listItems: MAX_LIST_ITEMS, listItemChars: LIST_ITEM_CHARS, triggerChars: TRIGGER_CHARS } = REPORT_LIMITS;
...
export function normalizeReportNodeDetail(value, { nodeId, historySeq = null, expectedGeneration = null } = {}) {
	...
	if (historySeq != null && (value.historical_seq !== historySeq || value.historical_generation !== expectedGeneration)) return null;
...
export function normalizeRunReport(value) {
	...
	result.trigger = boundedAdvisoryText(field(value, "trigger"), TRIGGER_CHARS, { remaining: TRIGGER_CHARS }, true);
	...
	result.published_at = typeof publishedAt === "number" && Number.isFinite(publishedAt) && publishedAt > 0 && publishedAt <= 253402300799 ? publishedAt : null;
	return result;
}
...
export function reportNarrativeCoverage(report, currentNodeCount) {
	const total = Number.isSafeInteger(currentNodeCount) && currentNodeCount >= 0 ? currentNodeCount : null;
	const atNode = report && Number.isSafeInteger(report.at_node) && report.at_node >= 0 ? report.at_node : null;
	const base = {
		status: report ? "unknown" : "absent",
		atNode,
		currentNodeCount: total,
		staleBy: null
	};
	if (!report || atNode == null || total == null) return base;
	if (atNode < total) return {
		...base,
		status: "stale",
		staleBy: total - atNode
	};
	if (atNode > total) return {
		...base,
		status: "inconsistent"
...
export function reportCoverageText(coverage) {
	...
	return coverage.currentNodeCount == null ? "Node coverage unknown." : `Node coverage unknown for ${coverage.currentNodeCount} current nodes.`;
}
