...
export function scopeReportPublicationUnconfirmed(value) {
	return value?.exists === false && value?.quarantined === true && value?.code === SCOPE_PUBLICATION_UNCONFIRMED;
}
...
export function scopeReportAuthority(value) {
	...
	const authoritative = value?.exists === true && value?.authoritative === true && content?.schema === SCOPE_CONTENT_SCHEMA;
	const freshness = !authoritative ? "unknown" : value?.stale === false ? "fresh" : value?.stale === true ? "stale" : "unknown";
...
export function scopeReportGenerationError(error) {
	...
	if (error?.status === 413 || code === "scope_report_too_large" || code === "scope_report_source_too_large") {
...
export function scopeObservationRows(group) {
	...
	if (!rows || rows.length > 64 || group?.contract_authority !== "declared" || group?.outcome_policy !== "observations-only-v1" || group?.winner !== null || !Array.isArray(group?.tied_winners) || group.tied_winners.length !== 0 || !rows.every((row) => row?.authority === "declared")) return null;
	return rows;
}
// JSON tuple encoding avoids identity collisions between values containing separators.
export const scopeReportKey = (scope) => JSON.stringify([String(scope?.type ?? ""), String(scope?.id ?? "")]);
