...
// unchanged.
import { assertRunMutationAllowed } from "./runMode.mjs";
import { splitRouteHash } from "./runRouteState.mjs";
...
export const runApiPath = (runId, suffix = "") => `/api/runs/${encodeURIComponent(String(runId))}${suffix}`;
...
export const runNodeApiPath = (runId, nodeId, suffix = "") => runApiPath(runId, `/nodes/${encodeURIComponent(String(nodeId))}${suffix}`);
...
function ownerToken() {
	...
	try {
		return sessionStorage.getItem(OWNER_TOKEN_KEY) || volatileOwnerToken;
	} catch {
...
export const COMMAND_FAILED = new Set([
	"failed",
	"rejected",
	"timed_out"
]);
...
const COMMAND_REQUEST_TIMEOUT_MS = 8e3;
...
const ASSISTANT_TRANSPORT_ACTIONS = new Set([
	"stop",
	"finalize",
	"resume",
	"pause",
...
const LAUNCH_PREFLIGHT_TIMEOUT_MS = 12e3;
const LAUNCH_SUBMISSION_TIMEOUT_MS = 12e3;
const LAUNCH_STATUS_TIMEOUT_MS = 5e3;
const MAX_LAUNCH_REQUEST_TIMEOUT_MS = 6e4;
...
const MAX_OBSERVED_RUN_GENERATIONS = 512;
...
const TRANSPORT_EVENT_BY_ACTION = Object.freeze({
	stop: "pause",
	finalize: "run_abort",
	resume: "resume"
});
...
const STORED_ERROR_KEYS = new Set([
	"code",
	"retryable",
	"existing_command_id"
]);
const STORED_ERROR_CODES = new Set([
	...
	"command_in_progress",
	"retry_existing_command",
	"finalize_payload_conflict",
	"finalize_in_progress",
	"engine_finishing",
	"engine_start_uncertain",
	"spawn_claim_confirmation_required",
	"engine_failed",
	"spawn_failed",
	"command_worker_failed",
	"approval_not_requested",
	"ratification_not_requested",
	"invalid_transition",
...
const storedErrorCopy = (code) => {
	const title = code === "engine_failed" ? "The run engine reported a failure" : code.replaceAll("_", " ");
...
const ASSISTANT_ENVELOPE_KEYS = new Set([
	...RUN_ENVELOPE_KEYS,
	"arg",
	"nodeGeneration"
]);
...
const LAUNCH_TRANSPORT_KEYS = new Set([
	"identity",
	"runId",
	"idempotencyKey",
	"updatedAt"
]);
export const commandEventForAction = (action, source = "assistant") => (source === "dock" ? TRANSPORT_EVENT_BY_ACTION : ASSISTANT_EVENT_BY_ACTION)[action] || null;
...
const safeIdentityText = (value) => typeof value === "string" && value.length > 0 && value.length <= 200 && !/[\u0000-\u001f\u007f]/.test(value);
...
export const commandRecordMatchesAction = (record, action, source = "assistant") => storedRecord(record, action, source) != null;
const protocolTransport = (runId, source, payload = null) => {
	...
	const expectedGeneration = validRunGeneration(payload?.expectedGeneration) ? String(payload.expectedGeneration) : "";
	return {
		runId: String(runId),
		action,
		arg: null,
...
export function commandCanRetry(record) {
	return !!record?.id && (record.status === "failed" || record.status === "timed_out") && record?.error?.retryable === true;
}
...
const safeLaunchText = (value, max) => typeof value === "string" && value.length > 0 && value.length <= max && !/[\u0000-\u001f\u007f]/.test(value);
const parsedLaunchTransport = (raw, identity) => {
	...
	if (!payload || typeof payload !== "object" || Array.isArray(payload) || !hasOnlyKeys(payload, LAUNCH_TRANSPORT_KEYS) || payload.identity !== String(identity) || !safeLaunchText(payload.runId, 255) || !safeLaunchText(payload.idempotencyKey, 200) || !Number.isFinite(payload.updatedAt)) {
		return { invalid: true };
	}
	return payload;
};
const launchTransportMatches = (record, identity, state) => !!record && !record.invalid && record.identity === String(identity) && record.runId === String(state?.runId || "") && record.idempotencyKey === String(state?.idempotencyKey || "");
const notifyLaunchTransports = () => {
	if (typeof window === "undefined" || typeof window.dispatchEvent !== "function" || typeof CustomEvent === "undefined") return;
	try {
		window.dispatchEvent(new CustomEvent(LAUNCH_TRANSPORT_EVENT));
	} catch {}
};
...
export function saveLaunchTransport(identity, state, storage = undefined) {
	...
	if (!target || !safeLaunchText(identity, 300) || !safeLaunchText(state?.runId, 255) || !safeLaunchText(state?.idempotencyKey, 200)) return false;
	const payload = {
	...
	try {
		...
		const saved = launchTransportMatches(parsedLaunchTransport(target.getItem(key), identity), identity, payload);
...
export function clearLaunchTransport(identity, storage = undefined, expectedState = null) {
	...
	try {
		const key = launchTransportKey(identity);
		const currentRaw = target.getItem(key);
		if (expectedState && currentRaw != null && !launchTransportMatches(parsedLaunchTransport(currentRaw, identity), identity, expectedState)) return false;
...
export function clearDamagedLaunchTransport(storageKey, storage = undefined) {
	const target = transportStorage(storage);
	if (!target || typeof storageKey !== "string" || !storageKey.startsWith(LAUNCH_TRANSPORT_PREFIX)) return false;
	try {
...
const notifyRunCommandLock = (runId) => {
	...
	try {
		window.dispatchEvent(new CustomEvent(RUN_COMMAND_LOCK_EVENT, { detail: { runId: String(runId) } }));
	} catch {}
};
const commandStatePending = (state) => !!state && (state.statusUnavailable || state.retrying || state.checking || !state.record || state.record.status === "submitting" || COMMAND_PENDING.has(state.record.status));
const compatibleCommandLock = (current, next) => !current || current.source === next.source && current.idempotencyKey === next.idempotencyKey && current.action === next.action && current.expectedGeneration === next.expectedGeneration && (!current.commandId || !next.commandId || current.commandId === next.commandId);
...
export function saveRunCommandLock(runId, state, storage = undefined) {
	...
	if (!target || !runId || source !== "dock" && source !== "assistant" || !safeIdentityText(action) || !safeIdentityText(idempotencyKey) || !validRunGeneration(expectedGeneration) || commandId && !COMMAND_ID_RE.test(commandId) || !STORED_COMMAND_STATUSES.has(status)) return false;
	const payload = {
		runId: String(runId),
		source,
		action,
...
export function loadRunCommandLock(runId, storage = undefined) {
	...
	try {
		...
		if (!payload || typeof payload !== "object" || Array.isArray(payload) || !hasOnlyKeys(payload, LOCK_KEYS) || payload.runId !== String(runId) || payload.source !== "dock" && payload.source !== "assistant" || !validRunGeneration(payload.expectedGeneration) || !safeIdentityText(payload.action) || !safeIdentityText(payload.idempotencyKey) || payload.commandId && !COMMAND_ID_RE.test(payload.commandId) || !STORED_COMMAND_STATUSES.has(payload.status) || typeof payload.statusUnavailable !== "boolean" || !Number.isFinite(payload.updatedAt)) return null;
		return payload;
	} catch {
		return null;
	}
...
export function clearRunCommandLock(runId, expected = {}, storage = undefined) {
	...
	try {
		const current = loadRunCommandLock(runId, target);
		if (current && (expected.source && current.source !== expected.source || expected.idempotencyKey && current.idempotencyKey !== expected.idempotencyKey || expected.action && current.action !== expected.action || expected.expectedGeneration && current.expectedGeneration !== expected.expectedGeneration || current.commandId && current.commandId !== String(expected.commandId || ""))) return false;
...
const commandTransportActions = (source) => source === "dock" ? TRANSPORT_ACTIONS : ASSISTANT_TRANSPORT_ACTIONS;
const commandTransportKey = (source, runId) => source === "dock" ? transportStorageKey(runId) : assistantTransportStorageKey(runId);
const commandEnvelopeKeys = (source) => source === "dock" ? RUN_ENVELOPE_KEYS : ASSISTANT_ENVELOPE_KEYS;
...
function saveCommandTransport(source, runId, state, storage) {
	const target = transportStorage(storage);
	if (!target || !runId || !commandTransportActions(source).has(state?.action) || !state?.idempotencyKey) return false;
	const record = storedRecord(state.record, state.action, source);
	if (!record) return false;
	const explicitId = String(state.commandId || "");
	if (explicitId && !COMMAND_ID_RE.test(explicitId) || explicitId && record.id && explicitId !== record.id) return false;
	...
	if (source === "assistant" && state.action === "approve") {
		...
		if (arg != null && (!Number.isSafeInteger(arg) || arg < 0) || nodeGeneration != null && (!Number.isSafeInteger(nodeGeneration) || nodeGeneration < 0)) return false;
...
function loadCommandTransport(source, runId, storage) {
	...
	try {
		...
		if (!payload || typeof payload !== "object" || Array.isArray(payload) || !hasOnlyKeys(payload, commandEnvelopeKeys(source)) || payload.runId !== String(runId) || !commandTransportActions(source).has(payload.action) || !validRunGeneration(payload.expectedGeneration) || !safeIdentityText(payload.idempotencyKey) || typeof payload.commandId !== "string" || payload.commandId && !COMMAND_ID_RE.test(payload.commandId) || typeof payload.statusUnavailable !== "boolean" || !OBSERVATION_KINDS.has(payload.observationKind) || typeof payload.retrying !== "boolean" || typeof payload.checking !== "boolean" || payload.committed !== true || !Number.isFinite(payload.updatedAt)) return protocolTransport(runId, source, payload);
		const record = storedRecord(payload.record, payload.action, source, { strict: true });
		if (!record || payload.commandId && record.id && payload.commandId !== record.id || !!payload.commandId !== !!record.id) return protocolTransport(runId, source, payload);
		let assistantFields = {};
		if (source === "assistant") {
			const arg = payload.action === "approve" && payload.arg != null ? Number(payload.arg) : null;
			const nodeGeneration = payload.action === "approve" && payload.nodeGeneration != null ? Number(payload.nodeGeneration) : null;
			if (payload.action === "approve" && (arg != null && (!Number.isSafeInteger(arg) || arg < 0) || nodeGeneration != null && (!Number.isSafeInteger(nodeGeneration) || nodeGeneration < 0) || !payload.commandId && (arg == null || nodeGeneration == null)) || payload.action !== "approve" && (payload.arg !== null || payload.nodeGeneration != null)) {
...
function clearCommandTransport(source, runId, storage, expected = {}) {
	...
	try {
		...
		if (source === "assistant" && saved && expected.idempotencyKey && saved.idempotencyKey !== expected.idempotencyKey) return false;
...
export function commandFailureRecord(error, previous = error?.commandRecord || null) {
	const localCode = error?.code === "COMMAND_PROTOCOL_ERROR" ? "command_protocol_error" : error?.code === "COMMAND_REQUEST_TIMEOUT" ? "command_request_timeout" : error?.code;
	return {
		...previous || {},
		...error?.commandId && !previous?.id ? { id: error.commandId } : {},
		status: "failed",
		error: {
			code: error?.status === 401 || error?.status === 403 ? "owner_access_required" : error?.status === 404 ? "command_record_missing" : localCode || "command_request_failed",
			message: error?.message || String(error),
			retryable: false,
			remediation: error?.remediation || (error?.status === 401 || error?.status === 403 ? "restore owner access, then check this command again" : error?.code === "COMMAND_PROTOCOL_ERROR" ? "check the same durable command again; do not submit a new intent" : error?.status === 404 ? "refresh the run; this server no longer has the durable command record" : "refresh run state before submitting another action"),
...
export async function submitCommand(promise, labels = {}, onToast) {
	...
	try {
		...
		feedback = {
			...
			status: "transport",
			transport: true,
			message: labels.transport || `${labels.failure || "Command failed"}: ${error?.message || error}`
		};
	}
...
export async function getRunGeneration(runId) {
	...
	if (payload?.generation == null) {
		throw runGenerationError("run_generation_unavailable", "The current run generation is not available yet.", "Refresh the run and wait for its initial event before submitting another action.");
	}
	if (!validRunGeneration(payload.generation)) {
		throw runGenerationError("invalid_run_generation", "The server returned an invalid run generation.", "Refresh the run before submitting another action.");
	}
...
async function commandFetch(path, options = {}, timeoutMs = COMMAND_REQUEST_TIMEOUT_MS, consume = (response) => response) {
	...
	try {
		...
		if (timedOut) throw cause?.code === "COMMAND_REQUEST_TIMEOUT" ? cause : commandTimeoutError(path, timeout, cause);
		throw cause;
	} finally {
		clearTimeout(timer);
		unlink();
...
const commandJson = (path, options, timeoutMs, { errorPath = path, submission = false } = {}) => commandFetch(path, options, timeoutMs, async (response) => {
	if (!response.ok) await _throw(response, errorPath);
	return commandResponseJson(response, errorPath, { submission });
});
const commandRead = (path, { errorPath = path, signal, requestTimeoutMs = COMMAND_REQUEST_TIMEOUT_MS } = {}) => commandJson(path, {
...
export async function submitRunCommand(runId, type, data = {}, { idempotencyKey = createIdempotencyKey(), expectedGeneration, requestTimeoutMs = COMMAND_REQUEST_TIMEOUT_MS } = {}) {
	...
	if (!validRunGeneration(expectedGeneration)) {
		throw runGenerationError("invalid_run_generation", "A verified run generation is required before submitting a command.", "Refresh the run before submitting another action.");
	}
	try {
...
async function awaitRunCommand(runId, record, { waitMs = 8e3, pollMs = 250, requestTimeoutMs = COMMAND_REQUEST_TIMEOUT_MS, onRecord = null } = {}) {
	...
	while (Date.now() < deadline) {
		...
		try {
			...
			if (isTransientCommandReadError(error)) {
				transientFailures += 1;
				nextDelay = Math.max(Number(error.retryAfterMs) || 0, Math.min(2e3, Math.max(25, baseDelay || 25) * 2 ** Math.min(5, transientFailures - 1)));
				continue;
			}
...
export async function retryRunCommand(runId, commandId, { waitMs = 8e3, pollMs = 250, requestTimeoutMs = COMMAND_REQUEST_TIMEOUT_MS, onRecord = null } = {}) {
	...
	try {
		...
		if (error?.status !== 409 && !isTransientCommandReadError(error) && !error?.submissionMayHaveSucceeded) {
			error.commandRecord = {
				id: String(commandId),
				status: "failed",
				error: null
	...
	return awaitRunCommand(runId, record, {
		waitMs,
		pollMs,
		requestTimeoutMs,
		onRecord
...
export async function runCommand(runId, type, data = {}, { waitMs = 8e3, pollMs = 250, idempotencyKey = createIdempotencyKey(), submitRetries = 1, retryMs = 150, requestTimeoutMs = COMMAND_REQUEST_TIMEOUT_MS, onRecord = null, expectedGeneration = undefined } = {}) {
	...
	const generation = expectedGeneration === undefined ? getObservedRunGeneration(runId) || await getRunGeneration(runId) : expectedGeneration;
	...
	try {
		return await awaitRunCommand(runId, record, {
			waitMs,
			pollMs,
			requestTimeoutMs,
			onRecord
...
const reportStorageError = (cause) => Object.assign(new Error("Report refresh storage is unavailable."), {
	code: "REPORT_REFRESH_STORAGE_UNAVAILABLE",
	cause
});
const reportRefreshStorageKey = (runId) => "ll.report-refresh." + encodeURIComponent(String(runId || ""));
...
export function reportRefreshIntent(runId, generation, completedKey = "", storage = undefined, inspectOnly = false) {
	...
	try {
		...
		if (inspectOnly) return candidate === null ? null : {
			generation,
		...
		throw reportStorageError(cause);
	}
}
async function paidConceptLensPost(runId, suffix, body, { idempotencyKey, signal, requestTimeoutMs = COMMAND_REQUEST_TIMEOUT_MS } = {}) {
	if (!validRunGeneration(body?.expected_generation)) {
		throw runGenerationError("invalid_run_generation", "A verified run generation is required before paid concept-lens work.", "Reload Concepts before continuing this request.");
	}
	...
	try {
		...
		if (error?.status == null || error?.code === "COMMAND_REQUEST_TIMEOUT" || error?.code === "COMMAND_PROTOCOL_ERROR") error.submissionMayHaveSucceeded = true;
		throw error;
	}
}
export const submitConceptLens = (runId, prompt, expectedGeneration, options) => paidConceptLensPost(runId, "/concepts/lens", {
	prompt,
	expected_generation: expectedGeneration
}, options);
export const abandonConceptLens = (runId, expectedGeneration, requestId, options) => paidConceptLensPost(runId, "/concepts/lens/abandon", {
	expected_generation: expectedGeneration,
...
export async function getConceptLensRecovery(runId, expectedGeneration, { signal, requestTimeoutMs = COMMAND_REQUEST_TIMEOUT_MS } = {}) {
	if (!validRunGeneration(expectedGeneration)) {
		throw runGenerationError("invalid_run_generation", "A verified run generation is required before paid concept-lens recovery.", "Reload Concepts before inspecting paid work.");
	}
...
export async function abandonRecoveredConceptLens(runId, expectedGeneration, requestId, expectedStartedSeq, { resolutionIdempotencyKey, signal, requestTimeoutMs = COMMAND_REQUEST_TIMEOUT_MS } = {}) {
	if (!validRunGeneration(expectedGeneration)) {
		throw runGenerationError("invalid_run_generation", "A verified run generation is required before resolving recovered paid work.", "Reload Concepts and inspect the current recovery receipt.");
	}
...
export const CONTROL = {
	...
	nodeAbort: (rid, id, generation) => runCommand(rid, "node_abort", {
		node_id: id,
		generation,
		reason: "ui"
	}),
	...
	resetNode: (rid, id, stage, generation) => runCommand(rid, "node_reset", {
		node_id: id,
		generation,
		from_stage: stage
	}),
	approve: (rid, id, generation) => runCommand(rid, "approval_granted", {
		node_id: id,
		generation
	}),
	...
	setEvalCeiling: (rid, seconds) => runCommand(rid, "budget_extend", { max_eval_seconds: seconds }),
	forceConfirm: (rid, id, generation) => runCommand(rid, "force_confirm", {
		node_id: id,
		generation
	}),
	forceAblate: (rid, id, generation) => runCommand(rid, "force_ablate", {
		node_id: id,
		generation
	}),
	fork: (rid, id, generation) => runCommand(rid, "fork", {
		from_node_id: id,
		generation
	}),
	annotate: (rid, id, text) => runCommand(rid, "annotation", {
		node_id: id,
		text
	}),
	...
	createComment: (rid, { nodeId, nodeGeneration, text }, options = {}) => runCommand(rid, "comment_created", {
		node_id: nodeId,
		node_generation: nodeGeneration,
		text
	}, options),
	editComment: (rid, { commentId, nodeId, nodeGeneration, expectedVersion, text }, options = {}) => runCommand(rid, "comment_edited", {
	...
	setCommentResolved: (rid, { commentId, nodeId, nodeGeneration, expectedVersion, resolved }, options = {}) => runCommand(rid, "comment_resolution_changed", {
		comment_id: commentId,
		node_id: nodeId,
		node_generation: nodeGeneration,
		expected_version: expectedVersion,
	...
	retagConcepts: (rid, { nodeId, nodeGeneration, concepts }, options = {}) => runCommand(rid, "concept_tag_edited", {
		node_id: nodeId,
		node_generation: nodeGeneration,
		concepts
	}, options),
	promote: (rid, id, generation) => runCommand(rid, "promote", {
		node_id: id,
		generation,
		alias: "champion"
	}),
	...
	inject: (rid, { idea, parent_id = null, parent_generation = null, code = null }) => runCommand(rid, "inject_node", {
		idea,
		parent_id,
		code,
		parent_generations: parent_id != null && parent_generation != null ? { [parent_id]: parent_generation } : undefined
	}),
	...
	merge: (rid, ids, parentGenerations = undefined, options = {}) => runCommand(rid, "inject_node", {
		idea: {
			operator: "merge",
			rationale: `merge ${ids.map((i) => "#" + i).join(" + ")}`
		},
		parent_ids: ids,
		parent_generations: parentGenerations
	}, options),
	...
	addHypothesis: (rid, statement) => runCommand(rid, "hypothesis_added", {
		statement,
		source: "human"
	}),
	abandonHypothesis: (rid, id) => runCommand(rid, "hypothesis_updated", {
		id,
		status: "abandoned"
	}),
	deleteHypothesis: (rid, id) => runCommand(rid, "hypothesis_updated", {
		id,
		status: "deleted"
	}),
	...
	reprioritizeCard: (rid, id, priority) => runCommand(rid, "card_reprioritized", {
		id,
		priority
	}),
	editCard: (rid, id, statement) => runCommand(rid, "card_edited", {
		id,
		statement
	}),
	pinCardResources: (rid, id, gpus, gpuMemMiB = null) => runCommand(rid, "card_resource_pinned", {
		id,
		gpus,
		...gpuMemMiB == null ? {} : { gpu_mem_mib: gpuMemMiB }
	}),
	dropCard: (rid, id, reason = "operator dropped") => runCommand(rid, "card_dropped", {
		id,
		reason
	}),
	...
	refreshReport: async (rid, { expectedGeneration, idempotencyKey, signal, requestTimeoutMs = COMMAND_REQUEST_TIMEOUT_MS } = {}) => {
		if (!validRunGeneration(expectedGeneration)) {
			throw runGenerationError("invalid_run_generation", "A verified run generation is required before refreshing the report.", "Reload the run before generating its report.");
		}
		if (!safeIdentityText(idempotencyKey)) {
		...
		const result = await jobAwait(response, {
			maxTransientErrors: 3,
			signal
		});
		if (result?.ambiguous !== true && (!validRunGeneration(result?.generation) || result.generation !== expectedGeneration)) {
...
export const resetRun = (rid, expectedGeneration, operationId, options = {}) => post(runApiPath(rid, "/reset"), {
	expected_generation: expectedGeneration,
	operation_id: operationId
}, {
	...options,
	allowRunMutationModes: [
		"start-over",
		"stale-link",
		"history"
	]
...
export const clearNodeTrace = (rid, id, { expectedGeneration, expectedTraceRevision, nodeGeneration, operationId, signal } = {}) => {
	if (!validRunGeneration(expectedGeneration)) {
...
const validLlmHealthIdentity = (value) => typeof value === "string" && /^probe-v1:[\da-f]{64}$/i.test(value);
const validLlmHealthFailureCode = (value) => typeof value === "string" && /^[a-z][a-z0-9_:-]{0,127}$/i.test(value);
...
export async function llmHealth(expectedSettingsRevision, expectedSecretRevision, operationId, options = {}) {
	if (!validSettingsRevision(expectedSettingsRevision) || !validSettingsRevision(expectedSecretRevision)) {
		throw llmHealthRevisionError("llm_health_revision_unavailable", "Saved settings revisions are required before testing the LLM configuration.");
	}
	if (!UUID_V4_RE.test(operationId || "")) {
		throw llmHealthRevisionError("llm_health_operation_unavailable", "A unique operation identity is required before testing the LLM configuration.");
	}
	...
	try {
		...
		const detail = error?.detail && typeof error.detail === "object" && !Array.isArray(error.detail) ? error.detail : null;
		const exactErrorEnvelope = detail && detail.operation_id === operationId && detail.expected_settings_revision === expectedSettingsRevision && detail.expected_secret_revision === expectedSecretRevision && validLlmHealthFailureCode(detail.code) && typeof detail.provider_attempted === "boolean" && typeof detail.outcome_unknown === "boolean" && (detail.ambiguous == null || typeof detail.ambiguous === "boolean") && !(detail.ambiguous === true && detail.outcome_unknown !== true);
		if (!exactErrorEnvelope) {
			throw llmHealthRevisionError("llm_health_identity_protocol_error", "The LLM health error did not prove which operation or saved configuration it belonged to.", {
				operation_id: operationId,
	...
	if (!payload || typeof payload !== "object" || Array.isArray(payload) || payload.operation_id !== operationId || payload.settings_revision !== expectedSettingsRevision || payload.secret_revision !== expectedSecretRevision || typeof payload.ok !== "boolean" || typeof payload.provider_attempted !== "boolean" || typeof payload.outcome_unknown !== "boolean" || payload.ambiguous != null && typeof payload.ambiguous !== "boolean" || !validLlmHealthIdentity(payload.effective_identity) || payload.ok === true && (payload.provider_attempted !== true || payload.outcome_unknown !== false) || payload.outcome_unknown === true && payload.provider_attempted !== true || payload.ambiguous === true && payload.outcome_unknown !== true || payload.ok === false && !validLlmHealthFailureCode(payload.code || payload.error_kind)) {
		throw llmHealthRevisionError("llm_health_identity_protocol_error", "The LLM health response did not match the requested operation, saved settings revisions, or provider-attempt contract.", {
			operation_id: operationId,
...
const _authHeaders = (base) => {
	...
	if (isReviewLocation()) {
		...
		return review ? {
			...base,
			"X-LoopLab-Review": review
		} : { ...base };
	}
	const owner = ownerToken();
	return owner ? {
		...base,
		"X-LoopLab-Token": owner
	} : { ...base };
};
...
async function _throw(r, path) {
	...
	const arrayDetail = Array.isArray(detail) ? detail.map((d) => {
	...
	const message = structured ? String(structured.message || structured.detail || structured.error || structured.code || `${path}: ${r.status}`) : arrayDetail ? arrayDetail : detail ? String(detail) : `${path}: ${r.status}`;
	...
	const existingCommandId = structured?.existing_command_id || structured?.existingCommandId || detailText.match(/\bcmd_[0-9a-f]{32}\b/i)?.[0];
	...
	if (retryAfter) {
		...
		const millis = Number.isFinite(seconds) ? seconds * 1e3 : Date.parse(retryAfter) - Date.now();
		if (Number.isFinite(millis) && millis > 0) err.retryAfterMs = Math.min(6e4, millis);
	}
	throw err;
}
...
export function createEventStreamParser(onEvent, initialLastEventId = "") {
	...
	const line = (rawLine) => {
		...
		if (field === "event") eventType = value;
			...
			retry = Math.min(Number(value), 6e4);
		}
	};
	return {
		push(text) {
...
export async function fetchEventStream(path, { signal, lastEventId = "", onEvent } = {}) {
...
export const deadlineGet = (path, timeout = 8e3, options) => deadlineRequest((signal) => get(path, {
	cache: "no-store",
	...options,
	signal
}), timeout);
...
export const deadlineSharedAssistant = (shareToken, timeout = 8e3) => deadlineRequest(async (signal) => {
...
const artifactGenerationQuery = (expectedGeneration) => {
	if (!validRunGeneration(expectedGeneration)) {
		throw runGenerationError("run_generation_unavailable", "A verified run generation is required before reading files.", "Wait for the current run identity, then reopen Files.");
	}
...
const validateArtifactGeneration = (payload, expectedGeneration, path) => {
	if (!payload || typeof payload !== "object" || Array.isArray(payload) || !validRunGeneration(payload.run_generation)) {
	...
	if (payload.run_generation !== expectedGeneration) {
		const error = runGenerationError("run_generation_changed", "The run changed while files were loading.", "Reload the file inventory for the current run generation.");
		error.status = 409;
		error.detail = {
			expected_generation: expectedGeneration,
...
const validateArtifactContentIdentity = (payload, expected, requestPath) => {
	if (payload.root !== expected.root || payload.path !== expected.path) {
		throw artifactProtocolError("artifact_path_protocol_error", "the response did not echo the requested file identity", requestPath);
	}
	const responseHasNodeIdentity = payload.node_id != null || payload.attempt != null;
	if (responseHasNodeIdentity !== expected.hasNodeIdentity || expected.hasNodeIdentity && (payload.node_id !== expected.nodeId || payload.attempt !== expected.attempt)) {
		throw artifactProtocolError("artifact_attempt_protocol_error", "the response did not echo the requested experiment attempt", requestPath);
	}
	return payload;
};
export async function getRunArtifactInventory(runId, expectedGeneration, options = {}) {
	...
	if (fenced.run_id !== String(runId)) {
		throw artifactProtocolError("artifact_run_protocol_error", "the response did not echo the requested run identity", path);
	}
	return fenced;
}
export async function getRunArtifactContent(runId, { root, path: artifactPath, expectedGeneration, nodeId = null, attempt = null, ...options } = {}) {
	if (typeof root !== "string" || typeof artifactPath !== "string") {
		throw artifactProtocolError("artifact_path_protocol_error", "the file inventory returned an invalid root or path", runApiPath(runId, "/artifact"));
	}
	const query = artifactGenerationQuery(expectedGeneration);
	query.set("root", root);
	...
	if (hasNodeIdentity) {
		if (!Number.isSafeInteger(nodeId) || nodeId < 0 || !Number.isSafeInteger(attempt) || attempt < 0) {
	...
	const payload = await get(requestPath, {
		...options,
		cache: "no-store"
	});
...
export const validAuthoringName = (value) => typeof value === "string" && value.length > 3 && [...value].length <= 255 && value.endsWith(".md") && !/[\\/]/.test(value) && !/\p{C}/u.test(value);
const validAuthoringOperationId = (value) => typeof value === "string" && value.length === 36 && value === value.toLowerCase() && UUID_V4_RE.test(value);
const validAuthoringRevision = (value) => typeof value === "string" && (value === AUTHORING_MISSING_REVISION || value.length === 71 && AUTHORING_REVISION_RE.test(value));
const validAuthoringDesiredRevision = (value) => value !== AUTHORING_MISSING_REVISION && validAuthoringRevision(value);
const validAuthoringResultRevision = (value) => typeof value === "string" && (value === AUTHORING_MISSING_REVISION || value === "oversized" || value.length === 71 && AUTHORING_RESULT_REVISION_RE.test(value));
export const validAuthoringTargetRootId = (value) => typeof value === "string" && value.length === 76 && AUTHORING_TARGET_ROOT_ID_RE.test(value);
...
const authoringOperationPath = (kind, name, operationId) => {
	...
	if (!AUTHORING_KINDS.has(normalizedKind) || !validAuthoringName(normalizedName) || !validAuthoringOperationId(normalizedOperationId)) {
		const error = new Error("Invalid authoring operation identity");
		error.code = "AUTHORING_PROTOCOL_ERROR";
		throw error;
	}
	return `/api/${encodeURIComponent(normalizedKind)}/${encodeURIComponent(normalizedName)}` + `/operations/${encodeURIComponent(normalizedOperationId)}`;
};
...
export async function authoringTextRevision(text, source = globalThis.crypto) {
	if (typeof text !== "string" || typeof TextEncoder === "undefined" || typeof source?.subtle?.digest !== "function") {
...
export function validateAuthoringReceipt(receipt, expected = {}) {
	if (!receipt || typeof receipt !== "object" || Array.isArray(receipt) || !hasOnlyKeys(receipt, AUTHORING_RECEIPT_KEYS) || Object.keys(receipt).length !== AUTHORING_RECEIPT_KEYS.size || receipt.schema !== "looplab.authoring-operation/v1" || !validAuthoringOperationId(receipt.operation_id) || !AUTHORING_KINDS.has(receipt.kind) || !validAuthoringName(receipt.name) || !validAuthoringTargetRootId(receipt.target_root_id) || !validAuthoringRevision(receipt.expected_revision) || !validAuthoringDesiredRevision(receipt.desired_revision) || ![
		"prepared",
		"succeeded",
		"conflict"
	].includes(receipt.status) || !Number.isSafeInteger(receipt.created_at) || receipt.created_at < 0 || !Number.isSafeInteger(receipt.updated_at) || receipt.updated_at < receipt.created_at) {
		throw authoringProtocolError("The server returned an invalid authoring receipt.", receipt);
	}
	const validTerminal = receipt.status === "prepared" ? receipt.result_revision === null && receipt.code === null : receipt.status === "succeeded" ? receipt.result_revision === receipt.desired_revision && receipt.code === null : validAuthoringResultRevision(receipt.result_revision) && ["authoring_revision_conflict", "authoring_intervening_write"].includes(receipt.code);
	if (!validTerminal || receipt.ok !== (receipt.status === "succeeded") || receipt.replayable !== (receipt.status === "prepared")) {
	...
	const expectedRevision = expected.expectedRevision == null ? null : String(expected.expectedRevision);
	const expectedTargetRootId = expected.expectedTargetRootId == null ? null : String(expected.expectedTargetRootId);
	const desiredRevision = expected.desiredRevision == null ? null : String(expected.desiredRevision);
	if (expectedOperationId != null && receipt.operation_id !== expectedOperationId || expectedKind != null && receipt.kind !== expectedKind || expectedName != null && receipt.name !== expectedName || expectedTargetRootId != null && receipt.target_root_id !== expectedTargetRootId || expectedRevision != null && receipt.expected_revision !== expectedRevision || desiredRevision != null && receipt.desired_revision !== desiredRevision) {
		throw authoringProtocolError("The authoring receipt does not match the requested operation identity.", receipt);
	}
	return receipt;
}
export async function putAuthoringOperation(kind, name, operationId, { text, expectedRevision, expectedTargetRootId, desiredRevision = null }, { signal } = {}) {
	const path = authoringOperationPath(kind, name, operationId);
	if (typeof text !== "string" || !validAuthoringRevision(expectedRevision) || !validAuthoringTargetRootId(expectedTargetRootId)) {
...
export async function getAuthoringOperation(kind, name, operationId, { signal, expectedRevision, expectedTargetRootId, desiredRevision } = {}) {
	if (!validAuthoringTargetRootId(expectedTargetRootId) || !validAuthoringRevision(expectedRevision) || !validAuthoringDesiredRevision(desiredRevision)) {
		throw authoringProtocolError("An exact authoring operation identity is required for receipt lookup.");
	}
	const path = authoringOperationPath(kind, name, operationId) + `?expected_target_root_id=${encodeURIComponent(expectedTargetRootId)}` + `&expected_revision=${encodeURIComponent(expectedRevision)}` + `&desired_revision=${encodeURIComponent(desiredRevision)}`;
	const receipt = await get(path, {
		signal,
		cache: "no-store"
	});
	return validateAuthoringReceipt(receipt, {
...
export const createProject = (name, parent_id = null) => post("/api/projects", {
	name,
	parent_id
});
...
const runOrganizationBody = (field, value, expectedGeneration, expectedCurrent) => {
	if (!validRunGeneration(expectedGeneration)) {
		throw runGenerationError("run_generation_unavailable", "An exact observed run generation is required before changing run organization.", "Refresh the Runs list and repeat the change on the intended run.");
	}
	if (expectedCurrent !== null && typeof expectedCurrent !== "string") {
		throw runGenerationError("run_organization_unavailable", "The exact current organization value is required before changing run organization.", "Refresh the Runs list and repeat the change from the current run card.");
	}
	return {
		[field]: value,
...
export const assignRun = (runId, project_id, expectedGeneration, expectedProjectId) => post(runApiPath(runId, "/project"), runOrganizationBody("project_id", project_id, expectedGeneration, expectedProjectId));
export const renameRun = (runId, label, expectedGeneration, expectedLabel) => send(runApiPath(runId), "PATCH", runOrganizationBody("label", label, expectedGeneration, expectedLabel));
export function submitRunDeletion(runId, expectedGeneration, expectedSeq, operationId, options = {}) {
	if (!validRunGeneration(expectedGeneration)) {
		throw Object.assign(new Error("An exact run generation is required to delete a run."), { code: "run_generation_unavailable" });
	}
	if (!Number.isSafeInteger(expectedSeq) || expectedSeq < -1) {
		throw Object.assign(new Error("An exact run sequence is required to delete a run."), { code: "run_sequence_unavailable" });
	}
	if (!UUID_V4_RE.test(operationId || "")) {
		throw Object.assign(new Error("A stable deletion operation id is required."), { code: "delete_operation_invalid" });
	}
...
export const createRunReview = (runId, { ttl_seconds, include_evidence = false, expected_generation, request_id, token_secret } = {}, options) => post(runApiPath(runId, "/reviews"), {
	ttl_seconds,
	include_evidence,
	expected_generation,
	request_id,
...
export const listRunReviews = (runId, options) => get(runApiPath(runId, "/reviews"), options);
export const revokeRunReview = (runId, linkId, options) => send(runApiPath(runId, `/reviews/${encodeURIComponent(linkId)}`), "DELETE", null, options);
...
export const runComments = (runId, { nodeId = null, nodeGeneration = null, includeResolved = true, limit = 100, cursor = null, signal } = {}) => {
	const query = new URLSearchParams();
...
export const commentHistory = (runId, commentId, { limit = 100, cursor = null } = {}) => {
	...
	query.set("limit", String(Math.max(1, Math.min(100, Math.trunc(Number(limit) || 100)))));
	if (cursor != null) query.set("cursor", String(cursor));
	return get(runApiPath(runId, `/comments/${encodeURIComponent(commentId)}/history?${query}`), { cache: "no-store" });
};
...
export const createSupertask = (name, task_id = null) => post("/api/supertasks", {
	name,
	task_id
});
...
export const assignSupertask = (runId, supertask_id, expectedGeneration, expectedSupertaskId) => post(runApiPath(runId, "/supertask"), runOrganizationBody("supertask_id", supertask_id, expectedGeneration, expectedSupertaskId));
export const gpuStat = () => get("/api/gpu");
// ---- settings + run launch ----
export const getSettings = () => get("/api/settings");
export const getSettingsSchema = (options = {}) => get("/api/settings/schema/2", options);
export const saveSettings = (settings, { expectedRevision, ...options } = {}) => send("/api/settings", "PUT", {
	settings,
...
export const saveSecret = (key, value, { expectedSettingsRevision, expectedSecretRevision, ...options } = {}) => send("/api/settings/secret", "PUT", {
	key,
	value,
	...expectedSettingsRevision == null ? {} : { expected_settings_revision: expectedSettingsRevision },
	...expectedSecretRevision == null ? {} : { expected_secret_revision: expectedSecretRevision }
}, options);
...
export const saveRunConfig = (rid, settings, { expectedRevision, expectedGeneration, ...options } = {}) => send(runApiPath(rid, "/config"), "PUT", {
	settings,
	...expectedRevision == null ? {} : { expected_revision: expectedRevision },
...
const crossRunRead = (path, options = {}) => get(path, {
	...options,
	cache: "no-store"
});
export function boundedAtlasText(value, max = 360) {
	...
	const limit = Number.isSafeInteger(max) ? Math.max(0, Math.min(2e3, max)) : 360;
...
const CROSS_RUN_COUNT_ARRAYS = new Set([
	"merges",
	"splits",
	"purges",
	"decisions",
...
function projectCrossRunValue(value, key = "", depth = 0) {
	...
	if (Array.isArray(value)) {
		...
		return value.slice(0, CROSS_RUN_STATE_CAPS[key] || 6).map((item) => projectCrossRunValue(item, key, depth + 1));
	}
...
export function projectResearchAtlasSource(key, value) {
	...
	if (key === "atlas") {
		...
		projected.n_contested = Math.max(Number.isSafeInteger(projected.n_contested) && projected.n_contested >= 0 ? projected.n_contested : 0, contested);
	}
	return projected;
}
...
const crossRunLimitArgs = (limitOrOptions, fallback, maximum, options) => limitOrOptions && typeof limitOrOptions === "object" ? {
	limit: fallback,
	options: limitOrOptions
} : {
	limit: boundedCrossRunInt(limitOrOptions, fallback, maximum, 1),
	options
};
...
export const getCrossRunClaims = (limitOrOptions = 80, offset = 0, options) => {
	...
	return crossRunRead(`/api/cross-run/claims?limit=${args.limit}&offset=${boundedCrossRunInt(offsetIsOptions ? 0 : offset, 0, 1e6)}`, args.options);
};
...
const boundedLaunchTimeout = (value, fallback) => {
	const parsed = Number(value);
	return Number.isFinite(parsed) ? Math.max(1, Math.min(MAX_LAUNCH_REQUEST_TIMEOUT_MS, Math.trunc(parsed))) : fallback;
};
const launchDeadline = async (read, timeoutMs, code, { submission = false } = {}) => {
	try {
		...
		const ambiguous = cause?.name === "TimeoutError" || cause?.status == null || Number.isFinite(status) && (status >= 500 || TRANSIENT_HTTP.has(status));
		if (cause?.name !== "TimeoutError" && !(submission && ambiguous)) throw cause;
		const error = Object.assign(new Error(submission ? "Startup submission did not return before its deadline." : code === "LAUNCH_PREFLIGHT_TIMEOUT" ? "Launch validation did not return before its deadline." : "Startup status did not return before its deadline."), {
...
export const preflightRunStart = (body, { requestTimeoutMs = LAUNCH_PREFLIGHT_TIMEOUT_MS } = {}) => launchDeadline((signal) => post("/api/start/preflight", body, { signal }), boundedLaunchTimeout(requestTimeoutMs, LAUNCH_PREFLIGHT_TIMEOUT_MS), "LAUNCH_PREFLIGHT_TIMEOUT");
...
export const startRun = (body, { requestTimeoutMs = LAUNCH_SUBMISSION_TIMEOUT_MS } = {}) => launchDeadline((signal) => post("/api/start", body, { signal }), boundedLaunchTimeout(requestTimeoutMs, LAUNCH_SUBMISSION_TIMEOUT_MS), "LAUNCH_SUBMISSION_UNKNOWN", { submission: true });
export const getStartStatus = (runId, idempotencyKey, { requestTimeoutMs = LAUNCH_STATUS_TIMEOUT_MS } = {}) => launchDeadline((signal) => get(`/api/start/${encodeURIComponent(runId)}/status`, {
	cache: "no-store",
	signal,
	headers: { "Idempotency-Key": String(idempotencyKey || "") }
}), boundedLaunchTimeout(requestTimeoutMs, LAUNCH_STATUS_TIMEOUT_MS), "LAUNCH_STATUS_TIMEOUT");
...
const _scopeActionUrl = (type, id, actionId) => `/api/scope-report-actions/${encodeURIComponent(actionId)}` + `?scope_type=${encodeURIComponent(type)}&scope_id=${encodeURIComponent(id)}`;
...
const scopeActionId = (value) => typeof value === "string" && UUID_V4_RE.test(value) ? value.toLowerCase() : null;
...
const scopeGenerationErrorIdentity = (cause, actionId, jobId = null, ambiguous = true) => {
	...
	try {
		...
		if (actionId) {
			wrapped.actionId = actionId;
			wrapped.action_id = actionId;
		}
		if (jobId) {
			wrapped.jobId = jobId;
			wrapped.job_id = jobId;
		}
		if (ambiguous) {
			wrapped.ambiguous = true;
			wrapped.submissionMayHaveSucceeded = true;
		}
		return wrapped;
	}
};
...
const scopeTerminalResult = (value, actionId, jobId = null) => {
	...
	if (value.status === "indeterminate" && value.ok === false && value.code === "scope_report_action_indeterminate") {
	...
	return retainedJobId && !value.job_id ? {
		...value,
		job_id: retainedJobId
	} : value;
};
export async function getScopeReportAction(type, id, actionId, { signal, requestTimeoutMs = COMMAND_REQUEST_TIMEOUT_MS } = {}) {
	...
	try {
		const value = await commandRead(path, {
			signal,
			requestTimeoutMs
		});
		if (!scopeGenerationRecord(value) || value.action_id !== actionId || ![
...
export async function abandonScopeReportAction(type, id, actionId, { signal, requestTimeoutMs = COMMAND_REQUEST_TIMEOUT_MS } = {}) {
	actionId = scopeActionId(actionId);
	if (!actionId) throw new Error("A valid scope report action id is required.");
	const path = `/api/scope-report-actions/${encodeURIComponent(actionId)}/abandon` + `?scope_type=${encodeURIComponent(type)}&scope_id=${encodeURIComponent(id)}`;
	try {
		...
		const explicitlyAbandoned = value?.status === "abandoned" && value?.ok === false && value?.code === "scope_report_action_abandoned";
		...
		if (!scopeGenerationRecord(value) || value.action_id !== actionId || !explicitlyAbandoned && !concurrentlySettled && !safelyUnknown) {
			throw scopeIdentityMismatch(actionId, scopeJobId(value?.job_id));
		}
		return value;
	} catch (error) {
...
export async function jobAwait(resp, { intervalMs = 1500, timeoutMs = 6e5, signal, maxTransientErrors = Number.POSITIVE_INFINITY, requestTimeoutMs = COMMAND_REQUEST_TIMEOUT_MS } = {}) {
	...
	try {
		while (Date.now() < deadline) {
			...
			const returnedIds = ["job_id", "jobId"].filter((key) => Object.hasOwn(j, key)).map((key) => j[key]);
...
export async function reconcileScopeReportGeneration(type, id, { actionId, jobId = null, signal, onJob, intervalMs = 1500, timeoutMs = 6e5, requestTimeoutMs = COMMAND_REQUEST_TIMEOUT_MS, maxTransientErrors = 3 } = {}) {
	actionId = scopeActionId(actionId);
	if (!actionId) throw new Error("A valid scope report action id is required.");
	...
	const action = await getScopeReportAction(type, id, actionId, {
		signal,
		requestTimeoutMs
	});
	...
	if (result?.status === "done" || result?.ambiguous !== true) {
		const settled = await getScopeReportAction(type, id, actionId, {
			signal,
			requestTimeoutMs
		});
...
export async function genScopeReport(type, id, { actionId, signal, onJob, intervalMs = 1500, timeoutMs = 6e5, requestTimeoutMs = COMMAND_REQUEST_TIMEOUT_MS, maxTransientErrors = 3 } = {}) {
	actionId = scopeActionId(actionId);
	if (!actionId) throw new Error("A valid scope report action id is required.");
	...
	try {
		...
		const activeActionId = error?.code === "scope_report_action_in_progress" ? scopeActionId(error?.detail?.action_id) : null;
		if (activeActionId) {
		...
		const ambiguous = error?.code === "scope_report_storage_conflict" || error?.submissionMayHaveSucceeded === true || error?.status == null || status >= 500 || status === 408 || status === 425;
...
export const assistantCreate = (title = "", mode = "plan", options) => post("/api/assistant/sessions", {
	title,
	mode
}, options);
export const assistantGet = (sid, options) => get(`/api/assistant/sessions/${encodeURIComponent(sid)}`, options);
export const assistantDelete = (sid, options) => send(`/api/assistant/sessions/${encodeURIComponent(sid)}`, "DELETE", undefined, options);
const validAssistantForkActionId = (value) => typeof value === "string" && value === value.toLowerCase() && UUID_V4_RE.test(value);
...
export const assistantFork = (sid, { actionId = createIdempotencyKey(), expectedMessages = null } = {}, options = {}) => {
	const identity = assistantForkIdentity(actionId);
	if (expectedMessages != null && (!Number.isSafeInteger(expectedMessages) || expectedMessages < 0)) {
...
export const assistantForkStatus = (sid, actionId, { expectedMessages = null, ...options } = {}) => {
	if (expectedMessages != null && (!Number.isSafeInteger(expectedMessages) || expectedMessages < 0)) {
		const error = new Error("Invalid Assistant fork source version");
		error.code = "ASSISTANT_FORK_PROTOCOL_ERROR";
		throw error;
...
export async function assistantMessageStream(sid, instruction, mode, cbs = {}, signal, display = null, acknowledgedLiveShareIds = []) {
	const body = {
		instruction,
		mode,
...
export const spanDetail = (runId, spanId) => get(runApiPath(runId, `/spans/${encodeURIComponent(spanId)}`));
...
export const nodeConversation = (runId, nid, options) => get(runNodeApiPath(runId, nid, "/conversation"), options);
...
export const assistantCancel = (sid, options) => post(`/api/assistant/sessions/${encodeURIComponent(sid)}/cancel`, {}, options);
export const assistantProgress = (sid, options = {}) => get(`/api/assistant/progress?session=${encodeURIComponent(sid)}`, options);
...
export const assistantRevert = (change, options) => {
	...
	if (!path || !recoveryId || typeof exists !== "boolean" || exists && (typeof digest !== "string" || !/^[0-9a-f]{64}$/.test(digest)) || exists && (!Number.isInteger(mode) || mode < 0 || mode > 4095) || !exists && (digest != null || mode != null)) {
	...
	return post("/api/assistant/revert", {
		...
		expected_postimage: {
			exists,
			digest: exists ? digest : null,
			mode: exists ? mode : null
		}
	}, options);
};
...
export const assistantShare = (sid, live = false, options) => post(`/api/assistant/sessions/${encodeURIComponent(sid)}/share`, { live }, options);
export const assistantUnshare = (sid, options) => send(`/api/assistant/sessions/${encodeURIComponent(sid)}/share`, "DELETE", null, options);
// Pending human-in-the-loop confirm requests for a session, and resolving one.
export const assistantPermissions = (sid = null, options = {}) => get(sid == null ? "/api/assistant/permissions" : `/api/assistant/permissions?session=${encodeURIComponent(sid)}`, options);
export const assistantResolve = (reqId, decision) => post(`/api/assistant/permissions/${encodeURIComponent(reqId)}`, { decision });
export const attentionFeed = (limit = 200, cursor = null, options = {}) => get(`/api/attention?limit=${encodeURIComponent(limit)}` + (cursor == null ? "" : `&cursor=${encodeURIComponent(cursor)}`), options);
