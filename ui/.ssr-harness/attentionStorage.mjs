...
import { attentionIdValid } from "./attentionModel.mjs";
...
const MAX_AGE_MS = 90 * 24 * 60 * 60 * 1e3;
...
const validTimestamp = (value) => Number.isSafeInteger(value) && value >= 0 && value <= Number.MAX_SAFE_INTEGER;
const normalizePairs = (value, now) => {
	if (!Array.isArray(value) || value.length > ATTENTION_MAX_IDS) return null;
	const byId = new Map();
	for (const row of value) {
		if (!Array.isArray(row) || row.length !== 2 || !attentionIdValid(row[0]) || !validTimestamp(row[1])) return null;
		if (row[1] >= now - MAX_AGE_MS && row[1] <= now + 6e4) byId.set(row[0], row[1]);
	}
	return [...byId.entries()].sort((left, right) => right[1] - left[1]).slice(0, ATTENTION_MAX_IDS);
};
export function parseAttentionState(raw, now = Date.now()) {
	...
	if (!parsed || typeof parsed !== "object" || Array.isArray(parsed) || Object.keys(parsed).some((key) => !STATE_KEYS.has(key)) || parsed.v !== ATTENTION_STATE_VERSION || typeof parsed.enabled !== "boolean" || !validTimestamp(parsed.armedAt) || parsed.enabled && parsed.armedAt <= 0) {
		return {
			state: defaultState(),
			valid: false
		};
...
export function recordAttentionIds(state, field, ids, now = Date.now()) {
	...
	const rows = [...next.entries()].filter(([, timestamp]) => timestamp >= now - MAX_AGE_MS).sort((left, right) => right[1] - left[1]).slice(0, ATTENTION_MAX_IDS);
	return {
		...defaultState(),
		...state,
		[field]: rows
...
export const attentionIds = (state, field) => new Set(Array.isArray(state?.[field]) ? state[field].map((row) => row[0]).filter(attentionIdValid) : []);
