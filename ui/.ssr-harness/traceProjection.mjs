...
export const tracePartial = (p) => p?.truncated === true || Math.max(count(p?.omitted_spans), count(p?.total_spans) - count(p?.visible_spans)) > 0;
...
export const unavailableTraceDetail = () => ({
	status: "unavailable",
	attributes: {},
	partial: false
});
