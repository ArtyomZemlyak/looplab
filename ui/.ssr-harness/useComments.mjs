...
import { runComments } from "./api.mjs";
import { commentMatchesSubject, mergeCommentPages, normalizeCommentsPage } from "./commentsModel.mjs";
...
const COMMENTS_TIMEOUT_MS = 12e3;
export function useComments({ runId, nodeId = null, nodeGeneration = null, expectedGeneration = null, includeResolved = true, limit = 100, enabled = true, refreshKey = null } = {}) {
	...
	const subjectValid = nodeId == null || Number.isSafeInteger(nodeId) && nodeId >= 0 && Number.isSafeInteger(nodeGeneration) && nodeGeneration >= 0;
	const resourceEnabled = enabled && !!runId && /^[0-9a-f]{64}$/.test(expectedGeneration || "") && subjectValid;
	...
	useEffect(() => {
		...
		timed.promise.then((payload) => {
			...
			if (!page || page.comments.some((comment) => !commentMatchesSubject(comment, nodeId, nodeGeneration))) {
	...
	const loadMore = useCallback(async () => {
		...
		try {
			...
			if (!page || page.comments.some((comment) => !commentMatchesSubject(comment, nodeId, nodeGeneration))) throw new Error("invalid comments page");
			...
			setResource((previous) => ({
				...previous,
				loadingMore: false,
				loadMoreError: error?.status === 409 ? "The comment list changed before the next page loaded. Refresh and try again." : "Older comments could not be loaded. Try again."
			}));
		} finally {
