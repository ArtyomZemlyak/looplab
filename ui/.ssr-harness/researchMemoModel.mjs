import { advisoryArrayLength as arrayLength, boundedAdvisoryText as text, isAdvisoryRecord as record } from "./reportModel.mjs";
...
const { memos: MAX_MEMOS, collectionChars: COLLECTION_CHARS, memoChars: MEMO_CHARS, summaryChars: SUMMARY_CHARS, reasoningChars: REASONING_CHARS, findings: MAX_FINDINGS, findingChars: FINDING_CHARS, directions: MAX_DIRECTIONS, directionChars: DIRECTION_CHARS, sources: MAX_SOURCES, sourceTitleChars: SOURCE_TITLE_CHARS, sourceUrlChars: SOURCE_URL_CHARS, sourceSnippetChars: SOURCE_SNIPPET_CHARS, claimChars: CLAIM_CHARS, verificationVerdicts: MAX_VERDICTS } = RESEARCH_MEMO_LIMITS;
...
function normalizeVerification(value, budget) {
	...
	const totalVerdicts = Number.isSafeInteger(declaredTotal) && declaredTotal >= rawTotal && declaredOmitted === declaredTotal - rawTotal ? declaredTotal : rawTotal;
...
function normalizeCollections(value, budget) {
	...
	const directions = textList(field(src, "recommended_directions"), MAX_DIRECTIONS, DIRECTION_CHARS, budget);
	const sources = [];
...
export function normalizeResearchMemos(value) {
	...
	return {
		memos,
		total,
		omitted: Math.max(0, total - memos.length)
	};
}
