...
export function danglingAssistantTurn(messages) {
	...
	return trailing && trailing.role === "user" && typeof trailing.turn_id === "string" && trailing.turn_id.length > 0 ? trailing : null;
}
...
export function assistantTurnIndex(messages, prior) {
	...
	if (typeof prior.turn_id === "string" && prior.turn_id) {
		userIndex = messages.findIndex((message) => message?.role === "user" && message.turn_id === prior.turn_id);
	...
	return durableUser.content === prior.content && durableRaw === payload.raw && durableUser.mode === payload.mode ? userIndex : -1;
}
...
export function assistantRecoveryFailure(error) {
	...
	if (error.status >= 500 || [
		...
		425,
		429
	].includes(error.status)) return null;
	if (error.status === 409 && !["assistant_turn_recovery_mismatch", "assistant_turn_recovery_required"].includes(error.code)) return null;
