...
export function queuedGenerationControls(state = {}) {
	...
	const confirms = confirmRequests.filter((r) => !confirmDone.has(pairKey(r))).concat(uniqueLegacy(state.confirm_requests, confirmTwinIds, confirmLegacyDone));
	...
	const ablateLegacyDone = new Set((state.ablations || []).map((a) => a?.parent_id).filter(Number.isInteger));
	const ablates = ablateRequests.filter((r) => !ablateDone.has(pairKey(r))).concat(uniqueLegacy(state.ablate_requests, ablateTwinIds, ablateLegacyDone));
	return {
		confirms,
		ablates
	};
