...
export function pendingApprovalTarget(state = {}) {
	...
	if (!Number.isSafeInteger(nodeId) || nodeId < 0 || !Number.isSafeInteger(nodeGeneration) || nodeGeneration < 0) return null;
	const node = state.nodes?.[nodeId];
	if (!node || node.tombstoned || node.status === "aborted" || node.attempt !== nodeGeneration) return null;
	return {
		nodeId,
		nodeGeneration
	};
...
export function dagEmptyPresentation({ displayed = {}, live = null, resourceStatus = "ready", connected = true, historyActive = false, reviewMode = false, sequence = null } = {}) {
	...
	if (resourceStatus !== "ready" || Object.keys(activeNodeMap(displayed?.nodes || {}, displayed)).length > 0) return null;
	const connectionNote = connected || historyActive || reviewMode ? "" : " The live connection is interrupted; this is the last received state.";
	...
	const result = (kind, tone, title, body, actions = [], liveRegion = "polite") => ({
		...
		tone,
		title,
		body: body + connectionNote,
		actions,
		liveRegion
	});
	const action = (id, label, emphasis = "secondary") => ({
		id,
		label,
		emphasis
	});
	if (historyActive) return result("history", "neutral", `No experiments at snapshot${sequence == null ? "" : ` seq ${sequence}`}`, "This point in the timeline predates the first experiment. Return to the live run to continue.", [action("return-live", "Return to live", "primary")]);
	if (reviewMode) return result("review", "neutral", "No experiments are available in this review", mode === "finished" ? "This run ended without an experiment card. The report may still explain why." : "The owner has not produced or shared an experiment card yet.", mode === "finished" ? [action("report", "View report", "primary")] : []);
	if (mode === "finalization-stalled") return result("finalization-stalled", "danger", "Finalization stopped before wrap-up completed", "The engine stopped before the report, lessons, and final cost were safely written.", [action("finalize", "Reattach finalization", "primary"), action("events", "Show events")], "assertive");
	if (mode === "finalizing" || mode === "finishing") return result(mode, "progress", "Wrapping up this run…", mode === "finishing" ? "The run is writing its terminal report, lessons, and cost. No recovery action is needed yet." : "Finalization is still active. The report will open only after terminal write-out completes.");
	...
	if (state.phase === "approval" || state.phase === "spec_approval") {
		...
		if (!command) return result("approval-incomplete", "danger", "Approval state is incomplete", "The run requests human approval but does not identify an experiment. Inspect the timeline instead of sending a guessed command.", [action("events", "Show events", "primary")], "assertive");
		return result("approval", "attention", state.phase === "spec_approval" ? "Evaluation spec needs approval" : "Human approval is required", `Review the pending decision, then continue with ${command} in Assistant.`, [action("assistant", `Open Assistant · ${command}`, "primary")], "assertive");
	}
	if (mode === "paused") return result("paused", "attention", "Paused before the first experiment", "Resume to continue setup and create the first experiment, or finalize to wrap up without one.", [action("resume", "Resume run", "primary"), action("finalize", "Finalize run", "danger")]);
	if (mode === "unknown") return result("unknown", "neutral", "Engine ownership is unknown", "LoopLab cannot verify whether an engine owns this run. Inspect Events and storage locking before taking recovery action.", [action("events", "Show events", "primary"), ...!connected ? [action("retry-connection", "Retry connection")] : []], "assertive");
	if (mode === "stalled") return result("stalled", "danger", "Engine stopped before the first experiment", "No process is advancing this run. Resume it to reattach safely, or finalize it without an experiment.", [
		action("resume", "Resume run", "primary"),
		action("finalize", "Finalize run", "danger"),
		action("events", "Show events")
	], "assertive");
	if (mode === "finished") return result("finished", "neutral", "Run finished without producing an experiment", "Open the report for the terminal explanation, or resume the run to continue searching.", [action("report", "View report", "primary"), action("resume", "Resume run")]);
	if (state.engine_running === true) return result("preparing", "progress", "Preparing the first experiment…", state.phase && !["running", "search"].includes(state.phase) ? `The run is in ${state.phase}. Setup and research events remain available in the timeline.` : "The engine is active. Setup and research events remain available in the timeline.", [action("events", "Show events")]);
	const actions = [action("events", "Show events", "primary")];
	if (!connected) actions.push(action("retry-connection", "Retry connection"));
	return result("empty", connected ? "neutral" : "attention", "No experiments yet", connected ? "The run has not produced its first experiment. Check the timeline for setup or research activity." : "The last received state contains no experiment cards.", actions, connected ? "polite" : "assertive");
}
...
export function indexProjects(projects = []) {
	...
	projects.forEach((project) => {
		(byParent[project.parent_id || null] ||= []).push(project);
	});
	...
	Object.values(byParent).forEach((items) => items.sort((a, b) => String(a.name || "").localeCompare(String(b.name || ""))));
	const subtree = (id) => {
...
export function filterRuns(runs = [], { project = ALL_RUNS, projects = [], query = "", task = ALL_RUNS, taskExact = task !== ALL_RUNS, supertask = ALL_RUNS, status = "all" } = {}) {
	let result = scopeRuns(runs, project, projects);
	const q = query.trim().toLowerCase();
	if (q) result = result.filter((run) => [
		run.label,
		run.run_id,
		run.task_id,
		run.goal
...
export function metricComparable(runs = []) {
	...
	return runs.length > 0 && tasks.size === 1 && !!runs[0].task_id && directions.size === 1 && ["min", "max"].includes(runs[0].direction);
}
