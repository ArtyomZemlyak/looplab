...
export const DEFAULT_REQUEST_TIMEOUT_MS = 12e3;
...
export function deadlineRequest(read, timeout) {
	...
	const promise = new Promise((resolve, reject) => {
		...
		Promise.resolve().then(() => read(controller.signal)).then((value) => finish(true, value), (error) => finish(false, error));
	});
	return {
		controller,
		promise,
...
export const boundedRequest = (read, ms = DEFAULT_REQUEST_TIMEOUT_MS) => deadlineRequest(read, ms).promise;
