import { normalizeConceptId } from "./conceptId.mjs";
const canonical = (value) => {
	if (!Array.isArray(value) || value.length === 0 || value.length > 64) return [];
	return new Set(value).size === value.length && value.every((item) => normalizeConceptId(item) === item) ? value : [];
};
export function crossRunPriorNarration(data) {
	const matched = canonical(data?.matched_concepts);
	const runCount = Array.isArray(data?.prior_runs) ? data.prior_runs.slice(0, 8).filter((run) => run && typeof run === "object" && !Array.isArray(run)).length : 0;
	...
	const history = runCount ? `${runCount} retained run${runCount === 1 ? "" : "s"}` : "match recorded";
