...
import { anchoredScrollTop, buildVirtualLayout, DEFAULT_TIMELINE_OVERSCAN, DEFAULT_TIMELINE_ROW_HEIGHT, tailScrollTop, timelineViewportAtTail, virtualIndexAt, virtualRange } from "./timelineWindow.mjs";
...
export default function VirtualTimeline({ rows, getKey = defaultKey, renderRow, identity = "timeline", className = "", followingTail = false, windowAtTail = false, unread = 0, unreadUnknown = false, busy = false, onFollowingTailChange, onJumpToLive, estimateSize = DEFAULT_TIMELINE_ROW_HEIGHT, overscan = DEFAULT_TIMELINE_OVERSCAN, ariaLabel = "Event timeline" }) {
	...
	const layout = useMemo(() => buildVirtualLayout(rows, measurements.current, (row, index) => `${identity}:${getKey(row, index)}`, estimateSize), [
		rows,
		getKey,
		identity,
		estimateSize,
	...
	const readViewport = useCallback((updateAnchor = false) => {
		...
		setViewport((previous) => previous.top === top && previous.height === height ? previous : {
			top,
			height
		});
		if (updateAnchor && layout.count) {
			...
			anchorRef.current = {
				...
				offset: top - layout.offsets[index]
			};
		}
	}, [layout]);
	...
	const pinFollowingTail = useCallback(() => {
		...
		const pin = () => {
			...
			return geometryReady && timelineViewportAtTail(element.scrollHeight, element.scrollTop, element.clientHeight);
		};
		pin();
		const verify = (stableFrames = 0) => {
			...
			const expectsOverflow = layout.totalHeight > element.clientHeight + 2;
			const geometryReady = !expectsOverflow || element.scrollHeight > element.clientHeight + 2;
			const aligned = geometryReady && timelineViewportAtTail(element.scrollHeight, element.scrollTop, element.clientHeight);
	...
	const onScroll = useCallback(() => {
		...
		const element = scrollRef.current;
		if (!element) return;
		const nearBottom = timelineViewportAtTail(element.scrollHeight, element.scrollTop, element.clientHeight, 64);
	...
	useLayoutEffect(() => {
		...
		if (followingTail) {
			...
			if (target != null && Math.abs(element.scrollTop - target) > .5) setProgrammaticScroll(target);
	...
	const measure = useCallback((key, height) => {
		...
		if (previous != null && Math.abs(previous - height) < .5) return;
	...
	for (let index = range.start; index < range.end; index += 1) {
		...
		visible.push(/* @__PURE__ */ _jsx(MeasuredRow, {
			itemKey: key,
	...
	return /* @__PURE__ */ _jsxs("div", {
		className: "timeline-virtual-wrap",
		children: [/* @__PURE__ */ _jsx("div", {
			ref: scrollRef,
			className: `timeline-virtual ${className}`.trim(),
			onScroll,
			role: "list",
		...
		}), !followingTail && (unreadUnknown || unread > 0) && /* @__PURE__ */ _jsx("div", {
			className: "timeline-unread-status",
			role: "status",
			"aria-live": "polite",
			"aria-atomic": "true",
			children: /* @__PURE__ */ _jsx("button", {
				type: "button",
				className: "timeline-unread",
				onClick: onJumpToLive,
				"aria-label": unreadUnknown ? "New activity; jump to live" : `${unread} new event${unread === 1 ? "" : "s"}; jump to live`,
				children: unreadUnknown ? "new activity · jump to live" : `${unread} new · jump to live`
			})
		})]
	});
}
