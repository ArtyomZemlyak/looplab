import { runApiPath } from "./api.mjs";
...
export const TIMELINE_RETAIN_LIMIT = 5e3;
export const TIMELINE_CATCHUP_DELAYS = Object.freeze([
	100,
	200,
	400
]);
export const TIMELINE_DIRECTIONS = new Set([
	"tail",
	"older",
	"newer",
	"around"
]);
const finiteSeq = (value) => value !== null && value !== undefined && value !== "" && Number.isSafeInteger(Number(value)) && Number(value) >= 0;
const validEventCount = (value) => typeof value === "number" && Number.isSafeInteger(value) && value >= 0 ? value : null;
...
export const timelineConflictMatchesGeneration = (error, displayedGeneration) => GENERATION_RE.test(String(displayedGeneration || "")) && error?.status === 409 && String(error?.detail?.actual_generation || "") === String(displayedGeneration);
...
export function createTimelineState({ retainLimit = TIMELINE_RETAIN_LIMIT } = {}) {
	return {
		...
		unreadUnknown: false,
		parkedFrontier: null,
		parkedTotalEvents: null,
		parkedEventCount: null,
		liveEventCount: null,
		tornTail: false,
		totalEvents: null,
		range: null,
		sourceTailLimited: false,
...
export function timelineRequestFailed(state, direction, error) {
	...
	return {
		...
		errors: {
			...state.errors,
			[direction]: error?.status === 401 || error?.status === 403 ? "Owner access is required to load timeline events." : direction === "older" ? "Older timeline events could not be loaded." : direction === "newer" ? "New timeline events could not be loaded." : direction === "around" ? "Replay timeline events could not be loaded." : "Timeline events could not be loaded."
		},
...
export function timelineBehindLive(state, liveSeq, liveEventCount) {
	...
	const loadedSeq = state?.rows?.reduce((max, row) => finiteSeq(row?.seq) ? Math.max(max, Number(row.seq)) : max, -1) ?? -1;
...
function retainedSegments(state, page, direction, replace) {
	...
	if (replace) segments = [incoming];
		...
		if (direction === "older") {
			...
			segments = [{
				...
				cursors: {
					...first.cursors,
					older: page.cursors.older
				},
				hasMore: {
					...first.hasMore,
					older: page.hasMore.older
				}
			}, ...segments.slice(1)];
		} else {
			...
			segments = [...segments.slice(0, -1), {
				...
				cursors: {
					...last.cursors,
					newer: page.cursors.newer
				},
				hasMore: {
					...last.hasMore,
					newer: page.hasMore.newer
				}
			}];
		}
	...
	if (segments.length === 1 && segments[0].rows.length > state.retainLimit) {
		...
		segments = [{
			...segment,
			rows: direction === "older" ? segment.rows.slice(0, state.retainLimit) : segment.rows.slice(-state.retainLimit)
		}];
	}
	return segments;
}
export function mergeTimelinePage(state, payload, { direction = "tail" } = {}) {
	...
	const disjoint = previous.length === 0 || page.rows.length === 0 || direction === "older" && incomingLast < previousFirst || direction === "newer" && incomingFirst > previousLast;
	...
	const cursors = {
		older: first.cursors.older,
		newer: last.cursors.newer
	};
	const hasMore = {
		older: first.hasMore.older,
		newer: last.hasMore.newer
	};
	...
	const totalUnread = !followingTail && state.parkedTotalEvents != null && nextTotalEvents != null ? Math.max(0, nextTotalEvents - state.parkedTotalEvents) : null;
	...
	const tornCountMismatch = page.tornTail && observedEventCount != null && nextTotalEvents != null && observedEventCount > nextTotalEvents;
	const countUnread = !followingTail && !tornCountMismatch && parkedEventCount != null && observedEventCount != null ? Math.max(0, observedEventCount - parkedEventCount) : null;
	...
	return {
		...
		unreadUnknown: followingTail ? false : tornCountMismatch ? true : exactUnread != null ? false : state.unreadUnknown,
...
export function parkTimeline(state, liveSeq = null, liveEventCount = null) {
	...
	if (!state.followingTail && (state.parkedFrontier != null || state.parkedEventCount != null || state.parkedTotalEvents != null)) return state;
	...
	return {
		...
		unread: 0,
		unreadUnknown: false,
		parkedFrontier: frontier >= 0 ? frontier : null,
		parkedTotalEvents: state.totalEvents,
		parkedEventCount: eventCount,
...
export function noticeTimelineFrontier(state, liveSeq, liveEventCount = null) {
	...
	const parked = finiteSeq(state.parkedFrontier) ? Number(state.parkedFrontier) : finiteSeq(liveSeq) ? Number(liveSeq) : null;
	...
	const nonAdvancingCount = baseline != null && eventCount != null && eventCount > baseline && finiteSeq(state.parkedFrontier) && finiteSeq(liveSeq) && Number(liveSeq) <= Number(state.parkedFrontier);
	const inferredTornTail = state.tornTail || nonAdvancingCount;
	const tornCountMismatch = inferredTornTail && eventCount != null && state.totalEvents != null && eventCount > state.totalEvents;
	const exactUnread = !tornCountMismatch && !nonAdvancingCount && baseline != null && eventCount != null ? Math.max(0, eventCount - baseline) : null;
	return {
		...state,
		parkedFrontier: parked,
		liveEventCount: eventCount ?? state.liveEventCount,
		windowAtTail: false,
		tornTail: inferredTornTail,
		unread: exactUnread ?? state.unread,
		unreadUnknown: exactUnread == null
	};
}
...
export function timelinePagePath(runId, { direction = "tail", cursor = null, generation = null, anchorSeq = null, limit = TIMELINE_PAGE_SIZE, byteLimit = null } = {}) {
	if (!TIMELINE_DIRECTIONS.has(direction)) throw new TypeError(`Unknown timeline direction: ${direction}`);
	const params = new URLSearchParams({
		direction,
		limit: String(Math.max(1, Math.min(500, Number(limit) || TIMELINE_PAGE_SIZE)))
	});
	if (cursor) params.set("cursor", String(cursor));
	if (generation) params.set("generation", String(generation));
	if (byteLimit != null) params.set("byte_limit", String(Math.max(1024, Math.min(524288, Number(byteLimit) || 262144))));
	if (direction === "around") {
