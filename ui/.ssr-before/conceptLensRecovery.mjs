import { abandonConceptLens, abandonRecoveredConceptLens, awaitConceptLensRecoveryJob, createIdempotencyKey, getConceptLensRecovery, jobAwait, submitConceptLens } from "./api.mjs";
...
const safeText = (value, max = 200) => typeof value === "string" && value.length > 0 && value.length <= max && !/[\u0000-\u001f\u007f]/.test(value);
const validGeneration = (value) => typeof value === "string" && GENERATION.test(value);
const validRequestId = (value) => value === null || typeof value === "string" && REQUEST_ID.test(value);
const validPrompt = (value) => typeof value === "string" && value === value.trim() && !!value && value.length <= 800 && new TextEncoder().encode(value).length <= 2048;
const plausibleTime = (value) => Number.isSafeInteger(value) && value >= 15778368e5 && value <= Date.now() + 3e5;
...
const storageError = (cause, code = "CONCEPT_LENS_STORAGE_UNAVAILABLE") => Object.assign(new Error(code === "CONCEPT_LENS_INTENT_CONFLICT" ? "A different paid concept-lens request is already saved for this run." : "Paid concept-lens recovery storage is unavailable."), {
	code,
	cause
});
const validateIntent = (payload, runId) => {
	if (!payload || typeof payload !== "object" || Array.isArray(payload) || Object.keys(payload).some((key) => !KEYS.has(key)) || Object.keys(payload).length !== KEYS.size || payload.version !== 1 || payload.runId !== String(runId) || !safeText(payload.runId, 255) || !validGeneration(payload.generation) || !validPrompt(payload.prompt) || !UUID.test(payload.idempotencyKey) || !STATES.has(payload.state) || !(payload.jobId === null || JOB_ID.test(payload.jobId)) || !validRequestId(payload.requestId) || payload.state === "running" && !payload.jobId || payload.state === "running" && !payload.requestId || ["ready", "submitting"].includes(payload.state) && (payload.jobId !== null || payload.requestId !== null) || payload.jobId !== null && payload.requestId === null || !plausibleTime(payload.updatedAt)) throw storageError();
	return payload;
};
...
export function acquireConceptLensIntent(runId, generation, prompt, storage = undefined) {
	const backing = storageTarget(storage);
	if (!backing || !safeText(String(runId || ""), 255) || !validGeneration(generation) || !validPrompt(prompt)) throw storageError();
...
export function clearConceptLensIntent(runId, idempotencyKey, storage = undefined) {
	...
	try {
		...
		try {
			backing.removeItem(storageKey(runId));
		} catch {}
		return true;
	} catch (cause) {
		throw storageError(cause);
...
const validAbandonmentReceipt = (result) => result?.code !== "concept_lens_abandoned" || result.ok === false && ABANDON_REASONS.has(result.reason) && result.abandoned === true && result.resolved === true && result.provider_outcome === "unknown" && result.billing_status === "unknown" && safeText(result.warning, 500);
const normalizeJobResult = (value) => {
	...
	return value.status === "done" && typeof value.complete === "boolean" ? {
		...value,
		status: value.complete ? "complete" : "partial"
	} : value;
};
const validateReceipt = (value, intent) => {
	...
	if (result.ambiguous === true) {
		...
		if (result.code === "job_identity_mismatch" || Object.hasOwn(result, "generation") && result.generation !== intent.generation || Object.hasOwn(result, "request_id") && (typeof result.request_id !== "string" || !REQUEST_ID.test(result.request_id) || intent.requestId && result.request_id !== intent.requestId || !intent.requestId && !Object.hasOwn(result, "generation")) || Object.hasOwn(result, "job_id") && (!JOB_ID.test(result.job_id || "") || result.job_id !== intent.jobId)) {
	...
	if (result.status === "running") {
		if (!JOB_ID.test(result.job_id || "") || result.generation !== intent.generation || typeof result.request_id !== "string" || !REQUEST_ID.test(result.request_id) || intent.requestId && result.request_id !== intent.requestId) {
			throw protocolError("Invalid running paid concept-lens receipt.");
		}
		return result;
	}
	if (result.generation !== intent.generation || typeof result.request_id !== "string" || !REQUEST_ID.test(result.request_id) || intent.requestId && result.request_id !== intent.requestId) {
...
const exactKeys = (value, names) => {
	...
	return Object.keys(value).length === expected.length && expected.every((name) => Object.hasOwn(value, name));
};
const safeSequence = (value, minimum = 0) => Number.isSafeInteger(value) && value >= minimum;
const terminalAfterClaim = (value, startedSeq) => safeSequence(value?.seq) && value.seq > startedSeq;
...
export function validateConceptLensRecovery(value, expectedGeneration) {
	if (!value || typeof value !== "object" || Array.isArray(value) || value.schema !== 1 || value.generation !== expectedGeneration || !validGeneration(value.generation) || !RECOVERY_STATES.has(value.state)) {
	...
	if (value.state === "conflict") {
		if (!exactKeys(value, "schema generation state code message") || !safeText(value.code) || !safeText(value.message, 500) || !value.code.startsWith("concept_lens_recovery_")) {
	...
	if (!REQUEST_ID.test(value.request_id || "") || !safeSequence(value.started_seq) || !safeSequence(value.input_seq, -1) || value.input_seq >= value.started_seq) {
	...
	if (value.state === "running") {
		if (!exactKeys(value, `${identityKeys} job_id status`) || !JOB_ID.test(value.job_id || "") || !["running", "done"].includes(value.status)) {
...
export async function discoverConceptLensRecovery(runId, generation, options = {}) {
	return validateConceptLensRecovery(await getConceptLensRecovery(runId, generation, options), generation);
}
...
export async function pollDiscoveredConceptLens(runId, recovery, { signal, pollIntervalMs = 1e3, pollTimeoutMs = 45e3, requestTimeoutMs = 8e3 } = {}) {
	...
	const result = validateReceipt(await awaitConceptLensRecoveryJob(inspected.job_id, {
		signal,
		intervalMs: Math.max(0, pollIntervalMs),
		timeoutMs: Math.max(1, pollTimeoutMs),
		maxTransientErrors: 3,
		requestTimeoutMs
	}), {
		generation: inspected.generation,
		requestId: inspected.request_id,
		jobId: inspected.job_id
	});
	if (result.ambiguous !== true && result.status !== "running" && !terminalAfterClaim(result, inspected.started_seq)) {
		throw recoveryProtocolError("Paid concept-lens job terminal does not follow its durable claim.");
	}
	return result;
}
export async function resolveOrphanedConceptLens(runId, recovery, resolutionIdempotencyKey, options = {}) {
	const inspected = validateConceptLensRecovery(recovery, recovery?.generation);
	...
	const response = validateReceipt(await abandonRecoveredConceptLens(runId, inspected.generation, inspected.request_id, inspected.started_seq, {
		...options,
...
export async function requestConceptLens(runId, intent, { signal, onReceipt, pollIntervalMs = 1e3, pollTimeoutMs = 45e3, requestTimeoutMs = 8e3 } = {}) {
...
export async function abandonUnknownConceptLens(runId, intent, options = {}) {
	...
	const response = validateReceipt(await abandonConceptLens(runId, intent.generation, intent.requestId, {
		...options,
		idempotencyKey: intent.idempotencyKey
	}), intent);
