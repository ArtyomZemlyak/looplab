import { getRunCommand, isTransientCommandReadError, runCommand } from "./api.mjs";
const INTENTS = {
	stop: () => ({
		type: "pause",
		data: {}
	...
	approve: (arg, nodeGeneration) => ({
		...
		data: {
			node_id: Number(arg),
			generation: Number(nodeGeneration)
		}
	})
};
...
export function assistantDirectIntent(action, arg = null, nodeGeneration = null) {
	...
	if (action === "approve" && (arg == null || !Number.isSafeInteger(Number(arg)) || Number(arg) < 0)) {
		throw new Error("Approve requires a valid node id");
	}
	if (action === "approve" && (nodeGeneration == null || !Number.isSafeInteger(Number(nodeGeneration)) || Number(nodeGeneration) < 0)) {
...
export function submitAssistantDirect(runId, action, arg, idempotencyKey, { expectedGeneration, nodeGeneration = null, execute = runCommand, onRecord = null } = {}) {
	...
	request.finally(() => {
		if (activeSubmissions.get(key) === request) activeSubmissions.delete(key);
	}).catch(() => {});
	return request;
}
...
export const assistantRunChanged = (previousRunId, nextRunId) => String(previousRunId ?? "") !== String(nextRunId ?? "");
...
export function assistantStorageFailureOwnsLock(failure, lock) {
	return failure?.record?.error?.code === "command_storage_unavailable" && lock?.source === "assistant" && !lock.commandId && String(failure?.runId ?? "") === String(lock?.runId ?? "") && failure?.name === lock?.action && failure?.idempotencyKey === lock?.idempotencyKey && failure?.expectedGeneration === lock?.expectedGeneration;
}
