...
import { loadRunStartOverIntent } from "./runStartOverRecovery.mjs";
...
export function historyMatches(resource, requestedSeq, requestedGeneration = null) {
	return resource?.requestedSeq === Number(requestedSeq) && resource?.requestedGeneration === historyGeneration(requestedGeneration);
}
...
function announce(runId) {
	...
	window.dispatchEvent(new CustomEvent("ll:run-access", { detail: {
		runId,
		...getRunAccess(runId)
	} }));
}
...
export function listStartOverRunAccesses() {
	return [...accessByRun.entries()].filter(([, access]) => access?.mode === "start-over").map(([runId, access]) => ({
		runId,
		kind: access.recoveryKind || "active"
	}));
}
...
export function assertRunMutationAllowed(path, { allowModes = [] } = {}) {
	...
	const error = new Error(review ? "This review link is read-only" : staleLink ? "This diagnostic link targets an earlier run generation — open the current generation before acting" : startOver ? "Start over is unresolved — retry or finish that exact request before changing the run" : `Historical snapshot seq ${access.seq} is read-only — return to live to act`);
	error.code = review ? "REVIEW_READ_ONLY" : staleLink ? "STALE_LINK_READ_ONLY" : startOver ? "START_OVER_RECOVERY_LOCK" : "HISTORICAL_READ_ONLY";
	error.runId = runId;
