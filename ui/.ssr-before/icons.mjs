...
const spriteUrl = "./looplab-icons-v1.svg";
import { jsx as _jsx } from "react/jsx-runtime";
const OP_ICON_NAMES = new Set("flag trending bug confluence gitbranch target dot search doc alert gear user bot bolt star pause play stop replay sliders chevron-up chevron-down chat bell folder clip map compass bulb check cross pencil link download printer crown list".split(" "));
...
function ensureSprite() {
	if (({ SSR: true, DEV: false, PROD: true, MODE: "production" }).SSR || typeof document === "undefined" || !document.body || document.getElementById(SPRITE_DOM_ID) || spriteLoad) return;
	if (typeof fetch !== "function") return;
	spriteLoad = fetch(spriteUrl, { credentials: "same-origin" }).then((response) => {
		if (!response.ok) throw new Error("icon sprite unavailable");
		return response.text();
	}).then((markup) => {
		if (!/^<svg[\s>]/.test(markup) || /<(?:script|foreignObject|image|use)\b/i.test(markup)) {
...
export function OpIcon({ name, size = 14, className }) {
	...
	return /* @__PURE__ */ _jsx("svg", {
		...
		"aria-hidden": "true",
		focusable: "false",
		children: /* @__PURE__ */ _jsx("use", { href: `#${glyph}` })
	});
}
