import { boundedAtlasText, projectResearchAtlasSource } from "./api.mjs";
...
const thinProjectionTotal = (atlas, included, fallback) => {
	...
	return Number.isSafeInteger(total) && total >= included && atlas.thin_coverage_omitted === total - included ? total : fallback;
};
const normalizeConceptSource = (atlas) => {
	...
	const valid = typeof complete === "boolean" && typeof receipt.source_store_complete === "boolean" && counts.every((value) => value >= 0) && store.every((value) => value >= 0) && counts[1] <= counts[0] && store[1] <= store[0] && store[1] === store[2] + store[3] + store[4] && receipt.source_store_complete === (store[1] === 0) && complete === (counts[0] === 0 && receipt.source_store_complete) && !(complete && counts.some(Boolean));
...
const normalizeReadSegment = (value) => {
	...
	if (typeof segment.read_complete !== "boolean" || values.some((item) => item < 0) || total !== retained + quarantined || quarantined !== malformed + invalid || segment.read_complete !== (quarantined === 0)) return null;
	return [
		segment.read_complete,
		total,
		retained,
		quarantined,
		malformed,
...
const normalizeClaimSource = (value) => {
	...
	const digestValid = typeof digest === "string" && (known ? /^[0-9a-f]{64}$/.test(digest) : digest === "");
	const readComplete = lessons[0] && research[0];
	const consistent = digestValid && (known ? receipt.read_complete === readComplete && (!receipt.research_source_complete || research[0]) && receipt.source_complete === (lessons[0] && receipt.research_source_complete) : receipt.source_complete === false && receipt.read_complete === false && receipt.research_source_complete === false);
...
const claimSection = (envelope, rows, hasRows = rows.length > 0) => {
	...
	const coherent = hasEnvelope && rows.every((row) => {
		...
		return Object.hasOwn(item, "claim_source") && sameReceipt(normalizeClaimSource(item.claim_source), source);
	});
	return {
		present: hasEnvelope || hasRows,
		source: coherent ? source : null
	};
};
const combinedClaimSource = (sections) => {
	...
	return first?.snapshot_digest && visible.every((section) => sameReceipt(section.source, first)) ? first : null;
};
const claimSourceView = (receipt) => {
	if (!receipt) return unknownClaimSource();
	return { status: receipt.receipt_known ? receipt.source_complete ? "complete" : "partial" : "unknown" };
};
...
export function isValidAtlasSourceEnvelope(key, value) {
	...
	if (key === "atlas") {
		return Array.isArray(value.explored) && Array.isArray(value.thin_coverage) && Array.isArray(value.contradictions) && value.explored.length <= ATLAS_TRANSPORT_LIMITS.atlas && value.thin_coverage.length <= ATLAS_TRANSPORT_LIMITS.atlas && value.contradictions.length <= ATLAS_TRANSPORT_LIMITS.atlas;
	}
	if (key === "claims") {
		return Array.isArray(value.claims) && value.claims.length <= ATLAS_TRANSPORT_LIMITS.claims;
	}
	if (key === "conceptCuration" || key === "claimCuration") {
		if (!Array.isArray(value.entries) || value.entries.length > ATLAS_TRANSPORT_LIMITS.curation || !value.entries.every((entry) => entry && typeof entry === "object" && !Array.isArray(entry))) {
			return false;
		}
		...
		return value.v === 1 && value.status === "complete" && value.complete === true && Number.isSafeInteger(value.n) && value.n >= value.entries.length && Number.isSafeInteger(value.limit) && value.limit >= 1 && value.limit <= 200 && value.entries.length <= value.limit;
	}
	return false;
}
...
export function reconcileAtlasSourceStatuses(previousValue, successfulValue, loadedAt, attemptedKeys = ATLAS_SOURCE_KEYS) {
	...
	for (const key of ATLAS_SOURCE_KEYS) {
		...
		statuses[key] = ["current", "retained-stale"].includes(prior.state) ? {
			...prior,
			state: "retained-stale"
		} : failed;
	}
	return statuses;
}
const take = (value, max) => list(value).slice(0, max);
const textList = (value, max, textMax = 180) => take(value, max).map((item) => boundedAtlasText(item, textMax)).filter(Boolean);
const comparisonContextText = (value, max) => typeof value === "string" ? boundedAtlasText(value, max) : "";
const normalizeRun = (value) => {
	...
	const task = comparisonContextText(row.task_id, 180) || comparisonContextText(row.task, 220);
	const scope = comparisonContextText(row.scope, 220) || comparisonContextText(row.task_scope, 220);
	...
	return {
		...
		optimizationOrientation: discloseMetric ? rawOrientation === "max" ? "maximize" : "minimize" : "",
...
const normalizeConcept = (value) => {
	...
	const runs = take(row.runs, ATLAS_RENDER_LIMITS.conceptRuns).map(normalizeRun).filter((run) => run.runId);
	return {
		concept,
...
const normalizeClaim = (value, source = unknownClaimSource()) => {
	...
	const mixed = nSupport > 0 && (contradicts.length > 0 || row.epistemic === "mixed" || nOppose > 0);
	const evidenceEpistemic = mixed ? "mixed" : nSupport > 0 ? "supported" : nOppose > 0 ? "refuted" : "inconclusive";
	...
	const epistemic = ["supported", "refuted"].includes(evidenceEpistemic) && source.status !== "complete" ? "inconclusive" : evidenceEpistemic;
	return {
...
export function mergeCurationLogs(conceptValue, claimValue) {
	const sources = [{
		kind: "concept",
		envelope: record(conceptValue)
	}, {
		kind: "claim",
		envelope: record(claimValue)
	}];
	...
	const entries = sources.flatMap(({ kind, envelope }) => list(envelope.entries).length ? [{
		...record(envelope.entries[0]),
		kind
	}] : []);
	const total = sources.reduce((sum, { envelope }) => sum + Math.max(count(envelope.n), list(envelope.entries).length), 0);
	return {
		entries,
		n: total
	};
}
export function mergeResearchAtlasPayload(previousValue, successfulValue, { allowPortfolioSwitch = false } = {}) {
	let previous = record(previousValue);
	const successful = record(successfulValue);
	const incomingIdentities = new Set(ATLAS_SOURCE_KEYS.filter((key) => Object.hasOwn(successful, key)).map((key) => atlasPortfolioId(successful[key])).filter(Boolean));
	...
	const source = (key) => Object.hasOwn(successful, key) ? record(successful[key]) : record(previous[key]);
	return {
		atlas: source("atlas"),
...
export function buildResearchAtlasView(atlasValue, claimsValue, curationValue) {
	...
	const claimReceipt = combinedClaimSource([claimSection(atlas, contradictionRows, rawContradictions.length > 0), claimSection(claimsEnvelope, claimRows, rawClaims.length > 0)]);
	...
	const thinTotal = thinProjectionTotal(atlas, rawThin.length, rawThin.length > ATLAS_RENDER_LIMITS.thin ? rawThin.length : thin.length);
	const invalidRows = {
	...
	return {
		...
		curation,
		invalidRows,
		hiddenConcepts: Math.max(0, totals.concepts - concepts.length, rawConcepts.length - ATLAS_RENDER_LIMITS.concepts),
		hiddenThin: Math.max(0, thinTotal - thin.length, rawThin.length - ATLAS_RENDER_LIMITS.thin),
		hiddenContradictions: Math.max(0, totals.contested - contradictions.length, rawContradictions.length - ATLAS_RENDER_LIMITS.contradictions),
		hiddenClaims: Math.max(0, totals.claims - claims.length, rawClaims.length - ATLAS_RENDER_LIMITS.claims),
		hiddenCuration: Math.max(0, totals.curation - curation.length, rawCuration.length - ATLAS_RENDER_LIMITS.curation),
		empty: totals.runs === 0 && totals.concepts === 0 && totals.claims === 0 && totals.contested === 0 && totals.curation === 0 && concepts.length === 0 && thin.length === 0 && contradictions.length === 0 && claims.length === 0 && curation.length === 0
	};
}
