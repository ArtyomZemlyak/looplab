...
export function filterSettingsGroups(groups, { mode = "all", query = "", only, hideSecret = false } = {}) {
	const needle = normalizeSettingsQuery(query);
	const allowedGroups = only ? new Set(only) : null;
	return groups.filter((group) => !allowedGroups || allowedGroups.has(group.title)).map((group) => ({
		...group,
...
const opaqueRevision = (value) => typeof value === "string" && value.length > 0 && value.length <= 256 && !/[\u0000-\u001f\u007f]/.test(value);
const configRevision = (value) => typeof value === "string" && /^[0-9a-f]{64}$/.test(value);
const resourceError = () => {
	throw new TypeError("Invalid settings resource");
};
...
export function validateCredentialState(value) {
	if (!settingsRecord(value) || Object.keys(value).length > 8 || !CREDENTIAL_SOURCES.has(value.source) || !CREDENTIAL_STATUSES.has(value.status) || typeof value.stored !== "boolean" || typeof value.effective !== "boolean" || typeof value.active !== "boolean" || typeof value.clearable !== "boolean" || value.clearable !== value.stored || value.source === "none" && (value.stored || value.effective || value.active || value.status !== "missing") || value.source === "stored" && (!value.stored || ["missing", "ambient_override"].includes(value.status)) || value.active && !value.effective || value.status === "active" && (!value.effective || !value.active) || value.status === "missing" && (value.effective || value.active) || value.status === "incomplete" && (value.effective || value.active || value.source === "none") || ["unbound", "endpoint_mismatch"].includes(value.status) && (!value.effective || value.active) || value.status === "ambient_override" && (!["environment", "dotenv"].includes(value.source) || !value.stored || !value.effective || !value.active || !value.clearable)) resourceError();
	return value;
}
function boundedJson(value, budget = { nodes: 0 }, depth = 0) {
	...
	if (typeof value === "string") {
		if (value.length > 1e6 || /[\u0000]/.test(value)) resourceError();
		return;
	}
...
function validateAgentControl(value, schema) {
	...
	for (const [key, granted] of Object.entries(value)) {
		if (!settingKey(key) || !Array.isArray(granted) || granted.length > roles.size || new Set(granted).size !== granted.length || granted.some((role) => typeof role !== "string" || !roles.has(role))) resourceError();
	}
}
function validateKnownSetting(field, value) {
	...
	if (field.type === "bool") {
		...
		if (!Array.isArray(value) || value.length > 4096 || value.some((item) => typeof item !== "string" || item.length > 16e3)) resourceError();
	} else if (field.type === "secret") {
		if (value !== "***") resourceError();
	} else if (typeof value !== "string" || value.length > 1e6 || /[\u0000]/.test(value)) {
		resourceError();
	}
}
export function validateSettingsRecord(value, schema, { complete = false, allowMissingSecret = false, allowEmpty = false } = {}) {
	if (!settingsRecord(value) || Object.keys(value).length > 512 || !allowEmpty && Object.keys(value).length === 0) resourceError();
	let known = 0;
	for (const [key, item] of Object.entries(value)) {
...
export function validateSettingsResource(value, schema) {
	if (!settingsRecord(value) || Object.keys(value).length > 16 || !opaqueRevision(value.settings_revision) || !opaqueRevision(value.secret_revision)) {
		resourceError();
	}
	validateCredentialState(value.credential);
	validateSettingsRecord(value.settings, schema, { complete: true });
	validateSettingsRecord(value.defaults, schema, {
		complete: true,
...
export function validateSettingsSaveAck(value, schema) {
	if (!settingsRecord(value) || value.ok !== true || Object.keys(value).length > 16 || !opaqueRevision(value.settings_revision) || !opaqueRevision(value.secret_revision)) resourceError();
	validateCredentialState(value.credential);
	validateSettingsRecord(value.settings, schema, { complete: true });
	validateSettingsRecord(value.overrides, schema, { allowEmpty: true });
	return value;
...
export function validateSecretSaveAck(value, expectedKey) {
	if (!settingsRecord(value) || value.ok !== true || value.key !== expectedKey || typeof value.set !== "boolean" || !opaqueRevision(value.settings_revision) || !opaqueRevision(value.secret_revision) || Object.keys(value).length > 8) resourceError();
...
export function splitRunConfigPayload(payload, schema = null) {
	...
	if (schema && (Object.keys(meta).length > metaKeys.size || Object.keys(meta).some((key) => !metaKeys.has(key)) || !configRevision(meta.config_revision))) resourceError();
	const cleanNames = (value) => {
		if (schema && (!Array.isArray(value) || value.length > 512 || value.some((name) => !settingKey(name)) || new Set(value).size !== value.length)) {
			resourceError();
		}
		return Array.isArray(value) ? [...new Set(value.filter((name) => typeof name === "string" && name.length > 0))] : [];
	};
	...
	const readOnlyFields = new Set([...pinnedFields, ...cleanNames(meta.run_read_only_fields)]);
	return {
		config,
...
export function validateRunConfigSaveAck(value, schema) {
	if (!settingsRecord(value) || value.ok !== true || typeof value.engine_running !== "boolean" || typeof value.trust_gate_event_appended !== "boolean" || !Array.isArray(value.changed) || !Array.isArray(value.normalized_pinned) || Object.keys(value).length > 16) resourceError();
	for (const names of [value.changed, value.normalized_pinned]) {
		if (names.length > 512 || names.some((name) => !settingKey(name)) || new Set(names).size !== names.length) resourceError();
	}
...
export function reconcileUnknownRecord(current, submitted, authoritative, uncertainKeys = []) {
	const currentRecord = current && typeof current === "object" && !Array.isArray(current) ? current : {};
	const submittedRecord = submitted && typeof submitted === "object" && !Array.isArray(submitted) ? submitted : {};
	const authoritativeRecord = authoritative && typeof authoritative === "object" && !Array.isArray(authoritative) ? authoritative : {};
