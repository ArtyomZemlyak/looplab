...
import { fmt, isSweep, operatorMeta } from "./util.mjs";
import { nodeTheme } from "./conceptId.mjs";
import { activeNodeMap, nodeIsActive } from "./nodeProjection.mjs";
import { normalizeRunReport, reportCoverageText, reportNarrativeCoverage } from "./reportModel.mjs";
	...
	ev.forEach((n) => {
		...
		if (best === null || bt(v, best.v)) {
			...
			steps.push({
				id: n.id,
				operator: n.operator,
				theme: nodeTheme(n, state),
				from: best ? best.v : null,
				to: v,
				delta: best ? v - best.v : null,
				params: n.idea?.params || {},
				rationale: n.idea?.rationale || "",
				diff: paramDiff(n, parent),
...
export const themeEffectiveness = (nodes, dir, state = null) => rollup(nodes, dir, (node) => nodeTheme(node, state));
...
export function directionProfit(state) {
	...
	const baseline = (improvements(nodes, dir, state)[0] || {}).to ?? null;
	const byDirection = new Map(themeEffectiveness(nodes, dir, state).map((row) => [row.key, row]));
	const directions = [...new Set(Object.values(nodes).sort((a, b) => a.id - b.id).map((node) => nodeTheme(node, state)).filter(Boolean))];
...
export const optimizationLabel = (direction) => direction === "max" ? "maximize" : direction === "min" ? "minimize" : "unknown";
...
export function mergeSummary(node, nodes, state = null) {
	...
	return node.parent_ids.map((pid) => {
		...
		return {
			parentId: pid,
			theme: nodeTheme(p, state),
			change: paramDiffLabel(paramDiff(p, gp))
		};
	});
}
...
export function hyperImportance(state) {
	...
	const nodes = Object.values(activeNodeMap(state.nodes || {}, state)).filter((n) => n.status === "evaluated" && n.metric != null && n.feasible !== false);
	const keys = new Set();
	nodes.forEach((n) => Object.entries(n.idea?.params || {}).forEach(([k, v]) => {
...
export function nodeChip(node, nodes, state = null) {
	...
	if (isSweep(node)) {
		...
		const tok = (k) => {
			...
			return vs.length > 1 && vs.every((v) => typeof v === "number") ? `${k}∈[${fmt(Math.min(...vs))}…${fmt(Math.max(...vs))}]` : k;
		};
	...
	if (!parent) {
		...
		return what ? `baseline · ${what}` : "baseline";
	}
...
export function analyze(state) {
	...
	return {
		...
		totalGain: steps.length > 1 ? dir === "max" ? steps[steps.length - 1].to - steps[0].to : steps[0].to - steps[steps.length - 1].to : 0,
...
export function trustCaveats(state, best) {
	...
	if (best && hacks.some((h) => h.node_id === best.id)) out.push({
		kind: "reward-hack",
		severity: "alarm",
		text: "champion flagged as a possible reward-hack",
		panel: "trust"
	});
	else if (hacks.length) out.push({
		kind: "reward-hack",
		severity: "warn",
		text: `${hacks.length} node(s) flagged as possible reward-hacks`,
		panel: "trust"
	});
	if (state.leakage?.leak) out.push({
		kind: "leakage",
		severity: "alarm",
		text: "data-leakage scan flagged this run",
		panel: "data"
	});
	if ((state.drifts || []).length) out.push({
		kind: "drift",
		severity: "warn",
		text: `${state.drifts.length} metric-drift divergence(s) caught`,
		panel: "trust"
	});
	const infeasible = Object.values(activeNodeMap(state.nodes || {}, state)).filter((n) => isEvaluated(n) && n.feasible === false);
	if (infeasible.length) out.push({
		kind: "infeasible",
		severity: "warn",
		text: `${infeasible.length} evaluated node(s) violated a constraint`,
		panel: "trust"
	});
	if (best && best.confirmed_mean == null) out.push({
		kind: "single-seed",
		severity: "warn",
		text: "champion is single-seed (not multi-seed confirmed)",
		panel: "trust"
	});
	return out;
}
...
export function verdict(state, a) {
	...
	if (!best || a.finalBest == null) {
		return {
			outcome: "none",
			robustness: "n/a",
			trust: caveats.length ? "caveats" : "unverified",
			best,
			caveats,
	...
	if (best.confirmed_mean != null && (best.confirmed_seeds || 0) >= 2) {
		...
		robustness = sd <= Math.abs(gain) || sd / mean < .1 ? "robust" : "fragile";
	}
	// trust rollup
	...
	let headline;
	if (outcome === "improved") {
		const rob = robustness === "robust" ? `robust across ${best.confirmed_seeds} seeds` : robustness === "fragile" ? `but the multi-seed spread is wide` : `single-seed so far`;
		headline = `Improved the metric by ${gainStr} over baseline — champion #${best.id} is ${rob}` + (trust === "suspect" ? "; the win is flagged, treat with caution." : trust === "caveats" ? " (with caveats)." : "; no trust flags are recorded, but detector coverage is not fully verified.");
...
const reportContext = (context) => ({
	...
	snapshotSeq: Number.isSafeInteger(context?.snapshotSeq) && context.snapshotSeq >= 0 ? context.snapshotSeq : null
});
const coverageRecord = (coverage) => ({
	status: coverage.status,
	at_node: coverage.atNode,
	current_node_count: coverage.currentNodeCount,
	stale_by: coverage.staleBy,
	basis: "node_count",
	full_state_freshness: "unknown"
});
export function buildModelCard(state, _best = null, context = {}) {
	...
	return {
		...
		champion: champion ? {
			...
			metric: champion.confirmed_mean ?? champion.metric,
			confirmed: champion.confirmed_mean != null,
			params: champion.idea?.params || {},
			lineage: champion.parent_ids || []
		} : null,
		verdict: v.headline,
		...
		deterministic_trust: {
			status: v.trust,
			caveats: v.caveats.map((caveat) => caveat.text)
		},
		counts: {
			nodes: nodeCount,
			evaluated: a.nEval
		...
		agent_narrative: rep ? {
			...
			verdict: rep.verdict,
			summary: rep.summary,
			champion_summary: rep.champion_summary,
			what_worked: rep.what_worked,
			learnings: rep.learnings,
			what_didnt: rep.what_didnt,
			next_directions: rep.next_directions,
			caveats: rep.caveats,
			coverage: coverageRecord(coverage),
			provenance: {
				published_event_seq: rep.published_seq,
				published_at: rep.published_at,
				published_at_unit: "unix_seconds",
				trigger: rep.trigger || null,
				node_count_at_publication: rep.at_node
			}
		} : null,
		provenance: {
			authority: "events.jsonl",
			run_generation: ctx.generation,
...
export function toMarkdown(state, _best, context = {}) {
	...
	if (rep) {
		...
		const receipt = [
			rep.published_seq != null ? `event #${rep.published_seq}` : "event unknown",
			rep.published_at != null ? new Date(rep.published_at * 1e3).toISOString() : "time unknown",
			rep.trigger ? `trigger ${rep.trigger}` : "trigger unknown"
		].join(" · ");
		quote(`**Published:** ${receipt}`);
		if (rep.headline) {
			L.push(">");
			quote(`**Agent headline:** ${rep.headline}`);
		}
		if (rep.verdict || rep.summary) {
			L.push(">");
			quote(rep.verdict || rep.summary);
		}
		const advisoryLists = [
			["Agent caveats", rep.caveats],
			["Champion note", rep.champion_summary ? [rep.champion_summary] : []],
