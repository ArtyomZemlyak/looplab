...
import { canonicalId, normalizeConceptId } from "./conceptId.mjs";
...
export function chipsAtPath(nodeConcepts = {}, rename = {}, path = "") {
	...
	const chips = [...counts.entries()].map(([id, set]) => ({
		id,
		label: id.split("/").pop(),
		count: set.size
	})).sort((a, b) => a.id < b.id ? -1 : a.id > b.id ? 1 : 0);
	if (here && here.size) chips.push({
		id: path,
...
function addSelection(selected, nextKey, rename = {}, toggle = false) {
	...
	const kept = selected.filter((key) => {
		...
		return !(next.id === value.id || next.id.startsWith(value.id + "/") || value.id.startsWith(next.id + "/"));
	});
	return [...kept, (next.exact ? "=" : "") + next.id];
}
export const addConceptSelection = (selected = [], nextKey, rename = {}) => addSelection(selected, nextKey, rename, false);
export const toggleConceptSelection = (selected = [], nextKey, rename = {}) => addSelection(selected, nextKey, rename, true);
...
export function parseConceptTagsInput(text = "") {
	...
	for (const token of tokens) {
		...
		if (concepts.length >= MAX_NODE_CONCEPTS) {
			dropped += 1;
			continue;
		}
		seen.add(id);
