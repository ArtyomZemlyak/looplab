...
export const ASSISTANT_MODES = [
	{
		id: "plan",
		label: "Plan",
		hint: "read-only — inspect & propose (safe)"
	},
	{
		id: "default",
		label: "Ask",
		hint: "confirm every change"
	},
	{
		id: "acceptEdits",
		label: "Auto-edit",
		hint: "edits apply; commands and risky actions ask"
	},
	{
		id: "auto",
		label: "Auto",
		hint: "routine changes run; high-risk actions ask"
	}
];
...
const OPERATOR_META = {
	draft: {
		icon: "flag",
		label: "draft — initial baseline solution"
	},
	improve: {
		icon: "trending",
		label: "improve — hill-climb around best"
	},
	debug: {
		icon: "bug",
		label: "debug — repair a failed parent"
	},
	merge: {
		icon: "confluence",
		label: "merge — combine multiple parents"
	},
	refine_block: {
		icon: "target",
		label: "refine — ablation-driven tweak"
	},
	fork: {
		icon: "gitbranch",
		label: "fork — operator-seeded branch"
	},
	random: {
		icon: "dot",
		label: "random — exploratory sample"
	},
	exploit: {
		icon: "trending",
		label: "exploit — refine the leader"
	},
	greedy: {
		icon: "trending",
		label: "greedy — best-first step"
	},
	ablate: {
		icon: "target",
		label: "ablate — sensitivity probe"
	},
	manual: {
		icon: "flag",
		label: "manual — operator-authored experiment"
	}
};
...
export function sweepInfo(node) {
	...
	if (tr.length) {
		...
		return {
			...
			best: undefined,
			ok: series.length,
			failed: tr.length - series.length,
			series
		};
	}
	const sp = node?.idea?.space || {};
