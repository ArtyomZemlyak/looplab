import { emptyRunRouteState, encodeRunRouteState } from "./runRouteState.mjs";
...
const COPY = Object.freeze({
	approval: [
		"Experiment approval needed",
		"Review the exact pending experiment lifecycle.",
		"Review run"
	],
	approval_incomplete: [
		"Approval state needs inspection",
		"No safe approval target is available. Inspect Events.",
		"Open Events"
	],
	spec_approval: [
		"Evaluation spec approval needed",
		"Review the pending evaluation specification.",
		"Review spec"
	],
	failure_spike: [
		"Experiment failures need attention",
		"Several current experiments failed. Inspect the failure evidence.",
		"Inspect failures"
	],
	run_failed: [
		"Run failure needs attention",
		"Open the run for the failure evidence and recovery options.",
		"Inspect run"
	],
	budget_exhausted: [
		"Run budget reached",
		"The run completed after reaching a configured budget.",
		"View report"
	],
	finished: [
		"Run finished",
		"The final report and durable wrap-up are ready.",
		"View report"
	],
	stopped: [
		"Run finalized",
		"The run was intentionally stopped and its durable wrap-up is ready.",
		"View report"
	],
	finalization_stalled: [
		"Finalization needs recovery",
		"The engine stopped before durable wrap-up completed.",
		"Open Events"
	],
	stalled: [
		"Run engine stopped",
		"No engine process is advancing this run.",
		"Open Events"
	],
	train_monitor: [
		"Training looks broken",
		"The live-log monitor judged this training likely wasted. Open the run to inspect the log and verdict.",
		"Inspect training"
	],
	asha: [
		"ASHA rank warning",
		"Inspect the live curve. Automatic stopping requires peers at the same declared progress.",
		"Inspect experiment"
	],
	assistant_permission: [
		"Assistant approval needed",
		"Open Assistant to review the exact action and scope.",
		"Open Assistant"
	]
});
const safeRunId = (value) => typeof value === "string" && value.length > 0 && value.length <= 255 && !CONTROL_RE.test(value) ? value : "";
const safeContextText = (value) => typeof value === "string" && value.trim().length > 0 && value.trim().length <= 160 && !CONTROL_RE.test(value) ? value.trim() : "";
...
export function attentionHref(item) {
	if (!item || item.source !== "run" || !safeRunId(item.runId) || !GENERATION_RE.test(item.generation || "")) return null;
	const state = {
		...emptyRunRouteState(),
	...
	else if (item.kind === "finished" || item.kind === "budget_exhausted" || item.kind === "stopped") state.view = "report";
...
export function normalizeRunAttention(raw) {
	...
	const severity = typeof raw.severity === "string" && SEVERITIES.has(raw.severity) ? raw.severity : "";
	const runId = safeRunId(raw.run_id);
	const generation = typeof raw.generation === "string" && GENERATION_RE.test(raw.generation) ? raw.generation : "";
	const seq = safeInteger(raw.seq);
	if (!id || !kind || !severity || !runId || !generation || seq == null || typeof raw.active !== "boolean" || typeof raw.browser !== "boolean" || typeof raw.derived !== "boolean" || raw.stale !== undefined && typeof raw.stale !== "boolean") return null;
	...
	const item = {
		...
		runLabel,
		taskId,
		contextLabel: runLabel || runId
	};
	return {
		...item,
		href: attentionHref(item),
		needsAction: NEEDS_ACTION.has(kind)
	};
}
export function normalizePermissionAttention(raw, now = Date.now()) {
	...
	if (!requestId || !session || !created || !expires || expires * 1e3 <= now) return null;
...
export function sortAttentionItems(items) {
	...
	return [...unique.values()].sort((left, right) => {
		...
		return severity || right.created - left.created || rightSeq - leftSeq || right.id.localeCompare(left.id);
	});
}
export function normalizeAttentionSources(attentionPayload, permissionsPayload, now = Date.now()) {
	const runs = Array.isArray(attentionPayload?.items) ? attentionPayload.items.map(normalizeRunAttention).filter(Boolean) : [];
	const permissions = Array.isArray(permissionsPayload?.pending) ? permissionsPayload.pending.map((item) => normalizePermissionAttention(item, now)).filter(Boolean) : [];
	return sortAttentionItems([...runs, ...permissions]);
}
export const attentionIdValid = (value) => typeof value === "string" && (RUN_ID_RE.test(value) || /^perm_[0-9a-f]{16}$/.test(value));
