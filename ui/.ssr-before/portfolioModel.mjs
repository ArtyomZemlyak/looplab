...
const scalarText = (value, fallback, maximum = MAX_PORTFOLIO_SCOPE_ID_LENGTH) => typeof value === "string" && [...value].length <= maximum && !CONTROL.test(value) ? value : fallback;
...
function normalizeView(value) {
	...
	const name = scalarText(typeof value.name === "string" ? value.name.trim() : "", "", MAX_PORTFOLIO_VIEW_NAME_LENGTH);
	if (!name) return null;
	...
	const compare = Array.isArray(value.compare) ? [...new Set(value.compare.map((id) => scalarText(id, "", MAX_PORTFOLIO_RUN_ID_LENGTH)).filter(Boolean))].slice(0, MAX_COMPARE_RUNS) : [];
	return {
		name,
		...
		view: value.view === "map" || value.view === "compare" && compare.length > 1 ? value.view : "list",
		compare,
...
export const portfolioViewSignature = (view) => {
	...
	try {
		return JSON.stringify(exactViewState(view));
	} catch {
		return "";
	}
};
export function preparePortfolioViewSave(views, name, state, { replaceName = "" } = {}) {
	...
	if (!view || view.name !== requestedName) return {
		ok: false,
		code: "name",
		views: current
	};
...
const configSignature = (config, key) => {
	...
	try {
		return `value:${JSON.stringify(config[key])}`;
	} catch {
		return "invalid";
	}
};
...
export function configDifferences(items, maximum = 160) {
	...
	const keys = [...new Set(available.flatMap((item) => Object.keys(item.config)))].filter((key) => key !== "_looplab_config_meta").sort();
	const different = keys.filter((key) => new Set(available.map((item) => configSignature(item.config, key))).size > 1);
	return {
...
const metricObservation = (run) => {
	if (run?.best_confirmed != null) return {
		runId: run.run_id,
		phase: typeof run.best_confirmed === "number" && Number.isFinite(run.best_confirmed) ? "confirmed" : "invalid",
		value: typeof run.best_confirmed === "number" && Number.isFinite(run.best_confirmed) ? run.best_confirmed : null
	};
	if (run?.best_metric != null) return {
		runId: run.run_id,
		phase: typeof run.best_metric === "number" && Number.isFinite(run.best_metric) ? "raw" : "invalid",
		value: typeof run.best_metric === "number" && Number.isFinite(run.best_metric) ? run.best_metric : null
	};
	return {
		runId: run?.run_id,
		phase: "missing",
...
export function comparableRunRanking(runs = []) {
	...
	if (!task || !["min", "max"].includes(direction) || runs.some((run) => run.task_id !== task || run.direction !== direction)) {
