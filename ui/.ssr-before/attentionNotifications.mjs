import { apiPrefix } from "./api.mjs";
import { attentionIds, loadAttentionState, notificationsDisabledState, recordAttentionIds, saveAttentionState } from "./attentionStorage.mjs";
...
const eligibleIds = (items) => (items || []).filter((item) => item?.notifyEligible === true).map((item) => item.id);
export async function mutateAttentionState(mutator, { navigatorApi = globalThis.navigator, storage = undefined, prefix = apiPrefix(), now = Date.now(), broadcast = null, requireLock = false } = {}) {
	const update = async () => {
		...
		let state;
		try {
			state = await mutator(loaded.state);
		} catch {
			return {
				ok: false,
	...
	if (typeof navigatorApi?.locks?.request === "function") {
		try {
			return await navigatorApi.locks.request(attentionLockName(prefix), { mode: "exclusive" }, update);
		} catch {
			return {
				ok: false,
...
export function notificationCapability(NotificationApi = globalThis.Notification, navigatorApi = globalThis.navigator) {
...
export async function enableAttentionNotifications(items, { NotificationApi = globalThis.Notification, navigatorApi = globalThis.navigator, storage = undefined, prefix = apiPrefix(), now = Date.now(), broadcast = null } = {}) {
...
export async function disableAttentionNotifications({ navigatorApi = globalThis.navigator, storage = undefined, prefix = apiPrefix(), now = Date.now(), broadcast = null } = {}) {
...
export async function deliverAttentionNotifications(items, { NotificationApi = globalThis.Notification, navigatorApi = globalThis.navigator, storage = undefined, prefix = apiPrefix(), now = Date.now(), broadcast = null, onNavigate = defaultNavigate, onOpenCenter = () => {}, focusWindow = () => {
	try {
		globalThis.focus?.();
	} catch {}
} } = {}) {
	if (notificationCapability(NotificationApi, navigatorApi) !== "granted") {
		return {
			delivered: 0,
			status: notificationCapability(NotificationApi, navigatorApi)
		};
	}
	let result = {
		delivered: 0,
	...
	await navigatorApi.locks.request(attentionLockName(prefix), { mode: "exclusive" }, async () => {
		...
		if (!loaded.available || !loaded.valid || !loaded.state.enabled) {
			result = {
				delivered: 0,
				status: loaded.available ? "disabled" : "storage-unavailable"
			};
			return;
		}
		const notified = attentionIds(loaded.state, "notified");
		const fresh = (items || []).filter((item) => item?.notifyEligible === true && typeof item.id === "string" && !notified.has(item.id) && Number.isFinite(item.created) && item.created * 1e3 >= loaded.state.armedAt);
		...
		try {
			notification = new NotificationApi("LoopLab needs attention", {
				body: fresh.length === 1 ? "One new item is ready to review." : `${fresh.length} new items are ready to review.`,
...
export function createAttentionChannel({ prefix = apiPrefix(), Channel = globalThis.BroadcastChannel, onInvalidate = () => {} } = {}) {
	...
	let channel;
	try {
		channel = new Channel(attentionChannelName(prefix));
	} catch {
		return {
			broadcast() {},
			close() {}
	...
	channel.onmessage = (event) => {
		...
		if (value && value.type === "invalidate" && value.v === 1 && Object.keys(value).length === 2) onInvalidate();
	};
	return {
		broadcast(value = {
			type: "invalidate",
