...
import { deadlineGet, fetchEventStream, normalizeRunGeneration, observeRunGeneration, runApiPath } from "./api.mjs";
import { withBuilding } from "./buildingModel.mjs";
...
export function usePoll(fn, ms, deps = [], { pauseHidden = false, immediate = true, enabled = true } = {}) {
	useEffect(() => {
		...
		if (immediate) tick();
		const t = ms != null ? setInterval(() => {
			if (!pauseHidden || !document.hidden) tick();
		}, ms) : null;
		const onVis = pauseHidden ? () => {
...
export function useRunState(runId, { pollOnly = false, pollMs = 4e3 } = {}) {
	...
	const [eventCountState, setEventCountState] = useState({
		runId,
		value: null
	});
	...
	useEffect(() => {
		...
		const MIN_BACKOFF = 1500, MAX_BACKOFF = 3e4;
		const TERMINAL_PROBE_MS = 6e4, TERMINAL_PROBE_MAX_MS = 3e5;
		...
		const identity = (payload, probe = false) => {
			...
			if (!probe && (!payload.state || typeof payload.state !== "object" || Array.isArray(payload.state))) {
		...
		const identityChanged = (next) => next[0] !== lastSeq || next[1] !== lastAlive || next[2] !== lastGeneration || next[3] !== lastEventCount;
		const commitSnapshot = (payload) => {
			const next = identity(payload);
			const sameGeneration = next[2] != null && next[2] === lastGeneration;
			if (sameGeneration && (next[0] < lastSeq || next[3] != null && lastEventCount != null && next[3] < lastEventCount)) {
		...
		const terminalSnapshot = (payload) => payload?.state?.finished === true && payload.state.engine_running === false && payload.state.phase !== "finalizing";
		const endOwnerRun = () => {
		...
		initialRequest.promise.then((p) => {
			if (stopped) return;
			commitSnapshot(p);
			setStatus("ready");
			setError(null);
			reviewRetryRef.current = 1500;
			if (!pollOnly) {
			...
			if (!pollOnly) {
				...
				reviewRetryRef.current = Math.min(delay * 2, MAX_BACKOFF);
	...
	useLayoutEffect(() => {
		observeRunGeneration(runId, generation);
	}, [runId, generation]);
	return {
		live,
		seq,
