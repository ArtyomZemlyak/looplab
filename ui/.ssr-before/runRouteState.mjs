...
export const REVIEW_SUMMARY_TABS = [
	"Overview",
	"Comments",
	"Trust",
	"Cost"
];
export const REVIEW_EVIDENCE_TABS = [
	"Overview",
	"Comments",
	"Code",
	"Trust",
...
export function splitRouteHash(hash = "") {
	...
	return marker < 0 ? {
		...
		path: raw.slice(0, marker),
		query: raw.slice(marker + 1)
	};
}
...
export function sanitizeRunRouteState(input = {}, { reviewMode = false } = {}) {
	...
	const nodeGeneration = state.nodeId != null && Number.isSafeInteger(input.nodeGeneration) && input.nodeGeneration >= 0 ? input.nodeGeneration : null;
	...
	if (state.nodeId != null && state.nodeGeneration != null && state.inspectTab === "Comments" && typeof input.commentId === "string" && COMMENT_ID_RE.test(input.commentId)) {
	...
	if (!reviewMode && typeof input.timelineFilter === "string" && input.timelineFilter.length <= MAX_FILTER_CHARS && !CONTROL_RE.test(input.timelineFilter)) {
...
export function parseRunRouteState(hash = "", { reviewMode = false } = {}) {
	...
	if (attemptSupplied && attempt == null) {
		issues.push(state.nodeId == null ? "Invalid node attempt was ignored." : "The node target had an invalid attempt and was not opened.");
		state.nodeId = null;
	...
	if (comment != null && comment !== "") {
		...
		else if (state.nodeId == null || state.nodeGeneration == null || state.inspectTab !== "Comments") {
	...
	if (!sanitized.generation && runRouteStateHasTarget(sanitized, { reviewMode })) {
		...
		return {
			state: emptyRunRouteState(),
			issues,
			hadState: query.length > 0
		};
	}
	return {
		state: sanitized,
...
export function runRouteStateHasTarget(state, { reviewMode = false } = {}) {
	...
	return value.view !== "dag" || value.nodeId != null || value.panel != null || value.sequence != null || !reviewMode && Number.isSafeInteger(state?.sequence) && state.sequence >= 0 || value.timelineFilter.trim() !== "" || value.timelineKinds.length > 0;
}
...
export function sameRunRouteState(left, right) {
	return encodeRunRouteState(left, { forceGeneration: true }) === encodeRunRouteState(right, { forceGeneration: true });
}
export function reconcileRunRouteStateUpdate(current, raw, { generation = null, reviewMode = false, forceGeneration = false } = {}) {
	let candidate = {
		...emptyRunRouteState(),
		...raw
	};
