...
import { Turn, PermCard } from "./AssistantChat.mjs";
import { OpIcon } from "./icons.mjs";
import { useMediaQuery, usePoll } from "./hooks.mjs";
import { useToast } from "./useToast.mjs";
import { getRunAccess } from "./runMode.mjs";
import { DIALOG_PRIORITY, useDialogFocus } from "./useDialogFocus.mjs";
import { AttentionLauncher, openAttentionCenter, useAttentionIndicator } from "./attentionIndicator.mjs";
import { pendingApprovalTarget } from "./runIndex.mjs";
import { assistantErrorInfo, assistantPreview } from "./assistantErrors.mjs";
import { reconcilePendingPermissions } from "./assistantPermission.mjs";
import { clearLaunchDraftSession, launchDraftKey, launchDraftSession, removeLaunchDraft, retainLaunchDraft } from "./launchDraftStore.mjs";
import { proposalLaunchChat } from "./launchProvenance.mjs";
import { assistantDirectObservationKind, assistantDirectStatus, assistantRunChanged, assistantStorageFailureOwnsLock, pollAssistantDirectOnce, presentAssistantCommandResult, restoreAssistantDirectEntry, submitAssistantDirect } from "./assistantCommand.mjs";
import { assistantDirectDecision, assistantDirectPresentation } from "./assistantDirectPolicy.mjs";
import { assistantRecoveryFailure, assistantRecoveryPayload, assistantReplyCompletesTurn, assistantTurnIndex, danglingAssistantTurn, unavailableAssistantRecovery } from "./assistantRecovery.mjs";

import { get, fmtAgo, fmtDate, ASSISTANT_MODES as MODES, tokText, assistantCreate, assistantMessageStream, assistantCommands, assistantRevert, assistantSessions, assistantGet, assistantDelete, assistantPermissions, assistantResolve, assistantCancel, assistantProgress, assistantFork, assistantForkStatus, assistantShare, assistantUnshare, commandActionForEvent, commandCanRetry, commandErrorMessage, commandEventForAction, commandFailureRecord, commandFeedback, commandRecordMatchesAction, getRunCommand, retryRunCommand, getObservedRunGeneration, normalizeRunGeneration, COMMAND_SUCCEEDED, COMMAND_FAILED, createIdempotencyKey, saveAssistantRunTransport, loadAssistantRunTransport, clearAssistantRunTransport, clearRunCommandLock, loadRunCommandLock, saveRunCommandLock, clearDamagedLaunchTransport, clearLaunchTransport, listLaunchTransports, loadLaunchTransport, subscribeLaunchTransports, subscribeRunCommandLock, storageGet, storageSet, storageRemove, runApiPath } from "./util.mjs";
import { boundedRequest, deadlineRequest } from "./requestDeadline.mjs";
...
const assistantForkRecoveryKey = (sid) => `ll.assistant-fork-recovery.${encodeURIComponent(String(sid || ""))}`;
const validAssistantForkRecovery = (value) => value && typeof value === "object" && !Array.isArray(value) && Object.keys(value).length === 2 && typeof value.actionId === "string" && ASSISTANT_FORK_ACTION_RE.test(value.actionId) && Number.isSafeInteger(value.expectedMessages) && value.expectedMessages >= 0;
...
const saveAssistantForkRecovery = (sid, recovery) => ASSISTANT_SESSION_RE.test(String(sid || "")) && validAssistantForkRecovery(recovery) && storageSet(assistantForkRecoveryKey(sid), JSON.stringify(recovery));
...
const messagesOwnLaunchIdentity = (messages, sessionId, identity) => Array.isArray(messages) && messages.some((message, messageIndex) => Array.isArray(message?.proposals) && message.proposals.some((proposal, proposalIndex) => launchDraftKey({
	sessionId,
...
const FREEZE = {
	success: "⏸ run stopped (not finalized)",
	noop: "⏸ run already stopped",
	executing: "Stop requested — waiting for freeze"
};
const FINALIZE = {
	success: "⏹ run finalized",
	noop: "⏹ run already finalized",
	executing: "Finalize requested — waiting for wrap-up"
};
const DIRECT = {
	stop: FREEZE,
	pause: FREEZE,
	...
	resume: {
		success: "▶ run resumed",
		noop: "▶ run already running",
		executing: "Resume requested — waiting for engine"
	},
	ratify: {
		success: "✓ eval spec ratified",
		noop: "✓ eval spec already ratified",
		executing: "Ratification requested — awaiting confirmation"
	},
	approve: {
		arg: true,
		success: (id) => `✓ approved #${id}`,
		noop: (id) => `✓ #${id} already approved`,
		executing: (id) => `Approval #${id} requested — awaiting confirmation`
	}
};
const directSpec = (name) => typeof name === "string" && Object.hasOwn(DIRECT, name) ? DIRECT[name] : null;
const UNKNOWN_DIRECT_SPEC = {
	success: "Run command completed",
	noop: "Run command was already satisfied",
...
const composerUsesRun = (input, files = [], pendingFileReads = 0) => {
	...
	return files.length > 0 || pendingFileReads > 0 || !NEW_RUN_DRAFT_RE.test(text) && text.length > 0;
};
const composerRunKey = (runId) => runId == null ? "" : String(runId);
const uiRunContext = (runId, refs) => {
	if (!runId) return "";
	const safe = String(runId).replace(/[\]"\r\n]/g, " ").slice(0, 200);
	const nodes = refs.length ? ` The user refers to ${refs.map((id) => "#" + id).join(", ")}; read them with run tools.` : "";
...
const HINTS = [
	{
		label: "Summarize my runs",
		text: "Summarize my runs: best results, active work, and failures."
	},
	{
	...
	{
		label: "Explain the best result",
		text: "Explain the best experiment and why it works."
	},
	{
		label: "What's next?",
		text: "Propose the highest-value next experiment and why."
	}
];
...
const FILE_CHAR_CAP = 2e4;
const MAX_FILE_BYTES = 2 * 1024 * 1024;
const SECRET_RE = /(^|\/)\.env(\.|$)|\.pem$|\.key$|(^|\/)(id_rsa|id_ed25519)$|secret|credential/i;
const NEW_CHAT_COMPOSER_KEY = "__new__";
const normalizeComposerMode = (value) => MODES.some((candidate) => candidate.id === value) ? value : "plan";
...
const validAssistantShareMeta = (meta) => {
	...
	if (ids == null || liveIds == null || liveIds.some((id) => !shareSet.has(id)) || typeof meta.shared !== "boolean" || meta.shared !== ids.length > 0 || !Number.isInteger(meta.share_count) || meta.share_count !== ids.length || typeof meta.share_live !== "boolean" || meta.share_live !== liveIds.length > 0) return false;
	return ids.length ? Number.isFinite(meta.share_expires_at) && meta.share_expires_at > 0 : meta.share_expires_at == null;
};
const assistantLiveShareAckRequired = (error) => error?.status === 409 && error?.code === "assistant_live_share_ack_required";
const assistantForkTurnInProgress = (error) => error?.status === 409 && error?.code === "assistant_turn_fork_in_progress";
...
const assistantShareReceipt = (value, expectedSession) => {
	...
	if (value?.ok !== true || String(value?.session || "") !== String(expectedSession || "") || value?.live !== false || !validAssistantShareId(shareId) || match?.[1] !== shareId || !Number.isFinite(expiresAt) || expiresAt <= 0) return null;
	return {
		shareId,
...
const validAssistantShareFallback = (value) => {
	if (!value || !validAssistantShareId(value.shareId) || !Number.isFinite(value.expiresAt) || value.expiresAt <= 0 || typeof value.url !== "string") return false;
	try {
		const parsed = new URL(value.url);
		const match = ASSISTANT_SHARE_URL_RE.exec(parsed.hash);
		return parsed.origin === location.origin && parsed.pathname === location.pathname && !parsed.search && match?.[1] === value.shareId;
	} catch {
		return false;
	}
};
...
const assistantRevertKey = (sessionId, change) => {
	...
	return path && recoveryId ? JSON.stringify([
		String(sessionId || NEW_CHAT_COMPOSER_KEY),
		path,
		recoveryId
	]) : "";
};
...
const readFileText = (file) => new Promise((resolve) => {
	...
	r.onload = () => resolve({
		name: file.name,
		size: file.size,
		content: String(r.result || "").slice(0, FILE_CHAR_CAP),
		truncated: (r.result || "").length > FILE_CHAR_CAP
	});
...
export default function AssistantBar({ runId, hidden = false, onReady }) {
	...
	const runAccess = runAccessState.runId === runAccessKey ? runAccessState.access : pendingRouteAccess;
	...
	const readOnlyAction = runLoadingLocked ? "Run access is still loading; Assistant actions stay paused until it is verified" : runUnavailableLocked ? "This run is unavailable; return to the run list or retry before using Assistant actions" : startOverLocked ? "Start over must be resolved before asking the Assistant to change this run" : reviewLocked ? "This review link is read-only" : staleDiagnostic ? "This diagnostic link targets an earlier run generation — open the current generation to act" : `History seq ${runAccess.seq} is read-only — return live to act`;
	const readOnlyShort = runLoadingLocked ? "Run loading · Assistant paused" : runUnavailableLocked ? "Run unavailable · Assistant paused" : startOverLocked ? "Start over unresolved · recover the exact request" : reviewLocked ? "Read-only review" : staleDiagnostic ? "Stale diagnostic link · open the current generation" : `History seq ${runAccess.seq} · return live`;
	...
	const [directPending, setDirectPending] = useState(null);
	...
	const commandBusy = directStarting || directPending != null || runCommandLock != null && !ownStorageFailureLock;
	...
	const [toast, flash, clearToast] = useToast();
	...
	const shareVerificationRef = useRef({
		seq: 0,
		sid: null,
		promise: null
	});
	...
	const beginShareMetaRead = React.useCallback((sessionId) => {
		...
		return {
			id,
			seq,
			mutationEpoch: shareMutationEpochRef.current,
			listRequestSeq: sessionsRequestSeqRef.current
		};
	}, []);
	const shareMetaReadCurrent = React.useCallback((read) => !!read?.id && shareMetaReadSeqRef.current.get(read.id) === read.seq && shareMutationEpochRef.current === read.mutationEpoch && sessionsRequestSeqRef.current === read.listRequestSeq, []);
	...
	const turnCaptureRef = useRef(false);
	const openSessionSeqRef = useRef(0);
	const openSessionPendingRef = useRef(null);
	const deferredRecoverySessionRef = useRef(null);
	const directCaptureRef = useRef(false);
	const directConfirmRef = useRef(null);
	...
	const clearDirectConfirmation = React.useCallback(({ focus = false } = {}) => {
		try {
			directConfirmRef.current?.requestController?.abort();
		} catch {}
		directConfirmRef.current = null;
		setDirectConfirm(null);
		if (focus) requestAnimationFrame(() => inputRef.current?.isConnected && inputRef.current.focus({ preventScroll: true }));
	}, []);
	...
	const readPermissionSnapshot = React.useCallback((session, { fresh = false } = {}) => {
		...
		record.promise = Promise.resolve().then(() => boundedRequest((signal) => assistantPermissions(id, { signal }), 8e3)).then((payload) => {
			...
			const pending = reconcilePendingPermissions(payload.pending, resolvedForSession || new Set());
	...
	const replyAttemptOwned = (attempt, id) => !!attempt && activeReplyAttemptRef.current === attempt && mountedRef.current && sidRef.current === id;
	...
	const announceReplyReady = React.useCallback((content, { sessionId = null, turn = null, attempt = null } = {}) => {
		...
		if (!turnKey && attempt && typeof attempt === "object" && announcedReplyAttemptsRef.current.has(attempt)) return false;
	...
	const composerDraftsRef = useRef(new Map([[NEW_CHAT_COMPOSER_KEY, newComposerDraft()]]));
	...
	const updateComposerFiles = React.useCallback((draft, update, runScope = undefined) => {
		...
		if (!usesRun) draft.runScope = null;
			...
			draft.runScope = runScope === undefined ? composerRunKey(currentRunIdRef.current) : runScope;
		}
	...
	const activateComposer = React.useCallback((key, { clear = false, seedMode = "plan" } = {}) => {
		const draft = clear ? newComposerDraft() : composerDraftsRef.current.get(key) || newComposerDraft(seedMode);
		...
		if (draft.runScope == null && composerUsesRun(draft.input, draft.files, draft.pendingFileReads)) {
	...
	const applyAssistantShareMeta = React.useCallback((sessionId, meta, read) => {
		...
		if (shareMetaValid) {
			setShareCopyFallbacks((current) => {
				...
				if (!fallback || validAssistantShareFallback(fallback) && meta.share_ids.includes(fallback.shareId)) return current;
				const next = { ...current };
				delete next[id];
				return next;
			});
	...
	const refreshSessionShareMeta = React.useCallback(async (sessionId) => {
		...
		applyAssistantShareMeta,
		beginShareMetaRead,
		setShareUnknown,
		shareMetaReadCurrent
	]);
	useEffect(() => {
		const accessKey = runId == null ? null : String(runId);
		setRunAccessState({
			runId: accessKey,
			access: getRunAccess(runId)
		});
		const onAccess = (e) => {
	...
	useEffect(() => {
		...
		if (view === "side") {
			b.classList.add("asst-side-open");
			b.style.setProperty("--asst-side-w", sideW + "px");
		} else b.classList.remove("asst-side-open");
		return () => {
			b.classList.remove("asst-side-open");
		};
	}, [view, sideW]);
	useEffect(() => {
		assistantCommands().then((r) => setCommands(r.commands || [])).catch(() => {});
	}, []);
	const feedOpen = view === "side" || view === "full";
	usePoll((alive) => get("/api/runs").then((r) => alive() && setRuns(r || [])).catch(() => {}), 6e3, [feedOpen], { enabled: feedOpen });
	...
	const refreshSessions = React.useCallback(async () => {
		...
		try {
			...
			if (!mountedRef.current || requestSeq !== sessionsRequestSeqRef.current || mutationEpoch !== shareMutationEpochRef.current) return null;
			...
			const visibleSessions = result.sessions.filter((session) => !sessionDeleteTombstonesRef.current.has(String(session?.id || "")));
			...
			setShareCopyFallbacks((current) => {
				...
				for (const id of Object.keys(current)) {
					...
					const fallback = current[id];
					if (!validAssistantShareFallback(fallback) || !session?.share_ids.includes(fallback.shareId)) {
	...
	const onFeedScroll = (e) => {
		...
		atBottomRef.current = f.scrollHeight - f.scrollTop - f.clientHeight < 80;
	};
	useEffect(() => {
		if (feedOpen && feedRef.current && atBottomRef.current) requestAnimationFrame(() => {
	...
	const patchLast = (patch) => setMsgs((m) => {
		...
		if (i >= 0) c[i] = {
			...c[i],
			...typeof patch === "function" ? patch(c[i]) : patch
		};
		return c;
	});
	const lastAssistant = () => {
	...
	const openSide = () => openCommandView("side");
	...
	useEffect(() => {
		...
		const revealActiveSession = () => {
			...
			frame = requestAnimationFrame(() => {
				sessionsRef.current?.querySelector(".asst-sess.active")?.scrollIntoView({
					block: "nearest",
					inline: "nearest"
				});
			});
	...
	const closeDeleteConfirm = React.useCallback(() => {
		...
		requestAnimationFrame(() => {
			const target = trigger?.isConnected ? trigger : focusAfterDelete?.isConnected ? focusAfterDelete : document.querySelector(".asst-sessions .asst-sess-open, .asst-side-h .btn.primary");
	...
	const openRunFromAssistant = (event, href) => followClientRoute(event, () => handoffAssistantRoute(href));
	const toggleSide = () => view === "side" ? collapseToBar() : openSide();
	useDialogFocus(fullDialogRef, collapseToBar, view === "full" && !hidden, { priority: DIALOG_PRIORITY.ASSISTANT_FULL });
	useDialogFocus(sideDialogRef, collapseToBar, view === "side" && !hidden, {
		modal: compactAssistant,
		priority: DIALOG_PRIORITY.ASSISTANT_SIDE
	});
	useDialogFocus(deleteDialogRef, deleteConfirmBusy ? null : closeDeleteConfirm, !!deleteConfirm && !hidden, { priority: DIALOG_PRIORITY.ASSISTANT_DELETE });
	useEffect(() => {
	...
	const resizeWithKeys = (event) => {
		...
		setSideW(Math.min(Math.max(next, 320), maxSideWidth()));
	};
	...
	const openSession = (id, options = {}) => {
		...
		if (turnCaptureRef.current && !sidRef.current) {
			...
			return Promise.resolve({
				ok: false,
				sessionId: id,
				reason: "turn-starting"
			});
		}
		const matchingRead = openSessionPendingRef.current;
		if (matchingRead && String(matchingRead.id) !== String(id) && String(sidRef.current) === String(id)) {
		...
		if (matchingRead && String(matchingRead.id) === String(id) && (matchingRead.allowsRecovery || !requestedAllowsRecovery)) return matchingRead.promise;
		...
		const operation = (async () => {
			...
			if (id === sidRef.current && !observeOnly && (!recover || runningRef.current) && !currentNeedsRecovery) {
			...
			try {
				...
				if (!mountedRef.current || seq !== openSessionSeqRef.current || sessionDeleteTombstonesRef.current.has(String(id)) || sessionRead && openSessionPendingRef.current !== sessionRead) {
					return {
						ok: false,
						sessionId: id,
						reason: "superseded"
				...
				if (dangling) {
					try {
						prog = await boundedRequest((signal) => assistantProgress(id, { signal }), 8e3);
						progressKnown = true;
					} catch {}
				}
				if (!mountedRef.current || seq !== openSessionSeqRef.current || sidRef.current !== id || sessionDeleteTombstonesRef.current.has(String(id)) || sessionRead && openSessionPendingRef.current !== sessionRead) {
					return {
						ok: false,
						sessionId: id,
						reason: "superseded"
				...
				deferredRecoverySessionRef.current = dangling && !observeOnly && historicalRef.current ? String(id) : null;
				...
				if (inFlight && mountedRef.current && sidRef.current === id) {
					...
					const act = (prog.steps || []).length ? [{
						type: "tools",
						labels: prog.steps
					}] : [];
					setMsgs((m) => m[m.length - 1] && m[m.length - 1].role === "assistant" && m[m.length - 1].streaming ? m : [...m, {
						role: "assistant",
						content: "",
						streaming: true,
						activity: act,
					...
					const startExactRecovery = async () => {
						if (historicalRef.current || observeOnly || !dangling || exactState !== "idle" || !replyAttemptCurrent(reattachAttempt, id)) return;
						exactState = "checking";
						let latest;
						...
						if (latestTurn.turn_id !== dangling.turn_id) {
							exactState = "settled";
							exactFailure = unavailableAssistantRecovery;
							return;
						}
						...
						exactState = "posted";
						recoveryCtrl = new AbortController();
						abortRef.current = recoveryCtrl;
						...
						assistantMessageStream(id, recovery.instruction, recovery.mode, {}, recoveryCtrl.signal, recovery.display, acknowledgedLiveShareIds).catch((error) => {
					...
					(async () => {
						while (polling && replyAttemptCurrent(reattachAttempt, id)) {
							...
							try {
								...
								if (replyAttemptCurrent(reattachAttempt, id) && ((pp.steps || []).length || pp.text)) patchLast((prev) => prev && prev.role === "assistant" && prev.streaming ? {
									...pp.text ? { content: pp.text } : {},
									activity: (pp.steps || []).length ? [{
										type: "tools",
										labels: pp.steps
									}] : prev.activity
								} : prev);
					...
					recoverReply(id, arr.length + 1, dangling, reattachAttempt, () => exactFailure).then((ok) => {
						...
						if (!ok && replyAttemptCurrent(reattachAttempt, id)) {
							...
							patchLast((prev) => prev && prev.role === "assistant" && prev.streaming ? {
								streaming: false,
								recovering: false,
								content: exactFailure?.message || prev.content || normalizedFailureText("connection unreachable"),
								recoveryNeeded: !!dangling && !exactFailure?.blocked,
								recoveryBlocked: !!exactFailure?.blocked
							} : prev);
						}
					}).finally(() => {
	...
	const onAttentionPermissionFocused = React.useCallback((requestId, token) => {
		...
		if (token !== attentionPermissionHandoffRef.current || attentionPermissionReceiptRef.current === token || focus.phase !== "ready" || focus.id !== requestId || focus.token !== token) return;
		attentionPermissionReceiptRef.current = token;
		try {
			window.dispatchEvent(new CustomEvent("ll:assistant-permission-opened", { detail: { requestId } }));
		} catch {}
	...
	const onAttentionPermissionFocusFailed = React.useCallback((requestId, token) => {
		const focus = attentionPermissionFocusRef.current;
		if (token !== attentionPermissionHandoffRef.current || focus.phase !== "ready" || focus.id !== requestId || focus.token !== token) return;
		setAttentionPermissionFocus({
			session: "",
			id: "",
			token,
	...
	useEffect(() => {
		if (hidden) return undefined;
		let active = true;
		const handoffCurrent = (handoff, session, sessionChoiceSeq = null) => active && attentionPermissionHandoffRef.current === handoff && (session == null || sidRef.current === session) && (sessionChoiceSeq == null || openSessionSeqRef.current === sessionChoiceSeq);
		...
		const focusFallback = (handoff, session, sessionChoiceSeq, preferFeed = false, releaseHandoff = false) => {
			requestAnimationFrame(() => {
				...
				const target = !preferFeed && !historicalRef.current && composer && !composer.disabled ? composer : feedRef.current;
				target?.focus({ preventScroll: true });
			});
		};
		const onOpenAttentionSession = async (event) => {
			...
			if (typeof session !== "string" || !/^[0-9a-f]{16}$/.test(session) || typeof requestId !== "string" || !/^[0-9a-f]{16}$/.test(requestId)) {
	...
	}, [hidden, readPermissionSnapshot]);
	const releaseOrphanedLaunchRecovery = (recovery, reason) => {
		openFull();
		const current = listLaunchTransports().find((item) => item.identity === recovery.identity || recovery.storageKey && item.storageKey === recovery.storageKey);
		if (!current) {
		...
		const orphanDescription = reason === "missing-session" ? "The Assistant chat that owns this startup recovery no longer exists" : reason === "missing-proposal" ? "The exact Assistant proposal that owns this startup recovery no longer exists in the saved chat" : "This startup recovery cannot be matched to an Assistant session";
		const recordDescription = current.invalid ? "The current local recovery record is damaged and cannot be checked automatically." : `The current local recovery is for "${stored.runId}".`;
		...
		const cleared = current.invalid ? clearDamagedLaunchTransport(current.storageKey) : clearLaunchTransport(current.identity, undefined, stored);
		flash(cleared ? "Local startup recovery released after explicit inspection acknowledgement. The original outcome is still not proven." : "Startup recovery changed while it was being reviewed and was retained. Open it again.");
	};
	...
	useEffect(() => {
		...
		if (!deferred || String(sidRef.current || "") !== deferred || runningRef.current || openSessionPendingRef.current) return;
		deferredRecoverySessionRef.current = null;
		openSession(deferred, { recover: true });
	}, [historical, sessionOpening]);
	...
	useEffect(() => {
		const onAttach = (e) => {
			...
			setInput((prev) => new RegExp(`#(?:node-)?${id}\\b`, "i").test(prev) ? prev : (prev.trim() ? prev.replace(/\s*$/, " ") : "") + `#${id} `);
	...
	const deleteSessionBlock = (id) => {
		const unresolved = listLaunchTransports().find((item) => launchDraftSession(item.identity) === String(id));
		if (unresolved) {
			return {
		...
		if (id === sidRef.current && (turnCaptureRef.current || runningRef.current || busy || pending.length > 0)) {
	...
	const requestDeleteSession = (session, e) => {
		...
		deleteFocusAfterRef.current = row?.nextElementSibling?.querySelector(".asst-sess-open:not(:disabled)") || row?.previousElementSibling?.querySelector(".asst-sess-open:not(:disabled)") || document.querySelector(".asst-side-h .btn.primary");
		setDeleteConfirm({
			id: session.id,
	...
	const confirmDeleteSession = async () => {
		...
		mutateSessionsLocally((ss) => ss.filter((s) => s.id !== id));
		let deleted = false;
		...
		try {
			...
			const requestWasAmbiguous = (candidate) => candidate?.name === "TimeoutError" || candidate?.name === "AbortError" || candidate?.status == null || Number(candidate.status) >= 500;
			...
			if (finalAmbiguous && !deleted) {
				try {
					const payload = await boundedRequest((signal) => assistantSessions({
						signal,
						cache: "no-store"
					}), 8e3);
					listedPresence = Array.isArray(payload?.sessions) ? payload.sessions.some((session) => session?.id === id) : null;
				} catch {}
			}
			if (!deleted) {
				...
				const message = shared ? "This chat still has an active public link. Unshare it first, then delete the chat." : forkInProgress ? "This chat is still being forked. It is shown again; try deleting it after the fork finishes." : stillStopping ? "The live Assistant turn is still stopping. The chat is shown again; try deleting it again shortly." : incomplete ? "The server could not completely remove the chat. Close anything using its files and try again." : finalAmbiguous && listedPresence === true ? "Neither bounded deletion attempt was confirmed, and the chat still appears available. You can try again." : finalAmbiguous ? "Neither bounded deletion attempt was confirmed. The chat is shown again for safety; retry or refresh." : "The chat was not deleted. It is shown again; you can cancel or try again.";
				flash(shared ? "Unshare this chat before deleting it" : forkInProgress ? "Wait for this chat to finish forking before deleting it" : stillStopping ? "Assistant work is still stopping" : incomplete ? "Assistant chat was not completely removed" : "Could not confirm chat deletion");
			...
			if (deleted && mountedRef.current) {
				...
				setDeleteConfirm(null);
				requestAnimationFrame(() => {
					const targetAfterDelete = focusAfterDelete?.isConnected ? focusAfterDelete : document.querySelector(".asst-sessions .asst-sess-open:not(:disabled), .asst-side-h .btn.primary");
	...
	const resolvePerm = async (reqId, decision) => {
		if (historical) {
			flash(readOnlyAction);
			return;
		}
		if (resolvingPermsRef.current.has(reqId)) return;
		const requestOwner = pending.find((req) => req.id === reqId)?.session;
		const requestSession = typeof requestOwner === "string" && /^[0-9a-f]{16}$/.test(requestOwner) ? requestOwner : sidRef.current;
		if (!requestSession || requestSession !== sidRef.current) {
			...
			requestAnimationFrame(() => {
				const composer = inputRef.current;
				const target = !historicalRef.current && composer && !composer.disabled ? composer : feedRef.current;
				target?.focus({ preventScroll: true });
			});
			return;
		}
		...
		try {
			...
			if (sidRef.current === requestSession) setPending((current) => {
				...
				requestAnimationFrame(() => {
					const target = next.length ? document.querySelector(".asst-perm-region .asst-perm-actions button:not([disabled])") : inputRef.current;
					target?.focus({ preventScroll: true });
				});
				return next;
			});
			...
			try {
				...
				const latest = requestSession && sidRef.current === requestSession ? await readPermissionSnapshot(requestSession, { fresh: true }) : null;
				if (latest?.ok && sidRef.current === requestSession && openSessionSeqRef.current === permissionChoiceSeq) setPending(latest.pending);
			} catch {}
		} finally {
	...
	const onRevert = async (change) => {
		...
		if (!key) {
			flash("This older file change has no exact undo receipt");
			return;
		}
		...
		try {
			...
			if (mountedRef.current) flash(error?.status === 409 || error?.code === "assistant_revert_conflict" ? "Undo stopped — the file changed or a newer recovery point now owns this path" : error?.name === "TimeoutError" || error?.name === "AbortError" ? "Undo outcome was not confirmed — retrying the same receipt is safe" : "Could not revert this file change");
		} finally {
	...
	const directLabels = (entry) => entry.spec.arg && entry.arg == null ? {
		...
		noop: "Run command was already satisfied",
		executing: "Run command is pending",
		failure: `/${entry.name} failed`
	} : {
		success: directCopy(entry.spec.success, entry.arg),
		noop: directCopy(entry.spec.noop, entry.arg),
		executing: directCopy(entry.spec.executing, entry.arg),
		failure: `/${entry.name} failed`
	};
	const flashDirect = (entry, message) => presentAssistantCommandResult(currentRunIdRef.current, entry.runId, () => flash(message));
	...
	const setCurrentDirect = (entry, next) => setDirectPending((current) => current?.runId === entry.runId && current?.idempotencyKey === entry.idempotencyKey ? next : current);
	...
	const clearUnsentDirectRecovery = (entry) => {
		...
		const transportCleared = clearAssistantRunTransport(entry.runId, undefined, { idempotencyKey: entry.idempotencyKey });
		...
		return (transportCleared || !remainingTransport) && (lockCleared || !assistantStorageFailureOwnsLock(entry, remainingLock));
	};
		...
		const failure = {
			...
			record: {
				...
				error: {
					...
					remediation: "Enable session storage or free browser storage, then try again.",
					retryable: false
				}
			}
		};
	...
	const unavailableDirect = (entry, error, record = entry.record) => {
		...
		if (recoveryRecord.id && !recoveryRecord.event_type && directSpec(entry.name)) {
			recoveryRecord = {
				...recoveryRecord,
				event_type: commandEventForAction(entry.name, "assistant")
			};
		}
		const next = {
			...entry,
			...
			statusUnavailable: true,
			observationKind: assistantDirectObservationKind(error),
			checking: false,
	...
	const failDirectObservation = (entry, error) => {
		...
		if (record.error) {
			delete record.error.message;
			delete record.error.remediation;
		}
		const failure = {
			...entry,
			record,
	...
	const executeDirect = async (entry, { recovery = false, onCommitted = null } = {}) => {
		...
		if (!persistDirect(submitting)) {
			localStorageFailure(bound);
			return;
		}
		try {
			onCommitted?.();
		...
		if (mountedRef.current) {
			setCurrentDirect(bound, submitting);
			setCurrentFailure(bound, null);
		}
		try {
			const record = await submitAssistantDirect(bound.runId, bound.name, bound.arg, bound.idempotencyKey, {
			...
			if (error?.commandUnknown || error?.submissionMayHaveSucceeded || record?.id && [
				"transport",
				"access",
				"protocol"
			].includes(kind)) {
			...
			} else if (error?.code === "run_generation_changed" || error?.code === "invalid_run_generation" || error?.code === "run_generation_unavailable") {
	...
	const runDirect = async (d, binding = {}) => {
		...
		if (!directRunId) {
			flash(`/${d.name} needs an open run`);
			return false;
		}
		...
		if (turnCaptureRef.current || runningRef.current) {
			flash("Assistant is already starting or responding");
			return false;
		}
		...
		if (directCaptureRef.current || loadRunCommandLock(directRunId) || loadAssistantRunTransport(directRunId) && !directFailure) {
			flash("Recover the stored run command before starting another");
			return false;
		}
		...
		if (!expectedGeneration) {
			flash(`/${d.name} not sent because the run generation is not verified`);
			return false;
		}
		if (binding.expectedGeneration && observedGeneration !== binding.expectedGeneration) {
			flash(`/${d.name} not sent because the run generation changed`);
			return false;
		}
		if (directFailure) clearAssistantRunTransport(directFailure.runId, undefined, { idempotencyKey: directFailure.idempotencyKey });
		...
		try {
			let nodeGeneration = Number.isSafeInteger(binding.nodeGeneration) ? binding.nodeGeneration : null;
			if (d.name === "approve" && nodeGeneration == null) {
				const payload = await boundedRequest((signal) => get(runApiPath(directRunId, "/state"), { signal }), 8e3);
				if (String(currentRunIdRef.current ?? "") !== String(directRunId)) {
					flash(`/${d.name} not sent because the open run changed`);
					return;
				}
			...
			requestAnimationFrame(() => inputRef.current?.isConnected && inputRef.current.focus({ preventScroll: true }));
		} finally {
	...
	const checkDirect = async () => {
		...
		if (directCaptureRef.current || turnCaptureRef.current || runningRef.current || openSessionPendingRef.current) {
	...
	const retryDirect = async () => {
		...
		commandFocusRequestedRef.current = true;
		setCurrentFailure(failure, null);
		setDirectPending(retrying);
		directCaptureRef.current = true;
		try {
	...
	useEffect(() => {
		...
		if (!saved) {
			if (lock?.source === "assistant") clearRunCommandLock(runId, {
				...
				action: lock.action,
				expectedGeneration: lock.expectedGeneration,
		...
		const lockMismatch = lock?.source === "assistant" && (lock.idempotencyKey !== saved.idempotencyKey || lock.action !== saved.action || lock.expectedGeneration !== saved.expectedGeneration || lock.commandId && saved.commandId && lock.commandId !== saved.commandId);
		if (saved.protocolInvalid || lockMismatch) {
			...
			saveRunCommandLock(runId, {
				...restored,
				source: "assistant",
				action: restored.name
			});
		...
		const restored = restoreAssistantDirectEntry(saved, spec, runId, {
			record: saved.record || (saved.commandId ? {
				id: saved.commandId,
				status: "accepted"
			} : { status: "submitting" }),
			statusUnavailable: !!saved.statusUnavailable,
			observationKind: saved.observationKind,
			checking: false,
			retrying: !!saved.retrying,
			lastError: ""
		});
		if (COMMAND_FAILED.has(saved.record?.status) && !saved.statusUnavailable && !saved.retrying && !saved.checking) {
		...
		if (saved.retrying || saved.checking) {
			const uncertain = {
				...
				statusUnavailable: true,
				observationKind: "transport",
				lastError: "Recovery was interrupted; check the same command."
			};
			persistDirect(uncertain);
			setDirectPending(uncertain);
			return;
		}
		if (!persistDirect(restored)) {
			protocolDirect(restored, restored.record, "Stored command envelope is invalid");
			return;
		}
		setDirectPending(restored);
	...
	useEffect(() => {
		...
		if (entry?.statusUnavailable || entry?.checking || !command?.id || command.status !== "accepted" && command.status !== "executing") return;
			...
			try {
				...
				if (kind === "transport") {
					if (++transientFailures < 3) {
						schedule(Math.max(Number(error.retryAfterMs) || 0, Math.min(6e3, 750 * 2 ** transientFailures)));
						return;
					}
					unavailableDirect(entry, error, command);
					return;
				}
		...
		runId,
		directPending?.record?.id,
		directPending?.statusUnavailable,
		directPending?.checking
	]);
	// ── attached files ──
	const onFiles = async (list) => {
		...
		const secret = txt.filter((f) => SECRET_RE.test(f.name));
		if (secret.length) flash(`skipped secret-looking: ${secret.map((f) => f.name).join(", ")}`);
		const big = txt.filter((f) => !SECRET_RE.test(f.name) && f.size > MAX_FILE_BYTES);
		...
		try {
			...
			if (read.length) updateComposerFiles(ownerDraft, (f) => {
				const seen = new Set(f.map((x) => x.name));
				return [...f, ...read.filter((r) => !seen.has(r.name))];
			}, ownerRunScope);
		} finally {
	...
	const runLLM = async (instruction, { userText = null, ensureVisible = false, context = null, retryFiles = null, turnMode = null, clearComposer = false, acknowledgedShareMeta = null } = {}) => {
		if (historicalRef.current) {
			flash(readOnlyAction);
			return;
		}
		...
		if (guardedSid && shareVerificationRef.current.sid === String(guardedSid) && shareVerificationRef.current.promise) {
		...
		if (turnCaptureRef.current || runningRef.current || directCaptureRef.current) {
			flash("Assistant or a run command is already starting");
			return;
		}
		...
		if (guardedSid) {
			...
			const guardedSession = acknowledgedShareMeta != null ? String(acknowledgedShareMeta?.id || "") === String(guardedSid) ? acknowledgedShareMeta : null : sessions.find((session) => String(session?.id || "") === String(guardedSid));
		...
		if (cancelReqRef.current) {
			try {
				await cancelReqRef.current;
			} catch {}
		}
		const commandClaimedDuringCancel = !!runId && (!!loadRunCommandLock(runId) || !!loadAssistantRunTransport(runId) && !directFailure);
		const accessBecameReadOnly = historicalRef.current;
		if (!mountedRef.current || activeReplyAttemptRef.current !== localAttempt || sessionSeq !== openSessionSeqRef.current || openSessionPendingRef.current || directCaptureRef.current || commandClaimedDuringCancel || accessBecameReadOnly) {
		...
		if (!id) {
			...
			try {
				const m = await boundedRequest((signal) => assistantCreate((userText || instruction).slice(0, 60), effectiveMode, { signal }));
				if (!mountedRef.current || sessionSeq !== openSessionSeqRef.current || activeReplyAttemptRef.current !== localAttempt) {
					if (activeReplyAttemptRef.current === localAttempt) {
						activeReplyAttemptRef.current = null;
						turnCaptureRef.current = false;
						setTurnStarting(false);
		...
		const localUserTurn = {
			role: "user",
			content: userText || instruction,
			context: hasCtx ? ctxInfo : null,
			retryPayload: {
				instruction,
				raw: fullInstruction.trim(),
				userText: userText || instruction,
				context,
				files: atts,
				mode: effectiveMode,
				historyLength: localHistoryLength
			},
			localAttempt
		};
		...
		(async () => {
			while (polling && replyAttemptCurrent(localAttempt, id)) {
				...
				try {
					...
					if (replyAttemptCurrent(localAttempt, id) && acc.length < (pp.text || "").length) patchLast((prev) => prev && prev.role === "assistant" && prev.streaming ? {
		...
		const safeAttempt = (fn) => (...a) => {
			if (replyAttemptCurrent(localAttempt, id)) fn(...a);
		};
		try {
			...
			const res = await assistantMessageStream(id, fullInstruction, effectiveMode, {
				...
				onText: safeAttempt((txt) => patchLast((prev) => ({ activity: [...prev.activity || [], {
					type: "text",
					content: txt
				}] }))),
				onStep: safeAttempt((s) => patchLast((prev) => {
					...
					return last && last.type === "tools" ? { activity: [...a.slice(0, -1), {
						...last,
						labels: [...last.labels, s]
					}] } : { activity: [...a, {
						type: "tools",
						labels: [s]
					}] };
			...
			}, ctrl.signal, userText || instruction, acknowledgedLiveShareIds);
			if (!replyAttemptCurrent(localAttempt, id)) return;
			...
			if (res?.ok === false) {
				...
				try {
					...
					const exactReply = durableMessages.length >= priorLen ? completedAssistantReply(durableMessages, localUserTurn) : null;
					...
					if (exactTurnIndex >= 0 && exactTurnIndex === durableMessages.length - 1) {
						...
						setPreview(previewText(failureText));
						setHasNew(false);
						const acknowledged = new Set(acknowledgedLiveShareIds);
						const liveStateChanged = shareMetaValid && assistantLiveShareIds(session.meta).some((shareId) => !acknowledged.has(shareId));
						if (liveStateChanged) {
							setShareAckNotice({
								sid: id,
			...
			if (shareStateChanged || forkTurnRejected) {
				...
				if (replyAttemptCurrent(localAttempt, id)) {
					...
					if (inputCleared && inputAtSend) {
						...
						const restoredInput = !currentInput ? inputAtSend : currentInput === inputAtSend || currentInput.startsWith(`${inputAtSend}\n\n`) ? currentInput : `${inputAtSend}\n\n${currentInput}`;
					...
					if (sidRef.current === id) {
						...
						setPreview(previewAtSend);
						setHasNew(hasNewAtSend && viewRef.current === "bar");
					}
					const restored = inputCleared || retryFiles == null && atts.length > 0;
					if (shareStateChanged) {
						setShareAckNotice({
							sid: id,
							message: restored ? "Nothing sent · live public-link state changed. Your draft was restored; verify the warning, then send again." : "Nothing sent · live public-link state changed. Verify the warning, then retry the original turn."
						});
						flash(restored ? "Nothing sent · draft restored after public-link state changed" : "Nothing sent · public-link state changed");
						refreshSessions();
						refreshSessionShareMeta(id);
					} else {
						flash(restored ? "Nothing sent · draft restored while this chat is being forked" : "Nothing sent · this chat is being forked");
					}
				}
				return;
			}
			...
			if (!replyAttemptCurrent(localAttempt, id) || ctrl.signal.aborted) {} else if (streamedFailure) {
				patchLast({
					content: streamedFailure,
					streaming: false
				});
			} else {
				...
				patchLast((prev) => {
					...
					return {
						content: acc.length >= observed.length ? acc : observed,
						streaming: true,
						recovering: true
					};
				});
				...
				if (replyAttemptCurrent(localAttempt, id) && !ok) patchLast({
					content: normalizedFailureText("connection unreachable"),
					streaming: false,
					recovering: false,
					recoveryNeeded: true
				});
	...
	const requestNewRun = (goal = "", options = {}) => runLLM(goal ? `Plan a new run for this goal and show me a launch card to start it: ${goal}` : "I want to start a new run. Propose a run spec (name, task, key settings) as a launch card I can start; ask me for anything you need first.", {
		userText: goal ? `/new ${goal}` : "/new",
		ensureVisible: true,
		...options
	});
	const retryTurn = async (assistantIndex) => {
		...
		if (turnCaptureRef.current || directCaptureRef.current || busy || commandBusy) {
			flash(commandBusy ? "A run command is pending" : "Assistant is busy");
			return;
		}
		if (historical) {
			flash(readOnlyAction);
		...
		if (!prior) {
			flash("The original message is no longer available");
			return;
		}
		...
		if (failedTurn?.recoveryNeeded) {
			...
			if (!id) {
				flash("The saved Assistant session is no longer available");
				return;
			}
			...
			try {
				...
				const session = await boundedRequest((signal) => assistantGet(id, { signal }));
				if (historicalRef.current || !replyAttemptOwned(retryCheckAttempt, id) || retrySessionSeq !== openSessionSeqRef.current || openSessionPendingRef.current) return;
				...
				if (!replyAttemptOwned(retryCheckAttempt, id) || retrySessionSeq !== openSessionSeqRef.current || openSessionPendingRef.current) return;
		...
		if (prior.turn_id) {
			...
			if (!persisted) {
				flash("The saved Assistant turn cannot be retried safely");
				return;
			}
			runLLM(persisted.instruction, {
				userText: persisted.display,
				ensureVisible: true,
		...
		if (prior.retryPayload) {
			...
			runLLM(payload.instruction, {
				userText: payload.userText,
				ensureVisible: true,
				context: payload.context || null,
				retryFiles: payload.files || [],
				turnMode: payload.mode || null,
				acknowledgedShareMeta
			});
			return;
		}
		...
		if (!userText) {
			flash("The original message is empty");
			return;
		}
	...
	const draftRunMismatch = draftRunScope != null && draftRunScope !== currentComposerRunKey && composerUsesRun(input, files, pendingFileReads);
	...
	const requestDirectCommand = async (command, sourceText) => {
		...
		if (decision === "inline") {
			...
			runDirect(command, { onCommitted: () => clearCommittedDirectDraft(owningDraft, sourceText) });
			return;
		}
		const directRunId = String(runId || "");
		if (!directRunId) {
			flash(`/${command.name} needs an open run`);
			return;
		}
		if (historicalRef.current) {
			flash(readOnlyAction);
		...
		if (command.name === "approve") {
			const approvalRead = deadlineRequest((signal) => get(runApiPath(directRunId, "/state"), { signal }), 8e3);
			const checking = {
				...confirmationBase,
				phase: "checking",
				requestController: approvalRead.controller
			};
			...
			try {
				...
				const fetchedGeneration = normalizeRunGeneration(payload?.generation);
				const target = pendingApprovalTarget(payload?.state);
				if (fetchedGeneration !== expectedGeneration || !target || target.nodeId !== command.arg || !Number.isSafeInteger(target.nodeGeneration)) {
	...
	const confirmDirectCommand = () => {
		...
		if (confirmation.composerKey !== composerKeyRef.current || confirmation.draft !== composerDraftRef.current || confirmation.sourceText.trim() !== composerDraftRef.current.input.trim() || confirmation.mode !== normalizeComposerMode(composerDraftRef.current.mode) || String(currentRunIdRef.current || "") !== confirmation.runId || historicalRef.current || observedGeneration !== confirmation.expectedGeneration) {
			clearDirectConfirmation({ focus: true });
		...
		runDirect(confirmation.direct, {
			...
			onCommitted: () => clearCommittedDirectDraft(confirmation.draft, confirmation.sourceText)
		});
	};
	const send = () => {
		if (historical) {
		...
		if (liveDraft.runScope != null && liveDraft.runScope !== composerRunKey(runId) && composerUsesRun(liveDraft.input, liveDraft.files, liveDraft.pendingFileReads)) {
		...
		const t = input.trim();
		const storedCommand = !!runId && (!!loadRunCommandLock(runId) || !!loadAssistantRunTransport(runId) && !directFailure);
		if (!t && files.length === 0 || turnCaptureRef.current || directCaptureRef.current || busy || commandBusy || storedCommand) {
		...
		if (direct) {
			requestDirectCommand(direct, t);
			return;
		}
		const pr = runId ? preRoute(t) : null;
		if (pr) {
			requestDirectCommand(pr, t);
			return;
		}
		const refs = runId ? refNodes(t) : [];
		const ctx = uiRunContext(runId, refs);
		runLLM((t || "See the attached file(s).") + ctx, {
			userText: t || "(attached files)",
			context: {
				run: runId || null,
				refs
			},
	...
	useEffect(() => {
		const onNewRun = (event) => {
			...
			if (busy || commandBusy || historical) {
				flash(historical ? readOnlyAction : commandBusy ? "A run command is pending" : "Assistant is busy");
				return;
			}
	...
	useEffect(() => {
		...
		const contextChanged = hidden || historical || view === "bar" || directConfirm.runId !== String(runId || "") || directConfirm.composerKey !== composerKeyRef.current || directConfirm.draft !== composerDraftRef.current || directConfirm.sourceText.trim() !== input.trim() || directConfirm.mode !== mode;
	...
	useEffect(() => {
		...
		const invalidateStaleGeneration = () => {
			...
			const observedGeneration = normalizeRunGeneration(getObservedRunGeneration(directConfirm.runId));
	...
	const stop = () => {
		...
		const id = sidRef.current || sid;
		if (id) {
			const pr = boundedRequest((signal) => assistantCancel(id, { signal }), 8e3).catch(() => {});
			cancelReqRef.current = pr;
			pr.finally(() => {
				if (cancelReqRef.current === pr) cancelReqRef.current = null;
			});
		}
	...
	const pendingCommandText = directStarting ? "Checking run command preconditions…" : directPending ? assistantDirectStatus(directPending) : externalCommandPending ? `/${externalCommandPending.action} is pending in the run timeline` : "";
	...
	const directFailureText = directFailure ? `/${directFailure.name} failed: ${commandErrorMessage(directFailure.record)}` : "";
	...
	const launchRecoveryButton = launchRecoveries.length > 0 ? /* @__PURE__ */ _jsxs("button", {
		type: "button",
		className: "btn sm asst-launch-global",
		onClick: openLaunchRecovery,
		"aria-label": `${launchRecoveryLabel}${launchRecoveries.length > 1 ? ` (${launchRecoveries.length})` : ""}`,
		title: nextLaunchRecovery?.invalid ? "Review a damaged durable startup recovery record" : "Reopen the Assistant proposal that owns this durable startup identity",
		children: [
			...
			/* @__PURE__ */ _jsx("span", {
				className: "asst-launch-global-label",
				children: launchRecoveryLabel
			}),
			launchRecoveries.length > 1 && /* @__PURE__ */ _jsx("span", {
				className: "asst-launch-global-count",
				children: launchRecoveries.length
			})
		]
	}) : null;
	useEffect(() => {
		...
		directPending?.record?.id,
		directPending?.record?.status,
		directPending?.statusUnavailable,
		directPending?.checking,
		directPending?.retrying,
		directFailure?.record?.id,
		directFailure?.record?.status,
		directStarting,
		busy,
		view
	]);
	...
	const ktok = (n) => n >= 1e3 ? (n / 1e3).toFixed(n < 1e4 ? 1 : 0) + "k" : String(n || 0);
	const ctxChip = lastCtxTok > 0 ? /* @__PURE__ */ _jsxs("span", {
		className: "asst-ctxtok",
		title: `≈${lastCtxTok.toLocaleString()} tokens in the assistant's context (grows with the conversation — start a new chat to reset) · ≈${chatTok.toLocaleString()} total tokens this chat`,
		children: [
			/* @__PURE__ */ _jsx(OpIcon, {
				name: "sliders",
				size: 10
	...
	const directModeHint = directModeDecision === "deny" ? "run control · unavailable in Plan" : directModeDecision === "ask" ? "run control · confirmation required" : "run control · executes immediately";
	const directNames = [{
		name: "new",
		desc: "draft a launch card — review before start"
	}, ...runId ? Object.keys(DIRECT).map((n) => ({
		name: n,
		desc: directModeHint
	})) : []];
	const suggestions = slashMatch ? [...directNames, ...commands.map((c) => ({
		name: c.name,
		desc: c.desc
	}))].filter((c) => c.name.startsWith(slashMatch[1].toLowerCase())).filter((c, i, a) => a.findIndex((x) => x.name === c.name) === i).slice(0, 6) : [];
	...
	const shareTurnIncomplete = sessionOpening || busy || commandBusy || pending.length > 0 || runningRef.current || turnCaptureRef.current || msgs[msgs.length - 1]?.role === "user";
	...
	const composerPaused = sessionOpening || turnStarting || retryChecking || sharePaused || forkingCurrentSession || !!directConfirm;
	...
	const shareStatusMessage = shareVerifying ? "Checking public-link status. You can keep editing this draft; nothing has been sent." : shareUnknown ? "Public-link status unknown. You can keep editing this draft; sending is paused until the status is verified or public links are revoked." : "";
	const rememberForkRecovery = (sourceSid, recovery) => {
		const stored = loadAssistantForkRecovery(sourceSid);
		const durable = stored?.actionId === recovery.actionId && stored.expectedMessages === recovery.expectedMessages ? true : saveAssistantForkRecovery(sourceSid, recovery);
	...
	const forgetForkRecovery = (sourceSid, actionId) => {
		...
		if (mountedRef.current) {
			setForkRecovery((current) => current?.sid === sourceSid && current.actionId === actionId ? null : current);
		}
	};
	const presentForkChild = async (child, sourceSid, recovery, sourceChoiceSeq) => {
		if (!mountedRef.current) return false;
		const childId = typeof child?.id === "string" && ASSISTANT_SESSION_RE.test(child.id) && child.parent === sourceSid && child.fork_action_id === recovery.actionId ? child.id : null;
	...
	const reconcileFork = async (sourceSid, recovery) => {
		...
		for (let attempt = 0; attempt < 5 && mountedRef.current; attempt++) {
			...
			try {
				const child = await boundedRequest((signal) => assistantForkStatus(sourceSid, actionId, {
					expectedMessages: recovery.expectedMessages,
					signal
				}), 4e3);
				return {
					kind: "created",
				...
				if (error?.code === "assistant_fork_in_progress" && (!reportedAction || reportedAction === actionId)) {
					sawPending = true;
					continue;
				}
				...
				if (error?.code === "assistant_fork_deleted") return {
					kind: "deleted",
					error
				};
				if (error?.code === "assistant_fork_action_conflict") return {
					kind: "conflict",
					error
				};
				if (error?.code === "assistant_fork_deleting") return {
					kind: "deleting",
					error
				};
				...
				const ambiguous = error?.name === "TimeoutError" || error?.name === "AbortError" || error?.status == null || status >= 500 || [
					408,
					425,
					429
				].includes(status);
	...
	const settleForkReconciliation = async (outcome, sourceSid, recovery, sourceChoiceSeq) => {
		...
		await refreshSessions();
		if (mountedRef.current) flash(outcome.kind === "pending" ? "Fork is still finishing · use check fork to recover this exact request" : "Fork status is uncertain · use check fork before starting another");
	};
	const forkCurrentSession = async () => {
		...
		if (!storedRecovery && (runningRef.current || turnCaptureRef.current || busy || commandBusy || pending.length > 0 || msgs[msgs.length - 1]?.role === "user")) {
		...
		const recovery = storedRecovery || {
			actionId: createIdempotencyKey().toLowerCase(),
			expectedMessages: msgs.length
		};
		...
		try {
			const child = await boundedRequest((signal) => assistantFork(forkSid, recovery, { signal }), 12e3);
			...
			} else if (error?.code === "assistant_fork_in_progress" && error?.detail?.action_id && error.detail.action_id !== recovery.actionId) {
				const activeActionId = String(error.detail.action_id).toLowerCase();
				const activeExpectedMessages = typeof error.detail.expected_messages === "number" ? error.detail.expected_messages : Number.NaN;
				const adopted = {
					actionId: activeActionId,
					expectedMessages: activeExpectedMessages
				};
				if (!ASSISTANT_FORK_ACTION_RE.test(activeActionId) || !Number.isSafeInteger(activeExpectedMessages) || activeExpectedMessages < 0 || activeExpectedMessages !== recovery.expectedMessages) {
			...
			} else if (error?.code === "assistant_fork_in_progress" || error?.code === "assistant_fork_deleting" || error?.code === "assistant_fork_child_deleting" || error?.name === "TimeoutError" || error?.name === "AbortError" || error?.status == null || Number(error.status) >= 500 || [
				408,
				425,
				429
	...
	const revokeCurrentShares = async () => {
		...
		if (!shareSid) return;
		if (shareActionSessionRef.current) {
			flash("Another share action is still in progress");
			return;
		}
		...
		try {
			...
			refreshSessions();
			flash(result.revoked ? `Revoked ${result.revoked} link${result.revoked === 1 ? "" : "s"}.` : "No active links.");
		} catch (error) {
			if (!mountedRef.current) return;
	...
	const retryHandlerFor = (assistantIndex) => {
		...
		if (composerPaused && !retryChecking || shareActionSessionRef.current || forkActionSessionRef.current === (sidRef.current || sid)) return null;
	...
	const revertState = (change) => {
		...
		return key ? {
			busy: revertingChanges.has(key),
			done: revertedChanges.has(key)
		} : {};
	};
	const attentionPermissionHandoffActive = attentionPermissionFocus.phase !== "idle";
	const attentionPermissionTargetVisible = attentionPermissionFocus.phase === "ready" && pending.some((request) => request?.id === attentionPermissionFocus.id);
	const renderThread = () => /* @__PURE__ */ _jsxs(_Fragment, { children: [
		msgs.length === 0 && /* @__PURE__ */ _jsxs("div", {
			className: "asst-empty",
			children: [/* @__PURE__ */ _jsxs("div", {
				...
				children: [
					"Ask anything — inspect the code, read your runs, steer or create runs",
					runId ? ` · run “${runId}” is open` : "",
					"."
				]
			}), !input.trim() && /* @__PURE__ */ _jsx("div", {
				className: "asst-hints",
				children: HINTS.map((h) => /* @__PURE__ */ _jsx("button", {
					className: "asst-hint",
		...
		msgs.map((m, i) => /* @__PURE__ */ _jsxs(React.Fragment, { children: [m.role === "user" && m.context && /* @__PURE__ */ _jsxs("div", {
			className: "asst-ctx-cap",
			title: "context attached to this message",
			children: [
				m.context.run && /* @__PURE__ */ _jsxs("span", {
					className: "asst-ctx-i",
					children: [
						/* @__PURE__ */ _jsx(OpIcon, {
							name: "folder",
							size: 10
				...
				(m.context.refs || []).map((r) => /* @__PURE__ */ _jsxs("span", {
					className: "asst-ctx-i",
					children: ["#", r]
				}, "r" + r)),
				(m.context.files || []).map((f) => /* @__PURE__ */ _jsxs("span", {
					className: "asst-ctx-i",
					children: [
						/* @__PURE__ */ _jsx(OpIcon, {
							name: "clip",
			...
			runsById,
			readOnly: historical,
			onRevert: historical ? null : onRevert,
			onRetry: retryHandlerFor(i),
			retryLabel: shareUnknown || shareVerifying ? shareVerifying ? "Checking status…" : "Verify status" : "Retry",
			retryBusy: shareVerifying || retryChecking,
			onOpenSettings: openAssistantSettings,
			onRunOpen: openRunFromAssistant,
			launchChat: launchChatThrough(i),
			revertState,
			launchSessionId: sid,
			launchMessageId: m.turn_id || m.id,
			launchMessageIndex: i,
			launchDrafts,
			launchDisclosures,
			onLaunchDraft: (key, draft) => setLaunchDrafts((current) => retainLaunchDraft(current, key, draft)),
			onLaunchDisclosure: (key, open) => setLaunchDisclosures((current) => retainLaunchDisclosure(current, key, open)),
			onLaunchStarted: (key) => {
		...
		!historical && pending.length > 0 && /* @__PURE__ */ _jsx("div", {
			className: "asst-perm-region",
			role: "region",
			"aria-label": `${pending.length} pending Assistant approval${pending.length === 1 ? "" : "s"}`,
			"aria-live": "assertive",
			"aria-atomic": "false",
			children: pending.map((req, index) => {
				const focusedFromAttention = attentionPermissionTargetVisible && req.id === attentionPermissionFocus.id;
				return /* @__PURE__ */ _jsx(PermCard, {
					req,
	...
	const fileChips = files.length > 0 && /* @__PURE__ */ _jsx("div", {
		className: "asst-files",
		children: files.map((f) => /* @__PURE__ */ _jsxs("span", {
			className: "chip xs file",
			title: `${(f.size / 1024).toFixed(1)} KB${f.truncated ? " · truncated" : ""}`,
			children: [
				/* @__PURE__ */ _jsx(OpIcon, {
					name: "doc",
					size: 11
	...
	const attachBtn = (cls) => /* @__PURE__ */ _jsx("button", {
		className: cls,
		"aria-label": "Attach text files",
		title: historical ? readOnlyShort : sessionOpening ? "Wait for the selected Assistant chat to finish opening" : shareUnknown ? "Attach to this draft; sending is paused until public-link status is verified" : shareBusy ? "Wait for the current public-link action" : forkingCurrentSession ? "Wait for this chat to finish forking" : draftRunMismatch ? draftRunMismatchMessage : pendingFileReads > 0 ? "Wait for the selected attachment to finish reading" : "attach text file(s)",
	...
	const modeRow = /* @__PURE__ */ _jsxs("div", {
		className: "asst-moderow",
		children: [/* @__PURE__ */ _jsx("div", {
			className: "asst-modes",
			children: MODES.map((x) => /* @__PURE__ */ _jsx("button", {
				"aria-pressed": x.id === mode,
				className: "asst-mode" + (x.id === mode ? " on" : ""),
				disabled: historical || composerEditingPaused,
				title: historical ? readOnlyShort : sessionOpening ? "Wait for the selected Assistant chat to finish opening" : shareUnknown ? "Choose a draft mode; sending is paused until public-link status is verified" : forkingCurrentSession ? "Wait for this chat to finish forking" : x.hint,
				onClick: () => {
					if (!openSessionPendingRef.current) setComposerMode(x.id);
				},
				children: x.label
			}, x.id))
		}), /* @__PURE__ */ _jsx("span", {
			className: "asst-modehint muted",
	...
	const suggestionPop = showSuggestions ? /* @__PURE__ */ _jsx("div", {
		className: "cmdbar-pop",
		id: "assistant-command-listbox",
		role: "listbox",
		"aria-label": "Assistant commands",
		children: suggestions.map((c, index) => /* @__PURE__ */ _jsxs("button", {
			id: `assistant-command-option-${index}`,
			className: "cmdbar-pop-item",
			role: "option",
			tabIndex: -1,
			...
			children: [/* @__PURE__ */ _jsxs("b", { children: ["/", c.name] }), /* @__PURE__ */ _jsxs("span", {
				className: "muted",
				children: [" ", c.desc]
			})]
		}, c.name))
	...
	const composer = (placeholder) => /* @__PURE__ */ _jsxs("div", {
		className: "chat-in asst-in" + (directConfirm ? " direct-confirming" : ""),
		children: [
			historical && /* @__PURE__ */ _jsx("div", {
				className: "assistant-history-lock",
				children: runLoadingLocked ? "Verifying this run · Assistant actions are paused." : runUnavailableLocked ? "Run unavailable · Assistant actions are paused. Retry the run or return to the run list." : startOverLocked ? "Start over unresolved · Assistant paused until the exact request is recovered." : reviewLocked ? "Read-only review · Assistant actions are unavailable." : staleDiagnostic ? "Diagnostic link generation mismatch · Assistant paused. Open the current generation to continue." : `History seq ${runAccess.seq} · Assistant paused. Return live to ask about or change this run.`
			}),
			directConfirm?.phase === "checking" && /* @__PURE__ */ _jsxs("div", {
				className: "assistant-command-pending",
				children: [/* @__PURE__ */ _jsx("span", {
					ref: directConfirmStatusRef,
					tabIndex: -1,
					role: "status",
					"aria-live": "polite",
					"aria-atomic": "true",
					children: "Verifying the exact approval target · nothing sent…"
				}), /* @__PURE__ */ _jsx("button", {
					ref: directConfirmCancelRef,
					type: "button",
					className: "btn sm ghost",
					onClick: cancelDirectConfirmation,
					children: "Cancel · keep draft"
				})]
			}),
			directConfirm?.phase === "ready" && /* @__PURE__ */ _jsxs("div", {
				...
				children: [
					/* @__PURE__ */ _jsxs("div", {
						...
						children: [
							...
							/* @__PURE__ */ _jsx("b", {
								id: "assistant-direct-confirm-title",
								children: directConfirm.title
							}),
							/* @__PURE__ */ _jsx("span", {
								className: "asst-perm-risk consequential",
					...
					/* @__PURE__ */ _jsxs("dl", {
						...
						id: "assistant-direct-confirm-details",
						children: [
							/* @__PURE__ */ _jsxs("div", { children: [/* @__PURE__ */ _jsx("dt", { children: "Run" }), /* @__PURE__ */ _jsx("dd", { children: /* @__PURE__ */ _jsx("code", { children: directConfirm.runId }) })] }),
							/* @__PURE__ */ _jsxs("div", { children: [/* @__PURE__ */ _jsx("dt", { children: "Version" }), /* @__PURE__ */ _jsx("dd", { children: /* @__PURE__ */ _jsx("code", {
								className: "assistant-direct-identity",
								children: directConfirm.expectedGeneration
							}) })] }),
							Number.isSafeInteger(directConfirm.nodeGeneration) && /* @__PURE__ */ _jsxs("div", { children: [/* @__PURE__ */ _jsx("dt", { children: "Attempt" }), /* @__PURE__ */ _jsxs("dd", { children: [
								"experiment #",
								directConfirm.direct.arg,
								" · generation ",
								directConfirm.nodeGeneration
							] })] }),
							/* @__PURE__ */ _jsxs("div", { children: [/* @__PURE__ */ _jsx("dt", { children: "Command" }), /* @__PURE__ */ _jsx("dd", { children: /* @__PURE__ */ _jsx("code", { children: directConfirm.commandText }) })] }),
							/* @__PURE__ */ _jsxs("div", { children: [/* @__PURE__ */ _jsx("dt", { children: "Mode" }), /* @__PURE__ */ _jsx("dd", { children: directConfirm.modeLabel })] }),
							/* @__PURE__ */ _jsxs("div", { children: [/* @__PURE__ */ _jsx("dt", { children: "Consequence" }), /* @__PURE__ */ _jsx("dd", { children: directConfirm.consequence })] })
						]
					}),
					/* @__PURE__ */ _jsxs("div", {
						...
						children: [/* @__PURE__ */ _jsx("button", {
							...
							onClick: confirmDirectCommand,
							children: directConfirm.actionLabel
						})]
					})
				]
			}),
			currentShareAckNotice && /* @__PURE__ */ _jsxs("div", {
				className: "assistant-command-pending error",
				role: "alert",
				"aria-live": "assertive",
				"aria-atomic": "true",
				children: [/* @__PURE__ */ _jsx("span", { children: currentShareAckNotice.message }), /* @__PURE__ */ _jsx("button", {
					type: "button",
					className: "btn sm ghost",
			...
			(shareUnknown || shareVerifying) && /* @__PURE__ */ _jsxs("div", {
				className: "assistant-command-pending assistant-share-pending error",
				children: [
					/* @__PURE__ */ _jsx("span", { children: shareStatusMessage }),
					/* @__PURE__ */ _jsx("button", {
						type: "button",
						className: "btn sm",
						onClick: retryShareStatus,
						disabled: shareBusy || shareVerifying || sessionsStatus === "loading" || sessionsStatus === "refreshing",
						children: shareVerifying || sessionsStatus === "loading" || sessionsStatus === "refreshing" ? "Checking status…" : "Retry status"
					}),
					/* @__PURE__ */ _jsx("button", {
						type: "button",
						className: "btn sm ghost",
						onClick: revokeCurrentShares,
						disabled: shareBusy || forkingCurrentSession,
						children: "Unshare"
					})
				]
			}),
			sessionOpening && /* @__PURE__ */ _jsx("div", {
				className: "assistant-command-pending",
				role: "status",
				"aria-live": "polite",
				"aria-atomic": "true",
				children: attentionPermissionFocus.phase === "loading" ? "Opening the selected Assistant chat; the exact approval will load next…" : "Opening the selected Assistant chat…"
			}),
			attentionPermissionFocus.phase === "loading" && !sessionOpening && /* @__PURE__ */ _jsx("div", {
				className: "assistant-command-pending",
				role: "status",
				"aria-live": "polite",
				"aria-atomic": "true",
				children: "Loading the exact Assistant approval…"
			}),
			retryChecking && /* @__PURE__ */ _jsx("div", {
				className: "assistant-command-pending",
				children: "Checking the saved Assistant turn…"
			}),
			(commandBusy || directFailure) && /* @__PURE__ */ _jsxs("div", {
				ref: commandStatusRef,
				tabIndex: -1,
				className: "assistant-command-pending" + (showDirectFailure ? " error" : ""),
				role: directNeedsAlert ? "alert" : "status",
				"aria-live": directNeedsAlert ? "assertive" : "polite",
				"aria-atomic": "true",
				children: [
					/* @__PURE__ */ _jsx("span", { children: showDirectFailure ? directFailureText : pendingCommandText }),
					canCheckDirect && /* @__PURE__ */ _jsx("button", {
						className: "btn sm",
						disabled: directRecoveryPaused,
						onClick: checkDirect,
						children: "Check same command"
					}),
					directPending?.protocolInvalid && /* @__PURE__ */ _jsx("button", {
						className: "btn sm ghost",
						disabled: !!directPending.checking,
						onClick: dismissProtocolDirect,
						children: "Dismiss"
					}),
					canRetryDirect && /* @__PURE__ */ _jsx("button", {
						className: "btn sm",
						disabled: directRecoveryPaused,
						onClick: retryDirect,
						children: "Retry same command"
					}),
					showDirectFailure && /* @__PURE__ */ _jsx("button", {
						className: "btn sm ghost",
						onClick: dismissDirectFailure,
						children: "Dismiss"
					})
				]
			}),
			draftRunMismatch && /* @__PURE__ */ _jsxs("div", {
				className: "assistant-command-pending error",
				role: "alert",
				"aria-live": "assertive",
				"aria-atomic": "true",
				children: [/* @__PURE__ */ _jsx("span", { children: draftRunMismatchMessage }), /* @__PURE__ */ _jsx("button", {
					type: "button",
					className: "btn sm ghost",
					disabled: sessionOpening,
					onClick: useDraftHere,
					children: runId ? "Use in this run" : "Use without a run"
				})]
			}),
			pendingFileReads > 0 && /* @__PURE__ */ _jsx("div", {
				className: "assistant-command-pending",
				role: "status",
				"aria-live": "polite",
				"aria-atomic": "true",
				children: "Reading selected attachment…"
			}),
			runId && refNodes(input).length > 0 && /* @__PURE__ */ _jsx("div", {
				className: "cmdbar-ctx",
				children: refNodes(input).map((id) => /* @__PURE__ */ _jsxs("span", {
					className: "chip xs",
					children: [
						"#",
						id,
						/* @__PURE__ */ _jsx("button", {
							className: "chip-x",
							"aria-label": `Detach experiment ${id}`,
							disabled: composerEditingPaused,
							onClick: () => {
								if (openSessionPendingRef.current) return;
			...
			/* @__PURE__ */ _jsxs("div", {
				...
				children: [
					...
					/* @__PURE__ */ _jsx("textarea", {
						...
						"aria-describedby": [draftingNewRun ? "assistant-new-run-hint" : "", shareUnknown || shareVerifying ? "assistant-share-status" : ""].filter(Boolean).join(" ") || undefined,
						...comboAria,
						disabled: historical || commandBusy || composerEditingPaused,
						onChange: changeInput,
						onKeyDown: onKey,
						placeholder: historical ? readOnlyShort : shareUnknown ? "Public-link status unknown · keep drafting; Send will verify it" : shareBusy ? "Finishing public-link action…" : forkingCurrentSession ? "Forking this chat…" : placeholder
					}),
					busy ? /* @__PURE__ */ _jsx("button", {
						...
						disabled: sessionOpening || retryChecking || turnStarting || historical || commandBusy || composerEditingPaused || draftRunMismatch || pendingFileReads > 0 || !input.trim() && files.length === 0,
						onClick: send,
						children: sessionOpening ? "Opening…" : retryChecking ? "Checking…" : turnStarting ? "Starting…" : shareUnknown || shareVerifying ? shareVerifying ? "Checking…" : "Verify to send" : commandBusy || shareBusy ? "Waiting…" : forkingCurrentSession ? "Forking…" : pendingFileReads > 0 ? "Reading…" : "Send"
					})
				]
			}),
			draftingNewRun && /* @__PURE__ */ _jsx("div", {
				id: "assistant-new-run-hint",
				className: "asst-new-run-hint",
				role: "note",
				children: "Describe the goal after /new, then Send. Send uses the configured model to draft a launch card. Nothing starts until you review it and press Start run."
			}),
			modeRow
		]
	});
	const hiddenFileInput = /* @__PURE__ */ _jsx("input", {
		ref: fileRef,
		type: "file",
		multiple: true,
		style: { display: "none" },
	...
	useEffect(() => {
		const expiryMs = Number(currentSession?.share_expires_at) * 1e3;
		...
		const arm = () => {
			...
			timer = window.setTimeout(arm, Math.min(remaining + 250, 2147e6));
		};
		arm();
		return () => {
			cancelled = true;
		...
		sid,
		currentSession?.share_expires_at,
		refreshSessions
	]);
	useEffect(() => {
		...
		const expiryMs = shareCopy.expiresAt * 1e3;
		let timer = null;
		let cancelled = false;
		const arm = () => {
			if (cancelled) return;
	...
	return /* @__PURE__ */ _jsxs(_Fragment, { children: [
		...
		/* @__PURE__ */ _jsx("output", {
			...
			"aria-live": "polite",
			"aria-atomic": "true",
			children: sessionOpening ? view === "bar" ? "Opening Assistant chat." : "" : retryChecking ? "Checking saved Assistant turn." : turnStarting ? "Starting Assistant response." : busy ? "Assistant is responding." : replyAnnouncement
		}),
		/* @__PURE__ */ _jsx("div", {
			id: "assistant-share-status",
			className: "sr-only",
			role: "status",
			"aria-live": "polite",
			"aria-atomic": "true",
			children: shareStatusMessage
		}),
		view === "bar" && /* @__PURE__ */ _jsx("div", {
			className: "cmdbar-wrap",
			children: /* @__PURE__ */ _jsxs("div", {
				className: "cmdbar-dock" + (sessionOpening || retryChecking || turnStarting || busy || commandBusy || forkingCurrentSession ? " thinking" : "") + (hasNew ? " fresh" : ""),
				children: [
					/* @__PURE__ */ _jsx("button", {
						className: "cmdbar-ic",
					...
					/* @__PURE__ */ _jsxs("button", {
						type: "button",
						className: `cmdbar-mode mode-${mode}`,
						...
						onClick: openSide,
						children: [/* @__PURE__ */ _jsx("span", {
							className: "cmdbar-mode-prefix",
							children: "Mode · "
						}), /* @__PURE__ */ _jsx("span", { children: activeMode.label })]
					}),
					/* @__PURE__ */ _jsxs("div", {
						className: "cmdbar-field",
						children: [
							(refNodes(input).length > 0 || files.length > 0) && /* @__PURE__ */ _jsxs("div", {
								className: "cmdbar-ctx",
								children: [runId && refNodes(input).map((id) => /* @__PURE__ */ _jsxs("span", {
									className: "chip xs",
									children: ["#", id]
								}, id)), files.map((f) => /* @__PURE__ */ _jsxs("span", {
									className: "chip xs file",
									children: [
										/* @__PURE__ */ _jsx(OpIcon, {
											name: "doc",
							...
							/* @__PURE__ */ _jsx("input", {
								...
								onChange: changeInput,
								onKeyDown: onKey,
								placeholder: historical ? readOnlyShort : shareUnknown ? "Public-link status unknown · keep drafting; Send will verify it" : shareBusy ? "Finishing public-link action…" : forkingCurrentSession ? "Forking this chat…" : runId ? "Command or ask…  /stop · pause · #12 to attach an experiment · or describe what to do" : "Describe a run to start, or ask the assistant…  ( / for commands )"
							})
						]
					}),
					attachBtn("cmdbar-attach"),
					shareUnknown || shareVerifying ? /* @__PURE__ */ _jsxs("span", {
						className: "cmdbar-status thinking recovery error",
						children: [
							/* @__PURE__ */ _jsxs("span", { children: [
								/* @__PURE__ */ _jsx("span", {
									className: "cmdbar-who",
									children: shareVerifying ? "checking public-link status" : "public-link status unknown"
								}),
								" · ",
								"draft available · nothing sent"
							] }),
							/* @__PURE__ */ _jsx("button", {
								type: "button",
								className: "btn sm",
								onClick: retryShareStatus,
								disabled: shareBusy || shareVerifying || sessionsStatus === "loading" || sessionsStatus === "refreshing",
								children: shareVerifying ? "Checking…" : "Retry status"
							}),
							/* @__PURE__ */ _jsx("button", {
								type: "button",
								className: "btn sm ghost",
					...
					}) : sessionOpening ? /* @__PURE__ */ _jsxs("span", {
						className: "cmdbar-status thinking",
						children: [/* @__PURE__ */ _jsx("span", { className: "cmdbar-pip" }), " opening selected chat..."]
					}) : retryChecking ? /* @__PURE__ */ _jsxs("span", {
						className: "cmdbar-status thinking",
						children: [/* @__PURE__ */ _jsx("span", { className: "cmdbar-pip" }), " checking saved turn..."]
					}) : turnStarting ? /* @__PURE__ */ _jsxs("span", {
						className: "cmdbar-status thinking",
						children: [/* @__PURE__ */ _jsx("span", { className: "cmdbar-pip" }), " starting response..."]
					}) : busy ? /* @__PURE__ */ _jsxs("span", {
						className: "cmdbar-status thinking" + (liveShareActive ? " assistant-live-share" : ""),
						children: [/* @__PURE__ */ _jsx("span", { className: "cmdbar-pip" }), liveShareActive ? /* @__PURE__ */ _jsxs(_Fragment, { children: [/* @__PURE__ */ _jsx("span", {
							className: "cmdbar-who",
							children: "live public link"
						}), " this reply remains public · thinking…"] }) : " thinking…"]
					}) : commandBusy ? /* @__PURE__ */ _jsxs("span", {
						ref: commandStatusRef,
						tabIndex: -1,
						...
						"aria-atomic": "true",
						children: [
							/* @__PURE__ */ _jsx("span", { className: "cmdbar-pip" }),
							" ",
							pendingCommandText || "Waiting for command…",
							canCheckDirect && /* @__PURE__ */ _jsx("button", {
								className: "btn sm",
								disabled: directRecoveryPaused,
								onClick: checkDirect,
					...
					}) : directFailure ? /* @__PURE__ */ _jsxs("span", {
						ref: commandStatusRef,
						tabIndex: -1,
						className: "cmdbar-status thinking recovery error",
						role: "alert",
						...
						"aria-atomic": "true",
						children: [
							/* @__PURE__ */ _jsx("span", { children: directFailureText }),
							canRetryDirect && /* @__PURE__ */ _jsx("button", {
								className: "btn sm",
								disabled: directRecoveryPaused,
								onClick: retryDirect,
					...
					}) : currentShareAckNotice ? /* @__PURE__ */ _jsxs("span", {
						className: "cmdbar-status thinking recovery error",
						role: "alert",
						"aria-live": "assertive",
						"aria-atomic": "true",
						children: [/* @__PURE__ */ _jsxs("span", { children: [/* @__PURE__ */ _jsx("span", {
							className: "cmdbar-who",
							children: "nothing sent"
						}), " · public-link state changed"] }), /* @__PURE__ */ _jsx("button", {
							type: "button",
							className: "btn sm ghost",
							onClick: () => setShareAckNotice((current) => current?.sid === sid ? null : current),
							children: "Dismiss"
						})]
					}) : draftRunMismatch ? /* @__PURE__ */ _jsxs("span", {
						className: "cmdbar-status thinking recovery error",
						role: "alert",
						"aria-live": "assertive",
						"aria-atomic": "true",
						title: draftRunMismatchMessage,
					...
					}) : forkingCurrentSession ? /* @__PURE__ */ _jsxs("span", {
						className: "cmdbar-status thinking",
						role: "status",
						"aria-live": "polite",
						"aria-atomic": "true",
						children: [/* @__PURE__ */ _jsx("span", { className: "cmdbar-pip" }), " forking this chat…"]
					}) : pendingFileReads > 0 ? /* @__PURE__ */ _jsxs("span", {
						className: "cmdbar-status thinking",
						role: "status",
						"aria-live": "polite",
						"aria-atomic": "true",
						children: [/* @__PURE__ */ _jsx("span", { className: "cmdbar-pip" }), " reading selected attachment…"]
					}) : liveShareActive ? /* @__PURE__ */ _jsxs("button", {
						className: "cmdbar-status preview assistant-live-share",
						type: "button",
						...
						children: [
							...
							" new messages remain public",
							/* @__PURE__ */ _jsx("span", {
								className: "cmdbar-more",
								children: " ▸"
							})
						]
					}) : preview ? /* @__PURE__ */ _jsxs("button", {
						className: "cmdbar-status preview",
					...
					busy ? /* @__PURE__ */ _jsx("button", {
						...
						className: "cmdbar-go",
						"aria-label": sessionOpening ? "Opening selected Assistant chat" : retryChecking ? "Checking saved Assistant turn" : turnStarting ? "Starting Assistant response" : shareUnknown || shareVerifying ? shareVerifying ? "Checking public-link status; nothing sent" : "Verify public-link status before sending" : "Send Assistant message",
						title: sessionOpening ? "Wait for the selected Assistant chat to finish opening" : retryChecking ? "Wait for the saved Assistant turn check to finish" : turnStarting ? "Wait for the Assistant response to start" : draftRunMismatch ? draftRunMismatchMessage : shareUnknown || shareVerifying ? shareVerifying ? "Checking public-link status; your draft remains editable" : "Messaging paused until public-link status is verified" : commandBusy ? "Waiting for the current run command" : historical ? readOnlyShort : forkingCurrentSession ? "Wait for this chat to finish forking" : pendingFileReads > 0 ? "Wait for the selected attachment to finish reading" : "send (Enter)",
						disabled: sessionOpening || retryChecking || turnStarting || historical || commandBusy || composerEditingPaused || draftRunMismatch || pendingFileReads > 0 || !input.trim() && files.length === 0,
						onClick: send,
						children: "▶"
					}),
					/* @__PURE__ */ _jsx("button", {
						className: "cmdbar-drawer-btn",
						"aria-label": "Open Assistant in side view",
						title: "open chat on the right (side view)",
						onClick: openSide,
						children: /* @__PURE__ */ _jsx(OpIcon, {
							name: "chat",
							size: 13
					...
					visibleToast && /* @__PURE__ */ _jsx("div", {
						className: "cmdbar-toast",
						role: "status",
						"aria-live": "polite",
						"aria-atomic": "true",
						children: visibleToast
					})
				]
			})
		}),
		view === "side" && compactAssistant && /* @__PURE__ */ _jsx("div", {
			className: "asst-side-backdrop",
			"aria-hidden": "true",
			onPointerDown: collapseToBar
		}),
		view === "side" && /* @__PURE__ */ _jsxs("aside", {
			ref: sideDialogRef,
			className: "asst-side-panel",
			"aria-label": "Assistant",
			role: compactAssistant ? "dialog" : undefined,
			"aria-modal": compactAssistant ? "true" : undefined,
			"aria-hidden": deleteConfirm ? "true" : undefined,
			inert: deleteConfirm ? "" : undefined,
			tabIndex: compactAssistant ? -1 : undefined,
			style: { width: sideW },
			children: [
				!compactAssistant && /* @__PURE__ */ _jsx("div", {
					className: "asst-resize",
					role: "separator",
				...
				/* @__PURE__ */ _jsxs("div", {
					className: "asst-drawer-h",
					children: [
						/* @__PURE__ */ _jsx("b", {
							className: "asst-drawer-ttl",
							children: "Assistant"
						}),
						ctxChip,
						launchRecoveryButton,
						sid && (currentSession?.shared || shareCopy || shareUnknown) && /* @__PURE__ */ _jsx("button", {
							type: "button",
							className: "btn sm assistant-share-state" + (shareUnknown || liveShareActive ? " warn" : ""),
							title: "Open the full Assistant to inspect or revoke public links",
							onClick: openFull,
							children: shareUnknown ? "share status unknown · messaging paused" : liveShareActive ? "LIVE · new replies public" : "snapshot public"
						}),
						/* @__PURE__ */ _jsx("span", {
							className: "spacer",
							style: { flex: 1 }
						}),
						compactAssistant && attentionIndicator.active && /* @__PURE__ */ _jsx(AttentionLauncher, {
						...
						/* @__PURE__ */ _jsx("button", {
							className: "btn sm ghost",
							"aria-label": "Start a new Assistant chat",
							title: turnStarting ? "Wait for the new Assistant chat to finish starting" : retryChecking ? "Wait for the saved turn check to finish" : directConfirm ? "Resolve the run-command confirmation first" : "new chat",
							disabled: turnStarting || retryChecking || !!directConfirm,
							onClick: newChat,
							children: "＋ Chat"
						}),
						/* @__PURE__ */ _jsx("button", {
							className: "btn sm ghost",
							title: "expand to the full view",
							onClick: openFull,
							children: "⤢ full"
						}),
						/* @__PURE__ */ _jsx("button", {
							...
							children: directConfirm ? "Cancel command" : "▾ bar"
						})
					]
				}),
				/* @__PURE__ */ _jsx("div", {
					className: "asst-drawer-feed",
					ref: feedRef,
					role: "log",
		...
		view === "full" && /* @__PURE__ */ _jsxs("div", {
			ref: fullDialogRef,
			className: "asst-view asst-full",
			role: "dialog",
			"aria-modal": "true",
			"aria-label": "Assistant",
			"aria-hidden": deleteConfirm ? "true" : undefined,
				...
				children: [/* @__PURE__ */ _jsxs("div", {
					...
					children: [
						...
						/* @__PURE__ */ _jsx("button", {
							className: "btn sm primary",
							"aria-label": "Start a new Assistant chat",
							title: turnStarting ? "Wait for the new Assistant chat to finish starting" : retryChecking ? "Wait for the saved turn check to finish" : directConfirm ? "Resolve the run-command confirmation first" : undefined,
							disabled: turnStarting || retryChecking || !!directConfirm,
							onClick: newChat,
							children: "+ Chat"
						})
					...
					ref: sessionsRef,
					className: "asst-sessions",
					"aria-busy": sessionsStatus === "loading" || sessionsStatus === "refreshing",
					children: [
						sessionsStatus === "loading" && /* @__PURE__ */ _jsx("div", {
							className: "asst-session-resource",
							role: "status",
							children: /* @__PURE__ */ _jsx("span", { children: "Loading chats…" })
						}),
						sessionsStatus === "refreshing" && /* @__PURE__ */ _jsx("div", {
							className: "asst-session-resource",
							role: "status",
							children: /* @__PURE__ */ _jsx("span", { children: "Refreshing chats…" })
						}),
						["error", "stale"].includes(sessionsStatus) && /* @__PURE__ */ _jsxs("div", {
							...
							children: [
								...
								/* @__PURE__ */ _jsx("span", { children: sessionsStatus === "stale" ? "Showing the last loaded chat list. Refresh failed." : "Chats could not be loaded." }),
								/* @__PURE__ */ _jsx("button", {
									type: "button",
									className: "btn xs",
						...
						sessionsStatus === "ready" && sessions.length === 0 && /* @__PURE__ */ _jsx("div", {
							className: "muted asst-session-empty",
							children: "No chats yet."
						}),
						sessions.map((s) => /* @__PURE__ */ _jsxs("div", {
							className: "asst-sess" + (s.id === sid ? " active" : "") + (String(openingSid || "") === String(s.id) ? " opening" : "") + (s.cleanup_required ? " cleanup-required" : ""),
							children: [/* @__PURE__ */ _jsxs("button", {
								type: "button",
								className: "asst-sess-open",
								"aria-current": s.id === sid ? "page" : undefined,
								"aria-busy": String(openingSid || "") === String(s.id) || undefined,
								"aria-label": String(openingSid || "") === String(s.id) ? `Opening chat ${s.title || "Chat"}` : s.cleanup_required ? "Incomplete chat deletion; use Delete to retry cleanup" : undefined,
								title: String(openingSid || "") === String(s.id) ? "Opening this Assistant chat…" : turnStarting ? "Wait for the new Assistant chat to finish starting" : retryChecking ? "Wait for the saved turn check to finish" : s.cleanup_required ? "This partial chat cannot be opened. Delete it to retry cleanup." : undefined,
								disabled: s.cleanup_required === true || turnStarting || retryChecking || !!directConfirm || String(openingSid || "") === String(s.id),
								onClick: () => openSession(s.id),
								children: [/* @__PURE__ */ _jsx("span", {
									className: "asst-sess-t",
									children: s.title || "Chat"
								}), /* @__PURE__ */ _jsx("span", {
									className: "asst-sess-m",
									children: String(openingSid || "") === String(s.id) ? "Opening…" : s.cleanup_required ? "Cleanup required · delete to retry" : s.shared ? `${Number(s.share_count) > 1 ? `${s.share_count} public links` : "Public link"} ${s.share_live ? "LIVE · new replies public" : "active"}${s.share_expires_at ? ` · until ${fmtDate(s.share_expires_at)}` : ""}` : fmtAgo(s.updated)
								})]
							}), /* @__PURE__ */ _jsx("button", {
								type: "button",
								className: "asst-sess-x",
								onClick: (e) => requestDeleteSession(s, e),
								disabled: forkBusySid === s.id || turnStarting || !!directConfirm || String(openingSid || "") === String(s.id) || s.id === sid && (retryChecking || busy || pending.length > 0),
								title: forkBusySid === s.id ? "Wait for this chat to finish forking" : turnStarting ? "Wait for the new Assistant chat to finish starting" : String(openingSid || "") === String(s.id) ? "Wait for this chat to finish opening" : s.id === sid && retryChecking ? "Wait for the saved Assistant turn check to finish" : s.id === sid && (busy || pending.length > 0) ? "Stop or finish this Assistant turn before deleting the chat" : undefined,
								"aria-label": s.cleanup_required ? `Retry cleanup for ${s.title || "incomplete chat deletion"}` : `Delete chat ${s.title || "Chat"}`,
								children: "✕"
							})]
						}, s.id))
					]
				...
				children: [
					/* @__PURE__ */ _jsxs("div", {
						...
						children: [
							...
							attentionIndicator.active && /* @__PURE__ */ _jsx(AttentionLauncher, {
								indicator: attentionIndicator,
								embedded: true,
								onClick: openAttentionCenter
							}),
							ctxChip,
							launchRecoveryButton,
							sid && /* @__PURE__ */ _jsx("button", {
								className: "btn sm ghost",
								type: "button",
								title: forkBusySid === sid ? "Forking this chat…" : forkBusy ? "Another Assistant fork is still in progress" : deletingCurrentSession ? "This chat is being deleted" : currentForkRecovery ? "Check the exact pending fork before starting another" : shareTurnIncomplete ? "Wait for the current Assistant reply to finish or recover it before forking" : shareBusy ? "Wait for the current share action before forking" : "Fork this complete chat into a new session",
								disabled: forkBusy || shareBusy || deletingCurrentSession || !!directConfirm || shareTurnIncomplete && !currentForkRecovery,
								onClick: forkCurrentSession,
								children: forkBusySid === sid ? "forking…" : currentForkRecovery ? "⑂ check fork" : "⑂ fork"
							}),
							sid && (currentSession?.shared || shareCopy || shareUnknown) && /* @__PURE__ */ _jsx("span", {
								className: "pill assistant-share-state" + (shareUnknown || liveShareActive ? " warn" : ""),
								children: shareUnknown ? "share state unknown" : liveShareActive ? "LIVE public link · new replies public" : "public snapshot active"
							}),
							shareUnknown && /* @__PURE__ */ _jsx("button", {
								className: "btn sm",
								type: "button",
								onClick: retryShareStatus,
								disabled: shareBusy || shareVerifying || sessionsStatus === "loading" || sessionsStatus === "refreshing",
								title: "Verify the exact active public-link capabilities before messaging",
								children: shareVerifying || sessionsStatus === "loading" || sessionsStatus === "refreshing" ? "checking share…" : "retry share status"
							}),
							sid && !currentSession && !shareCopy && !shareUnknown && /* @__PURE__ */ _jsx("button", {
								className: "btn sm ghost",
								type: "button",
								...
								onClick: refreshSessions,
								children: sessionsStatus === "loading" || sessionsStatus === "refreshing" ? "checking share…" : "retry share status"
							}),
							sid && currentSession && !deletingCurrentSession && !currentSession.shared && !shareUnknown && !shareCopy && /* @__PURE__ */ _jsx("button", {
								className: "btn sm ghost",
								title: forkingCurrentSession ? "Wait for this chat to finish forking before creating a snapshot" : shareTurnIncomplete ? "Wait for the current Assistant turn to finish before freezing a complete snapshot" : "Create and copy a frozen read-only snapshot",
								...
								onClick: async () => {
									...
									if (runningRef.current || turnCaptureRef.current || busy || commandBusy || pending.length > 0 || msgs[msgs.length - 1]?.role === "user") {
									...
									try {
										...
										mutateSessionsLocally((current) => current.map((session) => session.id === shareSid ? {
											...
											shared: true,
											share_count: 1,
											share_ids: [receipt.shareId],
											live_share_ids: [],
											share_expires_at: receipt.expiresAt,
											share_live: false
										} : session));
										...
										if (error?.status >= 400 && error.status < 500) {
											flash(error?.code === "assistant_share_snapshot_too_large" ? "This chat is too large for a complete snapshot · fork or share a shorter chat" : [
												"assistant_share_in_progress",
												"assistant_share_incomplete",
												"assistant_share_turn_active",
												"assistant_share_turn_incomplete"
											].includes(error?.code) ? "Wait for a complete Assistant reply before sharing" : "Share failed");
										} else {
											setShareUnknown(shareSid, true);
											refreshSessions();
								...
								children: shareBusySid === sid ? "working…" : "⤴ create snapshot"
							}),
							sid && !deletingCurrentSession && (currentSession?.shared || shareUnknown || shareCopy) && /* @__PURE__ */ _jsx("button", {
								className: "btn sm ghost",
								title: forkingCurrentSession ? "Wait for this chat to finish forking before revoking links" : "Revoke every share link",
								disabled: shareBusy || forkingCurrentSession,
								onClick: revokeCurrentShares,
								children: shareBusySid === sid ? "working…" : `⤫ ${shareUnknown ? "revoke pending" : "unshare"}`
							}),
							/* @__PURE__ */ _jsx("button", {
								className: "btn sm ghost",
					...
					shareCopy && /* @__PURE__ */ _jsxs("div", {
						className: "copy-link-fallback",
						role: "status",
						children: [
							/* @__PURE__ */ _jsxs("label", {
								htmlFor: `assistant-share-fallback-${sid}`,
								children: [
									"Frozen snapshot · expires ",
									fmtDate(shareCopy.expiresAt),
									" · link remains visible in this tab"
								]
							}),
							/* @__PURE__ */ _jsx("input", {
								id: `assistant-share-fallback-${sid}`,
								readOnly: true,
								value: shareCopy.url,
							...
							/* @__PURE__ */ _jsx("button", {
								...
								onClick: async () => {
									try {
										...
										flash("Clipboard blocked · select the snapshot link and copy it manually");
									}
								},
								children: "Copy link"
							}),
		...
		deleteConfirm && /* @__PURE__ */ _jsx("div", {
			className: "overlay assistant-delete-overlay",
			onPointerDown: (event) => {
				if (!deleteConfirmBusy && event.target === event.currentTarget) closeDeleteConfirm();
			},
			children: /* @__PURE__ */ _jsxs("section", {
				ref: deleteDialogRef,
				className: "modal assistant-delete-dialog",
				role: "alertdialog",
				"aria-modal": "true",
				"aria-labelledby": "assistant-delete-title",
				"aria-describedby": "assistant-delete-description assistant-delete-warning",
				"aria-busy": deleteConfirmBusy,
				tabIndex: -1,
				children: [/* @__PURE__ */ _jsx("div", {
					className: "modal-h",
					children: /* @__PURE__ */ _jsx("b", {
						id: "assistant-delete-title",
						children: "Delete this Assistant chat?"
					})
				}), /* @__PURE__ */ _jsxs("div", {
					className: "modal-b",
					children: [
						/* @__PURE__ */ _jsxs("p", {
							id: "assistant-delete-description",
							className: "assistant-delete-copy",
							children: [
								"You are deleting ",
								/* @__PURE__ */ _jsxs("strong", { children: [
									"“",
									deleteConfirm.title,
									"”"
								] }),
						...
						/* @__PURE__ */ _jsx("p", {
							id: "assistant-delete-warning",
							className: "assistant-delete-warning",
							children: "Its transcript will be permanently deleted, and any live Assistant work in this chat will be cancelled. This cannot be undone."
						}),
						deleteConfirmError && /* @__PURE__ */ _jsx("div", {
							ref: deleteErrorRef,
							className: "flag assistant-delete-error",
							role: "alert",
							tabIndex: -1,
							children: deleteConfirmError
						}),
						deleteConfirmBusy && /* @__PURE__ */ _jsx("div", {
							className: "assistant-delete-progress",
							role: "status",
							"aria-live": "polite",
							children: "Deleting chat and cancelling live work…"
						}),
						/* @__PURE__ */ _jsxs("div", {
							...
							children: [/* @__PURE__ */ _jsx("button", {
								...
								children: deleteConfirmBusy ? "Deleting chat…" : "Delete chat"
							})]
						})
					]
				})]
