import { computeGroups, nodeGroupMap } from './grouping.js'
...
export function dagLayoutProjection(state, groupMode = 'none') {
  ...
  const rows = Object.values(nodes)
    .sort((a, b) => a.id - b.id)
    .map(node => {
      const parents = [...new Set((node.parent_ids || [])
        .filter(parent => parent in nodes))].sort((a, b) => a - b)
      ...
      return [node.id, parents, nodeGroup.get(node.id) ?? null, rankInput]
    })
  return { nodes, groups, nodeGroup, key: JSON.stringify(rows) }
}
