...
const read = () => {
  try { return localStorage.getItem(KEY) === 'comfortable' } catch { return false }
}

function apply(comfortable, persist = true) {
  document.documentElement.toggleAttribute('data-comfortable', comfortable)
  if (persist) {
    try { localStorage.setItem(KEY, comfortable ? 'comfortable' : 'compact') } catch { /* optional */ }
  }
}
...
export default function DensityToggle() {
  ...
  useEffect(() => {
    const sync = event => {
      ...
      const next = read(); setComfortable(next); apply(next, false)
    }
