...
export const authoringRecoveryStorageKey = (kind, name) => AUTHORING_OPERATION_STORAGE_PREFIX
  + encodeURIComponent(`${String(kind || '')}\u0000${String(name || '')}`)
...
export function inspectAuthoringRecoveryStorage() {
  ...
  if (!storage) return { available: false, count: 0, records: [] }
  try {
    const records = []
    ...
    return { available: true, count: records.length, records }
  } catch {
    return { available: false, count: 0, records: [] }
  }
}
