...
test('RunView refreshes comment feeds only from comments_revision, never global seq', async () => {
  ...
  assert.match(resourceModel, /value\.scope === scope \? value : resourceInitial\(scope, status\)/,
    'a node, attempt, or generation switch must hide stale full detail before passive effects')
  // The `on` latch that used to gate this predicate moved into `settle`/the abort arm as a
  // `flight.current !== request` check, which is strictly stronger: the old flag guarded
