...
test('live node-detail and building-trace polls gate their setState on usePoll alive()', async () => {
  ...
  // Dock building-trace poll: callback receives (alive) and only sets trace when alive().
  assert.match(dock, /usePoll\(\(alive\) => get\(`\/api\/runs\/\$\{runId\}\/trace`\)[\s\S]*?\.then\(value => \{ if \(alive\(\)\)/)
})
