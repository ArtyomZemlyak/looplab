...
test('request errors are directional and keep already loaded rows visible', () => {
  ...
  state = timelineRequestFailed(state, 'older', new Error('password=must-not-render'))
  assert.equal(state.status, 'ready')
  assert.equal(state.errors.older, 'Older timeline events could not be loaded.')
  assert.doesNotMatch(state.errors.older, /password/)
  assert.equal(state.rows.length, 2)
  const denied = Object.assign(new Error('token=must-not-render'), { status: 403 })
  state = timelineRequestFailed(state, 'newer', denied)
  assert.equal(state.errors.newer, 'Owner access is required to load timeline events.')
