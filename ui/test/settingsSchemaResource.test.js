...
test('packaged settings metadata validates as one bounded versioned contract without losing copy', () => {
  ...
  //   180 -> 181 (2026-08-18): `cadence_while_evaluating` — whether the node-count cadences may
  //   fire at a creation decision point that still has an evaluation IN FLIGHT (backlog F1i).
  //   ELEVENTH occurrence, and the Python guard caught it first for the seventh consecutive
  ...
  //   207 + 2 -> 210 (2026-09-08, at the MERGE): two branches each added one row on the same day,
  ...
  //   211 -> 212 (2026-09-17): `exploit_strong_node_quantile`, B1 of docs/60 §60.9 — after a node
  //   lands in the top 1-q of this run's evaluated nodes the next action is forced to be an improve
  ...
  assert.equal(Object.keys(schema.fieldByKey).length, 229)
