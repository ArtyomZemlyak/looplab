...
export function layoutWithGroups(nodes, { collapsed = new Set(), nodeGroup = new Map(), groupMode = "none" } = {}) {
	const ent = (id) => {
		const k = nodeGroup.get(id);
		return k != null && collapsed.has(k) ? `super:${k}` : `n:${id}`;
	};
		...
		const sweep = (downward) => {
			(downward ? depths : [...depths].reverse()).forEach((d) => {
				...
				Object.entries(acc).forEach(([c, a]) => {
					cmean[c] = a.reduce((s, x) => s + x, 0) / a.length;
				});
				layer.sort((a, b) => cmean[clusterOf(a)] - cmean[clusterOf(b)] || (clusterOf(a) === clusterOf(b) ? bary[a] - bary[b] : clusterOf(a).localeCompare(clusterOf(b))) || order[a] - order[b]);
				layer.forEach((e, i) => {
					order[e] = i;
				});
			});
	...
	if (!banded) {
		...
		byKey.forEach((ids, key) => cells.push({
			key,
			ids
		}));
		return {
	...
	// Depth is a COARSE axis, so we relax it from a hard row into a soft BAND (K depths each). On a
	// desktop canvas, a coarser band spends the available width before adding another tall stack whose
	// lower branches cannot fit above the timeline. Members still fill a fixed-row,
	// column-major grid cell — compact,
	...
	const K = 4;
	const ROWS = 4;
	const PX = 206, PY = 96, GAPCOLS = .7, BAND_GAP = 64;
...
export function similarityRank(nodes, nodeGroup, mode) {
	...
	const sortKey = (key) => {
		...
		if (mode === "niche") {
			...
			return Object.keys(p).sort().flatMap((k) => {
				const v = p[k];
				const nv = typeof v === "number" ? v : Number(v);
				return [k, Number.isNaN(nv) ? String(v) : nv];
			});
		}
		return [Math.min(...ids)];
	};
	const keyOf = new Map([...members.keys()].map((k) => [k, sortKey(k)]));
