import { deadlineGet } from "./api.mjs";
export const SETTINGS_SCHEMA_VERSION = 2;
const SETTINGS_SCHEMA_TIMEOUT_MS = 15e3;
...
const schemaError = () => {
	throw new TypeError("Invalid settings schema");
};
...
export function validateSettingsSchema(value) {
	if (!record(value) || value.schema !== SETTINGS_SCHEMA_VERSION || typeof value.revision !== "string" || !/^[0-9a-f]{64}$/.test(value.revision) || !Array.isArray(value.groups) || !value.groups.length || value.groups.length > 32 || !record(value.agent_role_pills)) schemaError();
	...
	value.groups.forEach((group) => {
		...
		boundedText(group.sub, 1e3, true);
		if (groupNames.has(title) || !Array.isArray(group.fields) || !group.fields.length || group.fields.length > 256) schemaError();
		groupNames.add(title);
		group.fields.forEach((field) => {
			if (!record(field) || !/^[a-z][a-z0-9_]{0,119}$/.test(field.key) || Object.hasOwn(fieldByKey, field.key) || !FIELD_TYPES.has(field.type)) schemaError();
			boundedText(field.label, 500);
			for (const attribute of OPTIONAL_TEXT) {
				if (Object.hasOwn(field, attribute)) {
					boundedText(field[attribute], 16e3, true);
				}
			}
			...
			if (Object.hasOwn(field, "minimum") && Object.hasOwn(field, "exclusiveMinimum") || Object.hasOwn(field, "maximum") && Object.hasOwn(field, "exclusiveMaximum")) schemaError();
			const lower = field.minimum ?? field.exclusiveMinimum;
			const upper = field.maximum ?? field.exclusiveMaximum;
			if (lower != null && upper != null && (lower > upper || lower === upper && (Object.hasOwn(field, "exclusiveMinimum") || Object.hasOwn(field, "exclusiveMaximum")))) schemaError();
			if (field.type === "enum") {
			...
			if (Object.hasOwn(field, "agents")) {
				if (!Array.isArray(field.agents) || field.agents.length > roleNames.length || new Set(field.agents).size !== field.agents.length || field.agents.some((role) => !Object.hasOwn(agentRolePills, role))) schemaError();
...
export function createSettingsSchemaLoader(readSchema) {
	...
	return Object.freeze({
		...
		load: ({ reload = false } = {}) => {
			...
			let pending;
			pending = Promise.resolve().then(() => readSchema({ cache: reload ? "reload" : "no-cache" })).then(validateSettingsSchema).then((value) => {
				cached = value;
				return value;
			}).finally(() => {
...
const schemaLoader = createSettingsSchemaLoader(({ cache }) => {
	const timed = deadlineGet(`/api/settings/schema/${SETTINGS_SCHEMA_VERSION}`, SETTINGS_SCHEMA_TIMEOUT_MS, { cache });
...
const numericRangeError = (field, value) => {
	...
	const below = hasMin && value < field.minimum || hasExclusiveMin && value <= field.exclusiveMinimum;
	const above = hasMax && value > field.maximum || hasExclusiveMax && value >= field.exclusiveMaximum;
	...
	if (field.type === "int") {
		...
		if (!Number.isFinite(value)) {
			return {
				valid: false,
				value: INVALID_SETTING_VALUE,
				error: "Enter a finite whole number."
			};
		}
		if (!Number.isInteger(value)) {
			return {
				valid: false,
				value: INVALID_SETTING_VALUE,
				error: "Enter a whole number without decimals."
			};
		}
		if (!Number.isSafeInteger(value)) {
			return {
				valid: false,
				value: INVALID_SETTING_VALUE,
				error: "Enter a whole number within the safe range."
			};
		}
...
export function settingsSavePayload(form, agentControl, baselineForm, baselineAgentControl, schema) {
	...
	const control = agentControl && typeof agentControl === "object" && !Array.isArray(agentControl) ? agentControl : {};
	if (baselineAgentControl && typeof baselineAgentControl === "object" && !Array.isArray(baselineAgentControl)) {
		...
		for (const key of keys) {
			...
			const baselineValue = Object.hasOwn(baselineAgentControl, key) ? canonicalAgentRoles(baselineAgentControl[key]) : null;
