...
const visibleContent = (message) => {
	...
	return message?.role === "user" ? value.replace(/\n*\[UI context:[^\]]*\]\s*$/, "").trimEnd() : value;
};
...
export function proposalLaunchChat(messages, assistantIndex) {
	...
	if (start < 0) return [];
	return rows.slice(start, end + 1).filter((message) => (message?.role === "user" || message?.role === "assistant") && !message.streaming && visibleContent(message).trim()).map((message) => ({
		role: message.role,
