...
export function highlightSegments(text, query) {
	...
	if (end < value.length) segments.push({
...
export function searchConcepts(nodeConcepts = {}, rename = {}, query = "", limit = DEFAULT_LIMIT) {
	...
	return scored.slice(0, cap).map(({ id, label, count }) => ({
		id,
		label,
		count
	}));
...
export function experimentRefMatches(ref, query) {
	...
	const idMatch = Number.isFinite(nodeId) && (("#" + nodeId).includes(q) || String(nodeId).includes(q));
...
export function filterConceptTree(tree, experimentRefs = {}, query = "", opts = {}) {
	...
	for (const id of Object.keys(nodes)) {
		...
		const expHit = matchExperiments && has(refs, id) && Array.isArray(refs[id]) && refs[id].some((ref) => experimentRefMatches(ref, query));
