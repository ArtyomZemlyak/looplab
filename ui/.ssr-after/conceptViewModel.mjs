...
import { canonicalId, conceptMap } from "./conceptId.mjs";
...
export function relationshipProjectionCopy(rels = []) {
	...
	const relationTypes = source.map((rel) => rel === "co_occurs" ? "co-occurrence" : rel).join(", ") || "relationship";
	return {
		relationTypes,
		linkDescription: `projected ${relationTypes} links`,
		derivationNote: source.includes("co_occurs") ? "Co-occurrence links are derived from the concept memberships in this frame; they are not recorded edge claims." : ""
	};
}
const MAX_VISIBLE_ROWS = 1e4;
const MAX_TREE_DEPTH = 256;
const MAX_TREE_REFERENCES = 5e4;
...
export function visibleConceptRows(tree, expanded = new Set()) {
	...
	const stack = roots.slice().reverse().map((id) => ({
		id,
		depth: 0,
		path: null
	}));
