...
import { addConceptSelection, chipsAtPath, breadcrumb, matchingNodeIds, toggleConceptSelection } from "./conceptChips.mjs";
import { searchConcepts } from "./conceptSearch.mjs";
import { Marked } from "./Highlight.mjs";
import { canonicalId } from "./conceptId.mjs";
import { authoritativeNodeConcepts, conceptMaterializationStatus } from "./nodeProjection.mjs";
...
export default function ConceptChipBar({ state, onHighlight }) {
	...
	const hasConcepts = useMemo(() => Object.values(nodeConcepts).some((v) => v && v.length), [nodeConcepts]);
	const chips = useMemo(() => chipsAtPath(nodeConcepts, rename, path), [
		nodeConcepts,
		rename,
		path
	]);
	...
	const allResults = useMemo(() => searching ? searchConcepts(nodeConcepts, rename, query) : [], [
		searching,
		nodeConcepts,
		rename,
		query
	...
	const committed = useMemo(() => hasConcepts ? matchingNodeIds(nodeConcepts, selected, rename) : null, [
		hasConcepts,
		nodeConcepts,
		selected,
		rename
	...
	const preview = useMemo(() => searching && allResults.length ? matchingNodeIds(nodeConcepts, allResults.map((result) => result.id), rename) : null, [
		searching,
		allResults,
		nodeConcepts,
		rename
	...
	useEffect(() => {
		onHighlight && onHighlight(highlight);
	}, [sig]);
	useEffect(() => () => {
	...
	useEffect(() => {
		setCursor((current) => results.length ? Math.min(Math.max(current, 0), results.length - 1) : -1);
	}, [results.length]);
	...
	const onSearchKey = (event) => {
		if (event.key === "ArrowDown") {
			...
			if (r) commitConcept(r.id);
		} else if (event.key === "Escape") {
			event.preventDefault();
			query ? setQuery("") : closeSearch();
		}
	};
	...
	if (materialization === "unavailable" || !hasConcepts && withheld > 0) return /* @__PURE__ */ _jsx("div", {
		className: "concept-bar",
		role: materialization === "unavailable" ? "alert" : "status",
		children: /* @__PURE__ */ _jsxs("div", {
			className: "cb-head",
			children: [
				/* @__PURE__ */ _jsx("strong", { children: "Concepts" }),
				/* @__PURE__ */ _jsx("span", {
					className: "chip xs warn",
					children: materialization.toUpperCase()
				}),
				/* @__PURE__ */ _jsx("span", {
					className: "muted",
					children: materialization === "unavailable" ? "Membership unavailable; not empty." : `Membership withheld for all ${withheld} tagged experiment${withheld === 1 ? "" : "s"}; not empty.`
				})
			]
		})
	});
	...
	return /* @__PURE__ */ _jsxs("div", {
		...
		children: [
			/* @__PURE__ */ _jsxs("div", {
				...
				children: [
					/* @__PURE__ */ _jsx("strong", { children: "Concepts" }),
					withheld > 0 && /* @__PURE__ */ _jsxs("span", {
						className: "chip xs warn",
						title: `${withheld} experiment${withheld === 1 ? "'s" : "s'"} membership could not be materialized; counts are a lower bound and these experiments never match a filter.`,
						children: [
							"PARTIAL · ",
							withheld,
							" withheld"
					...
					/* @__PURE__ */ _jsxs("nav", {
						className: "cb-crumbs",
						"aria-label": "Concept breadcrumb",
						children: [/* @__PURE__ */ _jsx("button", {
							type: "button",
							className: "cb-crumb" + (path ? "" : " on"),
							onClick: () => setPath(""),
							"aria-current": path ? undefined : "true",
							children: "All"
						}), crumbs.map((c, i) => /* @__PURE__ */ _jsxs(React.Fragment, { children: [/* @__PURE__ */ _jsx("span", {
							className: "cb-sep",
							"aria-hidden": "true",
							children: "›"
						}), /* @__PURE__ */ _jsx("button", {
							type: "button",
							className: "cb-crumb" + (i === crumbs.length - 1 ? " on" : ""),
					...
					/* @__PURE__ */ _jsxs("div", {
						className: "cs",
						children: [!searchOpen ? /* @__PURE__ */ _jsx("button", {
							type: "button",
							className: "cs-icon",
							...
							children: results.length === 0 ? /* @__PURE__ */ _jsxs("div", {
								...
								className: "cs-pop-h",
								children: "Concepts · Enter to pin"
							}), results.map((r, i) => {
			...
			selected.length > 0 && /* @__PURE__ */ _jsx("div", {
				className: "cb-selected",
				...
				children: selected.map((key) => {
					...
					return /* @__PURE__ */ _jsxs("button", {
						...
						children: [
							keyExact(key) && /* @__PURE__ */ _jsx("span", {
								className: "cb-here",
								"aria-hidden": "true",
								children: "·"
							}),
							/* @__PURE__ */ _jsx("span", {
								className: "cb-pill-label",
								children: label
							}),
							/* @__PURE__ */ _jsx("span", {
								className: "cb-x",
			...
			/* @__PURE__ */ _jsx("div", {
				className: "cb-chips",
				"aria-label": path ? `Concepts under ${path}` : "Top-level concepts",
				children: chips.length === 0 ? /* @__PURE__ */ _jsx("span", {
					className: "muted cb-empty",
					children: "No concepts at this level."
				}) : chips.map((chip) => {
					const key = keyOf(chip);
					const on = selected.includes(key);
					return /* @__PURE__ */ _jsxs("span", {
						className: "cb-chip" + (on ? " on" : "") + (chip.atLevel ? " here" : "") + (!on && searching && matchedIds.has(chip.id) ? " match" : ""),
						children: [/* @__PURE__ */ _jsxs("button", {
							type: "button",
							className: "cb-chip-main",
							"aria-pressed": on,
							onClick: () => toggleSelect(key),
							title: `${chip.id} · ${chip.count} experiment(s)${chip.atLevel ? " tagged here (not deeper) — highlights only these" : ""}`,
							children: [
								chip.atLevel && /* @__PURE__ */ _jsx("span", {
									className: "cb-here",
									"aria-hidden": "true",
									children: "·"
								}),
						...
						}), !chip.atLevel && /* @__PURE__ */ _jsx("button", {
							type: "button",
							className: "cb-drill",
							"aria-label": `Open ${chip.id}`,
							onClick: () => setPath(chip.id),
							title: `Drill into ${chip.label}`,
							children: "›"
						})]
					}, chip.id + (chip.atLevel ? "#here" : ""));
				})
			})
		]
	});
