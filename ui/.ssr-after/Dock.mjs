...
import { get, fmt, workingId, getRunCommand, retryRunCommand, runCommand, commandFeedback, commandErrorMessage, commandFailureRecord, commandCanRetry, createIdempotencyKey, commandActionForEvent, commandRecordMatchesAction, commandEventForAction, loadRunTransport, saveRunTransport, clearRunTransport, isTransientCommandReadError, clearRunCommandLock, loadRunCommandLock, saveRunCommandLock, subscribeRunCommandLock, COMMAND_SUCCEEDED, COMMAND_FAILED, storageGet, storageSet, runApiPath, runNodeApiPath } from "./util.mjs";
import { usePoll } from "./hooks.mjs";
import Markdown, { stripMd } from "./markdown.mjs";
import { NodeTrace, TraceUnavailable } from "./Inspector.mjs";
import { OpIcon } from "./icons.mjs";
import { runLifecycle } from "./runIndex.mjs";
...
import { tracePartial, traceUnavailable } from "./traceProjection.mjs";
import { crossRunPriorNarration } from "./crossRunPrior.mjs";
import { buildingGenerations, buildingMarkers } from "./buildingModel.mjs";
...
const strategySummary = (strategy = {}) => {
	...
	if (lanes && typeof lanes === "object" && !Array.isArray(lanes)) {
		bits.push(`lanes ${Object.entries(lanes).slice(0, 5).map(([lane, width]) => `${lane}:${width}`).join(",")}`);
	}
...
const NARR = {
	...
	hint: (d) => `hint: ${d.text}`,
	promote: (d) => `promoted #${d.node_id} → ${d.alias || "champion"}`,
	...
	run_finished: (d) => d?.reason === "aborted" || d?.reason === "finalized" ? "run finalized (wrapped up)" : `run finished${d.reason ? " (" + d.reason + ")" : ""}`,
	...
	inject_node: (d) => {
		const i = d.idea || {};
		return `added experiment: ${i.operator || "improve"}${d.parent_id != null ? " from #" + d.parent_id : ""}${i.rationale ? " — " + stripMd(i.rationale).slice(0, 70) : ""}`;
	},
	...
	data_profiled: (d) => {
		const c = d.columns;
		const n = Array.isArray(c) ? c.length : Object.keys(c || {}).length;
		return `dataset profiled (${n} column${n === 1 ? "" : "s"})`;
	},
	data_provenance: (d) => {
		const n = Object.keys(d.assets || {}).length;
		return `dataset provenance pinned (${n} asset${n === 1 ? "" : "s"})`;
	},
	...
	node_tombstoned: (d) => {
		const ids = Array.isArray(d.node_ids) ? d.node_ids : d.node_id != null ? [d.node_id] : [];
		return ids.length ? `deleted node${ids.length === 1 ? "" : "s"} ${ids.map((n) => "#" + n).join(", ")}` : "deleted a node subtree";
	},
	...
	card_reprioritized: (d) => `Card ${d.id.slice(0, 80)} priority pinned to ` + `${Number.isSafeInteger(d.priority) ? d.priority + 1 : d.priority}`,
	...
	train_monitor_alert: (d) => `training monitor: #${d.node_id} ${d.log_role && d.log_role !== "training" ? `${d.stage ? `stage ${d.stage}` : "a non-training phase"} looks ${d.status} (advisory)` : `looks ${d.status}`}${d.reason ? " — " + String(d.reason).slice(0, 90) : ""}${d.confidence != null ? ` (${Math.round(d.confidence * 100)}% conf)` : ""}`,
	...
	asha_rank: (d) => `ASHA: #${d.node_id} ${fmt(d.intermediate)} ${d.underperforming === false ? "rank recovered" : `${d.endpoint_underperforming === false ? "same-resource" : "endpoint"} rank warning`}`,
	...
	asha_verdict: (d) => `ASHA judge: #${d.node_id} ${(d.stop_decided ?? d.kill) === true ? d.kill === false ? `STOP decided — superseded by ${d.kill_superseded_by || "another watchdog"}` : "STOP decided — early-kill claimed" : `keep running (${d.status || "unavailable"})`}${d.reason ? " — " + String(d.reason).slice(0, 90) : ""}${d.confidence != null ? ` (${Math.round(d.confidence * 100)}% conf)` : ""}`,
	restart: () => "run restart requested (pause-and-resume handoff)"
};
const ownValue = (value, key) => value !== null && typeof value === "object" && Object.hasOwn(value, key) && value[key] !== null && value[key] !== undefined;
...
const objectValue = (value, key) => ownValue(value, key) && value[key] !== null && typeof value[key] === "object" && !Array.isArray(value[key]);
...
const NARR_VALID = {
	...
	reward_hack_suspected: (d) => ownValue(d, "node_id") && Array.isArray(d?.signals) && d.signals.every((signal) => ownValue(signal, "signal")),
	novelty_rejected: (d) => ownValue(d, "near_node") && ownValue(d, "distance"),
	hypothesis_ranked: (d) => ownValue(d, "n") || Array.isArray(d?.order),
	foresight_selected: (d) => ownValue(d, "kind") && ownValue(d, "chosen") && (ownValue(d, "n") || Array.isArray(d?.order)),
	...
	data_profiled: (d) => ownValue(d, "columns") && d.columns !== null && typeof d.columns === "object",
	...
	cross_run_prior: (d) => Array.isArray(d?.matched_concepts) && Array.isArray(d?.prior_runs) && d.matched_concepts.length > 0 && d.prior_runs.length > 0,
	...
	card_edited: (d) => typeof d?.id === "string" && d.id.length > 0 && typeof d?.statement === "string",
	card_resource_pinned: (d) => typeof d?.id === "string" && d.id.length > 0 && ownValue(d, "gpus"),
...
function eventNodeAttempt(e) {
	const data = e?.data || {};
	const raw = Object.hasOwn(data, "generation") ? data.generation : e?.type === "node_repaired" ? data.attempt : 0;
...
const GROUPS = [
	[
		"proposal",
		"proposals",
		"node_building node_created"
	],
	[
		"eval",
		"results",
		"node_eval_started node_evaluated node_failed node_repaired node_confirmed best_confirmed proxy_scored ablate deps_installed confirm_done confirm_eval agent_validated holdout_evaluated stage_finished"
	],
	[
		"decision",
		"decisions",
		"policy_decision strategy_decision rung_promoted agent_decision set_strategy hypothesis_ranked foresight_selected coverage_snapshot"
	],
	[
		"research",
		"research",
		"research_completed deep_research hypothesis_added hypothesis_merged lessons_refreshed lessons_distilled cross_run_prior hypothesis_updated lessons_reconciled"
	],
	[
		"report",
		"report",
		"report_generated reflection_note report_refresh_failed"
	],
	[
		"trust",
		"trust",
		"reward_hack_suspected data_leakage spec_drift novelty_rejected drift_unavailable workspace_changed novelty_graded train_monitor_alert asha_rank asha_verdict"
	],
	[
		"control",
		"actions",
	...
	[
		"lifecycle",
		"lifecycle",
		"run_started run_finished llm_cost budget data_profiled data_provenance host_grading diversity_archive setup_started setup_step setup_finished workspace_seeded run_setup_started run_setup_finished env_changed log_repaired card_auto_dropped"
	]
];
const TYPE2GROUP = Object.fromEntries(GROUPS.flatMap(([group, , types]) => types.split(" ").map((type) => [type, group])));
...
const TYPE_GLYPH = {
	...
	agent_decision: "bot",
	run_reopened: "play",
	inject_node: "gitbranch",
	fork: "gitbranch",
	agent_validated: "target",
	hypothesis_ranked: "bulb",
	foresight_selected: "bulb"
};
function kindOf(type) {
	...
	const glyph = Object.hasOwn(TYPE_GLYPH, type) && TYPE_GLYPH[type] || Object.hasOwn(GROUP_GLYPH, g) && GROUP_GLYPH[g] || "dot";
	return {
		group: g,
		glyph
	};
...
const TRACE_OWNER_TYPES = new Set([
	"node_created",
	"node_evaluated",
	"node_failed",
	"node_repaired",
...
const CLOSED_EXPANSION = Object.freeze({
	open: false,
	touched: false
});
...
function collectThinking(trace, nid) {
	...
	const walk = (arr) => (arr || []).forEach((s) => {
		(s.events || []).forEach((ev) => {
			if (ev.name === "llm_call" && ev.thinking) out.push({
				op: s.name,
				text: ev.thinking
			});
		});
...
function LiveTrace({ runId, generation, active }) {
	...
	useEffect(() => {
		setLimit(TRACE_LIMIT_DEFAULT);
		stickRef.current = true;
		preserveRef.current = null;
	}, [scope]);
	usePoll((alive) => get(runApiPath(runId, `/trace/tail?limit=${limit}`)).then((r) => {
		if (alive()) setTailState({
			scope,
			items: Array.isArray(r?.tail) ? r.tail : [],
	...
	const current = tailState.scope === scope ? tailState : {
		items: [],
		projection: {},
		loaded: false
	};
	...
	const partialControl = !partial ? null : canLoadEarlier ? /* @__PURE__ */ _jsx("button", {
		type: "button",
		className: "lt-loadmore disclosure-button",
		onClick: loadEarlier,
		children: "↑ load earlier spans"
	}) : /* @__PURE__ */ _jsx("div", {
		className: "lt-note",
		role: "status",
		children: "Earlier history is in the node's full trace."
	});
	useLayoutEffect(() => {
		const el = bodyRef.current;
		if (!el || !open) return;
	...
	return /* @__PURE__ */ _jsxs("div", {
		className: "live-trace" + (open ? " open" : ""),
		children: [/* @__PURE__ */ _jsxs("button", {
			type: "button",
			className: "lt-toggle disclosure-button",
			"aria-expanded": open,
			onClick: () => setOpen((o) => !o),
			...
			children: !current.loaded ? /* @__PURE__ */ _jsx("div", {
				className: "muted lt-empty",
				role: "status",
				children: "loading trace…"
			}) : unavailable ? /* @__PURE__ */ _jsx(TraceUnavailable, { label: "Trace unavailable; retrying automatically." }) : /* @__PURE__ */ _jsxs(_Fragment, { children: [partialControl, !tail.length && !partial ? /* @__PURE__ */ _jsx("div", {
				className: "muted lt-empty",
				children: "waiting for the next agent step…"
			}) : tail.map((it, i) => it.kind === "generation" ? /* @__PURE__ */ _jsxs("div", {
				className: "lt-row lt-gen",
				children: [/* @__PURE__ */ _jsx("span", {
					className: "lt-ic",
					children: "🧠"
				}), /* @__PURE__ */ _jsx("span", {
					className: "lt-txt",
					children: it.text || /* @__PURE__ */ _jsxs("span", {
						className: "muted",
						children: [
							"(",
							it.model,
			...
			}, it.span_id || i) : /* @__PURE__ */ _jsxs("div", {
				className: "lt-row lt-tool",
				children: [
					/* @__PURE__ */ _jsx("span", {
						className: "lt-ic",
						children: "🔧"
					...
					/* @__PURE__ */ _jsx("span", {
						className: "lt-tool-name",
						children: it.tool
					}),
					it.arg && /* @__PURE__ */ _jsx("span", {
						className: "lt-arg",
						children: it.arg
					})
				]
...
function agentStatus(live, log) {
	...
	for (let i = log.length - 1; i >= 0; i--) {
		if (!STATUS_NOISE.has(log[i].type)) {
			last = log[i].type;
			break;
		}
	}
...
const Disclosure = ({ label, children }) => {
	const [open, setOpen] = useState(false);
	return /* @__PURE__ */ _jsxs("div", {
		className: "think-debug trace-disclosure",
		children: [/* @__PURE__ */ _jsxs("button", {
			type: "button",
			className: "role-think disclosure-button trace-disclosure-toggle",
			"aria-expanded": open,
			onClick: () => setOpen((v) => !v),
			children: [
				open ? "▾" : "▸",
...
function NodeCreatedDetail({ d, trace }) {
	...
	return /* @__PURE__ */ _jsxs("div", {
		...
		children: [
			/* @__PURE__ */ _jsx("div", {
				className: "section-h",
				children: "Conclusion — why this experiment next"
			}),
			idea.rationale ? /* @__PURE__ */ _jsx(Markdown, {
			...
			/* @__PURE__ */ _jsxs("div", {
				className: "ev-meta",
				children: [
					/* @__PURE__ */ _jsxs("span", { children: ["operator ", /* @__PURE__ */ _jsx("b", { children: idea.operator || d.operator })] }),
					(d.parent_ids || []).length > 0 && /* @__PURE__ */ _jsxs("span", { children: ["built from ", d.parent_ids.map((p) => "#" + p).join(", ")] }),
					Object.keys(params).length > 0 && /* @__PURE__ */ _jsxs("span", { children: ["params ", Object.entries(params).map(([k, v]) => `${k}=${fmt(v, 3)}`).join(", ")] }),
					Object.keys(space).length > 0 && /* @__PURE__ */ _jsxs("span", { children: ["sweep ", Object.entries(space).map(([k, v]) => `${k}∈[${(Array.isArray(v) ? v : [v]).join(", ")}]`).join("; ")] })
				]
			}),
			think.length > 0 && /* @__PURE__ */ _jsx(Disclosure, {
				label: "Researcher thinking (debug)",
				children: think.map((t, i) => /* @__PURE__ */ _jsx(Markdown, {
...
function PolicyDetail({ d }) {
	...
	return /* @__PURE__ */ _jsxs("div", {
		...
		children: [/* @__PURE__ */ _jsxs("div", {
			className: "section-h",
			children: ["Why this node", d.reason ? ` — ${d.reason}` : ""]
		}), entries.length === 0 ? /* @__PURE__ */ _jsxs("div", {
			className: "v muted",
			children: [
				"chose #",
				d.chosen,
			...
			caption: "Candidate scores for node selection",
			card: false,
			children: /* @__PURE__ */ _jsxs("table", {
				className: "tbl",
				children: [/* @__PURE__ */ _jsx("thead", { children: /* @__PURE__ */ _jsxs("tr", { children: [/* @__PURE__ */ _jsx("th", { children: "node" }), /* @__PURE__ */ _jsx("th", { children: "score" })] }) }), /* @__PURE__ */ _jsx("tbody", { children: entries.map(([nid, sc]) => /* @__PURE__ */ _jsxs("tr", {
					className: String(nid) === String(d.chosen) ? "chosen-row" : "",
					children: [/* @__PURE__ */ _jsxs("td", { children: [
						"#",
						nid,
						String(nid) === String(d.chosen) ? " ✓ chosen" : ""
					] }), /* @__PURE__ */ _jsx("td", { children: fmt(sc, 4) })]
				}, nid)) })]
			})
		})]
	});
	...
	return /* @__PURE__ */ _jsxs("div", {
		...
		children: [
			...
			/* @__PURE__ */ _jsxs("div", {
				className: "ev-meta",
				children: [
					/* @__PURE__ */ _jsxs("span", { children: ["policy ", /* @__PURE__ */ _jsx("b", { children: s.policy || "?" })] }),
					s.fidelity && /* @__PURE__ */ _jsxs("span", { children: ["fidelity ", s.fidelity] }),
					s.developer && /* @__PURE__ */ _jsxs("span", { children: ["developer ", s.developer] }),
					s.eval_parallel != null && /* @__PURE__ */ _jsxs("span", { children: ["eval parallel ", s.eval_parallel] }),
					s.llm_parallel != null && /* @__PURE__ */ _jsxs("span", { children: ["LLM total ", s.llm_parallel] }),
					s.llm_lane_limits && typeof s.llm_lane_limits === "object" && !Array.isArray(s.llm_lane_limits) && /* @__PURE__ */ _jsxs("span", { children: ["LLM lanes ", Object.entries(s.llm_lane_limits).slice(0, 5).map(([lane, width]) => `${lane}:${width}`).join(", ")] }),
					s.card_scoring && typeof s.card_scoring === "object" && !Array.isArray(s.card_scoring) && /* @__PURE__ */ _jsxs("span", { children: [
						"Card scoring ",
						s.card_scoring.stance || "balanced",
						" · novelty ",
						s.card_scoring.novelty_weight,
						" · ",
						"coverage ",
						s.card_scoring.coverage_weight
					] }),
					s.source && /* @__PURE__ */ _jsxs("span", { children: ["source ", s.source] })
				]
			}),
			ctxRows.length > 0 && /* @__PURE__ */ _jsxs(_Fragment, { children: [/* @__PURE__ */ _jsx("div", {
				className: "section-h",
				children: "Decision context"
			}), /* @__PURE__ */ _jsx("div", {
				className: "ev-meta",
				children: ctxRows.map(([k, v]) => /* @__PURE__ */ _jsxs("span", {
					className: "ev-ctx",
					children: [
						/* @__PURE__ */ _jsx("b", { children: k }),
						" ",
						String(v)
					]
				}, k))
...
function ResearchDetail({ d }) {
	...
	return /* @__PURE__ */ _jsxs("div", {
		...
		children: [
			...
			/* @__PURE__ */ _jsx("div", {
				className: "v",
				children: memo.summary || "—"
			}),
			(memo.findings || []).length > 0 && /* @__PURE__ */ _jsxs(_Fragment, { children: [/* @__PURE__ */ _jsx("div", {
				className: "section-h",
				children: "Findings"
			}), /* @__PURE__ */ _jsx("ul", {
				className: "bul",
				children: memo.findings.map((f, i) => /* @__PURE__ */ _jsx("li", { children: f }, i))
			})] }),
			(memo.recommended_directions || []).length > 0 && /* @__PURE__ */ _jsxs(_Fragment, { children: [/* @__PURE__ */ _jsx("div", {
				className: "section-h",
				children: "Recommended directions (fed to the Researcher)"
			}), /* @__PURE__ */ _jsx("ul", {
				className: "bul",
				children: memo.recommended_directions.map((x, i) => /* @__PURE__ */ _jsx("li", { children: x }, i))
			})] }),
			memo.reasoning && /* @__PURE__ */ _jsx(Disclosure, {
				label: "reasoning (debug)",
...
function reasoningDetail(e, trace) {
	const d = e.data || {};
	if (e.type === "node_created") return /* @__PURE__ */ _jsx(NodeCreatedDetail, {
		d,
		trace
	});
	if (e.type === "policy_decision") return /* @__PURE__ */ _jsx(PolicyDetail, { d });
	if (e.type === "strategy_decision") return /* @__PURE__ */ _jsx(StrategyDetail, { d });
	if (e.type === "research_completed") return /* @__PURE__ */ _jsx(ResearchDetail, { d });
	return null;
}
...
function GenericDetail({ e }) {
	...
	if (!rows.length) return /* @__PURE__ */ _jsx("div", {
		className: "ev-detail",
		children: /* @__PURE__ */ _jsx("pre", {
			className: "code event-json",
			children: JSON.stringify(e.data || {}, null, 2)
		})
	});
	return /* @__PURE__ */ _jsx("div", {
		className: "ev-detail",
		children: rows.map(([k, v], i) => /* @__PURE__ */ _jsxs(React.Fragment, { children: [/* @__PURE__ */ _jsx("div", {
			className: "section-h",
			children: k
		}), /* @__PURE__ */ _jsx("div", {
			className: "v",
...
function OpTrace({ runId, traceId }) {
	...
	useEffect(() => {
		let on = true;
		setTrace(null);
		get(runApiPath(runId, `/trace/by_trace/${encodeURIComponent(traceId)}`)).then((d) => on && setTrace({
	...
	if (trace === null) return /* @__PURE__ */ _jsx("div", {
		className: "muted trace-loading",
		role: "status",
		children: "loading trace…"
	});
	return /* @__PURE__ */ _jsx(NodeTrace, {
		spans: trace.spans,
...
function EventRow({ e, onFocusEvent, focusLabel, nodeCreatedAttempt = null, autoOpen, runId, readOnly = false, liveBuilding = null, expansion = null, onExpansionChange = null }) {
	...
	const changeOpen = (next, wasTouched = touched) => {
		if (controlled) onExpansionChange?.({
			open: next,
			touched: wasTouched
		});
		else {
			localTouched.current = wasTouched;
			setLocalOpen(next);
		}
	};
	...
	useEffect(() => {
		if (!touched && open !== autoOpen) changeOpen(autoOpen, false);
	}, [autoOpen]);
	...
	const traceNid = TRACE_OWNER_TYPES.has(e.type) ? e.data?.node_id ?? (e.type === "setup_started" ? -1 : null) : null;
	...
	const exactBuilding = liveBuilding != null && traceNid != null && traceGeneration != null && liveBuilding[traceNid] === traceGeneration;
	// Clear the error flag only on a SUCCESSFUL exact-attempt load (not eagerly at each poll tick).
	const loadNodeTrace = (alive) => get(runNodeApiPath(runId, traceNid, `/trace?attempt=${traceGeneration}&limit=${nodeTraceLimit}`)).then((d) => {
	...
	usePoll((alive) => loadNodeTrace(alive), 4e3, [
		open,
		readOnly,
		runId,
		traceNid,
		...
		nodeTraceNonce,
		nodeTraceLimit
	], { enabled: open && !readOnly && traceNid != null && traceGeneration != null && exactBuilding });
	useEffect(() => {
	...
	return /* @__PURE__ */ _jsxs("div", {
		...
		children: [/* @__PURE__ */ _jsx("div", {
			...
			children: [/* @__PURE__ */ _jsxs("div", {
				className: "fm-line",
				children: [expandable && /* @__PURE__ */ _jsx("button", {
					type: "button",
					className: "fm-tw",
					"aria-expanded": open,
					"aria-controls": detailsId,
					"aria-label": `${open ? "Collapse" : "Expand"} details for event ${e.seq}`,
					onClick: () => changeOpen(!open, true),
					children: open ? "▾" : "▸"
					...
					children: [/* @__PURE__ */ _jsx("span", {
						className: "fm-narr",
						children: narr
					}), nid != null && /* @__PURE__ */ _jsx("span", {
						className: "ev-go",
						children: "↗"
					})]
				})]
			}), open && expandable && /* @__PURE__ */ _jsxs("div", {
				className: "ev-detail-wrap",
				id: detailsId,
				children: [
					hasOmittedDetail && /* @__PURE__ */ _jsxs("div", {
						className: "notice",
						role: "note",
						children: [
							"Event details were not transferred: ",
							omittedBytes.toLocaleString(),
							" source bytes exceed the bounded page response."
						]
					}),
					hasReason && reasoningDetail(e, nodeTrace),
					hasGeneric && /* @__PURE__ */ _jsx(GenericDetail, { e }),
					hasTrace && nodeTrace == null && !nodeTraceError && /* @__PURE__ */ _jsx("div", {
						className: "muted",
						role: "status",
						children: "loading node trace…"
					}),
					hasTrace && nodeTraceError && /* @__PURE__ */ _jsxs("div", {
						className: "notice resource-error compact",
						role: "alert",
						children: [/* @__PURE__ */ _jsx("span", { children: "Could not load node trace." }), /* @__PURE__ */ _jsx("button", {
							type: "button",
							className: "btn sm",
							onClick: retryNodeTrace,
							children: "Retry"
						})]
					}),
					hasTrace && nodeTrace != null && /* @__PURE__ */ _jsx(NodeTrace, {
						spans: nodeSpans,
						projection: nodeTrace.projection,
						runId,
						onRetry: retryNodeTrace,
						onLoadMore: nodeTraceLimit < NODE_TRACE_CAP_MAX ? loadMoreNodeTrace : undefined
					}),
					opTraceId && /* @__PURE__ */ _jsx(OpTrace, {
						runId,
						traceId: opTraceId
					})
				]
...
const recoveryForRun = (runId) => {
	...
	if (saved.protocolInvalid) return {
		pending: {
			action: saved.action,
			...
			canResubmit: false,
			lastError: "Stored command recovery data is invalid."
		},
		failure: null
	};
	...
	const record = saved.commandId && !storedRecord.id ? {
		...storedRecord,
		id: saved.commandId
	} : storedRecord;
	...
	const knownStatus = record.status === "accepted" || record.status === "executing" || COMMAND_FAILED.has(record.status);
	...
	const entry = {
		...
		observationKind: saved.observationKind || (!knownStatus && record.id ? "protocol" : needsObservation ? "transport" : null),
		lastError: saved.retrying || saved.checking ? "The page reloaded while recovery was in progress; check the durable command status." : !knownStatus ? "Stored command state needs to be checked against the server." : ""
	};
...
export default function Dock({ runId, live, liveSeq, expectedGeneration, timeline, viewSeq, setViewSeq, onReturnToLive, onFocus, collapsed, onToggleCollapse, height = 230, onToast, readOnly = false, publishTransport = null, filter = "", onFilterChange = null, kindFilters = [], onKindFiltersChange = null, focusOnMount = false, onInitialFocus = null, collapseControlRef = null, startOverState = null, onStartOver = null }) {
	...
	useDialogFocus(startOverDialogRef, closeStartOverDialog, !!startOverDialogIntent, { priority: DIALOG_PRIORITY.START_OVER });
	...
	useEffect(() => {
		...
		if (lock && lock.source !== "dock" && (restored.pending || restored.failure)) {
			...
			const lockMismatch = lock?.source === "dock" && entry && (lock.idempotencyKey !== entry.idempotencyKey || lock.action !== entry.action || lock.expectedGeneration !== entry.expectedGeneration || lock.commandId && entry.record?.id && lock.commandId !== entry.record.id);
			if (lockMismatch) pending = {
				...entry,
				statusUnavailable: true,
				observationKind: "protocol",
				protocolInvalid: true,
			...
			setTransportFailure(lockMismatch ? null : restored.failure);
	...
	const toggleControls = () => {
		...
		setShowControls((v) => {
			const n = !v;
			storageSet("ll.dock.controls", n ? "1" : "0");
			return n;
		});
	};
	useEffect(() => {
	...
	const liveBuilding = readOnly || !atLiveView || timeline.generation !== expectedGeneration ? null : buildingGenerations(live);
	...
	const eventDestination = (e) => {
		...
		const nodeGeneration = e.type === "node_created" ? verifiableCreatedAttempt(e, currentAttempt) : null;
		const opensLiveTrace = !readOnly && e.type === "node_created" && atLiveView && timeline.generation === expectedGeneration && nodeGeneration != null && nodeGeneration === currentAttempt;
	...
	const eventFocusLabel = (e, destination = eventDestination(e)) => {
		if (destination.nodeId == null) {
			return destination.opensHistoricalSnapshot ? `Open event ${e.seq} in a read-only snapshot` : `Jump to event ${e.seq}`;
		}
		if (e.type !== "node_created") {
			return destination.opensHistoricalSnapshot ? `Open experiment #${destination.nodeId} from event ${e.seq} in a read-only snapshot` : `Open experiment #${destination.nodeId} from event ${e.seq}`;
		}
		...
		const attempt = destination.nodeGeneration == null ? "recorded attempt" : `attempt ${destination.nodeGeneration}`;
		return `Open ${attempt} for experiment #${destination.nodeId} at event ${e.seq} in a read-only snapshot`;
	};
	const toggleKind = (g) => setKinds((s) => {
		const n = new Set(s);
	...
	const searchableLog = useMemo(() => log.map((event) => {
		...
		let rawPreview = "";
		try {
			rawPreview = JSON.stringify(event.data ?? {}).slice(0, 500);
		} catch {}
		return {
			event,
			search: `${event.type || ""} ${narration} ${rawPreview}`.toLowerCase()
		};
	}), [log]);
	...
	const feed = useMemo(() => searchableLog.filter(({ event, search }) => isCuratedType(event.type) && (atLiveView || event.seq <= viewSeq) && kindMatch(event) && (!filterQuery || search.includes(filterQuery))).map((item) => item.event), [
		searchableLog,
		atLiveView,
		viewSeq,
		filterQuery,
	...
	useEffect(() => {
		...
		if (startOverDisabled || mode !== "finished" || startOverDialogIntent.runId !== String(runId) || startOverDialogIntent.expectedGeneration !== expectedGeneration) {
			setStartOverDialogIntent(null);
			restoreDockFocus();
		}
	}, [
		startOverDialogIntent,
		startOverDisabled,
		mode,
		runId,
		expectedGeneration
	...
	const transportLabels = (action) => ({
		stop: {
			success: "Stopped — frozen, not finalized",
			noop: "Run was already stopped",
			executing: "Stop requested — waiting for the run to freeze",
			failure: "Stop failed"
		},
		finalize: {
			success: "Finalized — report and wrap-up complete",
			noop: "Run was already finalized",
			executing: "Finalize requested — wrapping up",
			failure: "Finalize failed"
		},
		resume: {
			success: "Run resumed",
			noop: "Run was already running",
			executing: "Resume requested — waiting for the engine",
	...
	const verifiedTransportAction = (action, record, protocolInvalid = false) => {
		const actual = protocolInvalid || !TRANSPORT_INTENTS[action] ? commandActionForEvent(record?.event_type) : action;
		return actual && TRANSPORT_INTENTS[actual] && commandRecordMatchesAction(record, actual, "dock") ? actual : null;
	};
	const protocolTransportState = (action, idempotencyKey, record, message, lockIdentity = null, boundGeneration = transportPending?.expectedGeneration || lockIdentity?.expectedGeneration || "") => {
	...
	const acceptTransportRecord = (action, record, idempotencyKey, boundGeneration) => {
		...
		if (!actualAction) return protocolTransportState(action, idempotencyKey, record, "Command identity does not match the requested action", pendingState?.lockIdentity, boundGeneration);
		if (pendingState?.protocolInvalid) {
			const identity = pendingState.lockIdentity || {
				...
				expectedGeneration: pendingState.expectedGeneration,
		...
		if (feedback.kind === "pending") {
			...
			if (!persistTransport(entry)) {
				return protocolTransportState(actualAction, idempotencyKey, record, "Command accepted, but its updated durable status could not be stored", null, boundGeneration);
			}
			setTransportPending(entry);
			setTransportFailure(null);
		} else {
	...
	const unavailableTransport = (action, idempotencyKey, boundGeneration, record, error, extra = {}) => {
		...
		if (recoveryRecord.id && !recoveryRecord.event_type && TRANSPORT_INTENTS[action]) {
			recoveryRecord = {
				...recoveryRecord,
				event_type: commandEventForAction(action, "dock")
			};
		}
		const entry = {
			action,
		...
		if (!persistTransport(entry)) saveRunCommandLock(runId, {
			...entry,
			source: "dock"
		});
		setTransportPending(entry);
	...
	const runTransport = async (action, idempotencyKey = createIdempotencyKey(), { allowPending = false, boundGeneration = null } = {}) => {
		if (!allowPending && (transportPending || loadRunCommandLock(runId))) return;
		const intent = TRANSPORT_INTENTS[action];
		if (!intent) {
			protocolTransportState(action, idempotencyKey, transportPending?.record, "Stored command identity cannot be safely replayed", transportPending?.lockIdentity);
			return;
		}
		...
		if (!persistTransport(start)) {
			storageTransportFailure(action, idempotencyKey, generation);
			return;
		}
		...
		try {
			const record = await runCommand(runId, intent.type, intent.data, {
				...
				onRecord: (next) => {
					...
					setTransportPending((current) => current?.action === action && current?.idempotencyKey === idempotencyKey ? visible : current);
				}
			});
			...
			const kind = observationKind(error);
			if (error?.commandUnknown || record?.id && [
				"transport",
				"access",
				"protocol"
			].includes(kind)) {
	...
	const onRetryTransport = async () => {
		...
		try {
			...
			if ([
				...
				unavailableTransport(action, idempotencyKey, boundGeneration, error?.commandRecord || record, error);
			} else failTransport(action, idempotencyKey, boundGeneration, error, error?.commandRecord || record);
		}
	};
	const onCheckTransport = async () => {
		...
		if (!pending.record?.id) {
			...
			await runTransport(pending.action, pending.idempotencyKey, {
				allowPending: true,
				boundGeneration: pending.expectedGeneration
			});
			return;
		}
		try {
			const record = await getRunCommand(runId, pending.record.id);
			acceptTransportRecord(pending.action, record, pending.idempotencyKey, pending.expectedGeneration);
		} catch (error) {
			const kind = observationKind(error);
			if (pending.protocolInvalid) {
				protocolTransportState(pending.action, pending.idempotencyKey, pending.record, error?.message || "Stored command could not be verified", pending.lockIdentity, pending.expectedGeneration);
			} else if ([
				"transport",
				"access",
				"protocol"
			].includes(kind)) {
				unavailableTransport(pending.action, pending.idempotencyKey, pending.expectedGeneration, pending.record, error);
			} else failTransport(pending.action, pending.idempotencyKey, pending.expectedGeneration, error, pending.record);
		}
	};
	useEffect(() => {
		const command = transportPending?.record;
		if (transportPending?.statusUnavailable || transportPending?.retrying || transportPending?.checking || !command?.id || command.status !== "accepted" && command.status !== "executing") return;
		...
		const poll = async () => {
			try {
				...
				failTransport(action, idempotencyKey, boundGeneration, error, command);
				return;
			}
		};
	...
	const failureHeading = !transportFailure ? "" : failureCode === "owner_access_required" ? "Owner access required" : failureCode === "command_protocol_error" ? "Invalid command response" : transportFailure.record?.status === "rejected" ? `${transportLabels(transportFailure.action).failure} — rejected` : transportFailure.action === "finalize" && canRetryTransport ? "Finalization stalled" : transportLabels(transportFailure.action).failure;
	...
	const submitStartOver = () => {
		...
		if (!intent || startOverDisabled || mode !== "finished" || !onStartOver || intent.runId !== String(runId) || intent.expectedGeneration !== expectedGeneration) {
	...
	const startOverLabel = String(startOverDialogIntent?.label || live?.label || live?.run_id || runId);
	const startOverIdentity = startOverLabel === String(runId) ? /* @__PURE__ */ _jsx("code", { children: runId }) : /* @__PURE__ */ _jsxs(_Fragment, { children: [
		...
		/* @__PURE__ */ _jsxs("span", {
			...
			children: [
				"(run ",
				/* @__PURE__ */ _jsx("code", { children: runId }),
				")"
			]
		})
	] });
	...
	useLayoutEffect(() => {
		...
		externalTransportPending?.action,
		!!transportFailure
	]);
	return /* @__PURE__ */ _jsxs("div", {
		...
		children: [
			/* @__PURE__ */ _jsxs("div", {
				...
				children: [
					...
					/* @__PURE__ */ _jsx("button", {
						type: "button",
						className: "hist-tag-mini " + (visiblyLive ? "live" : "hist"),
						onClick: returnToLive,
						disabled: visiblyLive,
						title: visiblyLive ? "" : "Jump to latest verified event",
						children: atLiveView ? visiblyLive ? `live · ${liveSeq}` : "reading · jump latest" : `replay ${sliderVal}/${liveSeq} → live`
					}),
					/* @__PURE__ */ _jsx("span", { className: "spacer" }),
					/* @__PURE__ */ _jsxs("button", {
						className: "btn sm ghost" + (!collapsed && showControls ? " on" : "") + (filtersActive ? " filters-active" : ""),
						...
						"aria-expanded": !collapsed && showControls,
						"aria-controls": "run-timeline-controls",
					...
					/* @__PURE__ */ _jsx("button", {
						...
						"aria-controls": "run-events-timeline",
			...
			!collapsed && /* @__PURE__ */ _jsxs("div", {
				id: "run-events-timeline",
				className: "dock-body chat-body",
				style: { height },
				children: [
					showControls && /* @__PURE__ */ _jsxs("div", {
						id: "run-timeline-controls",
						className: "dock-controls",
						children: [/* @__PURE__ */ _jsxs("div", {
							className: "scrubber inline",
							children: [
								/* @__PURE__ */ _jsxs("button", {
									className: "btn sm",
									onClick: returnToLive,
									disabled: drag == null && visiblyLive,
									children: [/* @__PURE__ */ _jsx(OpIcon, {
										name: "play",
										size: 11
									}), " Live"]
								...
								/* @__PURE__ */ _jsx("input", {
									...
									"aria-valuetext": sliderVal >= liveSeq ? `live at ${liveSeq}` : `replay ${sliderVal} of ${liveSeq}`,
								...
								/* @__PURE__ */ _jsx("span", {
									className: sliderVal >= liveSeq ? "live-tag" : "hist-tag",
									children: sliderVal >= liveSeq ? `live · ${liveSeq}` : `replay · ${sliderVal}/${liveSeq}`
								})
							]
						}), /* @__PURE__ */ _jsxs("div", {
							className: "kind-chips",
							children: [/* @__PURE__ */ _jsx("input", {
								className: "text feed-filter",
								"aria-label": "Filter loaded events",
								placeholder: "filter events…",
								value: filter,
								onChange: (e) => onFilterChange?.(e.target.value)
							}), /* @__PURE__ */ _jsxs("div", {
								className: "kind-chip-strip",
								children: [GROUPS.map(([g, label]) => /* @__PURE__ */ _jsxs("button", {
									className: "kind-chip k-" + g + (kinds.has(g) ? " on" : ""),
									"aria-pressed": kinds.has(g),
								...
								}, g)), kinds.size > 0 && /* @__PURE__ */ _jsx("button", {
									className: "kind-chip clear",
					...
					/* @__PURE__ */ _jsxs("div", {
						...
						children: [
							...
							/* @__PURE__ */ _jsxs("span", {
								className: "muted",
								children: [timeline.totalEvents != null ? `${log.length} loaded of ${timeline.totalEvents}` : `${log.length} loaded`, feed.length !== log.length ? ` · ${feed.length} ${filter.trim() || kinds.size ? "matching" : "shown"}` : ""]
							}),
					...
					(filter.trim() !== "" || kinds.size > 0) && timeline.totalEvents != null && timeline.totalEvents > log.length && /* @__PURE__ */ _jsx("div", {
						className: "timeline-window-note",
						role: "note",
						children: "Filters search loaded events only; page for more."
					}),
					timeline.status === "loading" && log.length === 0 && /* @__PURE__ */ _jsx("div", {
						className: "timeline-resource muted",
						role: "status",
						children: "Loading timeline…"
					}),
					timeline.loading.around && /* @__PURE__ */ _jsxs("div", {
						className: "timeline-resource muted",
						role: "status",
						children: [
							"Loading around seq ",
					...
					timeline.errors.tail && /* @__PURE__ */ _jsxs("div", {
						...
						role: "alert",
						children: [/* @__PURE__ */ _jsx("span", { children: log.length ? "Refresh failed; window unchanged." : timeline.errors.tail }), /* @__PURE__ */ _jsx("button", {
							className: "btn sm",
					...
					timeline.errors.older && /* @__PURE__ */ _jsxs("div", {
						className: "notice resource-error compact",
						role: "alert",
						children: [/* @__PURE__ */ _jsx("span", { children: "Older events unavailable." }), /* @__PURE__ */ _jsx("button", {
							className: "btn sm",
							onClick: timeline.loadOlder,
							children: "Retry"
						})]
					...
					timeline.errors.newer && /* @__PURE__ */ _jsxs("div", {
						className: "notice resource-error compact",
						role: "alert",
						children: [/* @__PURE__ */ _jsx("span", { children: "Live refresh failed; events may lag." }), /* @__PURE__ */ _jsx("button", {
							className: "btn sm",
							onClick: timeline.loadNewer,
							children: "Retry"
						})]
					...
					timeline.tornTail && /* @__PURE__ */ _jsx("div", {
						...
						role: "status",
						children: timeline.sourceTailLimited ? "The raw log tail exceeds the safety limit; showing the last verified canonical prefix." : "The final source row is incomplete or non-canonical; showing the last verified event prefix."
					}),
					feed.length === 0 && timeline.status === "ready" && !timeline.loading.around ? /* @__PURE__ */ _jsx("div", {
						className: "timeline-resource muted",
						children: filter.trim() || kinds.size ? "nothing matches the loaded window" : log.length > 0 ? "only background bookkeeping in this window — page older for run events" : "no events yet"
					}) : /* @__PURE__ */ _jsx(VirtualTimeline, {
						rows: feed,
						getKey: timelineEventKey,
					...
					!showControls && (() => {
						...
						return /* @__PURE__ */ _jsxs("div", {
							className: "agent-status dock-agent-status",
							children: [/* @__PURE__ */ _jsxs("div", {
								className: "as-line",
								children: [/* @__PURE__ */ _jsx("span", { className: "as-dot" }), /* @__PURE__ */ _jsx("span", {
									className: "as-seg",
									children: pipeline
								})]
							}), /* @__PURE__ */ _jsx(LiveTrace, {
								runId,
					...
					/* @__PURE__ */ _jsxs("div", {
						className: "dock-foot",
						children: [/* @__PURE__ */ _jsx("span", {
							className: "muted dock-foot-hint",
							children: readOnly ? "Historical timeline — live controls and sidecar trace details are disabled." : /* @__PURE__ */ _jsxs(_Fragment, { children: [
								"Steer below by chat or ",
								/* @__PURE__ */ _jsx("code", {
									className: "cmd-hint",
									children: "/stop · /finalize · /resume · /approve #id"
								}),
								"."
							] })
							...
							className: "transport",
							children: [
								transportPending && /* @__PURE__ */ _jsxs("div", {
									className: "transport-message" + (transportPending.statusUnavailable ? " warning" : ""),
									role: transportPending.statusUnavailable ? "alert" : "status",
									children: [/* @__PURE__ */ _jsxs("span", { children: [transportPending.statusUnavailable ? transportPending.observationKind === "access" ? "Owner access required to check command status" : transportPending.observationKind === "protocol" ? "Invalid command status response" : "Command status unavailable — the same intent is preserved" : transportPending.checking ? "Checking the same command…" : transportPending.retrying ? "Retrying the same command…" : transportPending.record?.status === "submitting" ? `Submitting ${transportPending.action}…` : transportPending.action === "finalize" ? "Finalizing…" : transportPending.action === "stop" ? "Stop requested…" : "Resume requested…", transportPending.record?.id ? /* @__PURE__ */ _jsxs("span", {
										className: "transport-command-id",
										title: `Command ${transportPending.record.id}`,
										children: [
											" · ",
											String(transportPending.record.id).slice(0, 12),
											"…"
										]
									}) : null] }), transportPending.statusUnavailable && /* @__PURE__ */ _jsxs(_Fragment, { children: [
										/* @__PURE__ */ _jsx("span", {
											className: "transport-detail",
											children: transportPending.observationKind === "access" ? "Verify owner access, then check again." : transportPending.observationKind === "protocol" ? "Saved command response unverifiable." : "Reconnect and check before another action."
										}),
										/* @__PURE__ */ _jsx("button", {
											className: "btn sm",
											onClick: onCheckTransport,
											"aria-label": `Check preserved ${transportPending.action} command`,
											children: "Check command"
										}),
										transportPending.protocolInvalid && /* @__PURE__ */ _jsx("button", {
											className: "btn sm ghost",
											onClick: dismissProtocolTransport,
											children: "Dismiss"
										})
									] })]
								}),
								!transportPending && externalTransportPending && /* @__PURE__ */ _jsxs("div", {
									className: "transport-message",
									role: "status",
									"aria-live": "polite",
									"aria-atomic": "true",
									children: [/* @__PURE__ */ _jsxs("span", { children: [
										"/",
										externalTransportPending.action,
										" is pending in Assistant"
									] }), externalTransportPending.commandId && /* @__PURE__ */ _jsxs("span", {
										className: "transport-command-id",
										title: `Command ${externalTransportPending.commandId}`,
										children: [
											" · ",
											String(externalTransportPending.commandId).slice(0, 12),
											"…"
										]
									})]
								}),
								!transportBusy && transportFailure && /* @__PURE__ */ _jsxs(_Fragment, { children: [
									/* @__PURE__ */ _jsxs("div", {
										className: "transport-message error",
										role: "alert",
										children: [/* @__PURE__ */ _jsxs("span", { children: [
											failureHeading,
											failedCommandId ? /* @__PURE__ */ _jsxs("span", {
												className: "transport-command-id",
												title: `Command ${failedCommandId}`,
												children: [
													" · ",
													String(failedCommandId).slice(0, 12),
													"…"
												]
											}) : null,
											!failedCommandId && conflictingCommandId ? /* @__PURE__ */ _jsxs("span", {
												className: "transport-command-id",
												title: `Conflicting active command ${conflictingCommandId}`,
												children: [
													" · active ",
													String(conflictingCommandId).slice(0, 12),
													"…"
												]
											}) : null
										] }), /* @__PURE__ */ _jsx("span", {
											className: "transport-detail",
											children: commandErrorMessage(transportFailure.record)
										})]
									}),
									canRetryTransport && /* @__PURE__ */ _jsx("button", {
										className: "btn sm",
										onClick: onRetryTransport,
										title: "Retry the same durable command",
										children: "Retry command"
									}),
									/* @__PURE__ */ _jsx("button", {
										className: "btn sm ghost",
										onClick: dismissTransportFailure,
										title: "Dismiss result",
										children: "Dismiss"
									})
								] }),
								!transportBusy && !transportFailure && mode === "finalizing" && /* @__PURE__ */ _jsx("span", {
									className: "muted",
									role: "status",
									children: "Finalizing…"
								}),
								!transportBusy && !transportFailure && mode === "finishing" && /* @__PURE__ */ _jsx("span", {
									className: "muted",
									role: "status",
									children: "Finishing write-out…"
								}),
								!transportBusy && !transportFailure && mode === "finalization-stalled" && /* @__PURE__ */ _jsxs(_Fragment, { children: [/* @__PURE__ */ _jsx("span", {
									className: "muted",
									role: "alert",
									children: "Finalization stalled"
								}), /* @__PURE__ */ _jsx("button", {
									className: "btn sm",
									onClick: onFinalize,
									title: "Resume pending finalization",
									children: "Reattach finalization"
								})] }),
								!transportBusy && !transportFailure && mode === "running" && /* @__PURE__ */ _jsxs(_Fragment, { children: [/* @__PURE__ */ _jsx("button", {
									className: "btn sm",
									"aria-label": "Stop run without finalizing",
									title: "Stop now; resume or finalize later",
									onClick: onStop,
									children: /* @__PURE__ */ _jsx(OpIcon, {
										name: "pause",
										size: 13
									...
									"aria-label": "Finalize run",
									title: "Finalize: stop, report, lessons and cost",
									onClick: onFinalize,
									children: /* @__PURE__ */ _jsx(OpIcon, {
										name: "stop",
										size: 13
								...
								!transportBusy && !transportFailure && (mode === "paused" || mode === "stalled") && /* @__PURE__ */ _jsxs(_Fragment, { children: [/* @__PURE__ */ _jsx("button", {
								...
								!transportBusy && !transportFailure && mode === "finished" && /* @__PURE__ */ _jsxs(_Fragment, { children: [/* @__PURE__ */ _jsx("button", {
									className: "btn sm primary",
									"aria-label": "Resume finished run",
									...
									className: "btn sm danger start-over-trigger",
									"aria-haspopup": "dialog",
									"aria-label": "Start run over",
									title: startOverDisabled ? startOverState?.disabledReason || "Resolve the existing Start over outcome first" : "Archive this generation and start again from the saved task and settings",
									disabled: startOverDisabled,
									onClick: () => setStartOverDialogIntent({
										runId: String(runId),
			...
			startOverDialogIntent && /* @__PURE__ */ _jsx("div", {
				className: "overlay start-over-overlay",
				onMouseDown: (event) => {
					if (event.target === event.currentTarget) closeStartOverDialog();
				},
				children: /* @__PURE__ */ _jsxs("section", {
					ref: startOverDialogRef,
					className: "modal start-over-dialog",
					role: "alertdialog",
					"aria-modal": "true",
					...
					children: [/* @__PURE__ */ _jsx("div", {
						...
						children: [
							/* @__PURE__ */ _jsxs("p", {
								...
								className: "start-over-copy",
								children: [startOverIdentity, " will start again from its saved task and settings. Current events, nodes, traces, and chat leave the live view and remain archived on disk."]
							}),
							/* @__PURE__ */ _jsx("p", {
								id: "start-over-cost-note",
								className: "start-over-cost-note",
								children: "The engine starts immediately and may use provider and evaluation budget."
							}),
							/* @__PURE__ */ _jsxs("div", {
								...
								children: [/* @__PURE__ */ _jsx("button", {
									...
									onClick: closeStartOverDialog,
									children: "Keep current run"
								}), /* @__PURE__ */ _jsx("button", {
									type: "button",
									className: "btn sm danger",
									disabled: startOverDisabled || mode !== "finished" || startOverDialogIntent.expectedGeneration !== expectedGeneration,
									onClick: submitStartOver,
									children: "Archive & start over"
								})]
							})
						]
					})]
