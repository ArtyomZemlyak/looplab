...
const equalIndicator = (left, right) => left.active === right.active && left.ariaLabel === right.ariaLabel && left.badge === right.badge && left.showBadge === right.showBadge && left.tone === right.tone && left.verified === right.verified;
...
export function useAttentionIndicator() {
	return useSyncExternalStore(subscribeAttentionIndicator, getAttentionIndicatorSnapshot, getAttentionIndicatorSnapshot);
}
...
export function AttentionLauncher({ indicator, embedded = false, expanded, controls, onClick }) {
	const toneClass = indicator.tone === "action" ? " has-action" : indicator.tone === "unread" ? " has-unread" : "";
	const className = `attention-trigger${embedded ? " in-assistant" : ""}${toneClass}` + (indicator.verified ? "" : " is-unverified");
	return /* @__PURE__ */ _jsxs("button", {
		...
		children: [/* @__PURE__ */ _jsx(OpIcon, {
			name: "bell",
			size: 22,
			className: "attention-bell-icon"
		}), indicator.showBadge && /* @__PURE__ */ _jsx("span", {
