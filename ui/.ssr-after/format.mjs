...
export function fmt(v, p = 4) {
	...
	const a = Math.abs(v);
	if (a !== 0 && (a < .001 || a >= 1e6)) return v.toExponential(2);
...
export function chipFontSize(text, { max = 11, min = 8, width = 168 } = {}) {
	...
	if (!len) return max;
	const fit = width / (len * .56);
	return Math.max(min, Math.min(max, Math.round(fit * 2) / 2));
}
...
export function fmtDate(sec, withTime = true) {
	if (!sec) return "—";
	return new Date(sec * 1e3).toLocaleString(undefined, withTime ? {
		year: "numeric",
		month: "short",
		day: "numeric",
		hour: "2-digit",
...
export function fmtAgo(sec) {
	if (!sec) return "";
	const d = Date.now() / 1e3 - sec;
	if (d < 60) return "just now";
	if (d < 3600) return Math.floor(d / 60) + "m ago";
	if (d < 86400) return Math.floor(d / 3600) + "h ago";
	if (d < 7 * 86400) return Math.floor(d / 86400) + "d ago";
	return new Date(sec * 1e3).toLocaleDateString();
}
