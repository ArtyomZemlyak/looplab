...
const REMEMBERED_GRANT_MODES = new Set([
	"default",
	"acceptEdits",
	"auto"
]);
...
const safeText = (value) => {
	const text = typeof value === "string" ? value : typeof value === "number" && Number.isFinite(value) ? String(value) : typeof value === "boolean" ? String(value) : "";
...
const scopeText = (value) => {
	...
	return Object.entries(value).filter(([key]) => !HIDDEN_SCOPE_KEY.test(key)).slice(0, 10).map(([key, item]) => {
...
const expiryMillis = (req, action) => {
	const raw = req?.request_expires_at ?? req?.expires_at ?? req?.expiresAt ?? action?.request_expires_at ?? action?.expires_at ?? action?.expiresAt;
	if (typeof raw === "number" && Number.isFinite(raw) && raw > 0) {
		return raw < 0xe8d4a51000 ? raw * 1e3 : raw;
	}
	...
	return Number.isFinite(created) && created > 0 && Number.isFinite(ttl) && ttl > 0 ? (created + ttl) * 1e3 : null;
};
...
const durationLabel = (seconds) => seconds % 60 === 0 ? `${seconds / 60} min` : `${seconds} sec`;
export function permissionPresentation(req, now = Date.now()) {
	const action = req?.action && typeof req.action === "object" && !Array.isArray(req.action) ? req.action : {};
	const rawRisk = firstText(action.risk_level, action.riskLevel, action.risk, req?.risk_level, req?.riskLevel, req?.risk).toUpperCase();
	...
	const grantTtlSeconds = Number.isSafeInteger(grantTtl) && grantTtl > 0 && grantTtl <= 600 ? grantTtl : 0;
	...
	return {
		...
		expiryLabel: expiresMs == null ? "Exact expiry unavailable; the server timeout still applies." : expiryDate.toLocaleString(),
		expired,
		canAlways: rememberable && risk !== "HIGH" && risk !== "UNKNOWN" && !!toolKind && REMEMBERED_GRANT_MODES.has(mode) && hasDurableScopeIdentity && expiresMs != null && !expired && grantTtlSeconds > 0,
		grantTtlSeconds,
