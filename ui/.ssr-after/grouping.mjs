...
export function groupKey(node, mode, ctx, state = null) {
	...
	if (mode === "metric") {
		...
		return m == null || !ctx.bucketOf ? null : ctx.bucketOf(m);
	}
	if (mode === "niche") {
		const p = node.idea?.params || {};
		...
		return keys.map((k) => {
			const n = Number(p[k]);
			return `${k}=${Number.isFinite(n) ? Math.round(n) : p[k]}`;
		}).join(" · ");
	}
	return null;
}
...
export function regionGeometry(rects, pad = 26) {
	...
	return {
		...
		w,
		h,
		path: roundRectPath(pad / 2, pad / 2, w - pad, h - pad, 20)
	};
}
...
export function rerouteForCollapse(nodesObj, collapsed, nodeGroup) {
	...
	const endpoint = (id) => {
		...
		return k != null && collapsed.has(k) ? superId(k) : `n:${id}`;
	};
	Object.values(nodesObj).forEach((n) => {
		const k = nodeGroup.get(n.id);
		if (k != null && collapsed.has(k)) hidden.add(n.id);
	});
	const seen = new Set();
	const edges = [];
...
export function groupAggregate(memberIds, nodesObj, direction, state = null) {
	...
	members.sort((a, b) => a.id - b.id).forEach((n) => {
		...
		// collapsed-card best/trajectory must obey the same eligibility boundary as
...
export function themeFilteredGroupAggregate(memberIds, nodesObj, direction, themeFilter = null, state = null, highlightIds = null) {
	...
	const matchedIds = activeIds.filter((id) => (!themeFilter || nodeTheme(nodesObj[id], state) === themeFilter) && (!conceptFilterActive || highlightIds.has(id)));
	const filterActive = !!themeFilter || conceptFilterActive;
	const filterDescription = themeFilter && conceptFilterActive ? `primary concept axis ${themeFilter} and selected concepts` : themeFilter ? `primary concept axis ${themeFilter}` : conceptFilterActive ? "selected concepts" : null;
	return {
