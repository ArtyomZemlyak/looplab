...
import { createTimelineState, mergeTimelinePage, noticeTimelineFrontier, parkTimeline, setTimelineFollowing, timelineBehindLive, timelineConflictMatchesGeneration, timelineContainsSeq, timelinePagePath, timelineRequestFailed, timelineRequestStarted, TIMELINE_CATCHUP_DELAYS, TIMELINE_PAGE_SIZE, TIMELINE_RETAIN_LIMIT } from "./timelineModel.mjs";
...
export function useTimeline(runId, { liveSeq = null, liveEventCount = null, expectedGeneration = null, enabled = true, pageSize = TIMELINE_PAGE_SIZE, retainLimit = TIMELINE_RETAIN_LIMIT } = {}) {
	...
	const load = useCallback(async (direction = "tail", { anchorSeq = null, unfenced = false, catchupAttempt = 0 } = {}) => {
		...
		try {
			...
			const lagging = observesLiveEdge && timelineBehindLive(merged, latestLiveSeqRef.current, latestLiveEventCountRef.current);
			...
			const observed = lagging && merged.windowAtTail ? {
				...merged,
				windowAtTail: false
			} : merged;
			commit(observed);
			if (observed.generationChanged) queueMicrotask(() => loadRef.current?.("tail", { unfenced: !expectedGenerationRef.current }));
				...
				if (observed.hasMore.newer || lagging && catchupAttempt < 3) {
					...
					commit({
						...
						errors: {
							...parked.errors,
							newer: "Live events are still behind the displayed run state."
						}
					});
				}
			}
			...
			if (generationChanged(error)) {
				...
				const reset = {
					...fresh(),
					status: displayedGeneration && !sameGenerationBoundary ? "error" : "loading",
					generationChanged: true,
					followingTail: stateRef.current.followingTail
				};
				if (displayedGeneration && !sameGenerationBoundary) reset.errors = {
					...reset.errors,
					[direction]: "The run was replaced; waiting for the displayed generation to refresh."
				};
				commit(reset);
	...
	const setFollowingTail = useCallback((value) => {
		...
		return commit(value ? setTimelineFollowing(stateRef.current, true) : parkTimeline(stateRef.current, latestLiveSeqRef.current, latestLiveEventCountRef.current));
	...
	const loadNewer = useCallback(() => stateRef.current.cursors.newer ? loadRef.current?.("newer") : Promise.resolve(null), []);
	...
	const ensureSeq = useCallback((seq) => {
		commit(parkTimeline(stateRef.current, latestLiveSeqRef.current, latestLiveEventCountRef.current));
		const anchorSatisfied = stateRef.current.requestedAnchor === Number(seq) && (stateRef.current.matchedSeq != null && timelineContainsSeq(stateRef.current, stateRef.current.matchedSeq) || stateRef.current.totalEvents === 0);
		return timelineContainsSeq(stateRef.current, seq) || anchorSatisfied ? Promise.resolve(stateRef.current) : loadAround(seq);
	}, [commit, loadAround]);
	const jumpToLive = useCallback(async () => {
		...
		for (let attempt = 0; attempt < 4; attempt += 1) {
			...
			const lagging = timelineBehindLive(refreshed, latestLiveSeqRef.current, latestLiveEventCountRef.current);
	...
	const retry = useCallback((direction) => direction === "around" ? lastAnchorRef.current == null ? Promise.resolve(null) : loadAround(lastAnchorRef.current) : loadRef.current?.(direction), [loadAround]);
	...
	const identityCurrent = runRef.current === runId && (!expectedGeneration || !state.generation || state.generation === expectedGeneration);
	const visibleState = identityCurrent ? state : {
		...fresh(),
		status: enabled && runId ? "loading" : "idle",
		generationChanged: true
	};
	return {
		...visibleState,
