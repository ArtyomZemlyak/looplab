...
import { get, fmt, fmtAgo, fmtElapsedSeconds, normalizeRunGeneration, runApiPath } from "./util.mjs";
import { effectiveRunStatus } from "./runIndex.mjs";
import { comparableRunRanking, COMPARE_COLUMNS, configDifferences } from "./portfolioModel.mjs";
import { deadlineRequest } from "./requestDeadline.mjs";
import { hashWithRunRouteState } from "./runRouteState.mjs";
import { jsx as _jsx, jsxs as _jsxs } from "react/jsx-runtime";
const path = runApiPath;
const COMPARE_DETAIL_TIMEOUT_MS = 8e3;
const captureToken = (run) => normalizeRunGeneration(run?.generation) || normalizeRunGeneration(run?.deletion_generation);
...
const comparisonMetricFormatter = (runs) => {
	const values = runs.map((run) => run.best_confirmed ?? run.best_metric).filter((value) => typeof value === "number" && Number.isFinite(value));
	...
	for (let precision = 4; precision <= 17; precision += 1) {
		...
		if (new Set(labels).size === distinct.length) {
			...
			return (value) => typeof value === "number" && Number.isFinite(value) ? byValue.get(value) ?? fmt(value, precision) : "—";
		}
	}
	const byValue = new Map(distinct.map((value) => [value, String(value)]));
	return (value) => typeof value === "number" && Number.isFinite(value) ? byValue.get(value) ?? String(value) : "—";
};
export async function loadDetail(run, signal, timeoutMs = COMPARE_DETAIL_TIMEOUT_MS) {
	...
	const timed = deadlineRequest(async (requestSignal) => {
		...
		try {
			config = await get(path(run.run_id, "/config"), {
				signal: requestSignal,
				cache: "no-store"
			});
			configReady = true;
		...
		try {
			...
			probe = await get(path(run.run_id, "/lifecycle"), {
				signal: requestSignal,
				cache: "no-store"
			});
		} catch (error) {
	...
	try {
		await timed.promise;
		const stateStable = snapshot?.generation === expectedGeneration && snapshot?.seq === expectedSequence;
		const stable = stateStable && probe?.generation === expectedGeneration && probe?.seq === expectedSequence;
		return {
			runId: run.run_id,
...
export function championRunHref(run, detail) {
	...
	if (!generation || !Number.isSafeInteger(nodeId) || nodeId < 0 || !Number.isSafeInteger(sequence) || sequence < 0 || !Number.isSafeInteger(nodeGeneration) || nodeGeneration < 0) return null;
	return hashWithRunRouteState(`#/run/${encodeURIComponent(run.run_id)}`, {
		generation,
		nodeId,
		nodeGeneration,
		sequence
...
export default function RunCompare({ runs, columns, names, onColumns, onRemove, onOpen = null, headingRef = null, onFocusCapture = null }) {
	...
	useEffect(() => {
		...
		Promise.all(capture.runs.map((run) => loadDetail(run, controller.signal))).then((details) => {
			...
			setResource((current) => {
				const retained = retainedCapture(current, capture.runs);
				const message = error?.name === "TimeoutError" ? "Snapshot refresh timed out." : "Snapshot refresh failed.";
				return retained ? {
					...retained,
					identity,
					status: "stale",
	...
	const compatibleCapture = !!resource.loadedAt && runs.every((run) => sameRunIdentity(resourceRunById.get(run.run_id), run));
	const resourceMatches = resource.identity === identity;
	const captureVisible = compatibleCapture || resourceMatches && !!resource.loadedAt;
	const snapshotRuns = captureVisible ? runs.map((run) => resourceRunById.get(run.run_id)).filter(Boolean) : [];
	const selectedIds = new Set(snapshotRuns.map((run) => run.run_id));
	const snapshotDetails = captureVisible ? resource.details.filter((detail) => selectedIds.has(detail.runId)) : [];
	const snapshotNames = captureVisible ? resource.names : freezeNames(names);
	const snapshotStatus = resourceMatches ? resource.status : compatibleCapture ? "refreshing" : "loading";
	...
	const snapshotTime = resource.loadedAt ? new Date(resource.capturedAt || resource.loadedAt).toLocaleTimeString() : "";
	const snapshotBusy = snapshotStatus === "loading" || snapshotStatus === "refreshing";
	const snapshotPartial = snapshotStatus !== "partial" && snapshotDetails.some((detail) => detail.partial);
	const snapshotReceipt = snapshotStatus === "loading" ? "Loading a run comparison capture…" : snapshotStatus === "refreshing" ? `Refreshing… Showing capture from ${snapshotTime}.` : snapshotStatus === "stale" ? `${resource.error} Showing capture from ${snapshotTime}.` : snapshotStatus === "error" ? "Run comparison capture is unavailable." : snapshotStatus === "partial" ? `Summary capture completed ${snapshotTime}; run details could not be verified.` : `Comparison captured ${snapshotTime}.`;
	const rankingReceipt = ranking.status === "ranked" ? ranking.bestRunIds.length > 1 ? ` ${ranking.bestRunIds.length} runs tied for best ${ranking.phase === "confirmed" ? "confirmed mean" : "raw metric"} in this capture: ${formatMetric(ranking.bestValue)}.` : ` Best ${ranking.phase === "confirmed" ? "confirmed mean" : "raw metric"} in this capture: ${formatMetric(ranking.bestValue)}.` : "";
	const rankingWarning = ranking.status === "incompatible" ? "Metrics are shown but not ranked; selected runs use different tasks or objectives." : ranking.status === "mixed-phase" ? "Metrics are shown but not ranked because confirmed means and raw metrics are mixed." : ranking.status === "missing-metric" ? "Metrics are shown but not ranked because one or more selected runs lack a finite metric." : "";
	const refreshCapture = () => {
		if (snapshotBusy) return;
		setRetry((value) => value + 1);
	};
	const toggleColumn = (id) => onColumns(columns.includes(id) ? columns.filter((column) => column !== id) : [...columns, id]);
	...
	useLayoutEffect(() => {
		...
		const browserLostFocus = !active || active === document.body || active === document.documentElement || !active.isConnected;
	...
	return /* @__PURE__ */ _jsxs("section", {
		...
		children: [
			/* @__PURE__ */ _jsxs("div", {
				...
				children: [
					/* @__PURE__ */ _jsxs("div", { children: [/* @__PURE__ */ _jsx("h2", {
						ref: headingRef,
						id: "run-compare-title",
						tabIndex: -1,
						children: "Run comparison"
					}), /* @__PURE__ */ _jsxs("p", { children: [runs.length, " runs · The Run column stays pinned; choose the evidence columns you need."] })] }),
					/* @__PURE__ */ _jsxs("details", {
						ref: columnsDetailsRef,
						...
						children: [/* @__PURE__ */ _jsxs("summary", { children: ["Columns · ", columns.length] }), /* @__PURE__ */ _jsxs("fieldset", { children: [
							...
							/* @__PURE__ */ _jsx("button", {
								type: "button",
								className: "btn sm compare-columns-close",
								onClick: closeColumns,
								children: "Done"
							}),
							COMPARE_COLUMNS.map(([id, label]) => /* @__PURE__ */ _jsxs("label", { children: [
								/* @__PURE__ */ _jsx("input", {
									type: "checkbox",
					...
					/* @__PURE__ */ _jsx("button", {
						className: "btn sm",
						"aria-disabled": snapshotBusy || undefined,
						"aria-busy": snapshotBusy || undefined,
						onClick: refreshCapture,
						children: snapshotStatus === "loading" ? "Loading…" : snapshotStatus === "refreshing" ? "Refreshing…" : "Refresh snapshot"
					})
				]
			}),
			/* @__PURE__ */ _jsxs("div", {
				id: "run-compare-receipt",
				className: "compare-receipt",
				role: "status",
				"aria-live": "polite",
				children: [
					snapshotReceipt,
			...
			snapshotRuns.length > 0 && rankingWarning && /* @__PURE__ */ _jsx("div", {
				id: "run-compare-ranking-warning",
				className: "notice resource-warning",
				role: "status",
				children: rankingWarning
			}),
			/* @__PURE__ */ _jsx("div", {
				...
				children: /* @__PURE__ */ _jsx("div", {
					...
					role: "region",
					"aria-label": "Selected run comparison",
					"aria-describedby": `run-compare-receipt${rankingWarning ? " run-compare-ranking-warning" : ""}`,
					"aria-busy": snapshotBusy,
					tabIndex: 0,
					children: /* @__PURE__ */ _jsxs("table", {
						className: "tbl data-table compare-table",
						children: [
							/* @__PURE__ */ _jsx("caption", {
								className: "sr-only",
								children: "Comparison of selected runs"
							}),
							/* @__PURE__ */ _jsx("thead", { children: /* @__PURE__ */ _jsxs("tr", { children: [
								/* @__PURE__ */ _jsx("th", {
									scope: "col",
									className: "compare-pinned",
									children: "Run"
								}),
								columns.map((id) => /* @__PURE__ */ _jsx("th", {
									scope: "col",
									children: COMPARE_COLUMNS.find((column) => column[0] === id)?.[1]
								}, id)),
								/* @__PURE__ */ _jsx("th", {
									scope: "col",
							...
							/* @__PURE__ */ _jsxs("tbody", { children: [snapshotRuns.map((run) => {
										...
										/* @__PURE__ */ _jsxs("th", {
											...
											children: [
												/* @__PURE__ */ _jsx("button", {
													...
													onClick: () => openComparisonRoute(onOpen, run.run_id, `#/run/${encodeURIComponent(run.run_id)}`),
													children: label
												}),
												isBest && /* @__PURE__ */ _jsx("span", {
													className: "pill compare-best",
													children: tiedBest ? "Tied best" : "Best"
												}),
												run.label && /* @__PURE__ */ _jsx("small", { children: run.run_id }),
												detail?.partial && /* @__PURE__ */ _jsx("small", { children: "partial detail" })
											]
										}),
										columns.map((id) => /* @__PURE__ */ _jsx("td", { children: id === "champion" && championHref ? /* @__PURE__ */ _jsxs("button", {
											type: "button",
											className: "compare-link",
											...
											children: ["#", detail.state.best_node_id]
										}) : valueFor(id, run, detail, snapshotNames, formatMetric) }, id)),
										/* @__PURE__ */ _jsx("td", { children: /* @__PURE__ */ _jsx("button", {
											className: "btn xs ghost",
											"data-compare-remove-id": run.run_id,
							...
							}), !snapshotRuns.length && /* @__PURE__ */ _jsx("tr", { children: /* @__PURE__ */ _jsx("td", {
								colSpan: columns.length + 2,
								className: "muted",
								children: snapshotStatus === "error" ? "No verified run capture is available." : "Loading selected run capture…"
							}) })] })
						]
					})
				})
			...
			/* @__PURE__ */ _jsxs("details", {
				className: "compare-config",
				children: [/* @__PURE__ */ _jsxs("summary", { children: ["Configuration differences · ", config.total] }), snapshotStatus !== "loading" && config.rows.length ? /* @__PURE__ */ _jsxs("div", {
					className: "data-table-scroll",
					role: "region",
					"aria-label": "Configuration differences",
					tabIndex: 0,
					children: [/* @__PURE__ */ _jsxs("table", {
						className: "tbl data-table compare-config-table",
						children: [/* @__PURE__ */ _jsx("thead", { children: /* @__PURE__ */ _jsxs("tr", { children: [/* @__PURE__ */ _jsx("th", {
							scope: "col",
							children: "Setting"
						}), snapshotRuns.map((run) => /* @__PURE__ */ _jsx("th", {
							scope: "col",
							children: run.label || run.run_id
						}, run.run_id))] }) }), /* @__PURE__ */ _jsx("tbody", { children: config.rows.map((row) => /* @__PURE__ */ _jsxs("tr", { children: [/* @__PURE__ */ _jsx("th", {
							scope: "row",
							children: row.key
						}), row.values.map((value, index) => /* @__PURE__ */ _jsx("td", { children: value }, snapshotRuns[index].run_id))] }, row.key)) })]
					}), config.total > config.rows.length && /* @__PURE__ */ _jsxs("p", {
						className: "muted",
						children: [
							"Showing the first ",
							config.rows.length,
					...
					className: "muted",
					children: snapshotStatus === "loading" ? "Loading configuration capture…" : "No verified configuration differences are available for this capture."
				})]
			})
		]
	});
