...
export const LAUNCH_RUNTIME_FIELDS = Object.freeze([
	...
	{
		key: "card_driven_selection",
		label: "Card queue selection",
		type: "bool",
	...
	{
		key: "max_eval_seconds",
		label: "Eval budget (s)",
		type: "float",
		min: 0
	}
]);
const RESERVED_RUN_IDS = new Set([
	"assistant",
	"reports",
	".reviews",
	".reviews.lock",
...
export function createLaunchDraft(spec = {}) {
	...
	return {
		...
		setup_steps: Array.isArray(spec.setup_steps) ? spec.setup_steps.map((value) => String(value || "").trim()).filter(Boolean) : []
	};
}
export function parseObjectJson(text, label) {
	...
	try {
		...
		return {
			ok: false,
			error: `${label} must be valid JSON: ${error.message}`
		};
	}
...
export function validateLaunchDraft(draft) {
	...
	if (settings) {
		for (const field of LAUNCH_RUNTIME_FIELDS) {
			...
			if (field.type === "int" && (!Number.isInteger(value) || field.min != null && value < field.min || field.max != null && value > field.max)) {
				const range = field.min != null && field.max != null ? ` between ${field.min} and ${field.max}` : field.min != null ? ` of at least ${field.min}` : "";
				errors[`settings.${field.key}`] = `${field.label} must be an integer${range}`;
			}
			if (field.type === "float" && (typeof value !== "number" || !Number.isFinite(value) || field.min != null && value < field.min)) {
				errors[`settings.${field.key}`] = `${field.label} must be a number` + (field.min == null ? "" : ` of at least ${field.min}`);
			}
...
const cleanChat = (chat) => (Array.isArray(chat) ? chat : []).filter((turn) => turn && (turn.role === "user" || turn.role === "assistant") && String(turn.content || "").trim()).map((turn) => ({
	role: turn.role,
...
export function summarizeLaunchTask(draft) {
	if (draft?.source === "task_file") {
		...
		return [{
			label: "Source",
			value: path ? path.split(/[\\/]/).pop() : "No task file selected"
		}];
	}
	const parsed = parseObjectJson(draft?.task_json, "Task");
	if (!parsed.ok) return [{
		label: "Task",
		value: "Fix the JSON to preview this task",
		invalid: true
	}];
	const task = parsed.value;
	const kind = task.kind || (task.repo || task.editable_path || task.editables ? "repo" : task.kaggle || task.competition ? "Kaggle" : task.dataset || task.data || task.data_path ? "dataset" : task.benchmark || "composable");
	const rows = [{
		label: "Type",
		value: String(kind)
	}];
	if (task.goal) rows.push({
		label: "Goal",
		value: short(task.goal)
	});
	if (task.direction) rows.push({
		label: "Direction",
		value: String(task.direction)
	});
