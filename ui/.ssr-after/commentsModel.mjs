...
const safeTime = (value) => typeof value === "number" && Number.isFinite(value) && value >= 0 && value <= 864e10 ? value : null;
const safeCursor = (value) => typeof value === "string" && value.length > 0 && value.length <= 512 && !CONTROL_RE.test(value) ? value : null;
...
export function normalizeComment(raw) {
	...
	const text = raw.text;
	if (!commentIdValid(id) || nodeId == null || !legacy && nodeGeneration == null || version == null || version < 1 || createdAt == null || updatedAt == null || updatedAt < createdAt || typeof text !== "string" || !validUnicode(text) || utf8Bytes(text) > COMMENT_MAX_BYTES || typeof raw.resolved !== "boolean" || typeof raw.editable !== "boolean") return null;
...
export function normalizeCommentsPage(payload, expectedGeneration = null) {
	if (!payload || typeof payload !== "object" || Array.isArray(payload) || !Array.isArray(payload.comments) || !RUN_GENERATION_RE.test(payload.run_generation || "") || typeof payload.has_more !== "boolean") return null;
...
export function normalizeCommentVersion(raw, current) {
	...
	if (version == null || version < 1 || updatedAt == null || typeof text !== "string" || !validUnicode(text) || utf8Bytes(text) > COMMENT_MAX_BYTES || typeof resolved !== "boolean") return null;
	...
	const action = [
		...
		"reopened",
		"legacy"
	].includes(raw.action) ? raw.action : version === 1 ? "created" : "edited";
	return Object.freeze({
		version,
		text,
		resolved,
...
export function normalizeCommentHistory(payload, current, expectedGeneration = null) {
	if (!payload || typeof payload !== "object" || Array.isArray(payload) || payload.comment_id !== current?.id || !Array.isArray(payload.versions) || !RUN_GENERATION_RE.test(payload.run_generation || "") || typeof payload.has_more !== "boolean") return null;
	if (expectedGeneration && payload.run_generation !== expectedGeneration) return null;
...
export function commentMatchesSubject(comment, nodeId, nodeGeneration) {
	...
	return Number.isSafeInteger(nodeId) && nodeId >= 0 && Number.isSafeInteger(nodeGeneration) && nodeGeneration >= 0 && comment?.legacy === false && comment.nodeId === nodeId && comment.nodeGeneration === nodeGeneration;
}
...
export function commentMutationError(error, fallback = "Comment could not be saved.") {
	...
	if (error?.commandUnknown === true || ["accepted", "executing"].includes(error?.commandRecord?.status)) {
