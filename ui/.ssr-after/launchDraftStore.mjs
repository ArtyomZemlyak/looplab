...
export function launchDraftKey({ sessionId, messageId, messageIndex, proposalId, proposalIndex }) {
	const owner = proposalId ? `proposal:${proposalId}` : `message:${messageId || messageIndex}:proposal:${proposalIndex}`;
...
export function retainedLaunchDraft(draft) {
	...
	for (const field of DRAFT_FIELDS) {
		...
		clean[field] = field === "setup_steps" ? Array.isArray(draft[field]) ? draft[field].map((value) => String(value)) : [] : String(draft[field] == null ? "" : draft[field]);
	}
	return clean;
}
