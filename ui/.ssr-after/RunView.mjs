import React, { lazy, useCallback, useEffect, useLayoutEffect, useMemo, useRef, useState, useSyncExternalStore } from "react";
import { useMediaQuery, useRunState } from "./hooks.mjs";
import { useToast } from "./useToast.mjs";
import { useTimeline } from "./useTimeline.mjs";
import { takeRunPanelHistoryEntry, useRunRouteState } from "./useRunRouteState.mjs";
import { reviewInspectorTabs, reviewPanelAllowed, runRouteStateHasTarget } from "./runRouteState.mjs";
import { deadlineGet, fmt, fmtInt, fmtElapsedSeconds, phaseLabel, workingId, isSweep, CONTROL, commandFeedback, createIdempotencyKey, resetRun, storageGet, storageSet, runApiPath } from "./util.mjs";
import { computeGroups, autoCollapseSet } from "./grouping.mjs";
import EnergyToggle from "./EnergyToggle.mjs";
import { OpIcon } from "./icons.mjs";
import LazyBoundary from "./LazyBoundary.mjs";
import WhyStrip from "./WhyStrip.mjs";
import { DIALOG_PRIORITY, useDialogFocus } from "./useDialogFocus.mjs";
import { nextRovingIndex } from "./accessibility.mjs";
import { clearRunAccess, historyMatches, liveHistory, reconcileHistoricalSelection, rejectHistory, requestHistory, resolveHistory, setRunAccess } from "./runMode.mjs";
import { approvalCommandFor, dagEmptyPresentation, lifecyclePhaseLabel, runLifecycle, terminalReady } from "./runIndex.mjs";
import { initialDagOverviewDecision } from "./dagViewport.mjs";
import { captureMergeIntent, mergeIntentCommand, mergeIntentMatches, selectMergeTarget } from "./mergeIntent.mjs";
import { nodeIsActive } from "./nodeProjection.mjs";
import { createInspectorDraftStore } from "./inspectorDraftStore.mjs";
import { installNavigationLossGuard } from "./navigationLossGuard.mjs";
import { authoringRecoveryStorageKey, inspectAuthoringRecoveryStorage } from "./authoringRecoveryStorage.mjs";
import { clearCommentOperationIntent, clearDamagedCommentOperation, listCommentOperationRecoveries, readCommentOperationRecoveryRevision, refreshCommentOperationRecoveries, subscribeCommentOperationRecoveries } from "./commentRecoveryStorage.mjs";
import { clearRunStartOverIntent, createRunStartOverIntent, loadRunStartOverIntent, saveRunStartOverIntent } from "./runStartOverRecovery.mjs";
import { jsx as _jsx, jsxs as _jsxs, Fragment as _Fragment } from "react/jsx-runtime";
const lazyNamed = (load, name) => lazy(() => load().then((module) => ({ default: name === "default" ? module.default : module[name] })));
...
let traceClearRecoverySnapshot = {
	revision: 0,
	signals: new Map()
};
...
const publishTraceClearRecovery = (scope, kind) => {
	const signal = {
		scope,
		kind,
		revision: ++traceClearRecoveryRevision
	};
...
const commentDraftEntryUnsafe = ([scope, fields], runId) => {
	if (!scope.startsWith(`comment-composer:${runId}@`) && !scope.startsWith(`comment-card:${runId}@`) || !fields || typeof fields !== "object" || Array.isArray(fields)) return false;
	const busy = fields.busy === true || typeof fields.busy === "string" && !!fields.busy;
	const draft = typeof fields.text === "string" && fields.text.length > 0 || fields.dirty === true && typeof fields.draftText === "string";
	const recovery = !!fields.retryIntent || !!fields.uncertainIntent || !!fields.editRetryIntent || !!fields.uncertainEdit || !!fields.resolutionRetryIntent || !!fields.uncertainResolution || !!fields.damagedRecovery;
...
const HISTORY_SAFE_PANELS = new Set([
	"sensitivity",
	"importance",
	"failures",
	"pareto",
...
const RUN_CONFIG_REQUEST_TIMEOUT_MS = 15e3;
const HISTORICAL_SNAPSHOT_REQUEST_TIMEOUT_MS = 15e3;
const START_OVER_REQUEST_TIMEOUT_MS = 15e3;
const START_OVER_AUTO_RETRY_LIMIT = 3;
const RUN_GROUP_HISTORY_KEY = "looplab.runGroupNavigation";
const RUN_GROUP_HISTORY_MODES = new Set([
	"theme",
	"operator",
	"metric",
	"niche"
...
function normalizeRunGroupNavigation(value, runId, generation = null) {
	...
	if (value.runId !== String(runId) || !/^[0-9a-f]{64}$/.test(String(value.generation || "")) || generation && value.generation !== generation || !RUN_GROUP_HISTORY_MODES.has(value.mode) || typeof value.key !== "string" || !value.key || value.key.length > RUN_GROUP_KEY_LIMIT || RUN_GROUP_CONTROL_RE.test(value.key)) return null;
	...
	const scrollTop = Number.isFinite(rawScrollTop) && rawScrollTop >= 0 ? Math.min(rawScrollTop, 1e7) : 0;
	return {
		runId: String(runId),
...
function DagEmptyOverlay({ presentation, transport, onAction }) {
	...
	if (transport?.failure) {
		actions = [{
			id: "events",
			label: "Review command failure",
			emphasis: "primary"
		}, ...actions.filter((item) => !TRANSPORT_EMPTY_ACTIONS.has(item.id) && item.id !== "events")];
	}
	const pendingAction = transport?.pendingAction;
	return /* @__PURE__ */ _jsxs("section", {
		className: `dag-empty-card ${presentation.tone}`,
		role: presentation.liveRegion === "assertive" ? "alert" : "status",
		"aria-live": presentation.liveRegion,
		"aria-atomic": "true",
		children: [
			presentation.tone === "progress" && /* @__PURE__ */ _jsx("span", {
				className: "dag-empty-spinner",
				"aria-hidden": "true"
			}),
			/* @__PURE__ */ _jsx("span", {
				className: "dag-empty-eyebrow",
				children: "Search canvas"
			}),
			/* @__PURE__ */ _jsx("h2", { children: presentation.title }),
			/* @__PURE__ */ _jsx("p", { children: presentation.body }),
			transport?.failure && /* @__PURE__ */ _jsx("p", {
				className: "dag-empty-command-note",
				children: "The last run command needs attention. Its exact identity is preserved in Events & timeline."
			}),
			transport?.busy && /* @__PURE__ */ _jsx("p", {
				className: "dag-empty-command-note",
				children: pendingAction ? `${pendingAction} command is already in progress.` : "A run command is already in progress."
			}),
			actions.length > 0 && /* @__PURE__ */ _jsx("div", {
				className: "dag-empty-actions",
...
export default function RunView({ runId, onBack, reviewMode = false, reviewMeta = null }) {
	const { live, seq, generation, eventCount: liveEventCount, connected, status: runStatus, error: runError, retry: retryRun } = useRunState(runId, { pollOnly: reviewMode });
	const retryRunRef = useRef(retryRun);
	retryRunRef.current = retryRun;
	const [startOverRecovery, setStartOverRecovery] = useState(() => reviewMode ? {
		kind: "none",
		intent: null
	...
	const startOverAutoRetryRef = useRef({
		operationId: null,
		count: 0
	});
	...
	const publishPanelNavigationGuard = useCallback((controller) => {
		if (!controller || !["config", "authoring"].includes(controller.route) || typeof controller.dispose !== "function") return undefined;
	...
	const commentRecoveryRevision = useSyncExternalStore(subscribeCommentOperationRecoveries, readCommentOperationRecoveryRevision, readCommentOperationRecoveryRevision);
	...
	const traceClearRecoverySnapshot = useSyncExternalStore(subscribeTraceClearRecovery, readTraceClearRecoverySnapshot, readTraceClearRecoverySnapshot);
	const inspectorDraftRevision = useSyncExternalStore(inspectorDraftStoreRef.current.subscribeAll, inspectorDraftStoreRef.current.revision, inspectorDraftStoreRef.current.revision);
	const retainedCommentEntries = useMemo(() => reviewMode ? [] : inspectorDraftStoreRef.current.entries().filter((entry) => commentDraftEntryUnsafe(entry, String(runId))), [
		reviewMode,
		runId,
		inspectorDraftRevision
	]);
	const retainedCommentScopes = useMemo(() => [...new Set(retainedCommentEntries.map(([scope]) => scope))], [retainedCommentEntries]);
	const retainedCommentDrafts = useMemo(() => retainedCommentEntries.map(commentDraftText).filter(Boolean), [retainedCommentEntries]);
	const retainedCommentRecovery = useMemo(() => reviewMode ? {
		available: true,
		valid: [],
		damaged: []
		...
		reviewMode,
		runId,
		commentRecoveryRevision
	]);
	const retainedCommentDurableCount = retainedCommentRecovery.valid.length + retainedCommentRecovery.damaged.length;
	...
	const retainedCommentProtectedCreateCandidates = [
		...retainedCommentRecovery.valid.filter((intent) => intent.kind === "create" && (!generation || intent.expectedGeneration === generation)),
		...retainedCommentRecovery.damaged.filter((recovery) => recovery.identity?.kind === "create" && (!generation || recovery.identity.expectedGeneration === generation)).map((recovery) => recovery.identity),
		...retainedCommentEntries.flatMap(([, fields]) => {
			const candidates = [fields?.uncertainIntent?.recovery, fields?.damagedRecovery?.identity];
			return candidates.filter((identity) => identity?.kind === "create" && (!generation || identity.expectedGeneration === generation));
		})
	];
	const retainedCommentProtectedCreates = [...new Map(retainedCommentProtectedCreateCandidates.map((identity) => [[
		identity.expectedGeneration,
		identity.nodeId,
		identity.nodeGeneration
	].join("\0"), identity])).values()];
	const retainedCommentDamagedProtectedCreateCount = retainedCommentProtectedCreates.filter((identity) => !identity.operationId).length;
	const retainedCommentValidProtectedCreateCount = retainedCommentProtectedCreates.length - retainedCommentDamagedProtectedCreateCount;
	const retainedCommentScopeHasProtectedCreate = (scope) => retainedCommentProtectedCreates.some((identity) => {
	...
	const retainedCommentReleasableEntries = retainedCommentRecoveryUnavailable ? [] : retainedCommentEntries.filter(([scope]) => !retainedCommentScopeHasProtectedCreate(scope));
	const retainedCommentReleasableIntents = retainedCommentRecovery.valid.filter((intent) => intent.kind !== "create" || generation && intent.expectedGeneration !== generation);
	const retainedCommentReleasableDamaged = retainedCommentRecovery.damaged.filter((recovery) => recovery.identity?.kind !== "create" || generation && recovery.identity.expectedGeneration !== generation);
	const retainedCommentReleasableCount = retainedCommentReleasableEntries.length + retainedCommentReleasableIntents.length + retainedCommentReleasableDamaged.length;
	const retainedCommentWorkUnsafe = retainedCommentEntries.length > 0 || retainedCommentDurableCount > 0 || retainedCommentRecoveryUnavailable;
	...
	const activePanelNavigationGuard = panelNavigationGuard?.route === panel ? panelNavigationGuard : null;
	const retainedConfigDraft = panel === "config" ? inspectorDraftStoreRef.current.readField(`panel:config:${String(runId)}`, "draft", null) : null;
	const retainedConfigStoredDraftUnsafe = retainedConfigDraft?.schema === "looplab.config-draft/v1" && retainedConfigDraft?.unsafe === true;
	const retainedConfigDraftUnsafe = retainedConfigStoredDraftUnsafe || activePanelNavigationGuard?.route === "config" && activePanelNavigationGuard.unsafe === true;
	const retainedConfigScope = `panel:config:${String(runId)}`;
	const retainedAuthoringScope = "panel:authoring";
	const retainedAuthoringDocuments = panel === "authoring" ? inspectorDraftStoreRef.current.readField(retainedAuthoringScope, "documents", null) : null;
	const retainedAuthoringDocumentEntries = retainedAuthoringDocuments && typeof retainedAuthoringDocuments === "object" && !Array.isArray(retainedAuthoringDocuments) ? Object.entries(retainedAuthoringDocuments).filter(([, document]) => document && typeof document === "object" && !Array.isArray(document)) : [];
	const retainedAuthoringDocumentValues = retainedAuthoringDocumentEntries.map(([, document]) => document);
	const retainedAuthoringDraftCount = retainedAuthoringDocumentValues.filter((document) => document.draftText !== document.savedText).length;
	const retainedAuthoringUncertainSaves = panel === "authoring" ? inspectorDraftStoreRef.current.readField(retainedAuthoringScope, "uncertainSaves", null) : null;
	const retainedAuthoringDamagedRecoveries = panel === "authoring" ? inspectorDraftStoreRef.current.readField(retainedAuthoringScope, "damagedRecoveries", null) : null;
	const retainedAuthoringUncertainSaveEntries = retainedAuthoringUncertainSaves && typeof retainedAuthoringUncertainSaves === "object" && !Array.isArray(retainedAuthoringUncertainSaves) ? Object.entries(retainedAuthoringUncertainSaves).filter(([, recovery]) => recovery && typeof recovery === "object" && !Array.isArray(recovery)) : [];
	const retainedAuthoringDamagedRecoveryValues = retainedAuthoringDamagedRecoveries && typeof retainedAuthoringDamagedRecoveries === "object" && !Array.isArray(retainedAuthoringDamagedRecoveries) ? Object.values(retainedAuthoringDamagedRecoveries).filter((recovery) => recovery && typeof recovery === "object" && !Array.isArray(recovery)) : [];
	const retainedAuthoringFenceActive = panel === "authoring" && (generationMismatch || generationPending);
	const retainedAuthoringStorageRecovery = retainedAuthoringFenceActive ? inspectAuthoringRecoveryStorage() : {
		available: true,
		count: 0,
		records: []
	};
	...
	const retainedAuthoringHasMemoryRecovery = retainedAuthoringUncertainSaveEntries.length > 0 || retainedAuthoringDamagedRecoveryValues.length > 0 || retainedAuthoringDocumentEntries.some(([, document]) => document.recoveryOperationId || document.recoveryStorageRaw);
	...
	if (retainedAuthoringFenceActive) {
		...
		for (const [scope, document] of retainedAuthoringDocumentEntries) {
			...
			const represented = addRetainedAuthoringRecovery(authoringRecoveryStorageKey(document.kind, document.name), document.recoveryStorageRaw, "memory");
	...
	let retainedAuthoringMemoryOnlyRecoveryCount = retainedAuthoringFenceActive ? retainedAuthoringOpaqueMemoryRecoveryCount : retainedAuthoringHasMemoryRecovery ? 1 : 0;
	...
	const retainedAuthoringRecoveryCount = retainedAuthoringDurableRecoveryCount + retainedAuthoringMemoryOnlyRecoveryCount;
	const retainedAuthoringDraftUnsafe = retainedAuthoringDraftCount > 0 || retainedAuthoringRecoveryCount > 0 || activePanelNavigationGuard?.route === "authoring" && activePanelNavigationGuard.unsafe === true;
	const retainedPanelDraftUnsafe = retainedConfigDraftUnsafe || retainedAuthoringDraftUnsafe;
	const retainedPanelScope = retainedConfigDraftUnsafe ? retainedConfigScope : retainedAuthoringDraftUnsafe ? retainedAuthoringScope : "";
	const retainedPanelRoute = retainedConfigDraftUnsafe ? "config" : retainedAuthoringDraftUnsafe ? "authoring" : null;
	const retainedAuthoringDiscardItems = [retainedAuthoringDraftCount > 0 ? `${retainedAuthoringDraftCount} unsaved in-memory Authoring draft${retainedAuthoringDraftCount === 1 ? "" : "s"}` : "", retainedAuthoringMemoryOnlyRecoveryCount > 0 ? `${retainedAuthoringMemoryOnlyRecoveryCount} recovery snapshot${retainedAuthoringMemoryOnlyRecoveryCount === 1 ? "" : "s"} that ${retainedAuthoringMemoryOnlyRecoveryCount === 1 ? "exists" : "exist"} only in this tab` : ""].filter(Boolean);
	const retainedAuthoringDiscardStatement = retainedAuthoringDiscardItems.length > 0 ? `Leaving this run will discard ${retainedAuthoringDiscardItems.join(" and ")}.` : "No in-memory Authoring draft will be discarded.";
	const retainedPanelLeaveSummary = activePanelNavigationGuard?.unsafe && activePanelNavigationGuard.route === retainedPanelRoute && activePanelNavigationGuard.leaveSummary ? activePanelNavigationGuard.leaveSummary : retainedConfigDraftUnsafe ? "Leaving this run will discard an unsaved Run settings draft." : `${retainedAuthoringDiscardStatement}${retainedAuthoringDurableRecoveryCount > 0 ? ` ${retainedAuthoringDurableRecoveryCount} durable recovery record${retainedAuthoringDurableRecoveryCount === 1 ? "" : "s"} will remain protected in browser storage.` : ""}`;
	const retainedPanelLeaveMessage = `${retainedPanelLeaveSummary} Leave this run?`;
	const retainedPanelCloseMessage = activePanelNavigationGuard?.unsafe && activePanelNavigationGuard.route === retainedPanelRoute && activePanelNavigationGuard.closeMessage ? activePanelNavigationGuard.closeMessage : retainedConfigDraftUnsafe ? "This tab is retaining an unsaved Run settings draft. Close the panel and discard it?" : `${retainedAuthoringDiscardItems.length > 0 ? `Closing Authoring will discard ${retainedAuthoringDiscardItems.join(" and ")}.` : "No in-memory Authoring draft will be discarded."}${retainedAuthoringDurableRecoveryCount > 0 ? ` ${retainedAuthoringDurableRecoveryCount} durable recovery record${retainedAuthoringDurableRecoveryCount === 1 ? "" : "s"} will remain protected in browser storage.` : ""} Close Authoring?`;
	const retainedCommentLeaveMessage = [
		retainedCommentDrafts.length > 0 ? `${retainedCommentDrafts.length} unsaved comment draft${retainedCommentDrafts.length === 1 ? "" : "s"} will leave this in-memory workspace` : "",
		retainedCommentDurableCount > 0 ? `${retainedCommentDurableCount} exact comment recovery record${retainedCommentDurableCount === 1 ? "" : "s"} will remain protected in this browser tab` : "",
		retainedCommentEntries.length > retainedCommentDrafts.length ? `${retainedCommentEntries.length - retainedCommentDrafts.length} other active Comments state${retainedCommentEntries.length - retainedCommentDrafts.length === 1 ? "" : "s"} will leave the in-memory workspace` : "",
		retainedCommentRecoveryUnavailable ? "Comments recovery storage cannot be inspected, so an exact saved command may still be protected in this tab" : ""
	].filter(Boolean).join("; ");
	const retainedRunLeaveMessage = [retainedPanelDraftUnsafe ? retainedPanelLeaveSummary : "", retainedCommentWorkUnsafe ? retainedCommentLeaveMessage : ""].filter(Boolean).join(" ") + " Leave this run?";
	...
	const retainedNavigationTarget = (targetHash) => {
		...
		if (!sameRun) return {
			sameRun: false,
			panel: null,
			keepsMutablePanel: false
		};
		const queryIndex = targetHash.indexOf("?");
		const params = queryIndex < 0 ? new URLSearchParams() : new URLSearchParams(targetHash.slice(queryIndex + 1));
		...
		const keepsMutablePanel = targetPanels.length === 1 && targetGenerations.length === 1 && targetPanel === retainedPanelRoute && !params.has("seq") && targetGeneration === generation;
		return {
			sameRun: true,
			panel: targetPanel,
			keepsMutablePanel
	...
	useEffect(() => {
		...
		return installNavigationLossGuard({
			...
			message: (targetHash) => retainedNavigationTarget(targetHash).sameRun ? retainedPanelCloseMessage : retainedRunLeaveMessage,
		...
		retainedPanelCloseMessage,
		retainedCommentScopes.join("\0"),
		retainedPanelDraftUnsafe,
		retainedPanelRoute,
		retainedPanelScope,
		retainedGuardedHash,
		retainedGuardedHistoryState
	]);
	...
	const leaveRetainedPanelRoute = () => {
		if (!retainedPanelDraftUnsafe && !retainedCommentWorkUnsafe) {
			onBack?.();
			return;
		}
	...
	const [groupNavigation, setGroupNavigation] = useState(() => readRunGroupNavigation(runId));
	...
	const deepLinkLandingRef = useRef(route.hadState);
	...
	const autoReport = !historyActive && liveTerminalReady && !landedRef.current && !deepLinkLandingRef.current && !groupNavigation && !runRouteStateHasTarget(routeState, { reviewMode });
	...
	const onWorkspaceToolbarKeyDown = (event) => {
		const controls = [...workspaceToolbarRef.current?.querySelectorAll("[data-workspace-control]:not(:disabled)") || []];
	...
	const reportTimelineFocusPending = timelineActivation.runId === runId && timelineActivation.focusPending;
	...
	const timelineDeferred = view === "report" && panel !== "events" && !reportTimelineActivated && !live?.finished;
	...
	useEffect(() => setCompactGoalExpanded(false), [
		runId,
		generation,
		compactWorkspace
	]);
	...
	const startOverMutationBlocked = startOverRecovery.kind === "active" || startOverRecovery.kind === "corrupt";
	const mutationReadOnlyMode = readOnlyMode || runAuthorityBlocked || startOverMutationBlocked;
	const mutationReadOnlyReason = reviewMode ? "review" : startOverMutationBlocked ? "start-over" : "history";
	...
	const setPanel = (value, options = {}) => {
		...
		if (retainedPanelDraftUnsafe && routeState.panel === retainedPanelRoute && nextPanel !== retainedPanelRoute && !confirmRetainedPanelClose()) return routeState;
		return route.update((current) => ({
			...current,
			panel: nextPanel
		}), options);
	};
	const setView = (value, options = {}) => route.update((current) => ({
		...current,
		view: typeof value === "function" ? value(current.view) : value
	}), options);
	const changeViewSeq = (next) => {
		if (reviewMode || routeFenceBlocked) return routeState;
		const value = Number(next);
		const n = next == null || value >= seq ? null : Number.isSafeInteger(value) && value >= 0 ? value : null;
		return route.update((current) => ({
			...current,
			sequence: n
		}));
	...
	useLayoutEffect(() => {
		...
		setRunAccess(runId, {
			...
			seq: viewSeq,
			mode: reviewMode ? "review" : startOverMutationBlocked ? "start-over" : routeFenceBlocked ? "stale-link" : historyActive ? "history" : runStatus === "loading" ? "loading" : runAuthorityBlocked ? "unavailable" : "live"
		});
		...
		return () => {
			if (!startOverMutationBlocked) clearRunAccess(runId);
		};
	}, [
		runId,
		reviewMode,
		mutationReadOnlyMode,
		viewSeq,
		historyActive,
		routeFenceBlocked,
		runStatus,
	...
	const groupSurfaceReady = runStatus === "ready" && !!live && !routeFenceBlocked && (!historyActive || historyMatches(history, viewSeq, generation) && history.status === "ready");
	...
	const commitGroupNavigation = (next, { mode = "replace", focusExit = false } = {}) => {
		const normalized = next ? normalizeRunGroupNavigation(next, runId, generation) : null;
	...
	useEffect(() => {
		const hydrateGroupNavigation = () => {
			...
			const hasSnapshot = !!browserState && typeof browserState === "object" && !Array.isArray(browserState) && Object.prototype.hasOwnProperty.call(browserState, RUN_GROUP_HISTORY_KEY);
			const next = readRunGroupNavigation(runId, generation);
			if (hasSnapshot && !next) writeRunGroupNavigation(null, "replace");
			const previousBelongsHere = previous?.runId === String(runId) && (!generation || previous.generation === generation);
	...
	useLayoutEffect(() => {
		...
		setCollapsed((current) => current.has(groupNavigation.key) ? current : new Set(current).add(groupNavigation.key));
	...
	const selectedLiveGroupValid = useMemo(() => {
		...
		live,
		generation,
		groupNavigation?.generation,
		groupNavigation?.mode,
		groupNavigation?.key
	]);
	...
	useLayoutEffect(() => {
		...
		const decision = initialDagOverviewDecision({
			...
			explicitContext: historyActive || selectedId != null && !selectedLiveGroupValid || selectedLiveGroupValid && groupMode !== "operator"
		});
		...
		selectedId,
		selectedGroup,
		selectedLiveGroupValid,
		groupMode,
		historyActive
	]);
	useEffect(() => {
		if (panel && !panelAllowed(panel)) {
			const reason = historyActive ? `${panel} is not snapshot-safe and was not opened with historical data.` : startOverMutationBlocked ? `${panel} is unavailable while Start over is being resolved.` : `${panel} is not included in this review link.`;
	...
	const focusInspectorFromGroup = (nodeId) => {
		...
		setGroupInspectorFocusRequest({
			id: targetId,
			sequence: ++groupInspectorFocusSequenceRef.current
		});
	};
	...
	const closeCompactInspector = () => {
		...
		requestAnimationFrame(() => {
			const target = compactInspectorTriggerRef.current || [...document.querySelectorAll("[data-node-select-id]") || []].find((element) => element.dataset.nodeSelectId === String(selectedId));
	...
	useDialogFocus(compactInspectorRef, closeCompactInspector, compactWorkspace && compactInspectorOpen && !overlayPanelOpen, {
		modal: false,
		priority: DIALOG_PRIORITY.NONMODAL
	});
	useLayoutEffect(() => {
		...
		const frame = requestAnimationFrame(() => {
			...
			(surface.querySelector("[data-dialog-initial-focus]") || surface).focus({ preventScroll: true });
		});
	...
	useLayoutEffect(() => {
		if (!groupSurfaceReady || !groupInspectorFocusRequest || groupInspectorFocusAppliedRef.current === groupInspectorFocusRequest || selectedId !== groupInspectorFocusRequest.id || view !== "dag" || overlayPanelOpen) return;
		...
		const focus = () => {
			...
			const surface = compactInspectorRef.current;
			const target = surface?.querySelector("[role=\"tab\"][aria-selected=\"true\"]") || surface?.querySelector(".insp-body");
		...
		groupSurfaceReady,
		groupInspectorFocusRequest,
		selectedId,
		view,
		overlayPanelOpen
	]);
	useLayoutEffect(() => {
		if (!groupSurfaceReady || view !== "dag" || selectedId != null || !groupNavigation || groupSurfaceFocusAppliedRef.current === groupNavigation || overlayPanelOpen) return;
		...
		const focus = () => {
			...
			const member = navigation.returnNodeId == null ? null : [...surface?.querySelectorAll("[data-group-member-id]") || []].find((element) => element.dataset.groupMemberId === String(navigation.returnNodeId));
	...
	useLayoutEffect(() => {
		if (!groupSurfaceReady || groupNavigation || !groupExitFocus || groupExitFocusAppliedRef.current === groupExitFocus || selectedId != null || view !== "dag") return;
		...
		const focus = () => {
			...
			const target = [...document.querySelectorAll("[data-group-select-key]") || []].find((element) => element.dataset.groupSelectKey === returning.key);
			if (target) {
				if (target !== lastTarget || document.activeElement !== target) {
					if (target.dataset.restoredFocus !== "true") {
						target.dataset.restoredFocus = "true";
						const clearMarker = () => {
							if (target.isConnected) delete target.dataset.restoredFocus;
						};
	...
	useEffect(() => {
		...
		requestAnimationFrame(() => {
			...
			const selectedNode = [...document.querySelectorAll("[data-node-select-id]") || []].find((element) => element.dataset.nodeSelectId === String(selectedId));
			let target = null;
			if (focusOwner === "timeline-splitter" || focusOwner === "timeline-body" && timelineCollapsed) {
				...
				target = sideC ? sideRailRef.current : compactInspectorCloseRef.current || compactInspectorRef.current;
			} else if (!wasCompact && [
				"desktop-side-rail",
				"inspector-surface",
				"side-splitter"
	...
	useEffect(() => {
		...
		storageSet("ll.sideW", sideW);
		storageSet("ll.dockH", dockH);
		storageSet("ll.sideC", sideC ? "1" : "0");
		storageSet("ll.dockC", dockC ? "1" : "0");
	}, [
		sideW,
		dockH,
		sideC,
	...
	const clampDock = (value) => Math.max(MIN_DOCK_HEIGHT, Math.min(Math.max(MIN_DOCK_HEIGHT, window.innerHeight - 470), value));
	useEffect(() => {
		if (compactWorkspace) return;
		const clampPanes = () => {
			// A phone-sized resize must not overwrite the user's persisted desktop pane geometry.
			if (window.matchMedia?.("(max-width: 900px)").matches) return;
			setSideW((w) => clampSide(w));
			setDockH((h) => clampDock(h));
		};
		clampPanes();
	...
	const configAuthority = reviewMode ? `review:${reviewMeta?.id || runId}` : `owner:${runId}`;
	const configKey = runStatus === "ready" && !routeFenceBlocked ? `${configAuthority}:${generation || "pending"}` : null;
	...
	useEffect(() => {
		...
		let active = true;
		const request = deadlineGet(runApiPath(runId, "/config"), RUN_CONFIG_REQUEST_TIMEOUT_MS);
	...
	useEffect(() => {
		...
		const request = deadlineGet(runApiPath(runId, `/state?seq=${want}`), HISTORICAL_SNAPSHOT_REQUEST_TIMEOUT_MS);
		request.promise.then((p) => {
			if (alive) setHistory((current) => resolveHistory(current, want, generation, p));
		}).catch((error) => {
			if (!alive) return;
			const failure = error?.name === "TimeoutError" ? new Error("Historical snapshot loading timed out. Check the connection and retry.") : error;
	...
	const copyRetainedCommentWork = async () => {
		const lines = [...retainedCommentDrafts.map((text, index) => `Unsaved comment draft ${index + 1}:\n${text}`), ...retainedCommentRecovery.valid.map((intent, index) => {
		...
		if (!lines.length) {
			showToast("There is no readable comment text to copy.");
			return;
		}
		try {
	...
	const discardRetainedCommentWork = () => {
		...
		if (!total) {
			showToast(retainedCommentProtectedCreates.length > 0 ? "Current-generation new-comment recovery cannot be discarded without a terminal server outcome or intact exact recovery data." : "There is no releasable Comments work in this tab.");
			return;
		}
		if (!window.confirm(`Discard ${total} releasable Comments work item${total === 1 ? "" : "s"} from this browser tab? Current-generation append-only recovery stays protected because it is not safe to release.`)) return;
		let cleared = true;
		...
		showToast(cleared ? retainedCommentProtectedCreates.length > 0 ? "Releasable Comments work discarded. Exact new-comment recovery remains protected." : "Retained Comments work discarded. Review the thread before creating another command." : "Some Comments recovery changed and was not discarded. Refresh this run before continuing.");
	};
	const currentCommentRecovery = retainedCommentRecovery.valid.find((intent) => intent.expectedGeneration === generation);
	...
	const startOverIntent = startOverRecovery.kind === "active" ? startOverRecovery.intent : null;
	const startOverRequestPending = !!startOverRequestRef.current;
	const startOverHandoff = !!(startOverIntent?.replacementGeneration && generation && generation === startOverIntent.replacementGeneration);
	...
	const startOverReplacementSuperseded = !!(startOverIntent?.replacementGeneration && generation && generation !== startOverIntent.replacementGeneration && generation !== startOverIntent.expectedGeneration);
	const persistStartOverPhase = (intent, phase, replacementGeneration = intent.replacementGeneration) => {
		const next = createRunStartOverIntent(intent.runId, intent.expectedGeneration, intent.operationId, phase, Date.now(), replacementGeneration);
		if (!next) return null;
		const stored = saveRunStartOverIntent(next, undefined, intent.operationId);
		if (!stored) {
			const restored = loadRunStartOverIntent(intent.runId);
			if (restored.kind === "active" && restored.intent.operationId !== intent.operationId) {
	...
	const executeStartOver = async (intent, { initialRequest = false } = {}) => {
		...
		try {
			const result = await resetRun(intent.runId, intent.expectedGeneration, intent.operationId, { signal: controller.signal });
			if (startOverRequestRef.current !== request) return;
			const replacementGeneration = String(result?.generation || "");
			if (!result || result.ok !== true || result.operation_id !== intent.operationId || result.expected_generation !== intent.expectedGeneration || !/^[0-9a-f]{64}$/.test(replacementGeneration) || replacementGeneration === intent.expectedGeneration || intent.replacementGeneration && replacementGeneration !== intent.replacementGeneration) {
			...
			const foreignResetConflict = error?.code === "reset_operation_conflict" && String(error?.detail?.operation_id || "") && String(error.detail.operation_id) !== intent.operationId;
			const pendingReceipt = status === 425 && error?.code === "reset_pending" && error?.detail?.operation_id === intent.operationId && error?.detail?.expected_generation === intent.expectedGeneration && ["pending", "accepted"].includes(error?.detail?.status);
			if (pendingReceipt) {
			...
			const authoritativeRejection = initialRequest && ([
				...
				422,
				428
			].includes(status) || generationChanged || foreignResetConflict || error?.code === "replay_task_invalid" || error?.code === "replay_config_invalid" || error?.code === "replay_config_unavailable");
			if (authoritativeRejection && clearStartOverRecovery(intent)) {
				showToast(foreignResetConflict ? "Another Start over operation already owns this run. This request was not submitted." : error?.message || "Start over was rejected; the run was not changed.");
				retryRunRef.current();
				return;
			}
			persistStartOverPhase(intent, "unknown");
			showToast(generationChanged ? "The run changed, but this operation is not yet verified. Retry the exact request." : "Start-over outcome is not confirmed. Retry this exact request before doing anything else.");
			retryRunRef.current();
		} finally {
	...
	const submitStartOver = (confirmedGeneration) => {
		...
		const currentCommentMemoryUnsafe = inspectorDraftStoreRef.current.entries().some((entry) => commentDraftEntryUnsafe(entry, String(runId)));
		const currentCommentRecovery = listCommentOperationRecoveries(String(runId));
		const currentCommentWorkUnsafe = currentCommentMemoryUnsafe || !currentCommentRecovery.available || currentCommentRecovery.valid.length > 0 || currentCommentRecovery.damaged.length > 0;
		...
		const currentConfigDraft = inspectorDraftStoreRef.current.readField(`panel:config:${String(runId)}`, "draft", null);
		const currentAuthoringDocuments = inspectorDraftStoreRef.current.readField("panel:authoring", "documents", {});
		...
		const currentPanelWorkUnsafe = currentPanelController?.unsafe === true || currentConfigDraft?.schema === "looplab.config-draft/v1" && currentConfigDraft.unsafe === true || currentAuthoringDocuments && typeof currentAuthoringDocuments === "object" && !Array.isArray(currentAuthoringDocuments) && Object.values(currentAuthoringDocuments).some((document) => document && typeof document === "object" && !Array.isArray(document) && (document.draftText !== document.savedText || document.recoveryOperationId || document.recoveryStorageRaw)) || currentAuthoringRecoveryUnsafe;
		...
		if (reviewMode || startOverRecovery.kind !== "none" || confirmedGeneration !== generation || !/^[0-9a-f]{64}$/.test(confirmedGeneration || "")) {
			showToast("Start over was not submitted because the run changed before confirmation.");
			return;
		}
		const intent = createRunStartOverIntent(runId, confirmedGeneration, createIdempotencyKey().toLowerCase());
	...
	const finishStartOverHandoff = (intent, { superseded = false } = {}) => {
		...
		if (!route.openCurrentGeneration({ mode: "replace" })) {
			...
			showToast(superseded ? "Start over is verified, but the current run address could not be opened. Retry." : "The new run is ready, but its address could not be updated. Retry opening it.");
			return false;
		}
		setStartOverRouteSyncFailed(false);
		...
		if (clearStartOverRecovery(intent)) {
			showToast(superseded ? "Start over completed, and the run changed again. The current generation is open." : "Previous run generation archived. The new run is open.");
		} else {
			showToast("The current run is open, but saved recovery evidence could not be cleared safely.");
		}
		requestAnimationFrame(() => {
			;
			(routeMainRef.current || document.querySelector("[data-route-main]"))?.focus?.({ preventScroll: true });
		});
		return true;
	};
	...
	const retryStartOverStorage = () => {
		const restored = loadRunStartOverIntent(runId);
		setStartOverRecovery(restored);
		showToast(restored.kind === "unavailable" ? "Recovery storage is still unavailable in this tab." : "Recovery storage is available again.");
	...
	useLayoutEffect(() => {
		if (!startOverIntent?.replacementGeneration || !generation || generation !== startOverIntent.replacementGeneration) return;
		...
		runId,
		generation,
		startOverIntent?.expectedGeneration,
		startOverIntent?.operationId,
		startOverIntent?.replacementGeneration,
		startOverIntent?.phase,
		startOverRouteSyncAttempt
	]);
	useEffect(() => {
		...
		tracker.count += 1;
		const timer = setTimeout(() => executeStartOver(startOverIntent), 900 + tracker.count * 450);
		return () => clearTimeout(timer);
	}, [
		startOverIntent?.operationId,
		startOverIntent?.phase,
		startOverIntent?.updatedAt
	]);
	useEffect(() => {
		if (!startOverIntent || startOverIntent.phase !== "accepted" || generation && generation !== startOverIntent.expectedGeneration) return;
		...
		const timer = runStatus === "loading" ? setTimeout(() => {
			showToast("Start over is verified, but the replacement run is taking longer than expected to open.");
		}, Math.max(1e3, 45e3 - age)) : setTimeout(() => retryRunRef.current(), 900);
		return () => clearTimeout(timer);
	}, [
		runStatus,
		generation,
	...
	useEffect(() => {
		...
		if (!members) {
			...
			setRouteNotice((current) => [current, `Saved group “${staleKey}” is no longer available in this run state; opened the graph.`].filter(Boolean).join(" "));
			if (selectedId == null) {
				setCompactInspectorOpen(false);
				requestAnimationFrame(() => routeMainRef.current?.focus({ preventScroll: true }));
			}
		...
		if (groupNavigation.returnNodeId != null && !members.some((id) => Number(id) === groupNavigation.returnNodeId)) {
	...
	const groupMembers = useMemo(() => {
		...
		return selectedGroup != null && ns ? computeGroups(ns, groupMode, groupState).get(selectedGroup) || [] : [];
	}, [
		groupState,
		groupMode,
		selectedGroup
	...
	const selectNodeFromPanel = (id) => {
		...
		route.update((current) => ({
			...routeWithSelectedNode(current, id),
			view: "dag",
			panel: null
		}), { mode: "replace" });
		revealSelectedNode(id);
	...
	const mergeSubmittingRef = useRef(false);
	...
	const closeMergeChooser = (restoreFocus = true) => {
		...
		requestAnimationFrame(() => {
			const fallback = document.querySelector(`[data-node-action-id="${source}"]`) || document.querySelector(`[data-node-select-id="${source}"]`);
	...
	const openMergeChooser = (source, target = null, returnFocus = null) => {
		...
		if (!Number.isSafeInteger(sourceId) || targetId != null && (!Number.isSafeInteger(targetId) || targetId === sourceId)) {
		...
		showToast(targetId == null ? `Choose a destination for #${sourceId}, then confirm the merge` : `Review merge #${sourceId} + #${targetId}, then confirm`);
		requestAnimationFrame(() => {
	...
	useEffect(() => {
		...
		setRouteNotice((current) => [current, `${inspectTab} is unavailable for this node or access level; opened Overview instead.`].filter(Boolean).join(" "));
		setInspectTab(effectiveInspectTab, {
			mode: "replace",
			preserveIssues: true
		});
	}, [inspectTab, effectiveInspectTab]);
	useEffect(() => {
		if (startOverHandoff || !live2 || selectedId == null || historyActive && hist == null) return;
		if (nodeIsActive(live2.nodes?.[selectedId], live2)) return;
		setRouteNotice((current) => [current, `Experiment #${selectedId} is not available in this run state.`].filter(Boolean).join(" "));
		route.update((current) => ({
			...current,
			nodeId: null,
			nodeGeneration: null,
	...
	const exactNodeAttemptMismatch = nodeAttemptEvidenceReady && routeState.nodeGeneration != null && live2?.nodes?.[selectedId] != null && routeState.nodeGeneration !== currentSelectedAttempt;
	const commentAttemptMatches = !routeState.commentId || Number.isSafeInteger(routeState.nodeGeneration) && routeState.nodeGeneration === currentSelectedAttempt;
	useLayoutEffect(() => {
		...
		setAttemptFenceNotice(historyActive ? routeState.commentId ? `The linked comment targets attempt ${routeState.nodeGeneration}, but this historical snapshot contains attempt ${currentSelectedAttempt ?? "unavailable"} for experiment #${selectedId}. The requested attempt was not opened.` : `The link targets attempt ${routeState.nodeGeneration}, but this historical snapshot contains attempt ${currentSelectedAttempt ?? "unavailable"} for experiment #${selectedId}. The requested attempt was not opened.` : routeState.commentId ? `The linked comment belongs to attempt ${routeState.nodeGeneration}, but experiment #${selectedId} is now attempt ${currentSelectedAttempt ?? "unavailable"}. The newer attempt was not opened.` : `The link targets attempt ${routeState.nodeGeneration}, but experiment #${selectedId} is now attempt ${currentSelectedAttempt ?? "unavailable"}. The newer attempt was not opened.`);
		route.update((current) => ({
			...current,
			nodeId: null,
			nodeGeneration: null,
		...
		exactNodeAttemptMismatch,
		routeState.nodeGeneration,
		routeState.commentId,
		selectedId,
		currentSelectedAttempt,
		historyActive
	]);
	useLayoutEffect(() => {
	...
	const pendingDescendants = (rootId) => {
	...
	const onNodeAction = async (action, arg, context = null) => {
		if (mutationReadOnlyMode) {
			showToast(startOverMutationBlocked ? "Start over must be resolved before changing this run." : reviewMode ? "This review link is read-only" : `Historical snapshot seq ${viewSeq} is read-only`);
			return;
		}
		if (action === "merge" && mergeSubmittingRef.current) {
			showToast("A merge is already being submitted");
			return;
		}
		try {
			if (action === "merge" && arg && typeof arg === "object") {
			...
			if (action === "explore") {
				const f = checkedCommand(await CONTROL.fork(runId, id, generation), {
					success: `Fork from #${id} applied — the engine is processing it`,
					noop: `Branch from #${id} already exists`,
					executing: `Fork from #${id} requested — waiting for the engine`,
					failure: "Fork failed"
				});
				showToast(f.message);
			} else if (action === "ablate") {
				const f = checkedCommand(await CONTROL.forceAblate(runId, id, generation), {
					success: `Ablation of #${id} applied — the engine is processing it`,
					noop: `Ablation of #${id} was already satisfied`,
					executing: `Ablation of #${id} requested — waiting for the engine`,
					failure: "Ablation failed"
				});
				showToast(f.message);
				...
				setComparePair([live2?.best_node_id ?? id, id]);
				setPanel("compare");
				...
				if (!ids.length) {
					showToast(`#${id}: nothing pending to kill (already evaluated)`);
					return;
				}
				let waiting = 0;
				for (const k of ids) {
					const f = checkedCommand(await CONTROL.nodeAbort(runId, k, live2?.nodes?.[k]?.attempt), {
						success: `Cancelled #${k}`,
						noop: `#${k} was already settled`,
						executing: `Cancellation of #${k} requested`,
						failure: `Could not cancel #${k}`
					});
					if (f.kind === "pending") waiting++;
				}
				showToast(waiting ? `Cancellation requested for ${ids.length} experiment(s); ${waiting} still pending` : `Cancelled ${ids.length} pending experiment(s) under #${id}`);
			}
		} catch (e) {
			showToast(e.message || "Run command failed");
		}
	};
	...
	const onCanvasSelect = (id) => {
		if (mergeFrom != null && mergeSubmittingRef.current) {
			showToast("A merge is already being submitted");
			return;
		}
		if (mutationReadOnlyMode && mergeFrom != null) {
			closeMergeChooser(false);
			return;
		}
	...
	const focusNode = (id, tab, eventSeq, { preserveGroup = false, nodeGeneration = null, preserveExactSequence = false } = {}) => {
		const value = Number(eventSeq);
		const validEventSeq = Number.isSafeInteger(value) && value >= 0 ? value : null;
		const targetSeq = eventSeq == null ? null : preserveExactSequence ? validEventSeq : validEventSeq != null && validEventSeq < seq ? validEventSeq : null;
		const targetNodeGeneration = Number.isSafeInteger(nodeGeneration) && nodeGeneration >= 0 ? nodeGeneration : null;
		route.update((current) => {
	...
	const selectGroup = (key) => {
		...
		commitGroupNavigation(null, {
			mode: selectedId != null ? "replace" : "push",
			focusExit: true
		});
		setCompactInspectorOpen(false);
	};
	...
	const mergeCandidates = useMemo(() => Object.values(live2?.nodes || {}).filter((node) => node.id !== mergeFrom).sort((left, right) => left.id - right.id), [live2, mergeFrom]);
	const submitMergeTarget = async (event) => {
		...
		try {
			const feedback = checkedCommand(await CONTROL.merge(command.runId, command.ids, command.parentGenerations, { expectedGeneration: command.expectedGeneration }), {
				success: `Merge #${source} + #${target} applied — the engine is processing it`,
				noop: "That merge was already satisfied",
				executing: `Merge #${source} + #${target} requested — waiting for the engine`,
	...
	const routeFocusPhase = !live ? `resource:${runStatus}` : routeFenceBlocked ? `fence:${generationMismatch ? "mismatch" : "pending"}` : historyActive && !hist ? `history:${history.status}` : "ready";
	const workspaceRouteLabel = `${live?.label || live?.run_id || runId} run workspace · ${view === "concepts" ? "Concepts" : view === "report" ? "Report" : "Search"}`;
	useEffect(() => {
		...
		const frame = requestAnimationFrame(() => {
			if (!document.querySelector("[aria-modal=\"true\"], [data-route-focus-guard=\"true\"]")) {
				;
				(panelTarget && document.contains(panelTarget) ? panelTarget : routeMainRef.current || document.querySelector("[data-route-main]"))?.focus({ preventScroll: true });
			}
		});
		return () => cancelAnimationFrame(frame);
	}, [
	...
	if (!live && startOverRecovery.kind !== "none" && !startOverIntent) return /* @__PURE__ */ _jsxs("div", {
		className: "app",
		children: [
			/* @__PURE__ */ _jsxs("div", {
				className: "topbar run-head",
				children: [/* @__PURE__ */ _jsxs("span", {
					...
					onClick: leaveRetainedPanelRoute,
					children: "← runs"
				})]
			}),
			/* @__PURE__ */ _jsxs("main", {
				ref: startOverNoticeRef,
				...
				children: [
					/* @__PURE__ */ _jsx("div", {
						className: "resource-state-icon",
						"aria-hidden": "true",
						children: "!"
					}),
					/* @__PURE__ */ _jsx("h1", {
						id: "run-state",
						children: startOverRecovery.kind === "unavailable" ? "Start over recovery storage is unavailable" : "Saved Start over recovery is invalid"
					}),
					/* @__PURE__ */ _jsx("p", { children: startOverRecovery.kind === "unavailable" ? "This tab cannot safely preserve a Start over request. No request can be submitted until recovery storage works again." : "LoopLab cannot identify an earlier Start over request safely. Run changes remain locked until its saved recovery evidence is resolved." }),
					/* @__PURE__ */ _jsxs("div", {
						className: "resource-state-actions",
						children: [startOverRecovery.kind === "unavailable" && /* @__PURE__ */ _jsx("button", {
							type: "button",
							className: "btn primary",
							onClick: retryStartOverStorage,
							children: "Try storage again"
						}), onBack && /* @__PURE__ */ _jsx("button", {
							type: "button",
							className: "btn",
							onClick: leaveRetainedPanelRoute,
	...
	if (!live && startOverIntent) return /* @__PURE__ */ _jsxs("div", {
		className: "app",
		children: [
			/* @__PURE__ */ _jsxs("div", {
				className: "topbar run-head",
			...
			/* @__PURE__ */ _jsxs("main", {
				...
				children: [
					runStatus === "loading" && /* @__PURE__ */ _jsx("div", {
						className: "history-spinner",
						"aria-hidden": "true"
					}),
					/* @__PURE__ */ _jsx("h1", {
						id: "run-state",
						children: startOverRequestPending ? "Submitting the same Start over request…" : runStatus === "not_found" ? "Run files are switching…" : runStatus === "error" ? "Start over status is unavailable" : startOverIntent.phase === "unknown" ? "Start over outcome is not confirmed" : startOverIntent.phase === "pending" ? "Checking the saved Start over request…" : "Starting this run over…"
					}),
					/* @__PURE__ */ _jsx("p", { children: runStatus === "error" ? `${runError || "The server could not read this run."} Recovery remains locked so another operation cannot overlap it.` : runStatus === "not_found" ? "The previous event log may already be archived while the replacement generation is starting. The exact request identity is preserved." : startOverIntent.phase === "unknown" ? "The result is not confirmed. Retry the exact saved request; LoopLab will not create a second Start over operation." : startOverIntent.phase === "pending" ? "The server preserved this exact request. LoopLab is checking the same operation automatically; no duplicate will be created." : startOverIntent.phase === "submitting" ? "The exact request is being submitted. No archive is assumed until the server returns its matching operation receipt." : "The server accepted this exact operation. Waiting for the replacement run to become readable." }),
					/* @__PURE__ */ _jsx("div", {
						className: "resource-state-actions",
						children: /* @__PURE__ */ _jsx("button", {
							type: "button",
							className: "btn primary",
							onClick: retryStartOver,
							disabled: startOverRequestPending,
							children: startOverRequestPending ? "Waiting for response…" : "Retry exact request"
						})
					})
				]
			}),
	...
	if (!live) return /* @__PURE__ */ _jsxs("div", {
		className: "app" + (reviewMode ? " review-mode" : ""),
		children: [/* @__PURE__ */ _jsxs("div", {
			...
			"aria-live": "polite",
			"aria-labelledby": "run-state",
			children: runStatus === "not_found" ? /* @__PURE__ */ _jsxs(_Fragment, { children: [
				/* @__PURE__ */ _jsx("div", {
					className: "resource-state-icon",
					"aria-hidden": "true",
					children: "404"
				...
				/* @__PURE__ */ _jsxs("p", { children: [/* @__PURE__ */ _jsx("code", { children: runId }), " does not exist or may have been removed."] }),
				/* @__PURE__ */ _jsxs("div", {
					className: "resource-state-actions",
					children: [onBack && /* @__PURE__ */ _jsx("button", {
						className: "btn primary",
						onClick: leaveRetainedPanelRoute,
						children: "Back to runs"
			...
			] }) : runStatus === "gone" ? /* @__PURE__ */ _jsxs(_Fragment, { children: [
				/* @__PURE__ */ _jsx("div", {
					className: "resource-state-icon",
					"aria-hidden": "true",
					children: "×"
				...
				/* @__PURE__ */ _jsx("h1", {
					id: "run-state",
					children: "Review access ended"
				}),
				/* @__PURE__ */ _jsx("p", { children: runError || "This review link expired or was revoked." })
			] }) : runStatus === "error" ? /* @__PURE__ */ _jsxs(_Fragment, { children: [
				/* @__PURE__ */ _jsx("div", {
					className: "resource-state-icon",
					"aria-hidden": "true",
					children: "!"
				...
				/* @__PURE__ */ _jsx("h1", {
					id: "run-state",
					children: "Could not load run"
				}),
				/* @__PURE__ */ _jsx("p", { children: runError || "Check that the LoopLab server is reachable." }),
				/* @__PURE__ */ _jsxs("div", {
					className: "resource-state-actions",
					children: [/* @__PURE__ */ _jsx("button", {
						className: "btn primary",
				...
				/* @__PURE__ */ _jsx("h1", {
					id: "run-state",
					children: "Opening run…"
				}),
				/* @__PURE__ */ _jsx("p", { children: "Loading the latest search state." })
			] })
		})]
	});
	if (routeFenceBlocked && startOverIntent) return /* @__PURE__ */ _jsxs("div", {
		className: "app",
		children: [
			/* @__PURE__ */ _jsxs("div", {
				className: "topbar run-head",
			...
			/* @__PURE__ */ _jsxs("main", {
				ref: startOverNoticeRef,
				className: "run-resource-state stale-route-state",
				"data-route-main": true,
				tabIndex: -1,
				"aria-labelledby": "run-state",
				role: "alert",
				children: [
					...
					/* @__PURE__ */ _jsx("h1", {
						id: "run-state",
						children: startOverReplacementSuperseded ? "Start over completed, and the run changed again" : startOverHandoff ? "Opening the verified replacement run…" : "The run changed while Start over is unresolved"
					}),
					/* @__PURE__ */ _jsx("p", { children: startOverReplacementSuperseded ? "The exact saved operation produced its verified replacement generation. A newer generation is now visible, so this resolved recovery record can be cleared before opening the current run." : startOverHandoff ? startOverRouteSyncFailed ? "The matching operation receipt is verified, but this page could not update its address. Changes remain locked." : "The matching operation receipt is verified. Updating this diagnostic address to the new generation." : "A different generation is visible, but that alone does not prove which operation created it. Retry the exact saved request to read its receipt." }),
					/* @__PURE__ */ _jsxs("div", {
						className: "resource-state-actions",
						children: [/* @__PURE__ */ _jsx("button", {
							type: "button",
							...
							onClick: retryStartOver,
							disabled: startOverRequestPending,
							children: startOverRequestPending ? "Waiting for response…" : startOverReplacementSuperseded ? "Open current run" : startOverHandoff ? "Retry opening new run" : "Retry exact request"
						}), onBack && /* @__PURE__ */ _jsx("button", {
							type: "button",
							className: "btn",
							onClick: leaveRetainedPanelRoute,
	...
	if (routeFenceBlocked) return /* @__PURE__ */ _jsxs("div", {
		className: "app" + (reviewMode ? " review-mode" : ""),
		children: [/* @__PURE__ */ _jsxs("div", {
			className: "topbar run-head",
			children: [/* @__PURE__ */ _jsxs("span", {
			...
			children: [
				/* @__PURE__ */ _jsx("div", {
					className: "resource-state-icon",
					"aria-hidden": "true",
					children: generationMismatch ? "↺" : "…"
				}),
				/* @__PURE__ */ _jsx("h1", {
					id: "run-state",
					children: generationMismatch ? "This diagnostic link targets an earlier run generation" : "Verifying diagnostic link…"
				}),
				generationMismatch ? /* @__PURE__ */ _jsxs(_Fragment, { children: [
					/* @__PURE__ */ _jsx("p", { children: "The run was reset or replaced after this link was created. Node ids and sequence numbers may now mean something else, so LoopLab will not reinterpret the link." }),
					retainedConfigDraftUnsafe && /* @__PURE__ */ _jsx("p", {
						className: "notice",
						role: "status",
						children: retainedConfigStoredDraftUnsafe ? /* @__PURE__ */ _jsxs(_Fragment, { children: [/* @__PURE__ */ _jsx("b", { children: "Your unsaved Run settings draft is retained in this tab." }), " Nothing was sent automatically. Open the current generation to load its authoritative settings and review the retained fields."] }) : /* @__PURE__ */ _jsxs(_Fragment, { children: [
							/* @__PURE__ */ _jsx("b", { children: "A Run settings operation was interrupted by this generation change." }),
							" ",
							"Its server-side outcome may need verification; nothing will be replayed automatically. Open the current generation to inspect the authoritative state."
						] })
					}),
					retainedAuthoringDraftUnsafe && /* @__PURE__ */ _jsxs("p", {
						className: "notice",
						role: "status",
						children: [
							retainedAuthoringDraftCount > 0 && /* @__PURE__ */ _jsxs(_Fragment, { children: [
								/* @__PURE__ */ _jsxs("b", { children: [
									retainedAuthoringDraftCount,
									" unsaved in-memory Authoring draft",
									retainedAuthoringDraftCount === 1 ? " is" : "s are",
							...
							retainedAuthoringDurableRecoveryCount > 0 && /* @__PURE__ */ _jsxs(_Fragment, { children: [
								/* @__PURE__ */ _jsxs("b", { children: [
									retainedAuthoringDurableRecoveryCount,
									" durable Authoring recovery record",
									retainedAuthoringDurableRecoveryCount === 1 ? " remains" : "s remain",
							...
							retainedAuthoringMemoryOnlyRecoveryCount > 0 && /* @__PURE__ */ _jsxs(_Fragment, { children: [
								/* @__PURE__ */ _jsxs("b", { children: [
									retainedAuthoringMemoryOnlyRecoveryCount,
									" exact recovery snapshot",
									retainedAuthoringMemoryOnlyRecoveryCount === 1 ? " exists" : "s exist",
									" only in this tab."
								] }),
								" ",
					...
					retainedCommentWorkUnsafe && /* @__PURE__ */ _jsxs("div", {
						className: "notice",
						role: retainedCommentRecoveryUnavailable || retainedCommentRecovery.damaged.length > 0 ? "alert" : "status",
						children: [
							/* @__PURE__ */ _jsx("b", { children: "Comments work is retained in this tab." }),
							" ",
							retainedCommentDrafts.length > 0 ? `${retainedCommentDrafts.length} unsaved draft${retainedCommentDrafts.length === 1 ? "" : "s"} remain in memory. ` : "",
							retainedCommentRecovery.valid.length > 0 ? `${retainedCommentRecovery.valid.length} exact command recover${retainedCommentRecovery.valid.length === 1 ? "y is" : "ies are"} protected in browser storage. ` : "",
							retainedCommentRecovery.damaged.length > 0 ? `${retainedCommentRecovery.damaged.length} damaged recovery record${retainedCommentRecovery.damaged.length === 1 ? " needs" : "s need"} review. ` : "",
							retainedCommentRecoveryUnavailable ? "Recovery storage cannot be inspected, so Start over remains blocked until it is available again. " : "",
							"Nothing will be replayed automatically or rebound to the replacement generation.",
							(retainedCommentDrafts.length > 0 || retainedCommentRecovery.valid.length > 0) && /* @__PURE__ */ _jsxs("details", { children: [
								/* @__PURE__ */ _jsx("summary", { children: "View retained Comments work" }),
								retainedCommentDrafts.map((text, index) => /* @__PURE__ */ _jsx("pre", {
									className: "comment-recovery-payload",
									children: text
								}, `draft:${index}`)),
								retainedCommentRecovery.valid.map((intent) => /* @__PURE__ */ _jsxs("div", {
									className: "comment-recovery-payload",
									children: [
										/* @__PURE__ */ _jsx("b", { children: intent.kind === "create" ? "New comment" : intent.kind === "edit" ? "Comment edit" : intent.resolved ? "Resolve comment" : "Reopen comment" }),
										" ",
										"· experiment #",
										intent.nodeId,
										" · attempt ",
										intent.nodeGeneration,
										intent.text != null && /* @__PURE__ */ _jsx("pre", { children: intent.text })
									]
								}, intent.storageKey))
							] }),
							/* @__PURE__ */ _jsxs("div", {
								className: "resource-state-actions",
								children: [
									(retainedCommentDrafts.length > 0 || retainedCommentRecovery.valid.length > 0) && /* @__PURE__ */ _jsx("button", {
										type: "button",
										className: "btn",
										onClick: copyRetainedCommentWork,
										children: "Copy Comments work"
									}),
									retainedCommentRecoveryUnavailable && /* @__PURE__ */ _jsx("button", {
										type: "button",
										className: "btn",
										onClick: refreshCommentOperationRecoveries,
										children: "Retry recovery storage"
									}),
									retainedCommentReleasableCount > 0 && /* @__PURE__ */ _jsx("button", {
										type: "button",
										className: "btn danger",
										onClick: discardRetainedCommentWork,
										children: retainedCommentProtectedCreates.length > 0 ? "Discard other work" : "Discard Comments work"
									})
								]
							})
						]
					...
					/* @__PURE__ */ _jsxs("div", {
						className: "route-generation-detail",
						children: [
							/* @__PURE__ */ _jsxs("code", { children: ["link ", routeState.generation?.slice(0, 12)] }),
							/* @__PURE__ */ _jsx("span", { children: "≠" }),
							/* @__PURE__ */ _jsxs("code", { children: ["current ", generation?.slice(0, 12)] })
						]
					}),
					/* @__PURE__ */ _jsxs("div", {
						className: "resource-state-actions",
						children: [/* @__PURE__ */ _jsx("button", {
							...
							onClick: () => {
								...
								route.openCurrentGeneration(retainedPanelRoute ? {
									mode: "replace",
									panel: retainedPanelRoute
								} : undefined);
							},
							children: retainedConfigDraftUnsafe ? retainedConfigStoredDraftUnsafe ? "Open current generation with settings draft" : "Open current generation after settings operation" : retainedAuthoringDraftUnsafe ? "Open current generation with Authoring work" : retainedCommentWorkUnsafe ? "Open current generation with Comments recovery" : "Open current generation"
						}), onBack && /* @__PURE__ */ _jsx("button", {
							className: "btn",
							onClick: leaveRetainedPanelRoute,
							children: "Back to runs"
				...
				] }) : /* @__PURE__ */ _jsx("p", { children: "Confirming that this node and sequence still belong to the run generation named by the link." })
			]
		})]
	});
	...
	const finalizing = lifecycle.mode === "finalizing" || lifecycle.mode === "finalization-stalled" || lifecycle.mode === "finishing";
	const liveStatus = !connected ? "off" : finalizing ? "finalizing" : lifecycle.mode === "finished" ? "done" : lifecycle.mode === "stalled" ? "stalled" : lifecycle.mode === "unknown" ? "off" : lifecycle.mode === "paused" || lifecycle.mode === "approval" ? "paused" : "on";
	const liveLabel = !connected ? "offline" : lifecycle.mode === "finalization-stalled" ? "finalization stalled" : lifecycle.mode === "finishing" ? "finishing" : lifecycle.mode === "finalizing" ? "finalizing" : lifecycle.mode === "finished" ? "finished" : lifecycle.mode === "stalled" ? "stalled" : lifecycle.mode === "unknown" ? "engine ownership unknown" : lifecycle.mode === "paused" ? "paused" : lifecycle.mode === "approval" ? "approval needed" : "live";
	if (historyActive && !hist) return /* @__PURE__ */ _jsxs("div", {
		className: "app",
		children: [
			/* @__PURE__ */ _jsxs("div", {
				className: "topbar run-head",
				children: [
					...
					/* @__PURE__ */ _jsxs("span", {
						className: "live " + liveStatus,
						children: [
							/* @__PURE__ */ _jsx("span", { className: "led" }),
							"current run: ",
							liveLabel
						]
					})
			...
			/* @__PURE__ */ _jsxs("div", {
				...
				children: [
					...
					/* @__PURE__ */ _jsx("span", { children: "read-only" }),
					/* @__PURE__ */ _jsx("button", {
						className: "btn sm primary",
						onClick: returnToLiveAndFocusWorkspace,
						children: "Return to live"
					})
				]
			}),
			/* @__PURE__ */ _jsx("main", {
				className: "history-resource",
				"data-route-main": true,
				tabIndex: -1,
				"aria-labelledby": "run-state",
				children: currentHistory?.status === "error" ? /* @__PURE__ */ _jsxs(_Fragment, { children: [
					/* @__PURE__ */ _jsx("h1", {
						id: "run-state",
						children: "Snapshot unavailable"
					}),
					/* @__PURE__ */ _jsx("p", { children: currentHistory.error }),
					/* @__PURE__ */ _jsx("button", {
						className: "btn",
					...
					/* @__PURE__ */ _jsxs("h1", {
						id: "run-state",
						children: [
							"Loading snapshot seq ",
							viewSeq,
							"…"
						]
					}),
					/* @__PURE__ */ _jsx("p", { children: "The live workspace is hidden until this exact historical state resolves." })
				] })
			})
		]
	});
	...
	const configNoticeStatus = activeConfigResource?.status === "loading" && activeConfigResource.retrying ? "retrying" : ["error", "stale"].includes(activeConfigResource?.status) ? activeConfigResource.status : null;
	...
	const emptyPresentation = dagEmptyPresentation({
		...
		historyActive,
		reviewMode,
		sequence: history.resolvedSeq ?? viewSeq
	});
	...
	const onEmptyAction = (action) => {
		...
		if (action === "assistant") {
			if (!approvalCommand) {
				revealEvents();
				showToast("Approval target is missing; inspect the timeline before acting.");
				return;
			}
		...
		if (!exactController || exactController.runId !== runId || exactController.expectedGeneration !== generation) {
			showToast("Run controls are refreshing for the displayed generation. Try again in a moment.");
			return;
		}
		if (exactController.failure) {
			revealEvents();
			return;
		}
	...
	return /* @__PURE__ */ _jsxs("main", {
		...
		"data-route-main": true,
		tabIndex: -1,
		"aria-label": workspaceRouteLabel,
		children: [
			/* @__PURE__ */ _jsx("h1", {
				className: "sr-only",
				children: workspaceRouteLabel
			}),
			/* @__PURE__ */ _jsxs("div", {
				className: "topbar run-head",
				children: [
					...
					/* @__PURE__ */ _jsxs("button", {
						type: "button",
						className: "btn sm ghost copy-view-btn",
						onClick: copyViewLink,
						"aria-label": reviewMode ? "Copy read-only review context" : "Copy shareable run context",
						title: reviewMode ? "Copy this read-only capability and route context; local visual filters are not included" : "Copy the run route, selected evidence and snapshot; local graph filters are not included and recipients still need owner access",
						children: [
							...
							" ",
							/* @__PURE__ */ _jsx("span", {
								className: "copy-view-label",
								children: "Copy context"
							})
						]
					}),
					/* @__PURE__ */ _jsx(EnergyToggle, {}),
					/* @__PURE__ */ _jsxs("div", {
						ref: workspaceToolbarRef,
						className: "view-toggle",
						role: "toolbar",
						"aria-label": "Run workspace controls",
						"aria-orientation": "horizontal",
						onKeyDown: onWorkspaceToolbarKeyDown,
						onFocus: onWorkspaceToolbarFocus,
						children: [
							/* @__PURE__ */ _jsx("button", {
								type: "button",
								"data-workspace-control": "dag",
								tabIndex: workspaceTabStop === "dag" ? 0 : -1,
								"aria-pressed": view === "dag",
								className: "vt" + (view === "dag" ? " on" : ""),
								onClick: () => setView("dag"),
								children: "Search"
							}),
							/* @__PURE__ */ _jsx("button", {
								type: "button",
								"data-workspace-control": "concepts",
								tabIndex: workspaceTabStop === "concepts" ? 0 : -1,
								"aria-pressed": view === "concepts",
								className: "vt" + (view === "concepts" ? " on" : ""),
								disabled: reviewMode,
								onClick: () => setView("concepts"),
								title: reviewMode ? "Concept frames are not included in review capabilities" : "concept tree — group experiments by concept, any lens",
								children: "Concepts"
							}),
							/* @__PURE__ */ _jsxs("button", {
								type: "button",
								"data-workspace-control": "report",
								tabIndex: workspaceTabStop === "report" ? 0 : -1,
								"aria-pressed": view === "report",
								className: "vt report" + (view === "report" ? " on" : ""),
								onClick: () => setView("report"),
								title: "conclusion-first run report",
							...
							/* @__PURE__ */ _jsx("button", {
								type: "button",
								"data-workspace-control": "overview",
								tabIndex: workspaceTabStop === "overview" ? 0 : -1,
								"aria-pressed": panel === "overview",
								"aria-expanded": panel === "overview",
								className: "vt" + (panel === "overview" ? " on" : ""),
								...
								title: "at-a-glance run summary — best metric, budget, strategy, hints",
								children: "Overview"
							})
						]
					}),
					...
					compactWorkspace ? /* @__PURE__ */ _jsxs("button", {
						type: "button",
						className: "muted run-goal" + (compactGoalExpanded ? " expanded" : ""),
						title: state.goal || state.task_id,
						"aria-expanded": compactGoalExpanded,
						"aria-label": compactGoalExpanded ? "Show less task" : "Show full task",
						"aria-controls": "run-goal-text",
						"aria-describedby": "run-goal-meta run-goal-text",
						onClick: () => setCompactGoalExpanded((value) => !value),
						children: [
							/* @__PURE__ */ _jsxs("b", {
								id: "run-goal-meta",
								children: [
									state.label || state.run_id || runId,
									" · ",
									displayedPhase,
									" · gen ",
									gen
							...
							/* @__PURE__ */ _jsx("span", {
								className: "run-goal-toggle",
								"aria-hidden": "true",
								children: compactGoalExpanded ? "show less" : "show full task"
							})
						]
					}) : /* @__PURE__ */ _jsxs("span", {
						className: "muted",
						title: state.goal || state.task_id,
						children: [/* @__PURE__ */ _jsxs("b", { children: [
							state.label || state.run_id || runId,
							" · ",
							displayedPhase,
							" · gen ",
					...
					/* @__PURE__ */ _jsxs("span", {
						className: "live " + (reviewMode ? "off" : liveStatus),
						role: reviewMode ? undefined : "status",
						"aria-live": reviewMode ? undefined : "polite",
						...
						children: [
							...
							!reviewMode && /* @__PURE__ */ _jsx("span", {
								className: "sr-only",
								children: "Current run status: "
							}),
							reviewMode ? "read-only" : liveLabel,
							historyActive && /* @__PURE__ */ _jsx("span", {
								"aria-hidden": "true",
								children: " · history"
							})
						]
					...
					/* @__PURE__ */ _jsx("span", { className: "spacer" }),
					evalSec > 0 && /* @__PURE__ */ _jsxs("button", {
						type: "button",
						className: "chip run-metric-chip",
						disabled: historyActive,
						...
						children: [
							...
							" ",
							fmtElapsedSeconds(evalSec),
							maxEval != null ? ` / ${fmtElapsedSeconds(maxEval)}` : ""
						]
					}),
					cost && /* @__PURE__ */ _jsxs("button", {
						type: "button",
						className: "chip run-metric-chip",
						disabled: historyActive,
					...
					finalizing ? /* @__PURE__ */ _jsxs("span", {
						className: "chip warn",
						children: [/* @__PURE__ */ _jsx(OpIcon, {
							name: "stop",
							size: 11
						}), lifecycle.mode === "finalization-stalled" ? "finalization stalled" : lifecycle.mode === "finishing" ? "finishing" : "finalizing"]
					}) : live.paused && /* @__PURE__ */ _jsxs("span", {
						className: "chip warn",
						children: [/* @__PURE__ */ _jsx(OpIcon, {
							name: "pause",
							size: 11
			...
			configNoticeStatus && /* @__PURE__ */ _jsxs("div", {
				...
				children: [
					...
					/* @__PURE__ */ _jsx("span", { children: configNoticeStatus === "retrying" ? "Retrying run budget…" : configNoticeStatus === "stale" ? "Run budget may be stale. Showing the last loaded limit." : "Run budget unavailable. Current eval time is visible, but its limit could not be loaded." }),
					/* @__PURE__ */ _jsx("button", {
						type: "button",
						className: "btn xs",
						disabled: configNoticeStatus === "retrying",
						onClick: () => setConfigRetry((value) => value + 1),
						children: configNoticeStatus === "retrying" ? "Retrying…" : "Retry"
					})
				]
			}),
			!reviewMode && retainedCommentWorkUnsafe && /* @__PURE__ */ _jsxs("div", {
				...
				children: [
					...
					/* @__PURE__ */ _jsxs("span", { children: [
						retainedCommentEntries.length > 0 ? `${retainedCommentEntries.length} in-memory Comments work item${retainedCommentEntries.length === 1 ? " is" : "s are"} retained in this tab. ` : "",
						retainedCommentRecovery.valid.length > 0 ? `${retainedCommentRecovery.valid.length} exact Comments command recover${retainedCommentRecovery.valid.length === 1 ? "y is" : "ies are"} saved in this tab.` : "",
						retainedCommentRecovery.damaged.length > 0 ? ` ${retainedCommentRecovery.damaged.length} damaged recovery record${retainedCommentRecovery.damaged.length === 1 ? " needs" : "s need"} review.` : "",
						retainedCommentRecoveryUnavailable ? " Comments recovery storage cannot be inspected. Start over remains blocked; retry storage before continuing." : "",
						retainedCommentValidProtectedCreateCount > 0 ? ` ${retainedCommentValidProtectedCreateCount} current-generation new-comment recover${retainedCommentValidProtectedCreateCount === 1 ? "y stays" : "ies stay"} protected until the exact command reaches a terminal outcome.` : "",
						retainedCommentDamagedProtectedCreateCount > 0 ? ` ${retainedCommentDamagedProtectedCreateCount} damaged new-comment recover${retainedCommentDamagedProtectedCreateCount === 1 ? "y cannot" : "ies cannot"} be safely released; restore the exact recovery data before continuing.` : "",
						currentCommentRecovery ? " Open its experiment to check the same command; nothing replays automatically." : retainedCommentDurableCount > 0 ? " The saved commands belong to an earlier generation and will not be rebound." : " Review the retained work before leaving this run or starting over."
					] }),
					(retainedCommentDrafts.length > 0 || retainedCommentRecovery.valid.length > 0) && /* @__PURE__ */ _jsxs("details", { children: [
						/* @__PURE__ */ _jsx("summary", { children: "View" }),
						retainedCommentDrafts.map((text, index) => /* @__PURE__ */ _jsx("pre", {
							className: "comment-recovery-payload",
							children: text
						}, `draft:${index}`)),
					...
					currentCommentRecovery && /* @__PURE__ */ _jsx("button", {
						type: "button",
						className: "btn xs",
						onClick: openCurrentCommentRecovery,
					...
					retainedCommentReleasableCount > 0 && /* @__PURE__ */ _jsx("button", {
						...
						onClick: discardRetainedCommentWork,
						children: retainedCommentProtectedCreates.length > 0 ? "Discard other work…" : "Discard…"
					})
				]
			}),
			!reviewMode && startOverRecovery.kind !== "none" && /* @__PURE__ */ _jsxs("div", {
				ref: startOverNoticeRef,
				tabIndex: -1,
				className: `route-state-notice run-start-over-notice ${startOverRecovery.kind === "active" ? startOverIntent?.phase || "" : "error"}`,
				role: startOverRecovery.kind === "active" && startOverIntent?.phase !== "unknown" ? "status" : "alert",
				"aria-live": "polite",
				"aria-atomic": "true",
				children: [
					/* @__PURE__ */ _jsx(OpIcon, {
					...
					/* @__PURE__ */ _jsxs("span", { children: [startOverRecovery.kind === "active" ? startOverReplacementSuperseded ? "The saved Start over operation is verified, and a newer run generation is already visible. Open the current run to clear the resolved recovery lock." : startOverHandoff && startOverRouteSyncFailed ? "The new run is ready, but this page could not update its address. Changes stay locked until the current generation is opened safely." : startOverIntent.phase === "submitting" ? "Submitting this exact Start over request. No archive is assumed yet; other changes are locked." : startOverIntent.phase === "pending" ? "The server saved this exact Start over request. Checking the same operation automatically; other changes remain locked." : startOverIntent.phase === "accepted" ? "Start over was accepted. Waiting for the new run generation…" : "Start-over outcome is not confirmed. Retry the exact saved request; all other changes are locked." : startOverRecovery.kind === "unavailable" ? "Start over is disabled because this tab cannot preserve recovery state." : "Saved Start over recovery state is invalid. Changes are locked because an earlier request cannot be identified safely. Close this tab and reopen LoopLab only after confirming no Start over is still running.", startOverRecovery.storageUnavailable ? " This tab can still observe the current request, but its recovery record could not be updated." : ""] }),
					startOverRecovery.kind === "active" && /* @__PURE__ */ _jsx("button", {
						type: "button",
						className: "btn xs",
						onClick: retryStartOver,
						disabled: startOverRequestPending,
						children: startOverRequestPending ? "Waiting for response…" : startOverReplacementSuperseded ? "Open current run" : startOverHandoff && startOverRouteSyncFailed ? "Retry opening new run" : "Retry exact request"
					}),
					startOverRecovery.kind === "unavailable" && /* @__PURE__ */ _jsx("button", {
						type: "button",
						className: "btn xs",
						onClick: retryStartOverStorage,
						children: "Try storage again"
			...
			(routeNotice || attemptFenceNotice || route.issues.length > 0) && /* @__PURE__ */ _jsxs("div", {
				ref: routeNoticeRef,
				className: "route-state-notice",
				role: "status",
				tabIndex: -1,
				children: [
					/* @__PURE__ */ _jsx(OpIcon, {
					...
					/* @__PURE__ */ _jsx("button", {
						type: "button",
						className: "btn xs ghost",
						"aria-label": "Dismiss link-state notice",
						onClick: () => {
							setRouteNotice("");
							setAttemptFenceNotice("");
							route.clearIssues();
			...
			copyFallback && /* @__PURE__ */ _jsxs("div", {
				className: "copy-link-fallback",
				role: "status",
				children: [
					/* @__PURE__ */ _jsx("label", {
						htmlFor: "copy-view-fallback",
						children: "Clipboard blocked — select and copy this link"
					}),
					/* @__PURE__ */ _jsx("input", {
						id: "copy-view-fallback",
						readOnly: true,
						value: copyFallback,
			...
			reviewMode && /* @__PURE__ */ _jsxs("div", {
				...
				children: [
					...
					/* @__PURE__ */ _jsx("b", { children: "Read-only review" }),
					/* @__PURE__ */ _jsx("span", { children: (reviewMeta?.scopes || []).includes("evidence") ? "summary + redacted source evidence" : "summary only" }),
					reviewMeta?.expires_at && /* @__PURE__ */ _jsxs("span", { children: ["expires ", new Date(reviewMeta.expires_at * 1e3).toLocaleString()] }),
					/* @__PURE__ */ _jsx("span", { className: "spacer" }),
					!connected && /* @__PURE__ */ _jsx("span", {
						className: "review-refresh-warn",
						role: "alert",
						children: "Refresh interrupted — showing the last received data."
					}),
					/* @__PURE__ */ _jsx("span", { children: "Actions, Assistant, raw logs, artifacts, and owner settings are unavailable." })
				]
			}),
			historyActive && /* @__PURE__ */ _jsxs("div", {
				className: "history-banner",
				role: "status",
				children: [
					/* @__PURE__ */ _jsx("span", {
					...
					/* @__PURE__ */ _jsx("span", { children: "read-only · actions target live and are disabled" }),
					/* @__PURE__ */ _jsx("button", {
						className: "btn sm primary",
						onClick: returnToLiveAndFocusWorkspace,
						children: "Return to live"
			...
			!mutationReadOnlyMode && (live.phase === "approval" || live.phase === "spec_approval") && /* @__PURE__ */ _jsx("div", {
				className: "topbar",
				role: approvalCommand ? "status" : "alert",
				style: {
					background: "rgba(74,163,255,.12)",
					borderBottom: "1px solid var(--accent-dim)"
				},
				children: approvalCommand ? /* @__PURE__ */ _jsxs(_Fragment, { children: [
					/* @__PURE__ */ _jsx("b", { children: live.phase === "approval" ? `Human approval required for experiment #${live.approval_subject} — type ` : "Eval spec needs ratification — type " }),
					/* @__PURE__ */ _jsx("code", {
						className: "cmd-hint",
						children: approvalCommand
					}),
					/* @__PURE__ */ _jsx("b", { children: "\xA0in the chat below." })
				] }) : /* @__PURE__ */ _jsxs(_Fragment, { children: [
					/* @__PURE__ */ _jsx("b", { children: "Approval target is missing." }),
					/* @__PURE__ */ _jsx("span", { children: "Inspect Events before acting; no command has been guessed." }),
					/* @__PURE__ */ _jsx("button", {
						className: "btn sm",
			...
			/* @__PURE__ */ _jsx("div", {
				className: "topbar panel-bar" + (openHub ? " hub-open" : ""),
				style: {
					minHeight: 38,
					paddingTop: 4,
					paddingBottom: 4
				...
				children: /* @__PURE__ */ _jsxs("div", {
					...
					children: [
						/* @__PURE__ */ _jsx("div", {
							className: "panel-hubs",
							children: HUBS.map(([label, items]) => /* @__PURE__ */ _jsxs("div", {
								className: "more-wrap",
								children: [/* @__PURE__ */ _jsxs("button", {
									type: "button",
									className: "btn sm ghost" + (HUB_OF[panel] === label ? " on" : ""),
									"aria-haspopup": "menu",
									"aria-expanded": openHub === label,
									"aria-controls": hubMenuId(label),
									...
									onClick: (event) => {
										hubTriggerRef.current = event.currentTarget;
										setOpenHub((o) => o === label ? null : label);
									},
									children: [label, " ▾"]
								}), openHub === label && /* @__PURE__ */ _jsxs(_Fragment, { children: [/* @__PURE__ */ _jsx("div", {
									className: "menu-backdrop",
									"aria-hidden": "true",
									onClick: () => closeHub(true)
								}), /* @__PURE__ */ _jsx("div", {
									ref: hubMenuRef,
									id: hubMenuId(label),
									className: "run-menu more-menu",
									role: "menu",
									...
									children: items.map(([k, l]) => /* @__PURE__ */ _jsx("button", {
										type: "button",
										role: "menuitem",
										tabIndex: -1,
										className: "mi" + (panel === k ? " on" : ""),
										disabled: !panelAllowed(k),
										title: !panelAllowed(k) ? reviewMode ? k === "compare" && !reviewEvidence ? "Requires a review link with redacted evidence" : "Unavailable in read-only review" : "Unavailable while viewing a historical snapshot" : undefined,
										onClick: () => {
											if (!panelAllowed(k)) return;
											panelReturnFocusRef.current = hubTriggerRef.current;
											closeHub(false);
											setPanel(k);
										},
										children: l
						...
						/* @__PURE__ */ _jsx("span", { className: "panel-sep" }),
						/* @__PURE__ */ _jsx("button", {
							className: "btn sm ghost settings-panel-btn" + (panel === "config" ? " on" : ""),
			...
			mergeFrom != null && !mutationReadOnlyMode && /* @__PURE__ */ _jsxs("form", {
				className: "merge-destination-bar",
				"aria-label": `Choose a merge destination for experiment ${mergeFrom}`,
				onSubmit: submitMergeTarget,
				children: [
					/* @__PURE__ */ _jsxs("label", {
						htmlFor: "merge-destination-select",
						children: [
							"Merge ",
							/* @__PURE__ */ _jsxs("b", { children: ["#", mergeFrom] }),
							" with"
						]
					}),
					/* @__PURE__ */ _jsxs("select", {
						...
						}), mergeCandidates.map((node) => /* @__PURE__ */ _jsxs("option", {
							value: node.id,
							children: [
								"#",
								node.id,
								" · ",
								node.operator || "experiment",
								" · ",
								node.status
							]
						}, node.id))]
					...
					/* @__PURE__ */ _jsx("button", {
						...
						children: mergeSubmitting ? "Merging…" : "Confirm merge"
					}),
					/* @__PURE__ */ _jsx("button", {
						type: "button",
						className: "btn sm ghost",
						disabled: mergeSubmitting,
						onClick: () => closeMergeChooser(true),
						children: "Cancel"
					}),
					/* @__PURE__ */ _jsx("span", {
						className: "muted",
						children: "Graph gestures only choose the pair; nothing is sent until you confirm."
					})
				]
			}),
			view === "report" ? /* @__PURE__ */ _jsx("div", {
				className: "main",
				children: /* @__PURE__ */ _jsx("div", {
					className: "report-scroll",
					children: /* @__PURE__ */ _jsx(LazyBoundary, {
						label: "run report",
						resetKey: `${runId}:${history.resolvedSeq ?? "live"}`,
						children: /* @__PURE__ */ _jsx(ReportView, {
							state,
							runId,
							onToast: showToast,
							readOnly: mutationReadOnlyMode,
							historySeq: history.resolvedSeq,
							observedSeq: historyActive ? history.resolvedSeq : seq,
							expectedGeneration: reportGeneration,
							readOnlyReason: mutationReadOnlyReason,
							evidenceAvailable: !reviewMode || reviewEvidence,
			...
			}) : view === "concepts" ? /* @__PURE__ */ _jsx("div", {
				className: "main",
				children: /* @__PURE__ */ _jsx(LazyBoundary, {
					label: "concept tree",
					resetKey: `${runId}:${generation || "pending"}:${historyActive ? viewSeq : "live"}`,
					children: /* @__PURE__ */ _jsx(ConceptView, {
						runId,
						generation,
						sequence: historyActive ? viewSeq : null,
						state,
						onPickNode: (id) => {
							route.update((current) => ({
								...current,
				...
				/* @__PURE__ */ _jsx(LazyBoundary, {
					label: "concept filter",
					resetKey: `${runId}:${generation || "pending"}`,
					children: /* @__PURE__ */ _jsx(ConceptChipBar, {
						state,
						onHighlight: setConceptHighlight
					}, `concept-filter:${runId}:${generation || "pending"}`)
				}),
				/* @__PURE__ */ _jsx(WhyStrip, {
					state,
					onSelect: selectNode
				}),
				/* @__PURE__ */ _jsxs("div", {
					className: "main run-workspace" + (compactWorkspace ? " compact" : ""),
					children: [
						/* @__PURE__ */ _jsxs("div", {
							className: "canvas-wrap" + (emptyPresentation ? " dag-empty" : ""),
							inert: compactWorkspace && showInspector ? "" : undefined,
							"aria-hidden": compactWorkspace && showInspector ? "true" : undefined,
							children: [/* @__PURE__ */ _jsx(LazyBoundary, {
								label: "experiment graph",
								resetKey: `${runId}:${generation || "pending"}`,
								children: /* @__PURE__ */ _jsx(Dag, {
									state,
									selectedId,
									...
									onNodeAction: mutationReadOnlyMode ? null : onNodeAction,
									mergeArm: mutationReadOnlyMode ? null : mergeFrom,
									selectedGroup,
									onSelectGroup: selectGroup,
									themeFilter,
									highlightIds: conceptHighlight
								}, `experiment-graph:${runId}:${generation || "pending"}`)
							}), /* @__PURE__ */ _jsx(DagEmptyOverlay, {
								presentation: emptyPresentation,
								transport: transportController,
								onAction: onEmptyAction
							})]
						}),
						compactWorkspace && !showInspector && hasInspectorContext && /* @__PURE__ */ _jsx("button", {
							ref: compactInspectorTriggerRef,
							className: "workspace-pane-toggle",
							onClick: () => setCompactInspectorOpen(true),
							"aria-label": `Open ${groupDetailsOpen ? "group" : "inspector"} panel`,
							children: groupDetailsOpen ? "Group" : `Inspector · #${selectedId}`
						}),
						compactWorkspace && showInspector && /* @__PURE__ */ _jsx("button", {
							type: "button",
						...
						!compactWorkspace && hasInspectorContext && !showInspector ? /* @__PURE__ */ _jsxs("button", {
							ref: sideRailRef,
							className: "side-rail",
							title: "show panel",
							onClick: () => setSideC(false),
							children: ["‹ ", groupDetailsOpen ? "group" : "inspector"]
						}) : showInspector && /* @__PURE__ */ _jsxs(_Fragment, { children: [!compactWorkspace && /* @__PURE__ */ _jsx("div", {
							className: "splitter v",
							onPointerDown: startDrag("side"),
							onKeyDown: resizeWithKeys("side"),
							role: "separator",
							tabIndex: 0,
							...
							className: "side" + (compactWorkspace ? " compact-drawer" : ""),
							style: { width: sideW },
							...
							children: [/* @__PURE__ */ _jsxs("div", {
								...
								children: [
									...
									/* @__PURE__ */ _jsx("button", {
										...
										onClick: () => compactWorkspace ? closeCompactInspector() : collapseSideInspector(),
										children: "⟩"
									})
								]
							}), /* @__PURE__ */ _jsx(LazyBoundary, {
								label: groupDetailsOpen ? "group details" : "experiment inspector",
								resetKey: groupDetailsOpen ? `group:${selectedGroup}` : `node:${selectedId}`,
								children: groupDetailsOpen ? /* @__PURE__ */ _jsx(GroupSummary, {
									...
									state,
									live,
									tab: effectiveInspectTab,
									setTab: setInspectTab,
									onToast: showToast,
									draftStore: inspectorDraftStoreRef.current,
									traceClearRecoveryStore,
									traceClearRecoverySnapshot,
									publishTraceClearRecovery,
									readOnly: mutationReadOnlyMode,
									historySeq: history.resolvedSeq,
									expectedGeneration: generation,
			...
			(view === "dag" || view === "report") && /* @__PURE__ */ _jsxs(_Fragment, { children: [
				!reviewMode && (timelineDeferred || reportTimelineFocusPending) && /* @__PURE__ */ _jsx("div", {
					className: "dock timeline-deferred-trigger",
					"aria-busy": reportTimelineFocusPending,
					children: /* @__PURE__ */ _jsxs("div", {
						className: "dock-tabs",
						children: [
							/* @__PURE__ */ _jsx("strong", { children: reportTimelineFocusPending ? "Loading events & timeline…" : "Events & timeline" }),
							/* @__PURE__ */ _jsx("span", { className: "spacer" }),
							/* @__PURE__ */ _jsx("button", {
								type: "button",
								className: "btn sm ghost dock-collapse",
								"aria-label": "Load events and timeline",
								"aria-expanded": "false",
				...
				!reviewMode && !timelineDeferred && !compactWorkspace && !timelineCollapsed && /* @__PURE__ */ _jsx("div", {
					className: "splitter h",
					onPointerDown: startDrag("dock"),
					onKeyDown: resizeWithKeys("dock"),
					role: "separator",
					tabIndex: 0,
					"aria-orientation": "horizontal",
					"aria-label": "Resize timeline",
					"aria-valuemin": MIN_DOCK_HEIGHT,
					"aria-valuemax": Math.max(MIN_DOCK_HEIGHT, window.innerHeight - 470),
					"aria-valuenow": Math.round(dockH),
					title: "Drag or use arrow keys to resize"
				}),
				!reviewMode && !timelineDeferred && /* @__PURE__ */ _jsx(LazyBoundary, {
					label: "timeline",
					resetKey: `${runId}:${generation || "pending"}`,
					children: /* @__PURE__ */ _jsx(Dock, {
						runId,
						live,
						liveSeq: seq,
						expectedGeneration: generation,
						timeline,
						viewSeq,
						setViewSeq: changeViewSeq,
						onReturnToLive: returnToLive,
						onFocus: focusNode,
						filter: routeState.timelineFilter,
						onFilterChange: (value) => route.update((current) => ({
							...current,
							timelineFilter: value
						}), { mode: "replace" }),
						kindFilters: routeState.timelineKinds,
						onKindFiltersChange: (value) => route.update((current) => ({
							...current,
							timelineKinds: value
						})),
						onToast: showToast,
						startOverState: {
							blocked: retainedPanelDraftUnsafe || retainedCommentWorkUnsafe || startOverRecovery.kind !== "none" || !/^[0-9a-f]{64}$/.test(generation || ""),
							lifecycleBlocked: startOverRecovery.kind === "active" || startOverRecovery.kind === "corrupt",
							disabledReason: retainedPanelDraftUnsafe ? "Finish or explicitly discard retained Run settings or Authoring work first" : retainedCommentWorkUnsafe ? "Review retained Comments work or retry unavailable recovery storage first" : startOverRecovery.kind === "active" ? "Resolve the existing Start over outcome first" : startOverRecovery.kind === "unavailable" ? "Start over needs working recovery storage in this tab" : startOverRecovery.kind === "corrupt" ? "Saved Start over recovery evidence is invalid" : "Wait for the current run generation before starting over",
			...
			panel && panelAllowed(panel) && /* @__PURE__ */ _jsx(LazyBoundary, {
				label: `${HUB_OF[panel] || panel} panel`,
				mode: "overlay",
				resetKey: `${panel}:${runId}@${generation || "pending"}`,
				onClose: closePanel,
				children: /* @__PURE__ */ _jsxs(_Fragment, { children: [
					panel === "overview" && panelAllowed("overview") && /* @__PURE__ */ _jsx(OverviewPanel, {
						state,
						maxEval,
						onClose: closePanel,
						onOpenPanel: (p) => {
							if (panelAllowed(p)) setPanel(p, { mode: "replace" });
						}
					}),
					panel === "research" && panelAllowed("research") && /* @__PURE__ */ _jsx(ResearchPanel, {
						state,
						runId,
						onToast: showToast,
						onClose: closePanel
					}),
					panel === "trust" && panelAllowed("trust") && /* @__PURE__ */ _jsx(TrustPanel, {
						state,
						runId,
					...
					panel === "queue" && panelAllowed("queue") && /* @__PURE__ */ _jsx(QueuePanel, {
						state,
						runId,
						onSelect: selectNodeFromPanel,
						onToast: showToast,
					...
					panel === "hypotheses" && panelAllowed("hypotheses") && /* @__PURE__ */ _jsx(HypothesisBoard, {
						state,
						runId,
						runGeneration: generation,
						onSelect: selectNodeFromPanel,
					...
					panel === "sensitivity" && panelAllowed("sensitivity") && /* @__PURE__ */ _jsx(SensitivityPanel, {
						state,
						onSelect: selectNodeFromPanel,
						onClose: closePanel
					}),
					panel === "importance" && panelAllowed("importance") && /* @__PURE__ */ _jsx(HyperImportancePanel, {
						state,
						onClose: closePanel
					}),
					panel === "failures" && panelAllowed("failures") && /* @__PURE__ */ _jsx(FailuresPanel, {
						state,
						onSelect: selectNodeFromPanel,
						onClose: closePanel
					}),
					panel === "pareto" && panelAllowed("pareto") && /* @__PURE__ */ _jsx(ParetoPanel, {
						state,
						onSelect: selectNodeFromPanel,
						onClose: closePanel
					}),
					panel === "data" && panelAllowed("data") && /* @__PURE__ */ _jsx(DataQualityPanel, {
						state,
						onClose: closePanel
					}),
					panel === "compare" && panelAllowed("compare") && /* @__PURE__ */ _jsx(ComparePanel, {
						state,
						runId,
					...
					panel === "crossrun" && panelAllowed("crossrun") && /* @__PURE__ */ _jsx(CrossRunPanel, {
						state,
						onClose: closePanel
					}),
					panel === "collab" && panelAllowed("collab") && /* @__PURE__ */ _jsx(CollabPanel, {
						state,
						runId,
						onSelect: selectNode,
						...
						onToast: showToast,
						onClose: closePanel,
						reviewRouteState: routeState,
						reviewMode,
						draftStore: inspectorDraftStoreRef.current,
						expectedGeneration: generation,
						refreshKey: state?.comments_revision
					}),
					panel === "config" && panelAllowed("config") && /* @__PURE__ */ _jsx(ConfigPanel, {
						runId,
						expectedGeneration: generation,
						state,
						live,
						...
						onClose: closePanel,
						draftStore: inspectorDraftStoreRef.current,
						navigationGuardOwner: "run",
						publishNavigationGuard: publishPanelNavigationGuard
					}),
					panel === "authoring" && panelAllowed("authoring") && /* @__PURE__ */ _jsx(AuthoringPanel, {
						onToast: showToast,
						onClose: closePanel,
						draftStore: inspectorDraftStoreRef.current,
						navigationGuardOwner: "run",
					...
					panel === "memory" && panelAllowed("memory") && /* @__PURE__ */ _jsx(MemoryPanel, { onClose: closePanel }),
					panel === "registry" && panelAllowed("registry") && /* @__PURE__ */ _jsx(RegistryPanel, {
						state,
						onClose: closePanel
					}),
					panel === "gpu" && panelAllowed("gpu") && /* @__PURE__ */ _jsx(GpuPanel, { onClose: closePanel }),
					panel === "events" && panelAllowed("events") && /* @__PURE__ */ _jsx(EventExplorer, {
						runId,
						timeline,
						historyActive,
						onReturnToLive: returnToLive,
					...
					panel === "artifacts" && panelAllowed("artifacts") && /* @__PURE__ */ _jsx(ArtifactsPanel, {
						runId,
						expectedGeneration: generation,
						onToast: showToast,
						onClose: closePanel
					}, `artifacts:${runId}@${generation || "pending"}`)
				] })
			}),
			toast && /* @__PURE__ */ _jsx("div", {
				className: "toast",
