...
import { deadlineSharedAssistant, fmtDate } from "./util.mjs";
import { Turn } from "./AssistantChat.mjs";
import { jsx as _jsx, jsxs as _jsxs } from "react/jsx-runtime";
const SHARED_REQUEST_TIMEOUT_MS = 15e3;
const SHARED_MESSAGE_MAX = 400;
const SHARED_TEXT_MAX = 256e3;
const SHARED_TOTAL_TEXT_MAX = 2e6;
const SHARED_ITEMS_MAX = 128;
const SHARED_LABEL_MAX = 1e3;
...
const SHARED_STRUCTURE_MAX = 2e4;
...
const labelItems = (value) => value == null || boundedArray(value) && value.every((item) => record(item) && (item.label == null || boundedText(item.label, SHARED_LABEL_MAX)) && (item.tool == null || boundedText(item.tool, SHARED_LABEL_MAX)));
const sharedMessageWeight = (message) => {
	if (!record(message) || !["user", "assistant"].includes(message.role) || !boundedText(message.content) || !labelItems(message.steps) || !labelItems(message.applied) || message.todos != null && (!boundedArray(message.todos) || !message.todos.every((item) => record(item) && boundedText(item.content, SHARED_LABEL_MAX) && boundedText(item.status, 64))) || message.activity != null && (!boundedArray(message.activity) || !message.activity.every((item) => record(item) && (item.type === "text" && boundedText(item.content) || item.type === "tools" && boundedArray(item.labels) && item.labels.every((label) => boundedText(label, SHARED_LABEL_MAX)))))) return null;
	...
	for (const item of message.activity || []) {
		weight += item.type === "text" ? item.content.length : item.labels.reduce((total, label) => total + label.length, 0);
	}
	return weight;
};
const sharedMessageStructure = (message, available) => {
	let remaining = available - 6;
	const spend = (amount) => {
		remaining -= amount;
		return remaining >= 0;
	};
	const spendText = (value) => {
		...
		for (let index = 0; index < value.length; index += 1) {
			...
			const cost = code === 10 || code === 13 ? 2 : code === 42 || code === 64 || code === 91 || code === 95 || code === 96 ? 1 : 0;
			if (cost && !spend(cost)) return false;
		}
		return true;
	};
	if (!spendText(message.content)) return null;
	if (!spend((message.steps?.length || 0) + (message.applied?.length || 0) + (message.todos?.length || 0) * 2 + (message.activity?.length || 0) * 2)) return null;
	for (const item of message.activity || []) {
...
const validSharedSession = (value) => {
	if (!record(value) || !record(value.meta) || !boundedText(value.meta.title, 500) || typeof value.meta.live !== "boolean" || !Number.isFinite(value.meta.expires_at) || value.meta.truncated != null && typeof value.meta.truncated !== "boolean" || value.meta.expires_at <= 0 || !Array.isArray(value.messages) || value.messages.length > SHARED_MESSAGE_MAX) return false;
...
const sharedLoadFailure = (error) => {
	...
	return {
		terminal: false,
		message: error?.name === "TimeoutError" ? "Shared chat loading timed out. Check your connection and retry." : "Shared chat could not be loaded. Retry when the service is reachable."
	};
};
...
export default function SharedAssistant({ sid }) {
	...
	const statusMessage = resource.status === "loading" ? "Loading shared chat." : refreshing ? "Refreshing shared chat. Showing the last loaded transcript." : resource.status === "ready" ? `Shared chat loaded. ${messageCount} ${messageCount === 1 ? "message" : "messages"}.${truncated ? " Some messages or details are not included because this transcript reached its safety limit." : ""}` : "";
	return /* @__PURE__ */ _jsx("main", {
		ref: mainRef,
		className: "asst-view asst-shared",
		...
		children: /* @__PURE__ */ _jsxs("div", {
			...
			children: [
				/* @__PURE__ */ _jsxs("div", {
					...
					children: [
						/* @__PURE__ */ _jsx("h1", {
							...
							className: "ttl",
							style: { flex: 1 },
							children: sess?.meta.title || "Shared chat"
						}),
						/* @__PURE__ */ _jsxs("div", {
							className: "asst-shared-terms",
							role: "note",
							children: [/* @__PURE__ */ _jsx("span", {
								className: "pill",
								children: sess ? liveShare ? "live · read-only" : "frozen snapshot · read-only" : "public · read-only"
							}), expiry && /* @__PURE__ */ _jsxs("span", {
								className: "muted",
								children: ["Expires ", /* @__PURE__ */ _jsx("time", {
									dateTime: new Date(expiry * 1e3).toISOString(),
									children: fmtDate(expiry)
								})]
							})]
						}),
						sess && /* @__PURE__ */ _jsx("button", {
							className: "btn sm",
							type: "button",
							disabled: refreshing,
							...
							children: refreshing ? "Refreshing…" : liveShare ? "Refresh messages" : "Recheck link"
						})
					]
				}),
				/* @__PURE__ */ _jsx("div", {
				...
				/* @__PURE__ */ _jsxs("div", {
					...
					role: "log",
					"aria-live": "off",
					"aria-label": "Shared Assistant transcript",
					"aria-busy": resource.status === "loading" || refreshing ? "true" : undefined,
					tabIndex: 0,
					children: [
						truncated && /* @__PURE__ */ _jsx("div", {
							className: "notice",
							role: "note",
							children: "This public transcript reached its safety limit. Some messages or details are not included."
						}),
						resource.status === "loading" && /* @__PURE__ */ _jsx("div", {
							className: "notice",
							children: "Loading shared chat…"
						}),
						refreshing && /* @__PURE__ */ _jsx("div", {
							className: "notice",
							children: "Refreshing shared chat…"
						}),
						(resource.status === "error" || resource.status === "stale" || resource.status === "gone") && /* @__PURE__ */ _jsxs("div", {
							className: "notice",
							role: "alert",
							style: {
							...
							children: [/* @__PURE__ */ _jsx("p", { children: resource.status === "stale" ? `${resource.error} Showing the last loaded transcript.` : resource.error }), resource.status !== "gone" && /* @__PURE__ */ _jsx("button", {
								className: "btn sm",
								type: "button",
								onClick: retry,
								children: "Retry"
						...
						sess && sess.messages.map((message, index) => /* @__PURE__ */ _jsx(Turn, {
							m: message,
							readOnly: true,
							audience: "public"
						}, index)),
						sess && messageCount === 0 && /* @__PURE__ */ _jsx("div", {
							className: "muted",
							children: "This shared snapshot has no complete turns."
						})
					]
				})
			]
