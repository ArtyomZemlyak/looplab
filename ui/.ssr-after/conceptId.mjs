...
export function normalizeConceptId(raw) {
	...
	return value && [...value].length <= MAX_ID_CHARS && parts.length <= MAX_ID_DEPTH && parts.every((part) => validSegment.test(part)) ? value : "";
}
...
export function nodeTheme(node, state = null) {
	...
	if (memberships && typeof memberships === "object" && !Array.isArray(memberships) && nodeId != null && Object.hasOwn(memberships, String(nodeId))) {
		const rename = state?.concept_consolidation || {};
		const raw = Array.isArray(memberships[String(nodeId)]) ? memberships[String(nodeId)] : [];
		const axes = new Set();
