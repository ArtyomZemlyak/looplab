...
const storageScope = () => typeof window === "undefined" ? "" : `${window.location.origin}${window.location.pathname}`;
...
const commit = (next) => {
	if (snapshot.active === next.active && snapshot.blocked === next.blocked && snapshot.status === next.status && snapshot.reason === next.reason) return snapshot;
	snapshot = next;
...
export function beginOperation(owner, kind = "settings-write") {
	if (owner !== publisher || reconciliationRequired || reconciledPublisher !== owner || operations.size > 0) {
	...
	operations.set(lease, {
		owner,
		kind: cleanText(kind, "settings-write"),
		detached: false
	});
	operationEpoch += 1;
	reconciliationRequired = true;
...
export function publish(tokenOrValue, suppliedValue) {
	...
	const foreignOperationPending = [...operations.values()].some((operation) => operation.owner !== token || operation.detached);
	...
	if (value?.resolved === true) {
		...
		if (operations.size > 0 || reconciliationRequired || reconciledPublisher !== token || storageUncertain) {
			writeRetainedFence();
