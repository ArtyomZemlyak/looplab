...
export const SETTINGS_GROUPS = [
  ...
  },
  {
    ...
    fields: [
      ...
      { key: 'developer_run_tools', label: 'Developer run tools', type: 'bool',
        help: 'On (default): give the in-house repo Developer the SAME read-only run tools the Researcher has (read_experiment/read_code/read_logs/find_analogous over its own run; sibling/all-runs gated by the two toggles above) — so it can read how a prior/merged/failed node was implemented and why it broke. Pulled on demand, not injected. Off: legacy repo-scouts-only Developer.' },
      { key: 'developer_memory', label: 'Developer memory (impl. lessons)', type: 'bool',
        help: 'On (default, needs a Memory dir): a SEPARATE cross-run store of IMPLEMENTATION lessons (dataset/framework/build gotchas + techniques) the Developer self-authors via remember_dev_lesson, reads via search_dev_lessons, and sees a top-5 preview of — distinct from the Researcher’s lessons. The engine also distils a few at run end. See the “Dev lessons” tab in Memory.' },
