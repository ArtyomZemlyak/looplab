  ...
  },
  {
    ...
    fields: [
      ...
      { key: 'agent_self_plan', label: 'Self-plan (TODO)', type: 'bool',
        help: 'C1: give long-running agents a TodoWrite-style update_plan tool so they keep their OWN checklist; the current plan is re-surfaced every few turns to keep the goal in view across a long loop. Off by default.' },
      ...
      { key: 'agent_auto_summary', label: 'Auto-summary', type: 'bool',
        help: 'C2: when the tool-loop history exceeds Context budget, LLM-summarize the stale middle instead of only truncating it (needs Context budget > 0; one extra model call per over-budget turn). Off by default.' },
