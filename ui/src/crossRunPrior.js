...
export function crossRunPriorNarration(data) {
  ...
  const runs = Array.isArray(data?.prior_runs)
    ? data.prior_runs.slice(0, 8).filter(run => run && typeof run === 'object' && !Array.isArray(run))
    : []
  ...
  const history = runs.length
    ? `${runs.length} retained run${runs.length === 1 ? '' : 's'}`
    : 'match recorded'
