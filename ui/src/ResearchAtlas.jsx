...
const SOURCES = [
  { key: 'atlas', label: 'Coverage', read: signal => getCrossRunAtlas(24, { signal }) },
  { key: 'claims', label: 'Claims', read: signal => getCrossRunClaims(40, 0, { signal }) },
...
function ClaimCard({ claim, compact = false }) {
  ...
  const context = [
    ...claim.scopes.map(value => `scope · ${value}`),
  ...
  return <article className={`atlas-claim atlas-state-${claim.epistemic}`}>
    <div className="atlas-claim-head">
      <span className={`chip xs atlas-epistemic ${claim.epistemic}`}>{claim.epistemic}</span>
      <span className="pill">{claim.maturity.replaceAll('-', ' ')}</span>
    </div>
    <p>{claim.statement}</p>
    <div className="atlas-claim-counts" aria-label="Claim evidence counts">
      <span>support <b>{claim.nSupport}</b></span>
      <span>oppose <b>{claim.nOppose}</b></span>
      {claim.nUnverified > 0 && <span>unverified <b>{claim.nUnverified}</b></span>}
      {claim.scopes.length > 0 && <span title={claim.scopes.join(', ')}>
        {countLabel(claim.scopes.length, 'scope')}
      </span>}
    </div>
    {context.length > 0 && <div className="atlas-claim-context" aria-label="Bounded claim context">
    ...
    {!compact && (evidence > 0 || hiddenEvidence > 0) && <details>
      <summary>Show bounded evidence references</summary>
      <div className="atlas-evidence">
        ...
        {claim.support.map((ref, index) => <code key={`s-${index}`}>supports · {ref}</code>)}
        {claim.oppose.map((ref, index) => <code key={`o-${index}`}>opposes · {ref}</code>)}
...
function RouteState({ kind, errors, onRetry }) {
  if (kind === 'loading') return <div className="run-resource-state" role="status" aria-live="polite">
    ...
    <p>Reading bounded coverage, claims, and recent curation proposals from shared memory.</p>
  </div>
...
export default function ResearchAtlas({ onBack }) {
  ...
  const [resource, setResource] = useState({
    status: 'loading', view: null, payload: null, errors: [], stale: false, preserved: false,
  })
  ...
  useEffect(() => {
    ...
    setResource(current => current.view
      ? { ...current, status: 'refreshing', errors: [], stale: false, preserved: false }
      : { status: 'loading', view: null, payload: null, errors: [], stale: false, preserved: false })
    Promise.allSettled(SOURCES.map(source => source.read(controller.signal))).then(results => {
      ...
      setResource(current => loaded === 0
        ? (current.view
          ? { ...current, status: 'ready', errors, stale: true, preserved: false }
          : { status: 'error', view: null, payload: null, errors, stale: false, preserved: false })
        : (() => {
          ...
          return {
            ...
            errors, stale: false, preserved: current.view != null && errors.length > 0,
          }
        })())
    })
        ...
        : <div className="atlas-content">
          <header className="atlas-intro">
            <div>
              ...
              <p>This is not the complete Atlas: it has no frozen snapshot, CoverageFrame, or server-paged
                exploration. Refresh may observe newly finalized runs or governance, but cannot change claims,
                taxonomy, runs, or selection state. Claim identity and applicability remain experimental.
                Rows, context, and model-authored text are clipped to client render limits. “Thin” means
                observed in one run—not untried.</p>
            </div>
          </header>
          ...
          <section className="atlas-summary" aria-label="Portfolio summary">
            {[
              ...
              ['Contested', view.totals.contested],
          ...
          {!view.empty && <div className="atlas-grid">
            <section className="atlas-panel atlas-coverage" aria-labelledby="atlas-coverage-title">
              <div className="atlas-panel-head">
                <div><p className="atlas-eyebrow">Coverage</p><h2 id="atlas-coverage-title">Explored concepts</h2></div>
              ...
              <div className="atlas-thin">
                <h3>Thin coverage <span>{view.thin.length}</span></h3>
                {view.thin.length > 0
                  ? <div className="atlas-tags">{view.thin.map((name, index) => <span className="pill" key={`${name}-${index}`}>{name}</span>)}</div>
                  : <p className="atlas-section-empty">No single-run concepts in the bounded projection.</p>}
                {view.hiddenThin > 0 && <p className="atlas-inline-boundary">
                  +{view.hiddenThin} more thin-coverage concept{view.hiddenThin === 1 ? '' : 's'}
                </p>}
              </div>
            </section>
            ...
            <section className="atlas-panel atlas-contradictions" aria-labelledby="atlas-contradictions-title">
              <div className="atlas-panel-head">
                <div><p className="atlas-eyebrow">Counter-evidence</p><h2 id="atlas-contradictions-title">Contradictions</h2></div>
                <span className="chip xs warn">{view.totals.contested} contested</span>
              </div>
              {view.contradictions.length > 0
                ...
                : <p className="atlas-section-empty">No live contradictions in the bounded projection.</p>}
              {view.hiddenContradictions > 0 && <p className="atlas-boundary-note">
                {countLabel(view.hiddenContradictions, 'additional contradiction')} omitted by the bounded projection.
              </p>}
            </section>

            <section className="atlas-panel atlas-all-claims" aria-labelledby="atlas-claims-title">
              <div className="atlas-panel-head">
                <div><p className="atlas-eyebrow">Evidence ledger</p><h2 id="atlas-claims-title">Claims</h2></div>
              ...
              {view.claims.length > 0
                ...
                : <p className="atlas-section-empty">No claims are available.</p>}
            ...
            <section className="atlas-panel atlas-curation" aria-labelledby="atlas-curation-title">
              <div className="atlas-panel-head">
                <div><p className="atlas-eyebrow">Audit trail</p><h2 id="atlas-curation-title">Concept + claim governance</h2></div>
              ...
              {view.curation.length > 0
                ? <ol className="atlas-curation-list">{view.curation.map((entry, index) => {
                    ...
                    return <li key={`${entry.runId}-${entry.at}-${index}`}>
                      <div><strong>{entry.runId
                        ? <a href={`#/run/${encodeURIComponent(entry.runId)}`}
                             aria-label={`Open run ${entry.runId}`}>{entry.runId}</a>
                        : 'Portfolio steward'}</strong><span>{entry.at || (entry.revision
                          ? `revision ${entry.revision}` : 'time not recorded')}</span></div>
                ...
                : <p className="atlas-section-empty">No steward outcomes have been logged.</p>}
