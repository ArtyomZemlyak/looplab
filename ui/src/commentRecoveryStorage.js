...
const COMMENT_OPERATION_KINDS = new Set(['create', 'edit', 'resolution'])
const COMMENT_OPERATION_KEYS = new Set([
  'schema', 'operationId', 'kind', 'runId', 'expectedGeneration', 'nodeId',
  'nodeGeneration', 'commentId', 'expectedVersion', 'text', 'resolved', 'updatedAt',
])
...
const validCommentText = text => wellFormedText(text)
  && text.trim().length > 0 && utf8Bytes(text) <= COMMENT_MAX_BYTES
...
const validIdentity = identity => isRecord(identity)
  && COMMENT_OPERATION_KINDS.has(identity.kind)
  && safeText(identity.runId, 255) && wellFormedText(identity.runId)
  && RUN_GENERATION_RE.test(identity.expectedGeneration)
  && safeIndex(identity.nodeId) && safeIndex(identity.nodeGeneration)
  && (identity.kind === 'create'
    ? identity.commentId == null
    : COMMENT_ID_RE.test(identity.commentId || ''))

const identityParts = identity => [
  identity.runId, identity.expectedGeneration, identity.kind,
  String(identity.nodeId), String(identity.nodeGeneration), identity.commentId || '',
]

export const commentOperationStorageKey = identity => validIdentity(identity)
  ? COMMENT_OPERATION_STORAGE_PREFIX + encodeURIComponent(identityParts(identity).join('\u0000'))
  : null

const parseStorageIdentity = key => {
  if (typeof key !== 'string' || !key.startsWith(COMMENT_OPERATION_STORAGE_PREFIX)) return null
  try {
    const parts = decodeURIComponent(key.slice(COMMENT_OPERATION_STORAGE_PREFIX.length)).split('\u0000')
    if (parts.length !== 6 || !/^(?:0|[1-9]\d*)$/.test(parts[3])
        || !/^(?:0|[1-9]\d*)$/.test(parts[4])) return null
    const identity = {
      runId: parts[0], expectedGeneration: parts[1], kind: parts[2],
      nodeId: Number(parts[3]), nodeGeneration: Number(parts[4]),
...
const sameIdentity = (left, right) => validIdentity(left) && validIdentity(right)
  && identityParts(left).every((value, index) => value === identityParts(right)[index])

export const validCommentOperationIntent = value => isRecord(value)
  && Object.keys(value).length === COMMENT_OPERATION_KEYS.size
  && Object.keys(value).every(key => COMMENT_OPERATION_KEYS.has(key))
  && value.schema === COMMENT_OPERATION_SCHEMA
  && OPERATION_ID_RE.test(value.operationId || '')
  && validIdentity(value)
  && Number.isSafeInteger(value.updatedAt) && value.updatedAt >= 0
  && (value.kind === 'create'
    ? value.commentId === null && value.expectedVersion === null
      && validCommentText(value.text) && value.resolved === null
    : value.kind === 'edit'
      ? COMMENT_ID_RE.test(value.commentId) && Number.isSafeInteger(value.expectedVersion)
        && value.expectedVersion >= 1 && validCommentText(value.text) && value.resolved === null
      : COMMENT_ID_RE.test(value.commentId) && Number.isSafeInteger(value.expectedVersion)
        && value.expectedVersion >= 1 && value.text === null && typeof value.resolved === 'boolean')

export function createCommentOperationIntent({
  operationId, kind, runId, expectedGeneration, nodeId, nodeGeneration,
  commentId = null, expectedVersion = null, text = null, resolved = null,
}) {
  const intent = {
    schema: COMMENT_OPERATION_SCHEMA, operationId: String(operationId || '').toLowerCase(),
    kind, runId: String(runId || ''), expectedGeneration: String(expectedGeneration || ''),
    nodeId, nodeGeneration, commentId, expectedVersion, text, resolved, updatedAt: Date.now(),
  }
...
export function inspectCommentOperation(identity, storage = undefined) {
  ...
  if (!target || !key) return { kind: 'unavailable', key }
  try {
    const raw = target.getItem(key)
    if (raw == null) return { kind: 'none', key }
    const intent = parsedIntent(raw, identity)
    return intent
      ? { kind: 'valid', key, raw, intent: { ...intent, storageKey: key, storageRaw: raw } }
      : { kind: 'damaged', key, raw, identity: { ...identity } }
  } catch { return { kind: 'unavailable', key } }
}
  ...
  try {
    ...
    if (existingRaw != null) {
      ...
      if (!existing || existing.operationId !== intent.operationId
          || existing.expectedVersion !== intent.expectedVersion
          || existing.text !== intent.text || existing.resolved !== intent.resolved) return null
      publishRecoveryChange()
      return { ...existing, storageKey: key, storageRaw: existingRaw }
    }
    const raw = JSON.stringify(intent)
    target.setItem(key, raw)
    if (target.getItem(key) !== raw) return null
    publishRecoveryChange()
    return { ...intent, storageKey: key, storageRaw: raw }
  } catch { return null }
}
...
export function commentOperationIntentStored(intent, storage = undefined) {
  ...
  try { return target.getItem(intent.storageKey) === intent.storageRaw } catch { return false }
}
...
export function listCommentOperationRecoveries(runId, storage = undefined) {
  const target = storageTarget(storage)
  if (!target || !safeText(runId, 255)) return { available: false, valid: [], damaged: [] }
  const valid = [], damaged = []
  try {
    const keys = []
    ...
    for (const key of keys) {
      ...
      const raw = target.getItem(key)
      const intent = parsedIntent(raw, identity)
      if (intent) valid.push({ ...intent, storageKey: key, storageRaw: raw })
      else damaged.push({ key, raw: raw ?? '', identity })
    }
