...
const appendIssue = (issues, issue) => (
  issues.includes(issue) ? issues : [...issues, issue]
)
...
const rememberFallbackPanelEntry = (href, previousHref) => {
  fallbackPanelHistoryEntry = { href, version: 1, previousHref }
}
...
export function useRunRouteState({ generation = null, reviewMode = false } = {}) {
  ...
  const write = useCallback((state, mode = 'push', {
    ...
    const nextHistoryState = currentHistoryState && typeof currentHistoryState === 'object'
      ? { ...currentHistoryState } : {}
    const fallbackPanelEntry = fallbackPanelHistoryEntry?.href === previousHref
      ? fallbackPanelHistoryEntry : null
  ...
  const hydrate = useCallback(() => {
    ...
    const fallbackPanelEntry = fallbackPanelHistoryEntry?.href === currentHref
      ? fallbackPanelHistoryEntry : null
  ...
  const update = useCallback((patch, {
    ...
    if (!written && mode !== 'replace') {
      setResource(value => ({ ...value, issues: appendIssue(value.issues, ROUTE_WRITE_FAILED) }))
      return current
    }
    stateRef.current = candidate
    setResource(value => {
      const issues = preserveIssues ? value.issues : []
      return { ...value, state: candidate,
