...
import { get, reviewTokenFromLocation } from './reviewRouteApi.js'
...
const ClaimsCuration = lazy(() => import('./ClaimsCuration.jsx'))
...
//   #/ #/claims #/memory #/knowledge #/gpu #/settings — the LoopLab plane, true for the installation
// The legacy #/atlas and #/research-atlas aliases are replaced in place so links generated by the UI
// have one canonical form. (#/atlas became legacy with doc 29 F7, which renamed the Research Atlas to
// Claims & Curation after the three sections it uniquely holds and dropped the fourth.)
// #/memory, #/knowledge and #/gpu are the new homes of three panels that used to be
...
function parseHash() {
  ...
  if (h === '#/claims') return { view: 'claims' }
  // A deep link INTO one of the run list's four representations. It exists because Claims & Curation
  // no longer carries a concept list and points here instead; a link that landed on the List tab
  // would have been a worse answer than the section it replaced. The list still owns the view — the
  // hash only names which one to open with.
  if (h === '#/concepts') return { view: 'list', listView: 'concepts' }
...
const keepsListOrigin = view => view === 'list' || view === 'settings'
  || view === 'claims' || isInstallationRouteView(view)
...
export default function App() {
  ...
  // Settings did this and the claim ledger silently dropped the operator's place in the list.
  ...
  // `#/concepts` opens the list ON its Concepts view. RunList reads `initialNavigationState` once at
  // mount (an `undefined`-checked ref), so the request has to arrive as an initial snapshot AND
  // re-key the list; memoized because an unstable object identity here re-fires the list's own
  // restore/focus effects on every render.
  const requestedListView = route.listView || ''
  const listNavigationForRoute = useMemo(() => requestedListView
    ? { ...(listNavigationRef.current || {}), view: requestedListView }
    : null, [requestedListView])
