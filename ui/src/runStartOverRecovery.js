...
const PHASES = new Set(['submitting', 'pending', 'accepted', 'unknown'])
const RUN_GENERATION_RE = /^[0-9a-f]{64}$/
const OPERATION_ID_RE = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/
const INTENT_KEYS = new Set([
  'schema', 'runId', 'expectedGeneration', 'operationId', 'replacementGeneration',
  'phase', 'updatedAt',
])
const PREVIOUS_INTENT_KEYS = new Set([
  'schema', 'runId', 'expectedGeneration', 'operationId', 'phase', 'updatedAt',
])
const LEGACY_INTENT_KEYS = new Set([
  'schema', 'runId', 'expectedGeneration', 'phase', 'updatedAt',
])
...
const safeRunId = value => typeof value === 'string' && value.length > 0 && value.length <= 512
  && !/[\u0000-\u001f\u007f]/.test(value)
const hasExactKeys = (value, keys) => Object.keys(value).length === keys.size
...
export function validRunStartOverIntent(value, runId = value?.runId) {
  return !!value && typeof value === 'object' && !Array.isArray(value)
    && hasExactKeys(value, INTENT_KEYS) && value.schema === SCHEMA
    ...
    && (value.replacementGeneration === null
      || (typeof value.replacementGeneration === 'string'
        && RUN_GENERATION_RE.test(value.replacementGeneration)
        && value.replacementGeneration !== value.expectedGeneration))
    && (value.phase !== 'accepted' || value.replacementGeneration !== null)
...
const migrateLegacyIntent = (value, runId) => {
  if (value && typeof value === 'object' && !Array.isArray(value)
      && hasExactKeys(value, PREVIOUS_INTENT_KEYS) && value.schema === PREVIOUS_SCHEMA
      && safeRunId(value.runId) && value.runId === String(runId || '')
      && typeof value.expectedGeneration === 'string'
      && RUN_GENERATION_RE.test(value.expectedGeneration)
  ...
  if (!value || typeof value !== 'object' || Array.isArray(value)
      || !hasExactKeys(value, LEGACY_INTENT_KEYS) || value.schema !== LEGACY_SCHEMA
      || !safeRunId(value.runId) || value.runId !== String(runId || '')
      || typeof value.expectedGeneration !== 'string'
      || !RUN_GENERATION_RE.test(value.expectedGeneration)
      || !PHASES.has(value.phase)
...
export function createRunStartOverIntent(
  runId, expectedGeneration, operationId, phase = 'submitting', now = Date.now(),
  replacementGeneration = null,
) {
  const intent = {
    schema: SCHEMA,
    ...
    replacementGeneration: replacementGeneration == null
      ? null : String(replacementGeneration || '').toLowerCase(),
    phase,
...
export function loadRunStartOverIntent(runId, storage) {
  ...
  let raw
  try { raw = target.getItem(storageKey(runId)) } catch { return { kind: 'unavailable', intent: null } }
  if (raw == null) return { kind: 'none', intent: null }
  if (raw === '') return { kind: 'corrupt', intent: null }
  try {
    const parsed = JSON.parse(raw)
    const intent = validRunStartOverIntent(parsed, runId)
      ? parsed : migrateLegacyIntent(parsed, runId)
    if (!intent) return { kind: 'corrupt', intent: null }
    if (parsed !== intent) {
      try { target.setItem(storageKey(runId), JSON.stringify(intent)) }
      catch { return { kind: 'active', intent, storageUnavailable: true } }
    }
    return { kind: 'active', intent }
  } catch {
...
export function saveRunStartOverIntent(intent, storage, expectedOperationId = null) {
  ...
  try {
    ...
    return restored.kind === 'active'
      && JSON.stringify(restored.intent) === JSON.stringify(intent)
  } catch {
    return false
  }
}
  ...
  try {
    ...
    for (let index = 0; index < length && found.length < 64; index += 1) {
      ...
      if (recovery.kind === 'active' || recovery.kind === 'corrupt') {
        found.push({ runId, kind: recovery.kind })
      }
    }
  } catch { return found }
