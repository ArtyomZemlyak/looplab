...
require_git() {
  ...
  # И ЭТОГО МАЛО: НАЛИЧИЕ РЕПОЗИТОРИЯ — НЕ ТО ЖЕ, ЧТО ОТСЛЕЖИВАЕМЫЙ `results/`.
  #
  # Классификатор здесь — «отслеживается = чужое». Если под `results/` git не знает НИ ОДНОГО
  # файла, `ls-files` отвечает «не отслеживается» про всё, `is_foreign` не срабатывает ни разу, и
  # забор становится ровно той молчаливой пустышкой, ради которой написана проверка выше, — только
  # на уровень глубже. Проверка на `rev-parse` её не ловит: дерево-то git.
  #
  # ЭТО НЕ УМОЗРЕНИЕ, а прямое следствие того, что записано в `box-jhub-l40s.sh`: `/var/tmp` не
  # переживает рестарт контейнера, и стенд восстанавливают КОПИЕЙ из снимка. Копия, которой
  # `git init` сделали заново (или в которой `results/` просто не попал в снимок), даёт именно эту
  # картину. Воспроизведено 2026-08-30 на синтетическом дереве: `check` напечатал «all foreign
  # result directories are closed» и вернул 0, пока `results/Foreign Model 9/convex_hull/solver.py`
  # лежал на месте; `close` сказал «closed 0» и тоже вернул 0.
  #
  # Отказ — только когда есть ЧТО классифицировать. Пустой `results/` нечем заразить, и требовать
  # от него отслеживаемых файлов значило бы ломать чистое дерево ради несуществующей опасности.
  [ -d "$AT/results" ] || return 0
  for D in "$AT/results"/*/; do
    [ -d "$D" ] || continue
    if [ -z "$(git -C "$AT" ls-files -- results | head -1)" ]; then
      echo "ОТКАЗ: в $AT/results есть каталоги, но git не отслеживает под ним НИ ОДНОГО файла —" >&2
      echo "  значит правило «отслеживается = чужое» не различает ничего, и забор был бы" >&2
      echo "  пустышкой с нулевым кодом возврата. Так выглядит дерево, восстановленное КОПИЕЙ" >&2
      echo "  (см. benchmarks/box-jhub-l40s.sh: /var/tmp не переживает рестарт контейнера)." >&2
      echo "  Верни настоящий чекаут форка (git clone / git restore), либо укажи его через" >&2
      echo "  FENCE_ALGOTUNE_ROOT. Сейчас в results/ лежит: $(ls "$AT/results" | tr '\n' ' ')" >&2
      exit 3
    fi
    return 0
  done
