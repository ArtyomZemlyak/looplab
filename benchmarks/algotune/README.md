...
| `run_arm.sh` | A PAIRED arm: N batches of four probes, two per condition, lanes crossed by batch parity so each condition sees each lane equally (§266 — the old arm nearly nailed its labels to its lanes). The size and the treatment are READ from `PREREGISTERED-<ARM>.txt` and cannot be passed; a snapshot sits beside the data and a mid-arm edit refuses; `arm.jsonl` records each probe's condition and lane BEFORE it starts. It computes no outcome, deliberately. `ARM=B1 bash run_arm.sh` shows the spend, `ARM_GO=1` runs it. |
