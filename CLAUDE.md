...
- **Duck-typed seams are REGISTRY-GUARDED** — a rename that used to break silently is now a red
  ...
  raising, which is how a Settings field went unwired for a week); `UNSETTLED_KNOB_DEFAULTS` beside them: a
...
- Settings are flat on purpose (`LOOPLAB_<FIELD>` env vars map 1:1); never nest or rename fields —
  ...
  `Engine.__init__`) cannot move it with a generic non-default, and a `tests/test_options_divergence.py`
