...
`run_cmds`/`export_cmds` (also `export-sft`: the run's answered turns as SFT rows, each carrying its node's outcome)/`inspect_cmds`/`concept_cmds`/`governance_cmds`/`memory_cmds`/`audit_cmds`/`ui_cmds` —
