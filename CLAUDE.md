...
python -m looplab.serve.api_reference   # regenerate docs/guide/api-reference.md from app.openapi() (pinned by tests/test_api_reference.py)
python -m looplab.events.event_reference # regenerate docs/guide/event-reference.md from EVENT_PAYLOAD_KEYS (pinned by tests/test_event_payload_contract.py)
```

The suite runs fully offline in ~40 minutes (pass `--basetemp=<private dir>`: pytest's
tmp root is keyed by OS USER, so two concurrent runs delete each other's fixtures); live-LLM tests auto-skip (opt in with
...
safe to contain>`. Containment is the house posture (744 such handlers), so the rule is not "do not
...
| Path | Contents |
|---|---|
| `looplab/core/` | Foundation; imports nothing above itself. `models.py` (`Idea`/`Node`/`RunState`; card IDENTITY in `cards.py`, re-exported so both spellings are the same objects; `RunState.halted` is the ONE spelling of paused-or-finished-or-stop-requested — never `CardSessionGates.stopping`, which is wider), `config.py` (`Settings` schema; `LEGACY_CONFIG_SNAPSHOT_DEFAULTS` pins a pre-field snapshot to the historical behaviour) + `appconfig.py` (loader), `llm.py` + `llm_streaming`/`llm_toolcall`/`llm_transient` (every split name re-exported through `llm.py`; `model_override` is the ContextVar every request site reads, so a routed build changes only the MODEL asked of the same endpoint), `llm_broker.py` (run-local concurrency ADMISSION by lane, borrowed around the real request only; `BACKGROUND_LANE_PRODUCERS` is the registry a capped lane's producers must be in), `llm_budget.py` (`RunBudget`: the run's spend as a RESERVE-COMMIT budget the broker meters at `borrow()`; `llm_cost_limit`/`llm_token_limit` 0 = off), `containment.py` (`contain(reason, exc)`: the countable contain-and-continue — stamps the span, counts, logs, and RE-RAISES `BudgetExceeded`, wrapped in an `ExceptionGroup` or chained as a cause too — `refuse_budget_stop(exc)` over `core/errors.py::budget_stop_leaf` is that rule for any handler; a blind handler whose try-block reaches a provider call through ANY function re-raises `BudgetExceeded` first, pinned by `tests/test_containment_census.py` bar its shrink-only `FUNNEL_BACKLOG`), `evidence.py` (the ONE untrusted-evidence envelope: `EVIDENCE_LABEL`, `untrusted_evidence_guard`, the idempotent `fence_untrusted`/`is_fenced`, `envelope_enabled` + `fence_kwargs` (the keyword a consumer spreads, ABSENT when off); every consumer defaults it OFF at its constructor because a prompt is a contract; `EVIDENCE_CONSUMERS` is every call site that hands a tool loop a toolset, FENCED (naming its driving test) or EXEMPT (with the reason); text reaching a prompt outside a tool result takes the same switch — `agents/established.py`'s carried pages, `tools/mcp_tools.py::model_facing_mcp_spec`, a fencing loop's compaction summary), `pathsafe.py`/`atomicio.py` (the single spelling of reparse/reserved-name/file-identity rules; `durable_no_replace_rename`; `rmtree_readonly_aware` removes a tree holding a hardened read-only file, which `shutil.rmtree` cannot on Windows; `contained_member` is RESOLVED containment of an agent-authored member name — a lexical `..`/absolute check misses a linked component and a Windows drive-relative name; `resolve_settled` is `Path.resolve()` asked AGAIN while a `\\?\` prefix the caller did not write stands — on Windows a name that does not exist yet keeps one when another thread creates its parent between `ntpath.realpath`'s two probes, so a containment test over such a name (`engine/workspace.py::WorkspaceSeeder.materialize`) resolves through it), `envsafe.py` (`is_secret_env` + the DECLARED ENVIRONMENT rule `validate_env_map`/`merge_env` shared by the run, task and stage levels — a secret-shaped declaration is REFUSED, never redacted), `redact.py` (`redact_persisted_text`, `redact_env_values`; `Settings.redact_output` gates only the ENTROPY pass — known shapes and this box's env values are always masked at every WRITE boundary — `env=` names whose, and `replay.fold` passes `env={}` so a replay is a function of the log; `secret_env_values()` caches THIS process's screen keyed on `os.environ`'s whole backing dict compared by value — any change is a miss on the next call; `engine/audit.py::Engine._redact` is the one funnel every persisted tail goes through), `research_record.py` (the durable research record's pure builders: exact-span `evidence_item`, `parse_literature`, the deterministic `bind_claims_to_evidence`), `phase_events.py` (the inner-agent phase moments and the `memory_read` record as DIAGNOSTIC events through an engine-installed ContextVar sink; a no-op outside a run), `jsonutil.py` (`canonical_json(_digest)`, `valid_digest_ref` — the ONE reader of the digest shape; never re-derive it), `setup_identity.py` (the two run-start digests), `numeric.py` (`numeric_params`/`knn_idw`; `size_bytes_or_error`/`parse_mem_bytes` — the ONE size grammar Settings refuses by and the sandbox enforces with), `fitness.py` (`SearchFitness`; `counts_toward_best` is the ONE rule for what counts toward the best — the champion, the card record setters/evidence and the trajectory share it), `param_carriers.py` (what number a committed configuration DOCUMENT assigns a dotted path — YAML/JSON composed trees beside Python's `ast`; a document declaration naming two leaves is REFUSED), `jsonlio.py` (generic JSONL store I/O, re-exported by `events/eventstore.py`), `tracing.py` (`generation` delta-encodes the input over the RAW message list — each message sanitized once, when appended; `SpanHandle._set_sanitized` is the trusted write for values the tracer itself just sanitized; `retained_input_window` is the 64-message / 64 000-char window `events/traceview.py::hydrate_inputs` applies at READ time), `trace_files.py`/`trace_append.py` (the descriptor-first trace-sidecar boundary and the append receipt), `run_identity.py` (`run_ref` for GROUPING vs `row_belongs_to_run` for cascade ATTRIBUTION — deliberately not one rule; `tests/test_run_incarnation_identity.py` refuses a new comparison of a row's `run_id` in engine/core/trust), `memory_window.py` (the ONE bounded-tail snapshot rule for the cross-run JSONL stores; its index is window-relative), `claimpin.py` (`CLAIM[...] decided:` evaluation and `citation_defects()`), `node_evidence.py` (`read_bounded_regular_file`: the ONE reader of a file a candidate can write — `O_NOFOLLOW|O_NONBLOCK`, fstat regular, bounded, `None` never raises; `node_workdir` refuses a linked node dir), `profile.py` (the pure data profiler / leakage front-end). |
| `looplab/events/` | The event store and `types.py`, the event-type registry — add every new type there (`tests/test_event_types.py` partitions each into a fold handler or `DIAGNOSTIC_EVENTS`) WITH its `EVENT_PAYLOAD_KEYS` row (what the payload carries; `required` is checked against every literal writer, `stored_whole` re-derived by folding a marker key (the AST scan only nominates), and `docs/guide/event-reference.md` generated from it — invariant #5 is driven there, by folding every type with an empty payload); the writer seams `BACKGROUND_APPENDABLE` / `SETUP_THREAD_APPENDABLE` / `ASSISTANT_APPENDABLE` / `NON_CARD_SELECTION_BACKGROUND_APPENDABLE` (see invariant 1); `replay.py::fold` (event log → `RunState`; `_HANDLERS` is the UNION of `replay.py`'s own rows and each handler FAMILY's `HANDLERS` table — `replay_concepts` / `replay_journals` / `replay_cards` / `replay_selection` / `replay_requests`, over `replay_ctx` (`_FoldCtx` + the lifecycle-generation rules); a family never imports `replay`, a type two tables claim is an import-time `assert`, a moved name stays importable from `replay` but a PATCH must name the module that reads it, and a guard reading the fold's source goes through `tests/_source_scan.py::fold_source_paths` (`tests/test_replay_families.py`); `Node.stages` is the per-NAME last-wins projection and `Node.stage_attempts` the per-ATTEMPT ledger beside it, `stage_wall_clock()` its sum); `card_ledger.py` (the derived Card ledger, a LEAF that imports only `core`); `span_index.py` (the light span index; its `_anchored` is the ONE `?before=` seek rule and an unplaceable anchor is REFUSED at the boundary); the pure UI projections `traceview.py` (incl. `claimed_build_traces`, the one reading of a node's claim on the run-scoped trace that BUILT it), `htmlview.py`, `comment_projection.py`, `belief_projection.py`, `authoring_projection.py`; `trust_gate.py::apply_trust_gate` (the ONE write policy for `trust_gate_changed`, shared by the config PUT and the assistant tool — it returns an outcome and phrases no refusal); `notebook.py` (an export); `finalize_scope.py` / `finalize_protocol.py` (the finalization step vocabulary and `QUIET_FINALIZATION_SUFFIX`, which `engine/finalize.py` writes and `search/speculation_quality.py` checks — never hand-sync a copy); `prior_citations.py` (the prior citation-rate instrument over `prior_injected` + `memory_read`, `looplab prior-citations`); `state_delta.py` (the SSE stream's delta frames: `diff`/`apply` over two folded payloads, lists atomic, keyed on the seq the client last saw, every key data — `__proto__` included; `ui/src/stateDelta.js` is its browser twin, and both apply `tests/fixtures/state_delta_cases.json`, generated by `tests/test_state_delta.py`); `trajectory.py` (`running_best`: the run's running best as bounded CHANGE POINTS `[experiment, best, node_id]` on the summary row, derived from the fold the row is built from — the cross-run overlay reads it, never a fold per run); digest, exporters; `readmodel.py` (the SQLite sidecar whose watermark digests the ordered `(seq, type, ts, data)` prefix, so a row edited in place reads `stale`). |
| `looplab/runtime/` | Process execution ONLY; imports nothing above `core`. `sandbox.py` (subprocess/Docker tiers; `eval_deadline_env` is the ONE derivation of `LOOPLAB_EVAL_DEADLINE` + `LOOPLAB_EVAL_TIMEOUT_S`, set by `run_argv` via `setdefault` so a declaration wins), `command_eval.py` (staged command evaluation; `validate_stages` is the single definition of a valid stage manifest — `role: "training"` on at most one stage is what grants a live-watchdog kill authority; `expect.numeric` is the model-free contract (`runtime/numeric_contract.py`: a declared relation held to the last value the stage printed, fails closed, never salvaged); `epoch_floor_acquits` is the deterministic floor under `declared_condition_violated` and may only ever ACQUIT; `READER_PATH_KEYS` registers which spec keys become filesystem paths; `READER_KEYS` is each reader's whole vocabulary — `metric_spec_vocabulary_error`/`metric_spec_pattern_error` refuse at submit a gate reader nothing reads and a regex reader with no usable pattern; `HOST_STAGE_KEY` is engine-stamped, never declarable), `read_fence.py` (the source-tree READ FENCE: a per-run `sitecustomize` audit hook that REFUSES `open` under an editable source root with a non-`OSError` exception on purpose, watches `os.chdir` + the `MUTATION_EVENTS` family, and refuses WRITES under the run dir outside the launch's own workdir; the fence FILE self-protects — `harden_guarantee` states which half of `_harden` actually binds (mode bit + CAP_DAC_OVERRIDE) and `reassert` restores the file from the engine's in-memory copy at every `run_argv` launch, clearing `__pycache__` first because a pyc whose header copies the live source's mtime shadows a pristine file; a PATH fence, not an inode one, for measured cost reasons), `landlock.py` (the kernel twin: an opt-in READ allow-list `Settings.landlock`, and `NO_MUTATION_HANDLED`, always on for the Developer probe; masks follow the kernel ABI (`allowlist_masks`/`no_mutation_handled`, FS_TRUNCATE from ABI 3) and a FILE mount is a file rule (`rule_access`) — both refuse with `EACCES`, so both sit BESIDE the hook, never instead of it), `seccomp.py` (the SYSCALL rung on Sandlock's shape: a classic-BPF filter by an exec'd launcher, `Settings.syscall_fence` off|mutators|egress — `mutators` refuses `mknod`/`mknodat`, `egress` also `socket(AF_INET|AF_INET6)` LOOPBACK INCLUDED because a filter never sees the address; `NO_MUTATION_FUNCTION` always on for the probe; a kernel `EACCES`/`EPERM` is rewritten into the fence's sentence by `engine/failure_diagnosis.py::fence_refusal_note` at the triage intake and the repair headline), `read_allowlist.py` (the ONE derivation of what an eval may read, from the operator's declarations), `applied_params.py` (the metric's COORDINATES: what the configuration that actually ran said the declared `Idea.params` were worth — `committed` carriers vs the `resolved` config elected by `eval.metric.applied_config_glob`; SURFACES, never refuses; a conflicted coordinate rides in `conflicts` and raises the champion caveat), `metric_subject.py` (the metric's REFERENT, bound at the score stage's start; `subject_glob` binds ONLY on a unique match), `stage_identity.py` (`stage_input_key` + `stage_outputs`, recorded on every `stage_finished` row and read by nothing that decides — the cross-node reuse cache was measured a corruption primitive and REFUSED; `looplab stage-dups` is the instrument), `deps.py` (text may NOMINATE a distribution, `is_present` DECIDES by probing the eval interpreter). |
| `looplab/tools/` | Agent-facing tools. `_base.py` is the ToolProvider contract and the shared bounded-output rules (`clip`, `fit_rows`, `stream_tails`, `RESULT_CAP`): every agent-facing reader states the range it covered and the call that continues past it, and that call must be one the caller has NOT already spent. `env_inspect.py` (read-only env inspector) and `dev_probe.py` (`run_probe(code)`: a Python program against the REAL environment inside a disposable replica — no read of the editable source, no write anywhere (audit hook + Landlock `NO_MUTATION_HANDLED` + `RLIMIT_FSIZE 0` + the seccomp `mknod` filter), no new program, no GPU; the interpreter IS the surface because it is what the fence can cover). `log_tools.py` (`read_log` / `metric_series`: the live-eval judges LOOK at a named stage log instead of a slice; a search SWEEPS to the end of the log with an exact total; four roles use it through `train_monitor.monitor_log_tools` / `repair_log_tools` / `stage_check_tools` / `diagnosis_tools`). `skills.py` (bounded, addressable, tiered: `render_skill_body` keeps whole sections under `SKILL_RESULT_CAP` and names what it left out beside the `use_skill(name, section)` call; `skill_tier` = declared `tier:` else hand-written `global` / promoted auto `domain` / other auto `task`; `parse_skill_fingerprints` is the ONE parser of that frontmatter field, shared with `engine/memory.py`'s writer). `memory_tools.py`, `cross_run_tools.py` and `skills.py` each report a `memory_read` record through `core/phase_events.py::emit_memory_read`. `clock.py` (`remaining_time`: the agent's own clock, published by `drive_tool_loop` before every tool execution). `node_diff.py` (what separates two nodes; `EDIT_TYPES` + `classify_line` are the deterministic edit-kind vocabulary and `reintroduced_lines` the first-parent cycling detector behind the `edits` section and `looplab edit-types`). `run_tools.py::ForeignRunReader` carries the EVALUATION-CONTRACT receipt beside a foreign run's number (`engine/eval_contract.py`) and annotates, never withholds; `_research_memo` is addressable by `section=`. `vectorstore.py` + `memora.py` (embeddings + harmonic index); `reposcout.py`. `perm_modes.py::clip_approval_preview` is the ONE bound of an approval card's preview: a cut is SAID (what is not shown; approving applies all), never silent. `write_tools.py::_publish_bytes` is the ONE publish for the approved write/edit/delete and both Undo restores and a failed patch's rollback (`_pop_patch_snapshot`): a descriptor walk from the approved root with `O_NOFOLLOW` per component, so the pathname the containment proof was made on is never resolved again (Windows: by name plus `_ancestors_are_plain`). |
| `looplab/agents/` | LLM personas. `roles.py` (the plain roles; `_state_brief` is the PUSH channel of the deep-research signal, prefixed by the memo's own claim tally under `Settings.memo_verdict_cue`; the registries `DEVELOPER_OUTPUT_ATTRS` / `RESEARCHER_ACTION_ATTRS` / `RESEARCHER_HINT_ATTRS` / `LLM_PRESENCE_ATTRS` / `FACADE_STAGE_ATTRS` / `WRAPPED_ROLE_ATTRS`; `DeveloperResult` is the envelope every build reads its outputs off, captured under the instance's lock, never off the shared instance afterwards; `ValidatingDeveloper` forwards `implement_from(..., co_parents=)` only to an inner that accepts it), `providers.py` (the providers every agentic role shares — keeps every `search`/`tools`/`agents` import function-local), `tool_loop.py::drive_tool_loop` / `agentic_*` (the shared loop: history compaction, stuck detection, the `LoopOptions` bundle vs `EXPLICIT_ONLY_LOOP_ARGS` partition — a keyword reachable both ways silently degrades the Researcher to non-agentic; it publishes the `LoopClock`, reports the three `agent_phase_*` moments through `core/phase_events.py`, and fences every tool result when the envelope is on; compaction pins the request the loop was HANDED by identity (`core/context_budget.py::_pinned_request`), not only the first user turn — the wrappers and `structured_judge` carry it as an explicit `tool_result_label`; `answered_transcript` is the ONE re-send rule (no fallback re-sends an unanswered tool_call_id); `bound_toolset` hands each call a PER-CALL VIEW, never a shared toolset rebound under a concurrent caller), `agent.py` (`run_phase` + re-exports; `looplab.agents.agent.drive_tool_loop` is THE documented monkeypatch seam), `deep_research.py` (the memo, its plan/evidence/literature captured on the memo FROM THE FIRST TURN so a junk emit keeps the record), `unified_agent.py` (the facade; `_TRIAGE_EVIDENCE_GUARD` / `_REPAIR_CRITIC_EVIDENCE_GUARD`; the emit caps are decided per consumer, not inherited), `strategist.py` (`RuleStrategist` + the LLM/agent variants; `stall_rung` is the stall's identity; `validate_strategy` is the paranoid whitelist; `Strategy.operators.endgame_sweep` is the plan reserve's sweep switch), `cli_agent.py`, `preflight.py` (an unreachable endpoint REFUSES the run before any role is built). |
| `looplab/search/` | Search policies (`policy.py`: action kinds/meta keys, `GreedyTree`, `legal_actions`, `operator_yields` + `_bandit_pick`, `eval_cost_penalty`/`subtree_eval_cost` — the cost-constrained MCTS term under `Settings.mcts_cost_weight`, relative to the run's own mean eval second, unmeasured = average, inert at 0 — and the operator × MODEL router — `parse_model_arms`, `model_arm_yields`, `_model_arm_pick` with the gain divided by the arm's relative cost, stamped as `META_MODEL`; `policy_knobs` is the ONE builder of a policy's run-level knobs — launch and every Strategist rebuild, `RUN_OWNED_POLICY_KNOBS` the keys `policy_params` may not restate; inert without `operator_bandit` or a declared arm), `operators.py` (`merge_idea` does ARITHMETIC on `idea.params`), best-of-N, `surrogate.py` (k-NN BO-lite; `acq = pred ± explore × nearest`), `panel.py` (K LLM ideas ranked by that same ACQUISITION — the stack wires `Settings.surrogate_explore`; a bare `PanelResearcher` keeps the point estimate), `researcher_stack.py` (the ONE researcher-wrapper stack: `wrap_researcher` = the launch pair; `with_surrogate` = the surrogate layer's rule for the launch, the pool and a mid-run BOHB switch, decided on the OBJECTS never the raw `unified_agent` flag; `pooled_researcher` = the primary's FREE layers only — a panel on a pooled pair is a spend change with no writable receipt), `proxy.py` (the pre-eval kill; `should_skip` ABSTAINS when the candidate's nearest neighbour is beyond `support_radius`; `pairwise_accuracy` is the kill's own measured ordering accuracy over the candidates it LET THROUGH — `looplab proxy-accuracy`), `archive`, `foresight.py`, `hybrid_merge.py`, `card_selection.py` (`card_next_actions`, `_speculative_selection`; `_asha_mask_is_unsound` is consulted at the ONE lane that hands the policy a masked view), `coverage.py`, `lock_in.py`, `speculation_quality.py` (the paired-run calibration BENCHMARK — an ORDERED pipeline whose order IS the receipt; a changed derivation revokes every issued receipt), and the PART IV/V concept cluster in strict layer order `concept_graph.py` ← `concept_tagging.py` / `concept_lens.py` (`run_constant_split`, `project_concept_map`) ← `concept_analytics.py` ← `concept_map.py` — no re-export façade, and a cluster module reaches a sibling's functions through the module object so monkeypatch seams survive. `search` may import `agents` at module level; `agents` reaches `search` only through a deferred import. |
| `looplab/trust/` | Gates that keep results honest, in three tiers. DETERMINISTIC (pure Python, no model): `leakage.py` (`target_leakage` gates on Pearson OR rank; `categorical_leak` is the third, ADVISORY rung — group purity and a binned correlation ratio over the residue, reported and read by nothing; `multi_test_scan` since 2026-09-06 (doc 52 row 22): repeated evaluation on the TEST split followed by selection on those scores, AST-based, both halves required), `reward_hack.py`, `cv.py`, `gate.py` (the >1-SE rule), `confirm.py`. MODEL-BACKED and advisory: `critic.py`, `verifier.py`, `memo_verify.py` (D8: a synthesis claim and its check are decoupled; `finalize_verified_evidence` consults `unreliable_metric_ids` and refuses with a distinct reason; `provenance_coverage` is the per-section share of the memo's SYNTHESIS that names evidence the memo can resolve — recorded on every memo, read by nothing that decides), `lesson_guard.py`, `judge.py` (the ONE structured-judge invocation — the two verifiers, both live-training judges, the value estimate, the MLE-bench rule judge and BAITBENCH call it; `structured_judge(tools=…)`). Plus `findings.py` (the single finding shape), `harden.py` (hacker-fixer-solver), `cross_run.py` (bounded, always-redacted projections; `cross_run_text`). Whether any flag CHANGES selection is `Settings.trust_gate`, not this package (`audit`, the default, only surfaces). Redaction is NOT here — `core/redact.py` owns it. |
| `looplab/engine/` | **The orchestrator loop** + cross-run memory; see the invariants below. `Engine` spans `orchestrator.py` (the `run` spine and its create/dispatch decisions, the module-global `fold` seam every helper that folds must reach through — a direct import in another file is a SILENT narrowing of every existing monkeypatch; the loop-exit signal vocabulary is CLOSED; `_steady_state_build_lane` is the opt-in refilling build pool beside the chunk barrier — same main-task proposal and reservation, the join moved) and 27 mixins: `shared.py` (members every cluster uses and none owns: `_agent_may`, `_op_span`, `_cadence_due`, the two eval-timeout rules, `judge_evidence_kwargs` — the envelope's ONE reading for every engine-side loop over candidate text (judges, memory authors, the memo verifier), `{}` when off), `knobs.py` (`EngineKnobs`: every knob that is a pure function of ONE `EngineOptions` field, declared once as a NON-DATA descriptor — `settle_knobs` lands them in the instance dict at construction, an instance value always wins, an object that never ran `__init__` reads the library default; `EXPLICIT_IN_INIT` names each knob that lands explicitly, and why; `self.options` is the frozen launch record), `node_build.py` (the node-creation spine since ENG1-04 step 4c — `_create_node` → `_create_node_scoped` → `_commit_built_node`, the rerun and inject builds, `_offload_build`/`_reserve_on_main_task`; `_implement_result` with `co_parents`, `_emit_node_created` — the single emitter, optional keys LEFT OUT not None-filled; `_run_developer` is CLEAR, call, capture as one step under `developer_call_lock` — a build site that clears the footprint itself is an unlocked write to a shared Developer, and since the serial build left the loop thread that write lands inside another caller's window), `card_reservation.py` (the native-Card reservation ledger; every fold through `_fold` → `orchestrator.fold`), `speculation.py` (the producer lanes; `CARD_BUILD_SKIP_REASONS`; `_proposal_receipt_fence` fences a paid proposal on what its receipt asserts and excludes `DIAGNOSTIC_EVENTS` wholesale), `speculation_gate.py` (the calibrated speculation ENVELOPE as statable free functions; `engine_authored_artifacts` is the ONLY asserter of the `engine` extra-metric channel), `evaluate.py` (`_evaluate` is a DRIVER over `EvalAttempt` — the slots record of one lifecycle's evaluation — and ten `_eval_*` phases in one order: admit / recover_settled (`settled_recovery.py`: an `ok` settle with no terminal is finalized from its recorded result, never re-run) / prepare_workdir / seed_ledgers, then per attempt run_attempt -> settle_outcome -> salvage -> decide_repair -> apply_repair, and write_terminal; a phase reports the loop control it cannot execute as a CLOSED `PHASE_*` signal (`tests/test_eval_attempt_phases.py`), a guard over "the attempt loop" reads `tests/_source_scan.py::eval_attempt_source()` (the phases in driver order) or names its phase; all nine node terminals inside `_write_lock`, and nothing is AWAITED under it (the substrate is read before it, prepare_workdir runs in a worker — `tests/test_terminal_write_lock_is_not_held_across_io.py`); ADMIT takes the SERIAL dispatcher's own fold through `handed_admission_fold` (task-local, consumed once, tail-gated — a hand-off, never a shared memo); the proxy kill reads the k-NN pair; the phases' repair decisions are the pure rules in `eval_attempt_rules.py` — `repair_gate`, `triage_verdict_outcome`, `evaluated_terminal` — pinned byte for byte by `tests/test_repair_loop_golden.py`), `eval_canary.py` (the opt-in `Settings.eval_canary` preflight: RUN_ATTEMPT runs the node's own chain on the task's `eval.canary` slice in `<run>/canary/node_N` before the full eval; a failure is the attempt's crash and is never salvaged, a pass is remembered per code digest on the diagnostic `eval_canary_finished` row; its number is never the metric), `eval_dispatch.py` (`_dispatch_evals`, the non-Card dispatch, with the admission rules only it reads — `_eval_admission_current` & co.; the Card lanes import them function-locally), `eval_stages.py` (`_resolve_stages` appends the host-side `score` stage; `manifest_prefix_unchanged` is the per-ENTRY reuse clause), `crash_repair.py` + `triage.py` (`TRIAGE_ACTIONS`; `TRIAGE_RATIONALE_CAP` is the intake bound), `failure_diagnosis.py` (WHO may say what a failed eval failed of: `ENGINE_FINAL_REASONS` vs `DIAGNOSED_FAILURE_REASONS`, `unclassified`; `coerce_hypotheses` is the alternatives the diagnostician considered under `Settings.diagnosis_hypotheses` — recorded as `reason_hypotheses`, read by nothing that decides; the diagnostician IS the triage call and its verdict RECORDS evidence, never refuses on it), `repair_verify.py` (`REPAIR_VERDICTS`: `inert` decided on BYTES is the only verdict the loop acts on; `verified` means "the repair's vocabulary appears in what it changed", never "it did what it said"; `declared_param_overrides`), `metric_salvage.py` (deterministic rungs only; a salvaged node carries `metric_salvaged` and is out of `feasible_nodes` unless `metric_salvage_repair` re-measured it), `champion_caveats.py` (the caveats that SURVIVE selection, incl. `params_overridden`; `mislead_gap` is the Protocol Validity pair on the run row — the champion beside the node the fold's own selector (`events/replay_selection.py::select_best_node`) crowns over `promotion_eligible_nodes` minus hard flags and select-admitted salvages, and their gap; 0 on an intended champion), `train_monitor.py` (the live-log judge + the tick both watchdogs share; re-exports its pure halves `loss_trajectory`/`monitor_gates`/`eval_log_plan` as the SAME objects — patch where a name is READ; `LossTrajectoryTracker` may only VETO; `should_monitor_repair` hands `not_learning` to the inline repair loop; kill authority needs the declared `training` role, SPENT once the stage's artifact exists; `stamp_projected_overrun` rows a healthy stage the clock says cannot finish), `asha_monitor.py`, `research_cadence.py` (`_record_deep_research` appends the memo and `literature_retrieved`; the concurrent overlap `_spawn_research`/`_research_overlap_loop`), `strategy.py` (consult due on cadence AND on a plateau via `cadence.py::plateau_due`; `_apply_strategy` gates every knob on the governance matrix), `concept_cadence.py`, `verifier_tiebreak.py` (SELECTION machinery), `confirm_phase.py`, `noise_floor.py` (the eval NOISE FLOOR under `Settings.eval_noise_seeds`: N repeats of ONE candidate on the search's own protocol, `sem` on the >1-SE rule's own scale, recorded and read by nothing that decides), `ablation.py`, `novelty.py` (`_capture_proposal_events`: the offloaded proposal buffers its folded intents for the MAIN task; `BatchProposal` is `_propose_batch`'s WHOLE result — nothing of a batch lives on the engine, and the gate capability reaches `_prepare_node_idea` only as `already_gated=`; `literature_overlap`/`_literature_note` put the run's own retrieved papers on the novelty audit rows under `Settings.novelty_literature` — evidence, never a verdict, and its recall is a stated floor), `audit.py` (`_redact` funnel, `_append_phase_event` sink), `resources.py` (the host GPU-pool lease), `proposal_cues.py`, `plan.py` (the PLAN artifact: `build_plan` cuts `max_nodes` into seed/search/endgame from `endgame_reserve_frac`, `endgame_actions` is the ONE gate `_plan_gate` applies to every selected action set — top-2 ensemble once, then champion sweeps proposed by the k-NN surrogate — `replan` re-cuts on a budget change or a hard stall), `cadence.py` (`cadence_due` = the since-last node-count gate, `occupancy_due` = the eval-occupancy pace, `at_creation_boundary` = the precondition every node-count consumer shares; a third pace must record no `at_node`), `widths.py` (the live width settling rule and `per_experiment_gpu_budget`; pure — `width_settling.py::WidthSettlingMixin` is the Engine half that applies it: `_settle_proposal_width`, `_apply_control_overrides`, `_reconfigure_llm_broker`), `reentry.py::ReentryMixin` (invariant #6 at both ends: the `run_started` pin values beside the re-entry checks that adopt or refuse them — `_repin_settled_widths`, `_repin_declared_env`, `_require_pinned_speculation_receipt`, `_reentry_repin` — and the `RunStartPinError` family, which `orchestrator.py` re-exports as the SAME classes), `forced_requests.py::ForcedRequestsMixin` (the operator's forced steering served from its durable queues — fork, inject, forced ablation, confirm), `setup_phase.py::SetupPhaseMixin` (the one-time preflight that appends `run_started`; `_dirty_inputs`' two constants and `_task_declared_env` are spelled there and nowhere else, so a test patches them on `setup_phase`), `costs.py` (the durable `llm_usage`/`llm_cost` ledger; `seed_run_budget`; `persisted_usage_deltas` is the fold's reading of that ledger, the one both resume seeders use), `finalize.py`, `holdout.py` (the withheld scorer and the private grade run in a worker; `holdout_evaluated.eval_seconds` is charged once per lifecycle to the `holdout` eval bucket), `eval_contract.py` (may this run's number be read on that one's scale — tri-state, `None` never a guess), `memory.py` (auto-skills: `next_auto_skill_status` is the support edge, `reconcile_auto_skill_statuses` the contradiction edge) + `lesson_hygiene.py` (`lesson_id`, `lesson_utility`, `filter_useless` beside `filter_contradicted`) + `lessons_priors.py` (`_pick_role_prior` returns the text AND the receipt `record_prior_injection` writes) + `lessons_distill.py` (writes `lesson_utility.jsonl`) + `lessons_reconcile.py` + `concept_capsules.py` + `concept_shelf.py`, `curation_protocol.py` vs `steward_invocation.py` (two paid-curation writers, deliberately not one), `governance_health.py` (`_validate_curation_row` stays TOTAL over every row generation), `memory_stores.py` (`MEMORY_STORES`: the ONE row per store under `memory_dir` — row key, writer, deletion policy CASCADED or PRESERVED-with-reason; `memory_cascade.CASCADED_TIERS`/`PRESERVED_TIERS`, `governance_health`'s ledger tables and `CROSS_RUN_SOURCE_NAMES` are its views). In a mixin `self` IS the Engine — grep the engine package before renaming an engine attribute. `attribute_sites.py` is the declaring-site registry behind `tests/test_engine_attribute_sites.py` (doc 52 row 21): a read of an attribute nothing assigns is a red test, not a default. `bundle.py` is the RO-Crate reviewer bundle (`looplab export-bundle`: the run's own record copied and digested, nothing re-derived but the summary row; here and not in `events/` because it composes the fold with the engine's derivations). |
| `looplab/adapters/` | Task types (toy → dataset → repo → MLE-bench); the TaskAdapter contract is documented in `adapters/tasks.py` (`TASK_OPTIONAL_HOOKS` is the registry). `perception.py` (the ONE home of the bounded on-disk data readers shared by `dataset_task.py` and `repo_task.py::columns`/`data_samples`: the primary table per declared `data:` mount, 200 rows, ≤4 tables, ≤64 columns, never executing anything). `repo_task.py` (`DataSpec` per-source permissions — only `mount`/`edit` are enforced; `HostScorerSpec`, the host-side `score` stage; `eval_source_tree_command_paths` is a submit-time WARNING). `repo_developer.py` (the repo Developer's four phases; `co_parent_block` renders an ensemble's other parents — traces and the files that differ; `_scout_tools` is the one point all phases compose; `_time_budget_note`). `repo_write_tools.py` (the manifest-collision rule). `mlebench_real.py` + `mlebench_split.py` (the search is scored on an agent-invisible split; `holdout_fraction=0` is the explicit legacy protocol). `mlebench_extras.py` (the two official MLE-bench extras as POST-RUN instruments: the rule-violation judge against `MLEBENCH_RULES` + the task's own description, and the Dolos plagiarism pass; a record in `mlebench_extras.json`, never a selection — `looplab mlebench-extras`). `mlebench_campaign.py` (the ≥3-seed mean ± SEM table off each run's own record, the Mislead-adjusted column BESIDE the raw one — a number only on the legacy `holdout_fraction=0` protocol, since the gap is on the SEARCH scale and the default grades the champion privately; `adjusted_scale` is TOTAL, so an empty cell always states its cause; `mlebench_grade.py::percentile_rank` is the leaderboard percentile every official report now carries). |
| `looplab/serve/` | FastAPI server; `server.py` is a thin composition root, routes live in `serve/routers/*`. `http.py` (`REFUSALS`: every route answers an unreadable input with a coded 503, never a 500 or the `OSError` text; a GET read fence takes the generation through `run_commands.py::readable_run_generation`, never the writer-side `run_generation`; no GET — body or same-module helper — WAITS on the exclusive sequencer except `start_status` (a literal `timeout=0` try is not a wait)). `run_commands.py` (the durable command lifecycle) with `control_validation.py` split out (`normalize_control` + `ControlSpec` rows joined from FIVE tables asserted equal to `protocol.py::CONTROL_EVENTS` at import; the direction is one-way). `router_wiring.py` (`LATE_BOUND_ROUTER_CALLABLES`: no router imports a router; `mount_routers` refuses a live consumer without its producer). `durable_op.py` (the RECEIPT tier; `refuse_unless_quiescent` shares the probe set, never the words), `paid_work.py` + `paid_ledger.py` (the paid-work claim→terminal ledger; `conflict_policy` is an explicit spec field; `strict_fsync` may not be re-bound by a router), `authoring_store.py` / `llm_probe.py` / `memory_projection.py` (the services `routers/misc.py` calls: the authoring receipt store, the paid LLM-health probe with its app-local replay registry — NOT a `paid_ledger` spec, no run owns it — and the Memory panel's read model; misc.py defines no class but a pydantic shape and reaches no lock/durable-write primitive, `tests/test_misc_router_split.py`), `memory_cascade.py` (what a run's deletion may remove from the shared stores `engine/memory_stores.py` marks cascaded, keyed on `run_uid`; `orphan_survey` for out-of-band removals), `service_reaper.py`, `lifecycle.py::ServerLifecycle` (the app's ONE `lifespan=`: ordered `(name, step)` startup/shutdown lists — the resume-timer cancel precedes the reaper; a failing shutdown step never costs the later ones and still raises; FastAPI never runs an `@app.on_event` hook on an app given `lifespan=`, so none may be added), `engine_proc.py` (keeps each spawned engine's `Popen`: `child_exited` answers liveness — and reaps — before any OS probe; the kill-on-exit reaper arms only on `jupyter.py::REAP_ON_EXIT_ENV`, which the hub launcher alone sets — `_on_shared_hub` decides AUTH, never reaping; `spawn_snapshot_refusal` answers at ADMISSION what a spawned resume/finalize child would refuse in `config.snapshot.json`, through the one read both use, `core/config.py::read_config_snapshot`; every log-ledger spawner — `_claim_and_spawn_resume` and the helpers over it — takes a REQUIRED `spawn_inflight`: the launch-in-flight HANDSHAKE appends the resume claim THEN reads the command lease, while a command worker writes its lease THEN reads the claim, `engine/run_lifecycle.py`), `assistant_watch.py` (the durable standing watch; the TRIGGER is evaluated by the server, a wake-up runs at the mode pinned at arming; every read-modify-write holds `.watches/.store.lock` across processes — `capability_store.py::polled_store_lock`, also `SessionStore`'s per-session fences — and a claim names its store's owner lease, so a restart settles only a `waking` record whose lease is free), `launch.py::validate_launch` (`POST /api/validate`, the same funnel `/api/start` refuses through), `owner_token.py` (an unset `LOOPLAB_UI_TOKEN` on the shared hub origin FAILS CLOSED with a minted token), `principal.py` (WHO is asking — `owner` / `local` / `review` / `anonymous`, stamped by the auth middlewares; `portfolio_access(principal, settings)` is the ONE decision that mounts the cross-run providers, a property of the party and never of the process; a standing watch pins the party that armed it), `settings_ui_schema.py` (the curated catalogue; every `Settings` field is a row or a written-down omission), `run_projections.py`, `appstate.py::TraceFacts` (the trace family's narrow read: immutable values of one fold per log identity — never a `RunState`, never the public payload), `scope_report_store.py` / `scope_actions.py`, `jupyter.py`; never imported by the engine. `api_reference.py` writes `docs/guide/api-reference.md` from `app.openapi()` and the guard pins `(method, path, deprecated)` against the live app — a route lands documented or red. `tests/test_route_coverage.py` holds every live route to a RECORDED request that reached it (`tests/_route_recorder.py` -> `tests/data/route_coverage.txt`, merge-recordable per file): a route lands exercised or red. |
| `ui/` | React control plane (built artifacts served by `serve/server.py`). The house pattern: a **pure model beside its React half** — transitions in a plain ES module `node --test` can drive, the hook/component keeping only the choreography. Pairs: `resourceModel.js`/`useScopedResource.js`, `runCommandMachine.js`/`useCommandStatusPoll`, `narration.js`/`Dock.jsx`, `stageAttribution.js`/`Inspector.jsx::StagePipeline` (a superseded stage row draws `STAGE_SUPERSEDED_ICON`, never its old outcome), `runStateModel.js`/`useRunState` (+ `stateDelta.js`: a `state_delta` frame applies only to the exact snapshot the connection holds, else a full resync; a frame over `RUN_STATE_MAX_FRAME_CHARS` DEGRADES the owner connection to the `/lifecycle` probe + `/state` re-read, never a reconnect loop), `traceClearModel.js`/`useTraceClear.js`, `forkFromSeqModel.js`/`ForkFromSeqPanel.jsx` (branch-from-history is `inject_node` with a stamped `forked_from` receipt, a CONTENT compare-and-swap; the only node action a historical snapshot offers) + `forkProvenance.js`, `traceScrollModel.js`/`useTraceScroll`/`TraceReach` (the reach button is VISIBLE; `?before=` moves the window, the server ceiling stays), `traceEpisodeModel.js`/`TraceEpisodes`, `conceptForest.js`/`PortfolioConcepts.jsx`, `crossRunRank.js`/`CrossRunPanel` (ranks only inside one (task, direction) group, competition ranks, ties named; `trajectoryOverlay` draws ONE group's running-best change points per `charts.jsx::MultiTrajectory`, off the run row's `trajectory`), `claimsCurationModel.js`/`ClaimsCuration.jsx`, the `assistant*Model.js` family beside `AssistantBar.jsx` (whose three layouts stay render functions; its coverage is source pins, so `test/assistantBarResourceTruth.test.js` SSR-loads the module, plus one LIVE mount, `test/assistantTranscriptRenders.test.js`, that streams a reply and counts `Turn` renders: `Turn` is memoized on `assistantTranscriptModel.js::turnPropsEqual`, so a prop handed to it keeps its identity across a chunk — a handler goes through `useLatestHandler`/`latestHandlers`, never an inline closure). The three largest components MOUNT through `test/_mount.js` (the `cardKanban.test.js` pattern + jsdom globals + a path-keyed `fetch` stub): a static render runs no effects, so each `mount*.test.js` asserts one prop-decided gate flip AND that nothing was fetched; `mountLive` is the live half (effects run, a data flow is driven end to end — `test/runViewLiveState.test.js`: `/state` → `state_delta` → the workspace on one connection), and a file needing several Vite loads takes `sharedVite()` (one server per test file, a fresh module runner per caller), never a server per test. A test that needs a timer to FIRE uses node:test's `mock.timers`, never a production delay compressed into a shorter real one (`test/useAttentionDeadline.test.js`). `api.js` is a BARREL over `apiClient.js`/`commandModel.js`/`commandProtocol.js`/`commandStorage.js`; the one narrow exception is `reviewRouteApi.js`. `runRouteState.js::inspectorTabs` is the ONE Inspector-tab rule: RunView heals the URL with it for the DURABLE modes only, the Inspector draws its strip with it. Build into a STAGING dir (`--outDir .dist.stage`) when a live `test_server` is running. |
...
1. **The engine is the sole writer of domain events.** Background tasks return values; only the
   ...
   append `DIAGNOSTIC_EVENTS` (fold-ignored AND excluded wholesale from every seq-equality fence — the fence-neutral set is `events/types.py::FENCE_NEUTRAL_EVENTS`, diagnostics minus `finalize_step` plus `llm_usage` —
   `card_reservation.py::_proposal_receipt_fence` discards a paid proposal when what its RECEIPT
   asserts moved — NOT a whole-log max-seq, which is what `_proposal_authority_seq` was and which
   now fences only `_reserve_node_build`'s CAS retry, where no paid proposal is at risk. Before
   that, a non-diagnostic
   ...
   now runs in a worker through `node_build.py::_offload_build`, and its outputs are read off the
   ...
   `_drain_adopted_evals`). An eval CHILD never raises a spend ceiling — nor the run's refused setup
   (`core/errors.py::RunSetupRefusal`, latched once by `eval_dispatch.py::_ensure_run_setup`) — into
   its group: it defers it (`core/errors.py::deferrable_run_stop`) and its owner raises it after the
   drain/join (`speculation.py::_raise_deferred_eval_budget_stop`, `_dispatch_evals`); a Card
   PRODUCER parks a ceiling on the same sink (`speculation.py::_run_isolated_producer`), and a held
   stop closes the session's gates. When you add a non-folded event the question is not "does the
   fold read it?" but **"does any reader key on its position?"**.
...
- **Layering**: `core` imports nothing above itself; `events` only `core`; `runtime` nothing
  ...
  **`search` may import `agents` at module level; `agents` may reach `search` ONLY through a
  deferred (function-local) import** — four search modules import `agents` at module scope, so a
  module-level `looplab.search` import in `agents/` closes the cycle into an ImportError at
  startup (`tests/test_agents_search_direction.py`). `tools` reaches `engine` at module level only through engine LEAVES (`comparability`,
  `workspace_seed`, `governance_health`: modules importing nothing `tools` may not) and everything else
  function-locally. The whole matrix is `tests/test_package_layering.py` — module-level edges two-way,
  every deferred edge with its reason, `core`/`events`/`runtime` purity at any level
...
- **A phase that SPENDS must open a SPAN, not just a `phase_progress` beacon.** `_progress`
  appends an event and opens nothing, and `core/tracing.py::generation` yields a NULL handle
  when no span is open — so a beacon-only phase writes every provider call with
  `trace_id=null`: real money attributable to nothing, invisible to `looplab timings`, to the
  trace view and to every per-phase cost question. Use `SharedEngineMixin::_paid_progress`,
  which is the beacon AND the span. (Measured at 26 % of paid calls; doc 64 has the account.)
...
- **Duck-typed seams are REGISTRY-GUARDED** — a rename that used to break silently is now a red
  ...
  / `LLM_PRESENCE_ATTRS` / `FACADE_STAGE_ATTRS` / `WRAPPED_ROLE_ATTRS`; `core/prompts.py::PROMPT_KEYS`;
  ...
  `engine/speculation.py::CARD_BUILD_SKIP_REASONS`; `engine/card_reservation.py::RESERVATION_REFUSALS`
  (= `RESERVATION_RACES | RESERVATION_VERDICTS`: WHY `_reserve_node_build(refusal=[])` made no reservation —
  an operator inject reserves BEFORE its `inject_done`, stays queued on a race, is spent only on a verdict); `core/tracing.py::TRACE_WORKER_STOP_REASONS`;
  ...
  `engine/memory_stores.py::MEMORY_STORES` (a store joined under `memory_dir` or a bare `*.jsonl`
  literal with no row is red, and so is a row whose declared writer no longer writes it); `core/evidence.py::EVIDENCE_CONSUMERS` (keyed `<module>::<qualname> -> <callee>` at the site that BUILDS the toolset; `tests/test_evidence_consumers.py` derives the sites by AST through every forwarder of a toolset parameter, so a new loop over untrusted text is red until fenced or exempted with a reason);
  ...
  raising, which is how a Settings field went unwired for a week); `engine/knobs.py::EngineKnobs` + `EXPLICIT_IN_INIT` (together EXACTLY the attributes `attr_by_field()` derives; `tests/test_engine_knobs.py`); `UNSETTLED_KNOB_DEFAULTS` beside them: a
  knob read as `getattr(<engine>, "<knob>", <default>)` defaults to what a real `Engine()` settles it
  to (`tests/test_engine_knob_defaults.py`) or owns a reasoned row, and a module function names its
  engine parameter `engine`/`eng` so that census sees it. Note `search/foresight.py`'s
...
- **Containment is countable, not forbidden.** The one lint rule (`[tool.ruff]` selects `BLE`) is
  ...
  in the run path (`core/` and `runtime/` included) re-raises `BudgetExceeded` first. "Paid" is a
  CLOSURE over a kind-aware call graph from the provider entry points, not a list of callee names
  (the eight-name list saw 23 paid try-blocks, the closure 101, and they had missed 38 swallows);
  `PAID_CALLBACKS` names the model-backed callables `runtime/` is handed, and the sites still owed
  sit in the shrink-only `FUNNEL_BACKLOG`. It is a NAME graph: a paid callable invoked through an
  attribute or a local (`obj(...)`) is invisible to it. Prefer `core/containment.py::contain(reason,
...
- **A POSIX-mechanism test is gated by NAME; a Windows-meant one is fixed.**
  `@pytest.mark.posix_only("<mechanism>")` through `tests/_posix_gates.py` (bash harness, fork,
  killpg, affinity, dir fsync, renameat2, …); `tests/conftest.py` skips it off POSIX and refuses one
  that names no mechanism. Behaviour meant to work on Windows is fixed and its Windows branch driven
  ...
  (`serve/launch.py::_still_reads_back`). Python 3.12's Windows `time.monotonic` ticks at 15.625 ms, so a
  test whose verdict races two cutoffs or counts ticks in a wall-clock window DRIVES the clock it asks
  about (`tests/test_assistant_budget_notice.py::_LoopClock`, `tests/test_command_monitor_cost.py::_MonitorClock`),
  and a route that may answer `{status: running, job_id}` is read SETTLED (`tests/test_report.py::_settled`).
...
- Settings are flat on purpose (`LOOPLAB_<FIELD>` env vars map 1:1); never nest or rename fields —
  ...
  `tests/test_calibration_profile_home.py`, for an `EngineOptions` field a
  `tests/test_engine_options.py::_PERTURBED_VALUE` row only when `attr_by_field()` (derived by driving
  `Engine.__init__`) cannot move it with a generic non-default, its `Knob` in `engine/knobs.py::EngineKnobs` (or an `EXPLICIT_IN_INIT` reason), and a `tests/test_options_divergence.py`
  row when the two defaults differ.
