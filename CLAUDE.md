...
`ruff` installed, refuses a NEW blind handler that states no reason, and keeps the 99 pre-existing
...
- **A POSIX-mechanism test is gated by NAME; a Windows-meant one is fixed.**
  ...
  (`serve/run_commands.py::_windows_process_alive`).
