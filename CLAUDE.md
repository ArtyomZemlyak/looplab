...
1. **The engine is the sole writer of domain events.** Background tasks return values; only the
   ...
   `_drain_adopted_evals`). An eval CHILD never raises a spend ceiling into its group: it defers it
   (`core/errors.py::deferrable_budget_stop`) and its owner raises it after the drain/join
   (`speculation.py::_raise_deferred_eval_budget_stop`, `_dispatch_evals`). When you add a non-folded event the question is not "does the fold
