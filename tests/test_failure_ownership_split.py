...
def test_the_engine_loop_records_why_its_own_word_stood():
    ...
    from tests._source_scan import eval_attempt_dedented_source, eval_attempt_tree

    # THE PHASES, IN DRIVER ORDER, not `_evaluate` — the attempt loop became nine `_eval_*` methods
    # (doc 52 row 21) and reading the driver alone would see neither stamp and pass vacuously. The
    # concatenation is in driver order, so index order is still run order and the ordering assertion
    # below means what it always meant.
    # `a._override_refused` and not a bare local: the attempt's slots live on the `EvalAttempt`
    # carrier since the phase split, and `DIAGNOSIS_SLOTS` is what resets it between attempts.
    assert "a._override_refused" in _payload_stamps(
        eval_attempt_dedented_source()).get("reason_override_refused", set())
    nodes = [n for n in ast.walk(eval_attempt_tree()) if isinstance(n, ast.Call)]
    nodes.sort(key=lambda n: (n.lineno, n.col_offset))
    calls = [getattr(n.func, "id", getattr(n.func, "attr", "")) for n in nodes]
