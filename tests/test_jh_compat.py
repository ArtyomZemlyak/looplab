...
def test_fsync_timeout_env_parse_tolerates_garbage():
    ...
    load. A valid override is still honored."""
    import importlib
    import looplab.core.atomicio as aio
    assert aio._fsync_timeout() == 5.0  # sanity: default when unset
    with mock_env("LOOPLAB_FSYNC_TIMEOUT", "abc"):
        assert aio._fsync_timeout() == 5.0          # garbage -> default, no ValueError at import
    with mock_env("LOOPLAB_FSYNC_TIMEOUT", "12.5"):
        assert aio._fsync_timeout() == 12.5         # valid -> honored
    importlib.reload(aio)                            # restore module-level constant for other tests


def mock_env(key, val):
    import contextlib
