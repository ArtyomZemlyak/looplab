...
def _render(memo: dict) -> str:
    ...
    return tool.execute("read_research_memo", {})
...
def test_the_refusal_survives_the_agent_layer_truncation():
    ...
    On this memo the old render put the `Claims` section at char 3,889 of a 7,862-char answer, so a
    verdict rendered only beside its claim would have been cut off 111 chars later. The refusal must
    therefore lead, and it must lead the SUMMARY too — the summary is where this memo restates
    0.8776 as settled fact.
    """
    from looplab.agents.tool_loop import _cap_tool_result
    out = _render(json.loads(_V8_MEMO.read_text(encoding="utf-8")))
    delivered = _cap_tool_result(out)
    assert len(out) > _RESULT_CAP, "fixture no longer exercises truncation — re-derive this test"
    assert _V8_ANCHOR in delivered and _V8_ANCHOR_NOTE in delivered
    assert delivered.index("UNSUPPORTED") < delivered.index("Summary:"), \
        "the verdict must precede the summary that asserts the number"
    assert out.index(_V8_ANCHOR_NOTE) < out.index("Summary:")


def test_every_claim_carries_its_own_verdict():
    out = _render(json.loads(_V8_MEMO.read_text(encoding="utf-8")))
    body = out[out.index("Claims (with evidence"):out.index("Recommended directions:")]
...
def test_a_memo_with_no_verification_block_says_so():
    ...
    out = _render({"summary": "s", "claims": [{"statement": "the leader is overfit"}], "at_node": 3})
    assert "Verifier: NOT RUN" in out
    assert "not a pass" in out
    assert "the leader is overfit" in out, "the memo must still be delivered in full"
...
def test_a_claim_with_no_verdict_reads_unverified_not_clean():
    ...
    assert "1 of 2 claims were never checked" in out
...
def test_render_is_pure_and_never_raises_on_junk():
    ...
    for junk in ({"verification": []}, {"verification": {"verdicts": "no"}},
        ...
        assert isinstance(_render(junk), str)
...
def test_the_tool_contract_promises_only_what_it_renders():
    ...
    # ... and the promise is kept in the answer, not only in the description.
    assert "verifier" in out.lower()
