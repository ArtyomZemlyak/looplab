...
def test_a_hand_edited_row_missing_id_or_name_is_dropped_instead_of_500ing_every_mutator(tmp_path):
    ...
    `name` is checked for the same reason: it is REQUIRED on the Project model, so an id-only row
    survived an id-only filter and then 500'd anyway — `reparent` returns `Project(**p)` ->
    ValidationError, and `rename(pid, "")` reads `p["name"]` (an empty new name short-circuits the
    `or`) -> KeyError. `rename_supertask` reads `st["name"]` identically."""
    ...
    path.write_text(_json.dumps({
        "projects": [{"id": "p1", "name": "keep"}, {"name": "no id"}, {"id": "p2"}, "junk", None],
        "assignments": {}, "labels": {},
        "supertasks": [{"id": "s1", "name": "keep"}, {"id": "s2"}, {"nope": 1}],
        "supertask_assignments": {},
    }), encoding="utf-8")
    ...
    # The dropped rows now surface the DOCUMENTED 400-mapped error, not a 500 from deep inside.
    for call in (lambda: store.rename("p2", ""), lambda: store.reparent("p2", None),
