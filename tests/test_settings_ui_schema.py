...
def test_packaged_settings_ui_schema_preserves_copy_and_only_known_unique_fields():
    ...
    # 168 -> 169 when `eval_deadline_grace_s` joined Budgets & confirmation (doc 37 §2.2): the
    # one-shot, judge-granted extension a stage may get at its wall-clock deadline. A row rather
    # than an uncurated omission because it is an LLM-judged switch that spends GPU time when an
    # operator turns it on, and they have to be able to see the number they set.
