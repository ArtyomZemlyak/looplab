...
def test_packaged_settings_ui_schema_preserves_copy_and_only_known_unique_fields():
    ...
    assert len(keys) == SETTINGS_UI_SCHEMA_CATALOGUE_FIELD_COUNT == 224
    # 223 -> 224 on 2026-09-26: `card_verdict_support` (doc 67 67.1) -- whether a supported card's
    # row on the proposal board says what its verdict rests on. A ROW on the PROMPT ground: OFF is the
    # historical bytes. Verified by INTERSECTION: 223 keys common to the previous keyset plus exactly
    # that one, none removed.
    ...
    # 219 keys common to the previous keyset plus exactly that one, none removed.
    ...
    # back. Verified by INTERSECTION: 216 keys common to the previous keyset plus exactly that one.
    ...
    assert len(Settings.model_fields) == SETTINGS_UI_SCHEMA_SETTINGS_FIELD_COUNT == 259
