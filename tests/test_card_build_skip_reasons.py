...
def test_a_head_with_no_producer_pair_is_released_producer_unavailable_never_producer_failed():
    ...
    from speculative election on it. The no-pair close in `_start_request_producer` (which
    `_start_head_producer` delegates to since several producers exist) runs no producer, so
    ...
    tree = ast.parse(_SRC)
    fn = next(n for n in ast.walk(tree)
              if isinstance(n, ast.FunctionDef) and n.name == "_start_request_producer")
