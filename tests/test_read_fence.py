...
def test_the_warn_rung_bounds_stderr_the_same_way_it_bounds_the_log(tmp_path):
    ...
    with contextlib.redirect_stderr(captured):
        for i in range(300):
            ns["_report"](f"/src/repo/shard-{i}.bin")
        for _ in range(1000):
            ns["_report"]("/src/repo/hot.bin")               # the retry loop the bound exists for
    ...
    # Both sinks stop at the SAME point, so neither can report a path the other omitted.
    assert {ln.rsplit("\t", 1)[-1] for ln in log.read_text().splitlines() if ln.strip()} == {
