...
#   2026-09-23  + propose_brief_fit (Q-3: the proposal brief FITTED — the digest spends its budget in
#               whole rows with a cut receipt, each fact once). The 'field set changed too' branch,
#               verified the prescribed way: an AST scan of `Settings`' annotated assignments against
#               the pre-change tree reports exactly `['propose_brief_fit']` added and `[]` removed.
...
#   2026-09-24  + eval_canary (the node's own stage chain on the task's tiny slice before its full
#               evaluation, `engine/eval_canary.py`). The 'field set changed too' branch: exactly
#               `['eval_canary']` added and `[]` removed; `_EXPECTED_FIELD_COUNT` goes 254 -> 255 and
#               both pins are re-set. INERT for a calibration replicate (off on both surfaces, and the
#               toy workload declares no `eval.canary`); re-pinned on the COMPLETE-envelope rule.
#   2026-09-26  + card_verdict_support (doc 67 67.1: a supported card's board row says what its
#               verdict rests on). The 'field set changed too' branch: exactly
#               `['card_verdict_support']` added and `[]` removed; `_EXPECTED_FIELD_COUNT` goes
...
#   2026-09-26  + brief_node_frontier (doc 67 67.9: the proposal brief names the node frontier).
