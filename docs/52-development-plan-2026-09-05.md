...
*Closed 2026-09-07 (row 31 shipped, the code half): the marker `proxy-prediction-accuracy-unmeasured`
stood here. `search/proxy.py::pairwise_accuracy` is the number this module never had — over every
pair of scored-and-evaluated nodes whose realized metrics differ, how often the proxy ordered them
the way the evaluation did, with prediction TIES counted apart (no ordering is not a wrong one) and
`accuracy: None` below one ordered pair (which is not 0 %). `looplab proxy-accuracy` prints it, and
prints the caveat that makes it honest: a killed node has no metric, so the figure is measured over
the candidates the proxy LET THROUGH and the kill's own error rate is not in it — the only
counterfactual a record can hold is a killed node evaluated anyway, which the report counts
separately. Below 60 % on a run that armed the kill, the command names `proxy_kill_fraction=0`.
...
verification method and unsupported count, associated with the researcher role), one `ll:Claim`
entity per claim — generated by that pass, derived from the experiments it cites AND from the exact
evidence spans it is bound to — with the verifier's verdict, its note and the verifier's own
`complete` bit beside it. It invents nothing: a cited experiment the fold no longer holds is named
in `ll:cited_missing_nodes` rather than given a made-up solution entity, an evidence id with no
ledger row is kept as `ll:lifecycle: unrecorded` rather than dropped, and a verdict that cannot be
bound to its claim by position OR by a unique statement match exports `ll:verdict_binding:
unmatched` and NO verdict, so a claim can never wear its neighbour's. Bounded at 400 claims with
the cut counted on the graph (`tests/test_prov_claims.py`). Deleted per the index rule.*

*Closed 2026-09-07 (row 32 shipped): the marker
`failure-diagnosis-emits-one-cause-not-competing-hypotheses` stood here. The triage schema gains a
`hypotheses` array behind `Settings.diagnosis_hypotheses` (off — no extra provider call, the same
triage answer gains a field, but it changes that call's schema and prompt): each alternative carries
its own `confidence` (INDEPENDENT — two explanations can both be likely, which is the point) and a
`discriminator`, the thing that would tell it from the primary answer. `failure_diagnosis.py::
coerce_hypotheses` owns the durable shape — `HYPOTHESES_CAP = 3`, the confidence clamped into [0, 1]
with an unscored alternative at 0.0 and NOT 0.5, a `kind` outside `DIAGNOSED_FAILURE_REASONS`
dropped rather than recorded, every field redacted, the model's own ORDER kept rather than sorted by
confidence — and the rows land as `reason_hypotheses` on `node_failed` / `node_repaired`. Read by
NOTHING that decides, pinned by an AST guard over the tree: the repair still follows the one
`failure_kind`, because a second explanation is evidence for a human and for the next diagnosis, not
a second instruction for this one (`tests/test_diagnosis_hypotheses.py`). Deleted per the index rule.*
...
OPEN[parallel-build-is-a-bulk-synchronous-barrier] the DEFAULT parallel build is still a join over
a `parallel_build_batch` task group — a barrier, not the steady-state pool AIRA₂ dispatches into as
soon as any worker is free. Since 2026-09-07 the lane EXISTS
(`orchestrator.py::_steady_state_build_lane` behind `Settings.steady_state_build`, driven by
`tests/test_steady_state_build.py`: with two lanes and a slow first build, the third build starts
before the slow one ends, and the barrier arm of the same test proves the contrast). What is not
done is the DEFAULT flip, and it should not be made here: the lane asks the researcher for one idea
per lane instead of a chunk's worth, so the evidence it needs is a real multi-GPU run comparing
wall clock AND proposal quality — the box queue's `serial-node-build-holds-the-loop` measurement is
the same evidence. proof:`present:self.tracer.span("parallel_build_batch"@looplab/engine/orchestrator.py`
