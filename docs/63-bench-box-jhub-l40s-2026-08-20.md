# 63. Standing the arena up on a second box: JupyterHub + L40S (2026-08-20)

Companion to [doc 61](61-benchmark-landscape-and-local-plan-2026-08-19.md) (*why* AlgoTune, and the
parity analysis) and [doc 62](62-algotune-arm-operational-notes-2026-08-20.md) (*what was touched*
in the third-party checkout). This one is the **second machine** doc 62 §6 asks for: what a box that
...
$0.02-per-task-arm budget are doc 62's. What changes is the environment underneath, and one thing
...
| GPU | 1x L40S, 46 GB, idle | **Unused by this arena.** AlgoTune is CPU-bound in substance (doc 61 §5d), which is why the sm_120 problem that blocks FML-bench does not arise. |
...
| Python | 3.12 system, **3.11.13 via `uv` for the AlgoTune venv** | Same version as doc 62. |
...
failure; the quiet one matters more. This benchmark's cost is **process spawn and imports** (doc 62
...
the repository side is pushed. It is the same argument doc 62 makes for arm B running from a pinned
...
| Real throughput | **~96 tok/s** median, 400-token completions (`deepseek-v4-flash`); `qwen3.6-35b` ~160–320 tok/s | The campaign model is the slower of the two, and is chosen anyway: it is the model doc 61/62 sized the campaign around. |
| Reasoning channel | **none** — no `reasoning_content`, and the OpenRouter `provider` / `reasoning` blocks are accepted and ignored (HTTP 200) | doc 62's `reasoning: {effort: medium}` pin and the `siliconflow/fp8` provider pin control nothing here. They are set EMPTY rather than left in: a dead parameter in the record reads like a live control. |
...
doc 61 §5a chose — fetched 2026-08-20T10:16:47Z and **pinned in the file**, because a benchmark
...
## 3. Deviations beyond doc 62's list

doc 62 §1 enumerates what is patched in the third-party checkout; all of it applies unchanged and is
...
| Check | Result |
|---|---|
| AlgoTune install (`uv venv --python 3.11` + `uv pip install -e .`) | 158 packages in **2 m 07 s**. Same headline versions as doc 62: `torch 2.13.0`, `jax 0.7.1`, `scipy 1.17.1`, `numpy 1.26.4`, `ortools 9.11.4210`. `litellm` is 1.97.0 here vs 1.83.0 there. |
| The 27 packages the agent's prompt promises | **27/27 importable**, matching doc 61 §5d's audit on the other box. |
...
| **The evaluator, end to end, with no LLM in the loop** | Re-timed a SHIPPED reference solver: `GPT-5.4` on `discrete_log` → **0.9967x**, ~9 minutes cold on 2 pinned cores including the HuggingFace dataset fetch. This is doc 61 §5e's re-timing plan working on this box. |
...
- **Against doc 61/62's numbers from the 5090 box — no.** Different CPU count, different model
  ...
  qualitative.
- **Against AlgoTune's published table — no**, for the reason doc 61 §5e already gives: the 17
...
**Probe the endpoint before every campaign, with a nonce.** doc 61 §5a's rule — the catalogue is not
...
The original box had 8 cores and doc 61 called that its handicap. This one has the opposite problem:
...
task — which is the spread doc 61 §5f already recorded for `discrete_log` (1.0006 then 1.4468 on
consecutive runs). One task at n=1 cannot separate a regime effect from that noise; the honest
reading is the one doc 61 already prescribes — **read the aggregate over 20 tasks, never a row.**
...
doc 62 §7. The rebase cost two conflicts, and the second is worth recording: `settings_ui_schema.json`
