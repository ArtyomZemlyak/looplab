...
| 34 | **[34-fence-coverage-audit-2026-08-13.md](34-fence-coverage-audit-2026-08-13.md)** | Measured coverage audit of the source-tree read fence: which surfaces the CPython audit hook sees and which walk straight through it (safetensors and h5py do; numpy.memmap, numpy.load, torch.load and PIL do not, because they open through Python first — so a library allow-list could never have worked). Records the closed mutation hole, the structurally unclosable `os.open(dir_fd=…)` read, the `python -S/-E/-I` unfencing, and the Landlock-vs-namespace probe results. Shipped behavior for the mutation half; the kernel-boundary half is a recommendation carried by doc 35. |
