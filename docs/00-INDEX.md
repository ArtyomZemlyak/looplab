...
|---|-----|---------------------|
| 37 | **[37-node-workspace-worktree-measurement-2026-08-13.md](37-node-workspace-worktree-measurement-2026-08-13.md)** | Dated MEASUREMENT and a decision for doc 29 §F3: node workspaces stay a materialized tracked copy, `git worktree` is declined. `git worktree add` is a checkout, not a link — it costs 2.1 % MORE disk per node, its seed win is 0.010 pp of the heaviest run, it cannot seed the operator's uncommitted work (10 of 17 tracked files dirty on the v1 testbed), and `git add -A` from a node worktree writes into the SOURCE repo. Also establishes that seeding has never copied an untracked file on a real testbed and that the 727 GB was retained training checkpoints (`save_total_limit=3`), not the copy. Nothing shipped; §8 is the cheaper root fix in cost order. |
| 36 | **[36-agent-driven-decisions-2026-08-13.md](36-agent-driven-decisions-2026-08-13.md)** | A STANDING design principle, not a dated analysis: where the same evidence can mean different things depending on context, the decision belongs to an agent that can read the context — and the line that separates "decides what to do NEXT" (an agent may) from "decides what goes into the RECORD" (deterministic rungs over authenticated evidence only). Argue new work against it the way you argue against the engine invariants. |
| 33 | **[33-cross-turn-dispatch-options-2026-08-13.md](33-cross-turn-dispatch-options-2026-08-13.md)** | Dated option table for F1f — the Card session's `consumer_completed`/`yield_outer` latch that stops new work at the FIRST eval terminal while refusing to exit until the LAST one drains. Options, with the recommended shape hoisting the eval task group to run scope; not shipped behavior. |
| 32 | **[32-f1e-operator-declaration-options-2026-08-13.md](32-f1e-operator-declaration-options-2026-08-13.md)** | Dated option table for F1e — re-checking a repaired artifact declaration instead of leaving the metric SALVAGED. Options, not shipped behavior. |
| 31 | **[31-path-isolation-options-2026-08-13.md](31-path-isolation-options-2026-08-13.md)** | Dated option table for keeping a node's reads inside its own workspace, written after v6 node 4 scored a human's checkpoint an absolute path named. Superseded in part by the read fence that shipped in `c862045c`; the open core question is carried by doc 35. |
