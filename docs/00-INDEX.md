...
| 34 | **[34-review-deferred-decisions-2026-08-13.md](34-review-deferred-decisions-2026-08-13.md)** | The residue of the two-round, 18-angle review of `2c36b7e..HEAD`: findings that are real and reproducible but whose fix is a PRODUCT or ARCHITECTURE decision rather than a defect repair — the receipt-less agent-facing node purge, the trace exporter's per-span filesystem cost, the card-trace whole-run scan, the span index's per-row hashing, and the settings tripwire that has drifted four times. Each has a pointer comment at its site. Also records, deliberately, the four findings that were REFUTED with evidence, so a later reader does not "fix" them. |
