# AutoRND — Documentation Index

**Project:** AutoRND — an open, backend-flexible **autonomous ML/DS research engine** (an LLM agent that invents → implements → tests → improves ML solutions in a loop, returning the best *verified* result).
...
| 6 | **[06-implementation-plan.md](06-implementation-plan.md)** | *Status board / ToDo (what's built vs left).* The 22 iterations (I0–I22) across P0–P4 as a **checklist with live status** — ✅ done / 🟡 partial / ⬜ todo / 🧱 infra-seam — each pointing at its module + test. Has a snapshot table and a recommended next-steps order. **Read this to see where the implementation actually stands.** Running code is in `autornd/` (see [README.md](README.md)). |
