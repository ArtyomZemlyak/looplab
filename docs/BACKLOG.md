...
3. **The hardened exploit suite re-flags the grader import the detector just sanctioned (P1, S).**
   ...
   **[Line drift, 2026-08-14 (`d307542`):** the call sites moved to `engine/evaluate.py:828-840` —
   `detect_reward_hacks` with `grader_import_ok=` at `:828-836`, the unconditional
   `self._exploit_suite.scan(scan_src)` at `:840`. The finding itself is unchanged: `scan(self,
   code)` (`trust/harden.py:79`) still takes no sanction argument.]
...
- ⬜ **Reward-hack vs hardened-suite residual (S).** The mlebench grader-import false positive
  ...
  **[Line drift, 2026-08-14 (`d307542`): the call site is now `engine/evaluate.py:828-840` —
  `grader_import_ok=` at `:835-836`, the unconditional `scan(scan_src)` at `:840`. Same finding.]**
