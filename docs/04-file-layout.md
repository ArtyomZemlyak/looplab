...
> **⚠ Design vs. shipped (2026-07):** this is the original on-disk *design*. The implementation deliberately simplified it: a run dir holds `events.jsonl` (at the run root, not `_logs/`), `config.snapshot.json` + `task.snapshot.json` (not `config.yaml`/`manifest.json`), `engine.lock`, per-node workdirs, and derived projections (`readmodel.sqlite`, `trace.json`, `tree.html`). There is no content-addressed `store/objects/` and no `commands.jsonl` (control intents are events in `events.jsonl`). One live engine owns `RunState` reduction; authenticated UI-server controls can append events through the same cross-process serialization. The event fold does not reconstruct task/config snapshots, spans, chat, command records, cross-run stores, or external effects. The current contract is documented in [guide/concepts.md](guide/concepts.md).
>
...
| **Event/flow trace** | [MA] | append-only **JSONL** (`events.jsonl`) | Source of truth for the run timeline; streamable (`tail -f`/`jq`) and git-diffable. Ordinary writes are one physical line per logical event; a bounded crash-atomic `append_many` transaction is one reserved physical envelope line whose members are expanded by event-aware readers. Its guarded type is intentionally invalid to pre-batch `Event` readers, so rollback to an older binary fails closed instead of discarding nested actions. Event-aware readers also reject the first duplicate, backward, or gapped logical sequence. Raw `jq` sees storage envelopes, while the CLI/API/replay surfaces expose logical events. Add a SQLite/WAL projection only when `grep` stops scaling. |
