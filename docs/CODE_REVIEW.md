...
| 🟡 | cli.py:152 | `resume` silently falls back to fresh `Settings()` when `config.snapshot.json` is missing — can finish an approval-pending run **without approval**, the very case its own comment warns about. **[closed 2026-09-03 — `load_run_settings(strict=True)` now REFUSES an absent snapshot on a run whose `events.jsonl` exists, naming the settings that would have been lost. All three strict callers (`resume`, `finalize`, the finalization recovery) reach it only after `_require_run_dir` has proved that log exists, so for them the file is not optional; a bare `--out` path with no log is still ambient, which is what keeps `run` working. `tests/test_resume_without_snapshot.py` drives it, including the approval gate end to end.]** **OPEN[require-approval-not-pinned-at-run-start]** the remaining half is the PIN: `require_approval` is read live off `Settings` and is absent from `RUN_START_PINNED_FIELDS`, so invariant #6 ("settings recorded in `run_started` win on resume") does not cover the approval gate — a snapshot EDIT, as opposed to a deletion, still moves it. proof:absent:"require_approval",@looplab/core/config.py |
