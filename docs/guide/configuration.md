...
| `cross_run_advisory` | `LOOPLAB_CROSS_RUN_ADVISORY` | `true` | PART IV cross-run Step 5 (§21.20.5). Inject a **claim-count-bounded** context pack plus a lean coverage line into Researcher/Strategist prompts. Both paths exclude the current run and scope their source snapshot; Researcher accepts exact-task or fingerprint-related lessons/capsules and exact-task v2 D8, while Strategist is deliberately exact-task. They apply taxonomy/claim overlays, exclude rejected claims, quote persisted text as untrusted data, and persist a compact `{scope_task, excluded_run, source counts, corpus_digest, render_digest}` receipt with the resulting node/strategy event. New v2 D8 rows retain verifier verdict/method and evidence references; legacy rows remain unverified. These receipts still lack a frozen corpus watermark, ComparisonContract, access/redaction policy version and per-evidence family applicability, so this is not the full Atlas derivation contract. It never directly changes best-metric selection. Keep opt-in until false-transfer/injection/verification and frozen A/B gates pass. Empty when stores are empty |
