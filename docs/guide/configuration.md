...
server-owned curated catalogue with **162 of the 194 direct `Settings` fields in 10 groups**. The default
**Essential** disclosure mode contains 18 high-frequency keys; search spans all 162 catalogued keys.
...
| `train_monitor_kill` | `LOOPLAB_TRAIN_MONITOR_KILL` | `true` | INTERVENTION: let the monitor tree-kill a training it judges `broken` (diverged / silent CPU fallback / not learning) early. The node then fails normally (`reason=monitor_broken`) with no repair and no retry, so the gate is deliberately narrow: the verdict must be about a stage the engine can IDENTIFY as a training stage (the monitor lives across the whole eval, so `setup.log` and the always-appended protected `score` stage are never judged, and a log it cannot attribute stays advisory), and it must be CONFIRMED by a second consecutive `broken` verdict about the same log — a prompt re-look, not another full cadence. Off = observe only |
...
positive `speculation_depth` — so on stock defaults the Card lane pre-builds nothing, and the shipped
`card_driven_selection` is effectively inert for *selection*. Turning `speculation_depth` to `-1`
(AUTO) is the intended next step and everything around it is ready: AUTO resolution, the `run_started`
...
sibling child is closed exactly as before. And a discarded prefetch is not counted as a child at all,
because it never spent budget: the fold now reads the same
`is_unevaluated_speculative_discard` predicate the Card lane's own policy view reads (it moved down to
`core/models.py` so replay can reach it), instead of the two views disagreeing about whether the
failed parent still had work available. The same offline reproduction now runs **16 nodes minted, 12
charged, 11 evaluated, normal finish** at depth `1` — four prefetches discarded and refunded, so the
budget denominator matches the speculation-off control exactly.

The default is still `0`: flipping it to `-1` is a separate, deliberate step now that the anchor is
proven. Setting it yourself is supported and no longer degrades on runs whose nodes fail — the
`budget` receipt below is how you check what it actually did for you.
...
| `speculation.discarded` | Speculative builds that reached a terminal without being admitted (a miss) |
...
| `inline_repair_attempts` | `LOOPLAB_INLINE_REPAIR_ATTEMPTS` | `0` | Max in-place repair attempts per node (**0 = unlimited**, bounded by the anti-stuck guard). The number is **apportioned, not a single pool**: it bounds each of TWO ledgers separately, because two equally "mechanical" repairs can be different resources. A repair that only **reconciles the code with the installed libraries** (a moved import, a removed/renamed API, a major-version migration, a symbol an absent optional dependency left undefined) is charged to the *environment* ledger; everything else — the node's own logic, a modelling decision, a run that was too slow or too memory-hungry — is charged to the *experiment* ledger, which is what you are really budgeting. Worst case per node is therefore `2N` repairs. Deliberately **no second knob**: the two are not independently tunable quantities, they are the same allowance charged to whoever incurred the cost. *Why:* measured on a live dense-retrieval run with `inline_repair_attempts: 6`, ALL six attempts went on PyTorch-Lightning-2.x / transformers / accelerate migrations of a repo whose pinned deps were a year stale — the crash-triage rationale said "mechanical" every time — and the first genuine research question (a DDP `find_unused_parameters` modelling decision) arrived with the budget exhausted. Under one pool such a repo can never reach its own research question: every node re-pays the same migration. The environment ledger cannot become a second runaway: an exemption needs the triage agent's structured `repair_class`, the **engine's own** reading of the traceback (`engine/triage.py::_environment_failure`) and **forward progress** (a never-before-seen normalized error signature, or a later pipeline stage reached) to agree, and anything that disagrees is charged to the experiment budget — so a runaway whose failures all normalize to one signature buys exactly one exempt attempt and still stops at `inline_repair_stuck_repeat`. Each repair records which ledger paid for it (`node_repaired.repair_class`) |
...
| `inline_repair_stuck_repeat` | `LOOPLAB_INLINE_REPAIR_STUCK_REPEAT` | `4` | How many times ONE normalized error signature may fail a node before the in-place retry loop calls it "stuck" and abandons. Counted per signature over the whole node, **not** as a consecutive streak — an oscillating repair (fix A breaks B, fix B breaks A) never repeats a signature back-to-back and would defeat a streak counter. The signature (`engine/triage.py::_normalize_error_sig`) absorbs incidental *operands* — addresses, line numbers, paths, numeric literals and quoted identifiers (the module/attribute/symbol name a message quotes) — while keeping the exception class, the message template and the failing frame distinct, so a crash that genuinely moves as the agent fixes it still gets its attempts |
...
<!-- CODEX AGENT: the preceding "Agents have no cross-run mutation function" is true for autonomous
run roles, not the owner Assistant named in the same row. Outside plan mode it also receives
approval-gated ConceptGovernanceTools (merge/purge/split/clear) over the shared taxonomy; document that
...
| `concurrent_consolidate` | `LOOPLAB_CONCURRENT_CONSOLIDATE` | `true` | Dedup/merge the open Card belief board (1 card = 1 hypothesis) during a long eval. With Card queue selection, `hypothesis_merged` changes Card ownership/readiness, so consolidation is deferred to the joined between-node cadence. Needs `track_hypotheses` + a reflect client; background overlap also needs `concurrent_research`. Library default off |
| `track_hypotheses` | `LOOPLAB_TRACK_HYPOTHESES` | `true` | P1: ask the Researcher to state each experiment's hypothesis, register deep-research directions, and track them to a verdict on the single Card board (1 card = 1 hypothesis). Open board cards are shown to the Researcher and may be payoff-ordered by foresight, so they can steer later proposals without directly re-ranking evaluated nodes. Also drives AGENTIC paraphrase-merge of the belief board (hybrid retrieval + the Researcher decides; `hypothesis_merged` events, applied deterministically in the fold) |
