...
| `cross_run_concepts` | `LOOPLAB_CROSS_RUN_CONCEPTS` | `false` | PART IV cross-run Step 2 (§21.20). At run end write a per-run concept capsule to shared `memory_dir`; during `_graded_novelty_precheck`, separately surface overlapping earlier concepts as a `cross_run_prior` audit event. It is not fed into the gating grade and never rejects. Effective surfacing requires a shared `memory_dir`, `graded_novelty` and concept tags produced through `concept_pivot`; use `fingerprint_universal` consistently for non-Latin portfolios. The current similarity/scope contract is lean and experimental; keep opt-in |
| `cross_run_read_tools` | `LOOPLAB_CROSS_RUN_READ_TOOLS` | `false` | PART V §22. Adds read-only `cross_run_prior_attempts` / `cross_run_claims` / `cross_run_atlas` to in-house Researcher, Strategist, deep-research, Genesis and the in-house `LLMRepoDeveloper` lesson-role variant; external coding-agent Developer backends do not receive this provider. Agents have no cross-run mutation function, but consumers are **not fully scope-safe**: automatic paths can be portfolio-wide and Repo Developer is not state-bound. Stored text is untrusted prompt data and no derivation receipt is recorded. Keep off until §22.7 scope/redaction/receipt gates pass |
| `cross_run_advisory` | `LOOPLAB_CROSS_RUN_ADVISORY` | `false` | PART IV cross-run Step 5 (§21.20.5). Inject a **claim-count-bounded** context pack plus lean portfolio line into Researcher/Strategist prompts. This is live prompt influence (although it never directly changes best-metric selection), scans shared stores without the full scope/comparison contract, records no derivation receipt, and rejected claims can still leak through Atlas/Strategist paths. Keep opt-in until false-transfer/injection/rejected-projection tests and frozen A/B pass. Empty when stores are empty |
