...
| `select_verifier` | `LOOPLAB_SELECT_VERIFIER` | `false` | R1-c / Part IV. Break an EXACT metric tie in best-selection with a calibrated §12-verifier soundness score: when a node's robust metric ties an already-eligible node, the engine verifies the tied nodes (grounded on their realized result, `selection_criteria`) and records a per-node score (`node_verified`, generation-scoped); the fold's mean pick then prefers the more trustworthy node. Strictly a TIE-BREAK — never moves a node ahead of a strictly-better metric (§21.7). Lazy (only fires on a real tie), opt-in, needs an LLM client; enable only after validating calibration offline with `verifier.calibrate`. Recorded in `run_started` so replay applies the same rule |
