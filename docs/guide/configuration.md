...
| `developer_memory` | `LOOPLAB_DEVELOPER_MEMORY` | `true` | A SEPARATE cross-run store of IMPLEMENTATION lessons for the Developer (`<memory_dir>/dev_lessons.jsonl`), distinct from the Researcher's `lessons.jsonl`. The Developer self-authors them (`remember_dev_lesson`), searches them (`search_dev_lessons`), and sees a top-5 preview up front; the engine distils a few at run end. Needs `memory_dir` |
