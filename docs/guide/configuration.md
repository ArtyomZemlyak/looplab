...
| `concurrent_consolidate` | `LOOPLAB_CONCURRENT_CONSOLIDATE` | `true` | Dedup/merge the OPEN hypothesis board on the eval-window background loop (repeated research keeps adding near-duplicate directions, so a long eval's board bloats before the between-nodes merge). Advisory-only (`hypothesis_merged` is BACKGROUND_APPENDABLE — selection-neutral); needs `track_hypotheses` + a reflect client, and only runs while `concurrent_research` drives the overlap loop. Library default off |
