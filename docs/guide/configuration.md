...
| `redact_output` | `LOOPLAB_REDACT_OUTPUT` | `false` | Adds the HIGH-ENTROPY pass to the bounded event/span/UI stdout/stderr tail redactor. It does **not** switch redaction on: known credential shapes (`sk-…`, `AKIA…`, `ghp_…`, `Authorization:`, `key=value`, URL userinfo) and the values of the operator's own secret-named env vars are masked on every persisted tail regardless of this flag. Turning it on additionally masks a 24+ character high-entropy run **that mixes upper case, lower case and digits** — the composition of a base64 credential, and deliberately not that of a path, a slug or a CamelCase identifier; `/` is treated as a separator for the same reason. That screen is what keeps the pass off ordinary ML output: measured over every persisted tail in `runs/`, it changed 13 of 744 before the screen and all 13 were traceback file paths, and 0 of 744 after. The residual cost is the other direction — a base64 credential containing a `/` can now escape the pass (75–98 % masked by length), which is why this remains the opt-in half while known shapes and your own env values are masked either way. Raw node-workdir `setup.log`, stage logs, `eval.log`, code and artifacts are outside this redaction boundary and may contain secrets; protect and retain the run root accordingly |
