...
| `concurrent_research` | `LOOPLAB_CONCURRENT_RESEARCH` | `true` | Overlap a due research "think" with the GPU-bound eval; the memo is recorded immediately when it finishes, and its directions become standing hints/open cards that can steer the next proposal |
