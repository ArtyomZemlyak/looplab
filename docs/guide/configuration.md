...
| `metric_salvage_repair` | `LOOPLAB_METRIC_SALVAGE_REPAIR` | `true` | After salvaging, still ask the Developer to fix the DECLARATION that broke. It never re-evaluates — the experiment succeeded — so it costs one call and no GPU time, and it stops the next attempt walking into the identical failure. Rides on every inline-repair bound and is best-effort: a dead provider loses the fix, never the metric |
