...
| `inline_repair_reasons` | `LOOPLAB_INLINE_REPAIR_REASONS` | `["crash","timeout","oom","setup","no_metric","drift"]` | Which failure reasons are eligible for inline repair. **All of them by default since 2026-08-12** — none of these reasons is evidence the HYPOTHESIS is wrong, which is the only thing that should end a node without a repair. The spend is bounded by `inline_repair_attempts`, `inline_repair_retrain_cap` and the per-attempt triage judge, not by this list |
