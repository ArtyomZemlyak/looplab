...
| `eval_deadline_grace_s` | `LOOPLAB_EVAL_DEADLINE_GRACE_S` | `0.0` | The most extra wall clock a live-log judge may buy for a stage that has reached its deadline, ONCE per command. `0.0` (the default) keeps the unconditional tree-kill this always was, byte for byte. It exists because a deadline is a number that cannot see a progress bar: all four `stage_finished.status == "timeout"` rows in the shipped corpus land EXACTLY on their 4 h/6 h/8 h wall and together discarded 22.0 GPU-hours, and the whole captured record of `rubertlite-dense-retrieval` node 72 ends `100%|##########| 664/664 [00:17<00:00, 38.13it/s]`. At the wall, a stage two seconds from writing its checkpoint and one that will never finish present the identical fact; only something reading the log separates them. The judge answers ONE WORD, never a number, and fails CLOSED — the opposite direction from the stage checker, because there an unreadable answer saves work and here it spends it. The operator's number is the ceiling: the runtime clamps to it, so a judge cannot name its own extension. Off by default because turning it on lets a model reading the candidate's own live log spend GPU time. The seconds it buys are seconds the process actually receives: the STALL watchdog's silence kill is deferred for the granted window (a stage that went quiet writing its checkpoint is the case this exists for), and the stage row's `seconds` and `deadline_grace_s` therefore agree. |
