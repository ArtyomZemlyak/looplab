...
| `auto_extra_metrics` | `LOOPLAB_AUTO_EXTRA_METRICS` | `true` | MAY AN UNDECLARED NUMBER BECOME A METRIC? `extra_metrics` is filled from two channels and only one was ever guarded: the operator's own `eval.metrics` reader specs (which refuse an agent-authored `adapter` reader, the same rule `cross_check` follows) and AUTO-CAPTURE — every OTHER numeric key on the candidate's own stdout JSON line, with no declaration, no reader spec and no gate. **Measured across the preserved runs, declared produced 0 of 12 extra metrics and auto produced 12 of 12**, including `speculation_cuda_probe_v=1.0` (a schema VERSION number), `device_count`, `alloc_bytes` and `device_ordinal` recorded as metrics beside genuine ones like `train_auc`/`cv_mean_auc` — and all of them reached the operator's metrics table, the Pareto front, the MLflow export and the reviewer view in the same visual place as the protected primary metric. `true` (default) captures them and TAGS each value with its channel in `node_evaluated.extra_metrics_provenance` (`{name: "declared"|"auto"}`); `false` records only what an operator-declared reader produced. **The tag is what closes the defect and it is written either way** — the gate is for a run that wants no agent-authored number in its record at all, and it DELETES information that cannot be recovered later, including the CUDA-probe proof `search/speculation_quality.py` re-derives. A value carrying NO tag is a row written before 2026-08-14 and reads back `unknown` at every consumer — deliberately not `declared`, since every such preserved value was in fact auto-captured. Write-side only: the fold never reads this field, so it cannot change how an already-recorded run replays, which is why it is not pinned in `run_started` and needs no `LEGACY_CONFIG_SNAPSHOT_DEFAULTS` row (its default IS the pre-field behaviour) ([tasks](tasks.md)) |
