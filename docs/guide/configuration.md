...
| `cross_run_read_tools` | `LOOPLAB_CROSS_RUN_READ_TOOLS` | `false` | PART V §22. A read-only cross-run KNOWLEDGE tool (`cross_run_prior_attempts` / `cross_run_claims` / `cross_run_atlas`) for the reasoning roles (Researcher, Strategist, deep-research, and a role-scoped Developer variant scoped to dev-routed lessons), over the §21.20 read-models from `memory_dir`. ADVISORY only — an agent may cite what it finds but never mutates cross-run truth (facts are engine-written, verdicts operator-ratified). OPT-IN (default off; the stores exist only once Part-IV features have run) |
