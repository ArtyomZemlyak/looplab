...
| `foresight_verify_samples` | `LOOPLAB_FORESIGHT_VERIFY_SAMPLES` | `3` | Verifier sample count for `foresight_verify` (the §12 repeated-sampling expectation on a no-logprob backend). `3` tames the single-shot variance; `1` = single-shot (cheaper, noisier) |
...
| `select_verifier_samples` | `LOOPLAB_SELECT_VERIFIER_SAMPLES` | `3` | Verifier sample count for `select_verifier` (the §12 repeated-sampling expectation). `3` tames single-shot variance; `1` = single-shot (cheaper, noisier) |
...
| `graded_novelty` | `LOOPLAB_GRADED_NOVELTY` | `false` | PART IV Phase 2b (D3). Grade a fresh proposal over the concept graph in the LIVE novelty gate: a level-4 "same direction, DIFFERENT implementation" or a level-5 "re-opens a wrongly-abandoned FAILED direction" proposal is ALLOWED through (recorded as a `novelty_graded` audit event) instead of being rejected by the flat LLM/semantic dedup gate — which can't tell "this DCL tweak" from "the whole DCL branch". Levels 1/2/3 (identical / near-dup / prior-run) still defer to the flat gate. Deterministic (heuristic tagger), audit-only, replay-safe, never touches selection; no-op for a task with no curated concept skeleton |
...
| `cross_run_concepts` | `LOOPLAB_CROSS_RUN_CONCEPTS` | `false` | PART IV cross-run Step 2 (§21.20). Fill graded-novelty's `prior_concepts` from a shared cross-run store: at run end write a per-run **concept capsule** (the shipped `node_concepts` tags + best outcome, keyed by `task_fingerprint`) to `memory_dir`; when a later SIMILAR run proposes an idea whose concept was tried before, record a `cross_run_prior` audit event (folds into `RunState.cross_run_priors`) that SURFACES the prior run + outcome — never rejects (D3 level 3). Audit-only, off the selection path; needs a shared `memory_dir`. OPT-IN (default off) |
| `cross_run_read_tools` | `LOOPLAB_CROSS_RUN_READ_TOOLS` | `false` | PART V §22. A read-only cross-run KNOWLEDGE tool (`cross_run_prior_attempts` / `cross_run_claims` / `cross_run_atlas`) for the reasoning roles (Researcher, Strategist, deep-research; a Developer-scoped variant is a follow-up), over the §21.20 read-models from `memory_dir`. ADVISORY only — an agent may cite what it finds but never mutates cross-run truth (facts are engine-written, verdicts operator-ratified). OPT-IN (default off; the stores exist only once Part-IV features have run) |
| `cross_run_advisory` | `LOOPLAB_CROSS_RUN_ADVISORY` | `false` | PART IV cross-run Step 5 (§21.20.5). Fold the bounded cross-run **context pack** — evidence-grounded claims with BOTH support and counter-evidence (Step 4) + a portfolio-coverage line (Step 3) — into the Researcher's proposal prompt, exactly like the E4 cross-run prior note. The gated flip of Step 2 from audit-only to a live prompt cue. Advisory ONLY: prompt-grounding, never touches node selection (§21.7). Reads `memory_dir` (`lessons.jsonl` + `concept_capsules.jsonl`); empty when the store is. OPT-IN (default off); measure via a frozen A/B before defaulting on |
| `cross_run_structured_claims` | `LOOPLAB_CROSS_RUN_STRUCTURED_CLAIMS` | `false` | PART IV cross-run §21.20.13 (full CR of the lean fuzzy claim merge). Switch the claim read-model to the SCOPE+POLARITY-safe **structured claim key** (`engine/claim_key.py`): claims from different tasks never merge, opposite polarity ("X helps" vs "X never helps") is surfaced as a CONTRADICTION rather than collapsed, paraphrase/inflection variants group by exact structured key (no transitive over-merge), and operator governance is scope-precise (a decision in task A cannot reach a same-worded claim in task B). Affects the `cross_run_advisory` context pack; OPT-IN (default off) |
| `cross_run_curation` | `LOOPLAB_CROSS_RUN_CURATION` | `false` | PART IV cross-run §22.4 (AGENTIC taxonomy steward). At finalize, when an LLM client is available, the concept steward (`engine/concept_steward.py`) reviews the freshly-updated portfolio concept graph and PROPOSES a curation — merge duplicate slugs / split conflated ones / purge noise — LOGGED to `concept_curation_log.jsonl` for operator ratification. The LLM only PROPOSES; every write goes through the same reversible append-only governance path (`record_concept_alias`/`record_concept_split`), so `fold()` stays pure. Portfolio-scoped, decoupled from the run's terminal state (never blocks/retries finalize). OPT-IN (default off); needs `memory_dir` + an LLM backend |
...
| `capability_expansion` | `LOOPLAB_CAPABILITY_EXPANSION` | `false` | PART IV Phase 2b (D7). When action-space LOCK-IN is detected (the concept-graph cadence reports a long consecutive same-lever streak) on an `explore` stance, escalate the Researcher's novelty hint from "broaden" to a forced-JUMP directive — expand the action space / build the missing infra (new data pipeline, external-mined negatives, a different eval), not another variant of the saturated lever. Prompt-cue only (the scored capability-expansion policy operator waits on the R1/SearchFitness gates); reads the 2a snapshot, so it needs `concept_pivot` to record one. Never touches selection |
