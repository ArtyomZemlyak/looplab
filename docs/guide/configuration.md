...
server-owned curated catalogue with **183 of the 216 direct `Settings` fields in 10 groups**. The default
...
| `proposal_width` | `LOOPLAB_PROPOSAL_WIDTH` | `true` | Let the **research** decide the run's eval width, not the box alone (docs/29 F1). While the Card board holds open `selection_ready` proposals, an **AUTO** `eval_parallel` is re-pinned mid-run to `min(gpu_pool // widest declared footprint.gpus, the width run_started pinned)`, and an AUTO `llm_parallel` follows it wherever AUTO actually resolved to that width (a run whose build calls no LLM settled its AUTO build width to serial `1` for the byte-identical event order, and stays there) — so two Cards each declaring `{"gpus": 2}` on a two-GPU box run at width `1`, which is what was already happening while `run_started` claimed `2`. When the proposals ask for more concurrency than the box has, **the surplus queues and the width never oversubscribes the pool**: the per-experiment ceiling the Researcher is quoted is `pool // eval_parallel`, so a wider width would quote the next Card less than the Cards already on the board declared. Every change appends a durable `run_width_settled` event carrying the new widths plus the pool/demand/footprint evidence, and **a resume adopts that record rather than re-deriving a width from its own box** (engine invariant #6) — which is the whole reason this is a durable event and not a per-turn recomputation. Deliberately not a `budget_extend`: that is a human control intent, an engine-authored one would permanently disable the re-entry width refusal, and it is forbidden in speculation-calibration evidence. Narrow by construction — AUTO axes only (an explicitly spelled width is never overridden), never an axis an operator retuned live, never during a calibration run, only with `card_driven_selection` on, and only between batches with nothing in flight. `speculation_depth` is **not** re-derived from the new width: it is a `run_started` pin bound into the speculation gate's receipt, and moving it here would break the guarded re-entry path at the next resume. A snapshot written before this field existed resumes with it OFF. |
| `gpu_footprint_cue` | `LOOPLAB_GPU_FOOTPRINT_CUE` | `true` | What the two Researcher prompts say a LARGER `footprint.gpus` does. Both used to claim it "does NOT get this experiment more hardware" and that the run "serialises at the same per-experiment cost" — the scheduler contradicts both halves (a declared count beats AUTO, `_acquire_gpus` reserves exactly that many devices and the child is fenced to them), and no Card on this box has ever declared anything but `1`. ON states the trade instead: the ordinary share is a DEFAULT, a larger count is honoured and re-pins the run to `pool // gpus` concurrent experiments, plus the throughput/latency arithmetic in both directions, the per-device memory the scheduler reserves against, and an invitation to measure this box's own speedup with a short fixed-step probe rather than assume one. Widens the ACTION space only (docs/36): the pool clamp, `proposal_derived_width`, the metric, the champion and selection are all untouched, and an explicit count in the task statement still wins. `false` reproduces both historical paragraphs byte for byte |
...
| `train_monitor_kill` | `LOOPLAB_TRAIN_MONITOR_KILL` | `true` | INTERVENTION: let the monitor tree-kill a training it judges `broken` (diverged / silent CPU fallback / not learning) early. The node then fails normally (`reason=monitor_broken`) with no repair and no retry, so the gate is deliberately narrow. **Which log may kill:** only one the eval plan can PROVE is the run's own training — the classic single-command eval (`eval.log`, one command that trains *and* scores), or a one-stage pipeline, which is that same shape wearing a stage name. In a MULTI-stage pipeline the last stage is the scorer (that is where `command_eval` reads the metric) and every earlier stage is a work stage the engine cannot prove is the training step, so its verdicts stay **advisory for the KILL** — a `data_prep` stage printing framework warnings and a flat loss draws a confident `broken` even when the prompt names the stage. They are **not** advisory for the REPAIR-STOP: since 2026-08-19 a `broken` verdict the model attributes to `fault="implementation"` stops that stage with `reason=not_learning`, on every judged role, because being wrong there costs one restart of a run the judge has just called wasted rather than a destroyed experiment. Every other conjunct below applies to it unchanged. `setup.log` (dep install), the scorer, and any filename with two possible writers (a stage named `setup`) are never judged at all; a log the monitor cannot attribute is advisory. **On top of that**, a kill (or a repair-stop) must be CONFIRMED by a second consecutive `broken` verdict about the same log — a prompt re-look, not another full cadence — where a tick that produced no parseable verdict (endpoint failure, an answer outside the schema) re-arms the gate from zero, and the armed re-look is bounded to one identical re-ask and a five-minute window. **Which ticks arm** is asked as a counterfactual rather than re-listed: the engine re-asks both gates with the streak already satisfied, so a verdict that a repeat would act on arms and one it would refuse does not. That is what gives a work-stage repair-stop its prompt re-look — it used to wait a full cadence, and on a log that diverged and then went silent it never got one at all, because an unchanged digest produces no tick unless an arm is standing — and it is also what stops a `broken` below this bar from buying a re-ask and a digest bypass for a kill the gate will refuse anyway. **And on top of THAT**, the engine's own measured loss trajectory must not contradict the verdict: the judge sees the digested tail, which on a tqdm-style log is about ten loss values over ~30 seconds of a multi-hour run, so the engine accumulates one reduction of that tail per check and decides "is it still descending" deterministically over the whole eval. A `broken` verdict about a run whose measured loss has fallen by more than its own step-to-step noise floor since the first check neither kills nor arms — it stays an advisory row, stamped with the measurement (`trajectory`, `trajectory_veto`). A non-finite loss/`grad_norm` or an explosion withdraws that protection, and a log with no parseable loss at all is measured as `unknown` and keeps exactly the authority it had. Off = observe only |
...
| `train_monitor_tools` | `LOOPLAB_TRAIN_MONITOR_TOOLS` | `true` | Let the two live-eval watchdog **judges** QUERY the stage logs instead of only being handed a fixed slice of them (`looplab/tools/log_tools.py`). Two tools: `read_log` reads a named log like a file — tail, head, a record range, or a regex search with context, where a "record" is split on newline **or** carriage return because a tqdm bar writes a whole multi-hour run into one newline-delimited line. **tail/head read a bounded WINDOW; range and search SEEK** from this attempt's start, which is a different bound for a different question: as a tail window ceilinged by the reader's 32 MiB, the head 61.8 % of the corpus's largest log (88 MB) was unsearchable at any parameter and a restart marker printed 4 times in it was reported as 1 match, while the *default* 256 KiB window answered "no record matches 'Traceback'" about a log whose byte 0 is a traceback. A sweep **always runs to the end of the log** — it has two stops and each names the `from_byte=` that continues it: its own 128 MiB byte-parsed ceiling, and a 10-second wall clock checked between records, which exists because the pattern comes from a MODEL and `re` cannot be interrupted mid-match (a whole ceiling's worth of real work is ~1.4 s at the measured ~10 ms/MB, so the deadline never fires on a sane pattern; one record contributes at most 64 KiB to a match attempt, and clipped records are counted in the receipt). So the match count is a TOTAL rather than a floor, and the answer shows the FIRST matches *and* the LAST ones with the elided middle stated as an exact count — both ends, because a watchdog shown only the first `Traceback` can kill a healthy node over a crash a repair already fixed, and one shown only the last cannot tell a run that has always been broken from one that just broke; `metric_series` answers over the numeric series the log prints (`loss`, `grad_norm`, `learning_rate`, `epoch`, the step counter, `it/s`) across a time window at a granularity the judge picks — the last 10 seconds, the whole run bucketed hourly — reporting count/median/spread/min/max/first/last per bucket, so a coarser granularity aggregates MORE samples and never drops any. **It exists because the slice was measured insufficient**: on `runs/rubertlite-dr-unified-v7` the digest a judge received was about ten loss values, roughly 30 seconds of a five-hour run, and at the 600-second cadence consecutive tails do not even overlap — so it answered "pinned at ~23.0, no learning trend from its initialization value" about a node that had run 27.69 → 22.92. **What it does NOT widen:** everything these tools return is text the candidate's own training script wrote, so it may inform the verdict and never becomes a fact a record rests on — `should_monitor_kill`'s conjuncts are unchanged and the deterministic loss-trajectory veto stays engine-side (`docs/36-agent-driven-decisions-2026-08-13.md`). **The boundary:** a log is chosen by NAME from the map `eval_log_plan` derives from the resolved pipeline, never by path, so nothing outside the node's own workdir is expressible; a log the plan cannot attribute to a phase is not readable at all; and every read is a bounded seek whose answer states the byte range it covered and the call that continues past it. **Cost:** up to six extra model round trips per check (deliberately below the 15 the one-shot trust verifiers get, because this judge fires on a timer up to 200 times per node), and a default log read is a 256 KiB seek — measured 2.4 ms for `read_log`, 13-14 ms for `metric_series`, on 6-16 MB live logs. A whole-run scan is 0.7 s on a 5.9 MB log and 1.3 s on a 15.7 MB one, and the answer states the bytes it read. A search costs the same whatever it finds, because it never stops early: ~10 ms/MB, measured 2026-08-15 at 138-150 ms / 564-580 ms / 879-905 ms on the 15 / 45 / 88 MB logs, against ~54 ms/MB for a metric scan over the same bytes. Six sweeps of the largest log in one judge tick is ~5.4 s, i.e. 0.9 % of the 600-second cadence. Off restores the historical single completion byte for byte, prompt included. A snapshot written before this field existed resumes with it OFF. No-op unless `train_monitor` is on |
| `train_monitor_contract` | `LOOPLAB_TRAIN_MONITOR_CONTRACT` | `true` | Show the live training-log watchdog the watched stage's **own declared contract** — the `expect.assert` sentence and the `expect.files` it must produce, quoted from the manifest the agent wrote — and, when the trainer's own progress records say the schedule it configured cannot reach a declared epoch count, the engine's reading of that shortfall (`looplab/engine/train_monitor.py::stage_contract_context`). **It exists because that is the class the judge cannot see, and the split is stark**: over the committed 450-decision bench (`tests/data/judge_bench/train_monitor.v1.jsonl.gz`) the judge answers `broken` on 48 of the 53 decisions whose node trained fine and then scored ~0 — 91 % — and on 2 of the 38 whose stage EXITED 0 and was then failed by the engine on its own declaration, 5 %. It was never blind there; the training really did finish, and nothing had told it the stage had promised anything. 13.4 h of the 20.1 h an ideal judge could still save on that corpus sits in that class. The schedule reading pairs a step counter with the epoch printed **in the same record**, because 109 of the 109 corpus logs above 200 KB carry more than one progress-bar lane and the loose pairing reads a completed 10-epoch training that scored 0.716575 as a 4.02-epoch one; it uses the SAME whole-epoch tolerance as the end-of-stage floor (`runtime/command_eval.py::declared_epoch_completion`), so it can never call a stage short that the floor would acquit — driven against `runs/rubertlite-dr-unified-v9` node 0 (14.87 of a declared 15), where it reads 14.87-14.92 and says nothing on all 11 of that node's decisions. Scored as a standalone deterministic signal over all 450 rows: 12 stops, **12 wasted / 0 productive**. **What it does NOT widen:** it is EVIDENCE only (`docs/36-agent-driven-decisions-2026-08-13.md`) — `should_monitor_kill`, `should_monitor_repair`, the kill-eligible roles and the deterministic trajectory veto are untouched, and it can mint no metric, champion, violation or selection. **Cost: zero extra provider calls** — two sentences in the user message of a tick already being paid for. Off restores the historical message byte for byte. No-op unless `train_monitor` is on |
| `repair_log_tools` | `LOOPLAB_REPAIR_LOG_TOOLS` | `true` | The same permission for the **crash/timeout triage judge** — the role whose whole job is "why did this stage die, and what do I change" — over the stage logs of the eval that just failed (`train_monitor.repair_log_tools`, which shares `monitor_log_sources` with the row above so there is ONE derivation of what is lookable and ONE reading of the attempt byte floor). **It exists because that role's slice is the smallest in the engine and was measured wrong**: `_eval_failure_text` hands triage `res.stderr[-500:]`, five hundred *characters*, where the watchdog's digest at least comes off a 128 KiB read. On `runs/rubertlite-dr-unified-v8` node 3 the `train` stage hit its own declared 22,000 s ceiling at 22,003 s — after training had completed all 15 epochs (`10590/10590 [5:29:35]`, then `{'train_runtime': 19775.3, …, 'epoch': 14.98}`) and a RETRIEVAL phase with its own progress bar on a different total had run to `223/361 [31:29<19:50]`, about twenty minutes from finishing. The durable `node_repaired.error_in` is 522 characters and holds only the last two renders of that second bar; the verdict read its elapsed field as training progress ("still in epoch 1 at 31:20" — `31:20` is verbatim the `222/361` render) and prescribed halving the batch **and** cutting `n_epochs` 15→8. 6.1 GPU-hours were discarded twenty minutes from a result — and are being discarded again: that epochs cut **never landed** (`repair_verify` stamped `unmet` on the same row, `config.yaml` still reads `n_epochs: 15`), so attempt 6 re-ran the same 10,590 steps. **That projection — 22,096 s into the same 22,000 s ceiling — was FALSIFIED by the run and is retracted**: `train` passed in 19,915.75 s, `score` ran 3,130.3 s, and the node recorded 0.762048 to become v8's champion, because a SECOND edit in the same repair deleted the in-`train` `test_model()` call and moved the full-index retrieval into the protected `score` stage. The misdiagnosis stands — it was wrong about where the time went, and being rescued by an unrelated edit is not a diagnosis working. **What it does NOT widen:** the verdict vocabulary (`TRIAGE_ACTIONS`), both fail-closed degradations and the terminal's authenticated `reason` are untouched, so nothing read here can reach a metric, a champion, selectability or a violation — same rule as the row above, `docs/36`. **The boundary** is identical too: a log is named, never pathed; the map is `eval_log_plan` over the resolved pipeline; every read is a bounded seek that states its byte range; and `LogSource.floor` is `attempt_byte_floor` over the snapshot taken **before this attempt started**, so a repairer diagnosing attempt N cannot read attempt N-1's curve as its own. **Cost** (mean of 5, warm, on node 3's real workdir): **1.35 ms** of engine-side derivation per failed attempt for the whole added path (log plan + attempt-floor snapshot + source map), plus up to four extra model round trips whose tool calls cost 7.1 ms (a `read_log` tail) to 525 ms (a whole-run hourly `metric_series` over the real 10.0 MB `train.log`, all 10,017,726 bytes stated) — *four*, not the watchdog's six, because this grant is additive over a budget already sized for `read_code`/`find_analogous` and covers only the new shape (a whole-run series, one search, one follow-up, one spare); and it is paid once per failed attempt rather than on a timer. Off restores the historical ask byte for byte, prompt included. A snapshot written before this field existed resumes with it OFF. No-op for an eval that writes no named stage log (the toy/dataset `solution.py` paths) |
| `asha_live` | `LOOPLAB_ASHA_LIVE` | `true` | ASHA live-curve watchdog for command evals using `stdout_json` or `stdout_regex`: read the latest intermediate objective metric and rank it against finished siblings at the same declared resource value. Other metric readers have no live observation path. Advisory (a fold-ignored `asha_rank` diagnostic + span) unless the stricter kill contract below is satisfied; with `watchdog_reflection` on, the raw diagnostic can also advise a later proposal without changing the current champion. Needs at least `asha_live_min_siblings` comparable finished nodes. **A watchdog that cannot act says so**: when the metric contract makes a kill structurally unreachable, or when the training log has been writing for three consecutive checks without ever printing the objective, the watchdog opens one `asha_monitor` span carrying `kill_reachable: false` + `inert_reason` and logs one WARNING — once per reason per eval, naming what would have to be declared. Same attribute name and meaning as the training monitor's. Library default off |
| `asha_live_kill` | `LOOPLAB_ASHA_LIVE_KILL` | `true` | Intervention, only for `stdout_json` with an explicit `metric.resource_key`: a node whose comparable intermediate metric stays below the bar past the grace window becomes a candidate for an early stop (fails it `reason=asha_underperforming`). The rank test is only the evidence — the stop itself needs a confident `stop` verdict from the LLM judge (see `asha_live_kill_confidence`), so no LLM client means no kill. `stdout_regex` can be ranked but not killed; without a resource key the watchdog remains advisory — and in both cases it now STATES that on its own span and in the log rather than leaving `asha_live_kill: true` reading as an armed safety net (measured: zero `asha_rank` rows in six real runs, all with both switches on) |
...
| `eval_env` | `LOOPLAB_EVAL_ENV` | `{}` | RUN-LEVEL DECLARED ENVIRONMENT: variables set for every eval of every node — the per-node `setup`, the single `command`, and every stage on both sandbox tiers, and the solution.py sandbox path for a non-repo task. Write it as a mapping in a config file (`eval_env: {VS_LOCAL_DATA_ROOT: /data/local}`) or as `-s eval_env=NAME=VALUE` (comma-separated for several) on the command line. The run-wide layer of the same contract the task's `cmd.env` and a stage's own `cmd.stages[].env` carry — most specific wins (`eval_env` < `cmd.env` < `stages[].env`). This is what makes a fact like "the local corpus lives here" a property of the RUN instead of something each node's Developer rediscovers by crashing: it is recorded in `config.snapshot.json` AND in `run_started`, so a resume reproduces the same environment (engine invariant #6) and the log says what the nodes ran under, rather than the launching shell's history. LoopLab REFUSES a name or value that looks like a credential, and refuses any name it owns itself (`CUDA_VISIBLE_DEVICES`, `LOOPLAB_*`) — see [Declaring an eval's environment](tasks.md#declaring-an-evals-environment). NOT applied to the run-level `run_setup`, which runs once in your own source tree before any node exists |
...
`footprint.gpus` they declared?* — and re-pins `eval_parallel` (and, when it too was AUTO *and* that AUTO resolved to the eval width
rather than to the serial `1` a build calling no LLM settles at, `llm_parallel`) to the answer:

```
width = min( gpu_pool // widest declared footprint.gpus,   # what the box can serve at that footprint
             the width run_started pinned )                # never wider than the run was launched at
```

Two cards each declaring `{"gpus": 2}` on a two-GPU box settle the width to **1** — which is what
already happened in practice, except that `run_started` used to claim `2` while node 0 held both
devices for its whole lifecycle. Five cards each declaring `{"gpus": 1}` on that box stay at **2**,
and the other three **queue**.

The *count* of open cards is deliberately not a term, even though "one experiment per card" sounds
like it should be. The board holds one ready card most of the time — that is the ordinary state
between a terminal and the next research turn — so counting it would narrow the run to serial on
board churn and widen it back a turn later, appending a durable row each way. It would also buy
nothing: the dispatcher fills up to the width from whatever work exists, so a width above the number
of ready cards already dispatches exactly the ready cards. What the width has to express is the
SHAPE of the proposals (how much hardware one of them wants), not the momentary length of the queue.

**When the proposals ask for more than the box has, the surplus queues — the width never
oversubscribes the pool.** That is a decision, not an omission, and it has three separate reasons.
It keeps the per-experiment ceiling below honest (a width above `pool // need` would quote the next
Card a smaller budget than the Cards already on the board declared); an eval slot above the device
count buys a node holding that slot while blocked in `_acquire_gpus` rather than any parallelism; and
the aged-head escape hatch in the parallel dispatcher detects a drained pool by comparing free slots
against the batch's width, which cannot fire at all when the width exceeds the batch's own capacity.

Each change is recorded as a durable **`run_width_settled`** event carrying the new widths and the
evidence they were derived from (pool size, open-proposal count, widest declared footprint). That is
what makes it safe under invariant #6: a resume **adopts the recorded width and never re-derives
one**, so a run that narrowed itself to 1 on a two-GPU box continues at 1 when resumed on an
eight-GPU box, exactly as `run_started`'s own pin behaves. It is deliberately *not* a `budget_extend`
— that is a human control intent, the engine writing one would permanently disable the re-entry
refusal that protects the pin, and it is a forbidden event in speculation-calibration evidence.

The re-pin is narrow by construction. It moves only an axis that was launched **AUTO** (an explicitly
spelled `eval_parallel` is the operator's number and is never overridden), it stands aside on any axis
an operator has retuned live through `budget_extend`, it never fires during a speculation-calibration
run, it needs `card_driven_selection` (no Card board, no proposals), and it fires only between
batches with nothing in flight. `speculation_depth` is deliberately **not** re-derived from the new
width even though its own AUTO resolves off it: the depth is a `run_started` pin bound into the
speculation gate's receipt, and moving it here would make the run unresumable through the guarded
path at the *next* re-entry. Set `proposal_width=false` to keep the box-derived width.

**The Researcher is told the resulting per-experiment ceiling.** The footprint contract asks the
Researcher for a `gpus` count, so it needs to know what count is affordable — and until it was told,
the only channel carrying that was the operator's goal prose, which is how a run with
`eval_parallel: 2` on two GPUs came to declare `{"gpus": 2}` on every Card and go serial at double
the per-node cost. The engine now stamps a `GPU BUDGET` line into **both** Researcher prompts (the
plain and the agentic one) on every proposal:

`max(1, pool // eval_parallel)` — the same per-experiment share an *undeclared* footprint already
gets from `engine/resources.py::_resource_request_for_node` — with three deliberate edges:

* **No GPU on the host** -> the ceiling is `0`, not `1`, and the line says so: a positive `gpus` is
  `required_unavailable` and fails admission closed rather than queueing.
* **A CPU-locked task** (`gpu_capable() -> False`) is told nothing at all. Its undeclared footprint is
  already a zero-device request that never takes the pool lease, so there is no ceiling to state.
* **A width the box cannot serve** (an explicit `eval_parallel` above the GPU count) still yields `1`,
  never `0`: the devices exist and the scheduler queues for them.

The *width* in that arithmetic is the run's own pinned one — the line is computed per proposal, i.e.
after `run_started`'s pin has been restored and after any `budget_extend` retune — so a resume never
quotes a width re-derived from the box it landed on. The *pool* is deliberately the live box, because
that is what the reservation clamps against. It is a ceiling on what the Researcher is asked to
declare, not a clamp: a declared footprint remains authoritative at admission.
...
`CUDA_VISIBLE_DEVICES`, and the per-experiment cost is precisely what a second device changes. That
sentence was written from the v5 incident above, which was a *width* defect that `proposal_width`
closed — and it outlived its cause: every node of all six preserved runs on this box declared
`{"gpus": 1}`. With `gpu_footprint_cue` on (the default) the cue states the trade instead of
foreclosing it — a larger count is honoured and re-pins the run to `pool // gpus` concurrent
experiments; K devices at the same *per-device* batch take about `1/K` the steps, i.e. about the same
experiments per hour, each finishing sooner, but a **different** experiment (K× the effective batch,
and K× the in-batch negative pool for a contrastive loss); K devices at the same *global* batch give a
speedup below K, i.e. the same experiment, fewer per hour; and if the experiment does not fit on one
device the count is not a preference at all, since gradient accumulation restores the effective batch
and not the negative pool. It also names the per-device memory the scheduler reserves against (the
`_gpu_mem` inventory, silent when that inventory did not join losslessly), states that this box's own
speedup is **unmeasured** and invites a short fixed-step probe before hours are committed to a
footprint. `gpu_footprint_cue=false` restores the historical wording of both paragraphs byte for byte.
An explicit count in the operator's own task statement still wins over all of it — and that is the
usual reason a run's Cards are uniform: `runs/e5small-dr-unified-v2`'s goal text ends "EACH EXPERIMENT
GETS EXACTLY ONE GPU".
...
`eval_deadline_grace_s` does **not** change the announced ceiling. When it is on (which it now is by
default, as AUTO) the prompts name it as a rescue for a stage demonstrably about to finish, to be planned against as
...
| `developer_probe` | `LOOPLAB_DEVELOPER_PROBE` | `true` | **F2 · the Developer's PROBE** (`looplab/tools/dev_probe.py`). The repo Developer may run a short **Python program** against the REAL environment while it authors — "does this package actually import here", "does this CSV parse", "does the code I just staged get this API right" — instead of guessing and then writing a workaround around the guess (the observed shape was *"Since I have no shell/install ability, the cleanest repair is a small loguru shim module"*, i.e. a fake library). It is **not a shell**, and that is the design rather than a limitation: the source-tree read fence is a CPython audit hook, so a tool that could run `cat`/`cp`/`bash` would be an execution surface the fence does not reach. The probe process **cannot write a file anywhere** — three rungs, each stated for exactly what it covers: the audit hook for the message (and for metadata/truncation, which Landlock ABI 2 does not mediate) (a non-`OSError` refusal, so `except OSError:` cannot turn it into a silent skip), a **Landlock** ruleset handling every filesystem-mutating access right and granting nothing for a file's EXISTENCE in the kernel, and `RLIMIT_FSIZE 0` for its CONTENT — which together are what makes a mid-run `pip install` impossible without ever naming pip. The kernel rung is always on, has no `Settings` field and is unrelated to `landlock` below (that one is a READ allow-list for an eval; this one grants nothing and polices no read bit): it landed 2026-08-15 because `os.mknod`/`os.mkfifo` raise no audit event at all and the rlimit bounds bytes rather than existence, so a probe could create a regular zero-byte file — an empty `.py` shadowing a real module for every later node — while emitting no event, and `pyarrow`/`h5py` can do the same with no Python-level name to refuse. On a kernel without Landlock the probe still runs and says on its own stderr what is no longer covered, **cannot start another program**, **cannot see a GPU** (`CUDA_VISIBLE_DEVICES=""`, so it can never disturb a sibling node's training), and **cannot read the editable source tree** — it carries a fence derived from the same `read_fence.fence_inputs` an eval's does, always at `deny` regardless of `read_fence`, because turning the eval fence off must not open a second door. Its whole world is a temp directory holding a disposable replica of the node's staged files, deleted when the call returns. Having no side effect is why it needs no domain event: it is a `tool` span in `spans.jsonl` like every other Developer tool call. `false` removes the tool entirely and restores the previous system prompt byte-for-byte. A resumed pre-2026-08-13 run keeps `false` (`LEGACY_CONFIG_SNAPSHOT_DEFAULTS`) — re-entry never hands a run's Developer a subprocess-launching tool, nor a different system prompt (887 bytes apart, first divergence at offset 455), that the first half of its own event log never had |
| `developer_probe_timeout_s` | `LOOPLAB_DEVELOPER_PROBE_TIMEOUT_S` | `60` | Wall clock for ONE probe (hard max 300 in the tool). Deliberately short: a probe is a question, and anything needing longer belongs in a declared eval stage — the surface that has a metric contract, a live log and a repair loop. There is deliberately **no probe count budget**: the developer session already has a finite wall-clock ceiling (`developer_session_time_budget_s`) that a probe spends like any other turn, and a second fixed counter is the category error `docs/36` names |
...
| Setting | Env | Default | Description |
|---|---|---|---|
| `inline_repair` | `LOOPLAB_INLINE_REPAIR` | `true` | Repair a crashed node in place within the same eval (no extra node). **Off now means a crash simply ends the node** — the inter-node debug node it used to fall back to was removed on 2026-08-13 |
| `inline_repair_attempts` | `LOOPLAB_INLINE_REPAIR_ATTEMPTS` | `0` | **Hard upper limit on in-place repair attempts per node — and `0`, no count cap, is the shipped default since 2026-08-13.** The transition from *keep repairing* to *stop* is a JUDGMENT, not a number, because a count cannot tell a loop converging on a fix from one rewriting the same line for an hour. The two recorded disasters are both exactly that: `rubert-dr-0804` produced **369 distinct error signatures** on one broken registry, so nothing counting repetitions ever saw one; and v6 node 5 spent **three rounds halving a batch size** against an OOM that never happened, nowhere near any cap. Three things stop the loop now, and each answers a question a counter cannot: the **crash-triage model**, consulted once per attempt (the call the loop already made) with this node's whole repair history, whose `abandon` means *"I no longer know how to fix this"*; the **Developer's own declaration** — it may answer a repair ask with `(developer stuck: …)` instead of code, which ends the node cleanly and is neither a failed attempt nor a provider outage; and the **repair critic** (`repair_critic_after` below), a second model asked only whether successive attempts address different causes or circle one. None of them may decide *what the result was* — every stop terminalizes carrying the eval's own authenticated failure reason, so no metric, champion, selectability or violation moves on a model's word (`docs/36-agent-driven-decisions-2026-08-13.md`). **The floors are unchanged and are what keeps this from being an unbounded spend:** the engine's absolute ceiling of **50** repairs per node (`engine/evaluate.py::_UNLIMITED_REPAIR_CEILING`) — **12 when the crash-triage model is not wired at all** and the deterministic rule path is deciding blind (`engine/triage.py::_RULE_BLIND_CRASH_ATTEMPTS`), because 50 is the ceiling *under a judge that can say "I no longer know how to fix this"*, and in the configuration this default composes with on a resumed pre-versioning run there is neither a judge nor a critic (both read the same object) nor necessarily a seconds floor, which measured out at 51 full pipeline evaluations for one undiagnosable `RuntimeError`; it bounds only the undiagnosed branch, since a MECHANICAL crash names what to change in its own traceback, and it is a `min` with the operator's cap and so never widens one — the run's eval-time budget, `systemic_failure_stop`, and the LLM money ceiling, which raises `BudgetExceeded` at the client where the money is actually spent and therefore cannot be talked past by any judge. Unbounded on its own was measured to be worth exactly nothing — with an always-`repair` judge and no ceiling, one node ran **795 repairs / 796 full evals in 45 s with no terminal**. **Set a number to get the old fixed cap back**; the terminal always names which bound stopped the node. The budget is charged against the durable `node_repaired` events, not a process counter, so `looplab resume` continues a node's repair chain instead of granting it a fresh one — and the judges are likewise shown the repairs made by earlier processes. A judge that cannot answer is never read as permission to continue, and the two ways it can fail differ: an unreachable endpoint (`unanswerable`) stops the node with `developer_crash` **and** raises one run-level pause naming the provider, while a live model answering something outside the vocabulary (`unreadable`) stops only that node. Either way it is re-asked once first |
| `repair_critic_after` | `LOOPLAB_REPAIR_CRITIC_AFTER` | `3` | How many repairs a node must ALREADY have made before the repair critic is consulted, once per attempt from then on. **It can only stop the loop** — it never decides what the failure was, never selects a salvage and never touches the result, so it is an added veto rather than an added authority. Its evidence is authenticated: the per-attempt CAUSE it compares across attempts is the engine's own `_failure_reason`, read from the sandbox's out-of-band signal channel, and the stderr tail beside it is labelled in the prompt as candidate-controlled text — a critic that read the kind of a failure off a banner the failing script printed would hand the candidate the stop decision, which is exactly what `c862045c` took away from the failure classifier. It **fails OPEN**, the opposite of the triage judge: a critic that is unwired, unreachable or unreadable contributes nothing and the loop stops exactly where it would have without it, because one flapped socket must not kill a node every other participant considers healthy. `3` rather than `1` because the question is not askable earlier — with one attempt behind it there is no trajectory to compare, and the agent refuses an empty one outright. **0 disables it**. **Every consultation is recorded** (2026-08-15): one `repair_critic_verdict` row in `events.jsonl` per ask, carrying the verdict, its rationale, the threshold that fired, and the authenticated causes it judged — including for a `continue`, which changes nothing about the run and was therefore invisible. It also carries `source`, because `continue` is five different facts: the critic answered, it was never wired, its endpoint was down, it returned nothing, or it answered outside the vocabulary — and "n consultations, 0 stops" reads identically for all five, which is what made this setting impossible to calibrate from a run. The row is fold-ignored, so nothing selection-side can stand on it A resumed pre-2026-08-13 run keeps `0` (`LEGACY_CONFIG_SNAPSHOT_DEFAULTS`) — the critic is a paid second model with the authority to abandon a chain, and re-entry adds neither the call nor the stop to a run that had neither; it compounds with the `inline_repair_attempts: 0` row beside it, which restores the same runs' historical uncapped loop |
| `inline_repair_reasons` | `LOOPLAB_INLINE_REPAIR_REASONS` | `["crash","timeout","oom","setup","no_metric","drift","unclassified","expect_failed","check_failed","diverged","stalled","needs_failed","not_learning","check_false_positive"]` | Which failure reasons are eligible for inline repair. **Who CHOOSES the reason is a question of OWNERSHIP (2026-08-20, `engine/failure_diagnosis.py`):** the engine classifies only what it CAUSED, RAN or MEASURED and remembers doing — `timeout` (its own clock), `diverged`/`stalled`/`not_learning` (its watchdogs' kills, from `run_argv`'s out-of-band `signals`), `drift` (its cross-reader), `setup` (the return code of the setup command it ran, now a `RunResult.setup_failed` flag rather than a stderr prefix), and `needs_failed`/`expect_failed` (its own `stat` of a stage's declared input and output). No model may contradict one; the diagnostician is not even consulted. Everything else — `crash`, `no_metric` and `check_failed`, the last of which is another MODEL's reading of the stage's output rather than an engine computation — is EVIDENCE handed to a failure diagnostician, which rides the crash-triage call the loop already makes (so it costs no extra provider call), can read the dead eval's stage logs AND the code that wrote them, and must cite what it stood on. `oom` is ANSWER-ONLY: both of the engine's ways of naming it were text rules over the candidate's own stderr and both are deleted. `check_false_positive` (2026-08-21) is answer-only for the same structural reason: the stage check is another MODEL's reading of stdout, so "that reading was wrong" is a claim only a second reader can make and there is nothing for the engine to have observed. It exists because `check_failed` names the stage that REFUSED and says nothing about why — measured on `bench-out/cand.durable.jsonl`, of 22 `check_failed` rows 14 become `not_learning` and FIVE were the diagnostician refuting the checker with validation numbers out of the same log it read, with nowhere to put the finding. Like `not_learning` and `unclassified` it is absent from `NEVER_SALVAGED_REASONS`, so it can neither suppress a metric nor grant one; what it moves is the DIRECTIVE, which points the repair at the check instead of asking a Developer to rewrite an experiment whose numbers it has just been told are correct. `unclassified` is minted when the diagnostician was wired, asked, and could not answer — it repairs blind under `_RULE_BLIND_CRASH_ATTEMPTS` rather than the caller's cap, is never in `NEVER_SALVAGED_REASONS`, buys no extra attempts, and carries `reason_source: "undiagnosed"` so its rate is countable. With `unified_agent` off the deterministic classifier remains the whole answer, byte for byte. **It does not move THIS gate:** the eligibility check runs above the triage call on a `reason` recomputed from `_failure_reason` at the top of every attempt, so an operator who narrows this list is gated on the engine's own answer every time and no judged reason can switch inline repair on or off. Safe for the record by construction: every reason that suppresses a metric (`metric_salvage.NEVER_SALVAGED_REASONS`) is on the authenticated side, so a judged reason moves a repair DIRECTIVE and never a number — and `node_repaired`/`node_failed` carry `reason_source` + `engine_reason` beside `reason`, so the row says who chose it. `oom` itself covers TWO signatures that look nothing alike: that kernel kill, and the GPU allocator RAISING (`torch.OutOfMemoryError`), which exits 1 WITH a full traceback and so matches none of the kernel conjuncts. It was the kernel one only until 2026-08-20, which meant every GPU exhaustion classified as `crash` and never reached the memory-reduction directive — measured on `e5small-dr-unified-v3`, where all three nodes OOMed, all three recorded `reason: crash`, and the run stopped systemic having produced no metric. What the JUDGE adds on top of the marker list beside it: the markers are the shapes somebody has already been bitten by, and the judge is what covers the ones nobody has written down yet — a host `MemoryError`, `DefaultCPUAllocator: can't allocate memory`, an allocator wrapped in another library's exception — and, above all, a failure whose diagnosis is NOT in the captured stderr at all. That last case is the common one here: `torchrun`/`accelerate` swallow the child traceback and print only a `Root Cause … exitcode: 1` block, so the tail the deterministic classifier and the repair prompt both see names nothing, while the judge's log tools reach the stage log where the OOM actually is. Replayed over `runs/`: 26 of the 41 text-read failures in the five modern runs were out-of-memory failures recorded as `crash`, 9 of them resolvable only from the stage log. `diverged` and `stalled` are the two WATCHDOG verdicts and are separate members on purpose: both are tree-kills the engine itself issued, so both exit `-9` with no traceback — the exact signature the KERNEL half of the `oom` heuristic recognises — and "reduce memory" is the wrong instruction for both (v6 node 5 spent three repair rounds halving a batch size while its loss was going NaN). **All of them by default since 2026-08-12** — none of these reasons is evidence the HYPOTHESIS is wrong, which is the only thing that should end a node without a repair. The spend is bounded by `inline_repair_attempts`, `inline_repair_retrain_cap` and the per-attempt triage judge, not by this list. Every member is validated against the registry at construction: a mistyped reason used to be accepted and then matched nothing, i.e. it silently switched inline repair OFF for that class |
...
| `debug_depth` | `LOOPLAB_DEBUG_DEPTH` | `2` | **INERT since 2026-08-13 — kept for compatibility only.** It bounded the DEBUG NODE: how many times a failing lineage could open a fresh node to have another go at the experiment that just failed. That node was removed (a failure is repaired inside the node that failed, bounded by `inline_repair_attempts` and the judgment above it), so nothing reads this to make a decision. It is not deleted because Settings names are a compatibility surface twice over — `LOOPLAB_DEBUG_DEPTH` and every `config.snapshot.json` under `runs/` name it, and the calibrated speculation envelope pins it, so removing it would revoke receipts for a knob that no longer does anything |
...
| `inline_repair_retrain_cap` | `LOOPLAB_INLINE_REPAIR_RETRAIN_CAP` | `2` | Max FULL multi-stage re-runs (re-trains) the inline-repair loop may do before abandoning the node (there is no inter-node debug operator to abandon TO any more — it was removed on 2026-08-13). A late-stage fix (e.g. a broken `score` script that didn't touch `train`) reuses the completed train checkpoint and re-runs only from the failed stage — cheap, not counted. The reuse check is **fail-closed**: a full, counted re-train is forced not only when the repair changes earlier-stage code, but whenever reuse can't be *proven* safe — the repair deleted any file, changed a non-`.py` file (config/data inputs are invisible to import reachability), changed the stage MANIFEST at or before the reuse point (since 2026-08-18 an edit confined *strictly after* it — retuning the failed stage's own argv, or anything later — no longer forfeits: the compare is per stage ENTRY across the two manifests the engine itself committed, so it also catches a reorder, a rename, and a stage inserted or removed in front of the failed one), the eval runs under a non-default `cmd.cwd`, an earlier stage is opaque (a shell wrapper, a bare binary, or a `python -m <module>` the engine cannot resolve to a file inside the workdir — a resolvable one IS bounded, since `-m` names its entry point by import syntax), or the failed stage is missing from the post-repair pipeline. Exception: a FIRST-stage failure (no completed earlier-stage work exists to discard) is not charged this COUNT — but since 2026-08-15 it is charged the same cap in **seconds**, because "bounded by `inline_repair_attempts` and the triage model's stop decision" (what this row used to say) is false in the supported configuration where no triage model is wired: `inline_repair_attempts` defaults to `0`, which the engine reads as its ceiling of 50, and `repair_critic_after` consults the same object the triage judge would have been, so there is no critic either. Driven, that bought **51 full pipeline evaluations, 50 repairs and 0 charges** on a chain re-running an 82-minute first stage — **13 and 12 since 2026-08-15**, when the rule path's undiagnosable-crash branch got a bound of its own (`inline_repair_attempts` below). So a repair chain may also spend at most `(cap + 1)` times what the task DECLARES one full pipeline costs — the sum of the declared stage `timeout`s, or the single command's **resolved** budget (`command_eval.eval_spec_time_budget`: the largest of the base `cmd.timeout` and the profile timeouts, **not** the base alone — licensing against the base let a task whose active profile raises the timeout 36× refuse every repair on the first failure), so single-command evals no longer ignore this field entirely. It fails OPEN: `0` is still unlimited, and an eval that declares no timeout anywhere licenses no number and is not bounded by it (a floor that guessed a pipeline's cost would abandon nodes on a number nobody wrote). It is deliberately NOT "charge the count for a first-stage repair too": measured against `runs/rubertlite-dr-unified-v8` node 3 — which failed its first stage three times and PASSED it on the fourth attempt — that abandons the node one repair before the stage it was fixing worked, because the count is calibrated for a re-TRAIN (22000 s declared) and the stage being redone is a 5400 s `mine`. **A stage ROLLBACK is charged to this same cap** — when a repair names an earlier, already-successful stage as the real cause (`rollback_stage` on its `done` emit), the engine re-runs the pipeline from there, which discards completed work exactly as a forced full re-train does; a second counter would let a repair alternate the two and pay neither. A rollback is *additionally* bounded to at most one per suspect stage per node (read back off the `stage_rollback` events, so a resume does not refund it), and is refused outright unless that repair also CHANGED something the suspect stage runs or imports. 0 = unlimited (legacy). It bounds COST, which neither of those two does: they bound how many repairs happen, not what each one costs |
| `metric_salvage` | `LOOPLAB_METRIC_SALVAGE` | `"audit"` | What happens when a node fails for something other than "the metric is absent" and the run's OWN declared reader can still find the metric the eval already produced. `audit` recovers it, records `metric_provenance`, and attaches a `metric_salvaged` violation — so the node is evaluated and counted (budget, UI, lineage) but excluded from `feasible_nodes()`, which is what champion selection and breeding read: a value nobody measured must not win. `select` lets it compete on equal terms — and since 2026-08-15 the run-summary row SAYS so: a champion selected on a salvaged number carries `salvaged` in `best_metric_caveats` (`engine/champion_caveats.py`), because `/api/runs` publishes the number and nothing else about it and no client can derive the fact. `off` discards it, which is what cost one run a 76-minute training that had already printed `RECALL@100: 0.743250`. Only DETERMINISTIC rungs are used — the declared reader re-asked over the failed attempt's own output, and a declared file reader retried at the fresh same-basename file the stage actually wrote — never a model, because the agent writes the training script and therefore writes the text an extractor would read, which would be a route around the protected `score` stage. `kind: adapter` is refused outright (it EXECs agent code), file reads are held to this attempt's start so a prior attempt's artifact cannot be salvaged, and `expect.assert` failures are NOT salvageable — `files` is a claim about where and fails on a path typo, `assert` is a claim about what. **`select` reaches only OPERATOR-produced output**: the spec is always the operator's but the bytes it reads are whatever the failing stage printed, so a value out of a Developer-manifest stage keeps the `metric_salvaged` violation under every rung except `off` (the provenance records `producer`). The operator's constraints, extra readers and drift cross-check are applied to a salvaged metric exactly as to a measured one, and a value the cross-reader cannot corroborate is refused rather than admitted. Under `audit` the violation is permanent with ONE exception, and it is `metric_salvage_repair`'s, not this field's: a node whose corrected declaration PASSES the re-checked artifact contract was measured after all, so the violation and the exclusion go with it — restricted to operator-produced output, which is what keeps that path from being more permissive than `select`. A resumed pre-2026-08-12 run keeps `off` (`LEGACY_CONFIG_SNAPSHOT_DEFAULTS`) — re-entry never adds a selection policy or a paid call the run did not have |
| `metric_salvage_repair` | `LOOPLAB_METRIC_SALVAGE_REPAIR` | `true` | After salvaging, still ask the Developer to fix the DECLARATION that broke. It never re-evaluates — the experiment succeeded — so it costs one call and no GPU time, and it stops the next attempt walking into the identical failure. Its bounds are its own and are stated in `_repair_salvaged_cause`: the feature flags, `inline_repair_reasons`, a Developer that can repair, and ONCE per node lifecycle (gated on its own `salvage_cause_fix` event, so a resume never re-buys it). It deliberately does NOT consume `inline_repair_attempts` — that budget bounds re-evaluations and this buys none. Best-effort in the strict sense: a dead provider, or a budget stop, loses the fix and never the metric or the node's terminal. Since 2026-08-13 it can also UN-salvage the node: when the whole declared pipeline RAN and its LAST stage failed an artifact contract, and the fix changed EXACTLY `looplab_stages.json`, the artifact CHECK is re-asked against the corrected declaration (never the stage — no re-evaluation), held to that stage's OWN start, and a node whose contract now passes is recorded as MEASURED with no `metric_salvaged` violation and a `metric_provenance` of `salvaged: false, declaration_repaired: true`. A repair that touched code is re-run, not re-checked, and a pipeline that aborted early is never promoted — a corrected path says nothing about the stages that never ran, so no shipped pipeline shape reaches this today (`docs/guide/tasks.md`). A resumed pre-2026-08-12 run keeps it off |
...
| `read_fence` | `LOOPLAB_READ_FENCE` | `deny` | Source-tree READ FENCE. `seed_mode` decides what a node's own copy CONTAINS; this decides that the copy is the only place the node may read from. A generated `sitecustomize.py` under `<run>/.looplab-fence/` goes first on the eval's `PYTHONPATH` and installs an audit hook that inspects every `open`, every `os.chdir`, and the twelve MUTATION events (`os.remove`/`os.rename`/`os.truncate`/`os.chmod`/`os.chown`/`os.utime`/`os.mkdir`/`os.rmdir`/`os.symlink`/`os.link`/`os.setxattr`/`os.removexattr` — none of which raises `open`, so before 2026-08-13 a node could DELETE the operator's tree while every read of it was refused): `deny` (default) raises `LoopLabSourceReadRefused` in the child, so the refusal — which names the fix — is in the node's own stderr and reaches the repair loop; `warn` lets the read or the mutation through but logs one line per distinct path to stderr and to `<run>/.looplab-fence/violations.log`; `off` installs nothing. No-op for a non-repo task and for the Docker tiers (the source tree is never bind-mounted into a container). The node workdir, the run directory, `/tmp`, site-packages, the model cache and every `data:`/`references:` mount SOURCE stay readable — a mount is the sanctioned read channel even when it lives inside the editable tree. The one false positive is a large **untracked** in-tree input that `seed_mode="auto"` does not copy: the fix is a `data:` mount or `seed_mode: "all"`, and `warn` is the honest rung while you find out. It is a CPython audit hook and covers only what goes through CPython: a native reader (`safetensors`, `h5py`, `pyarrow`, `ctypes`), a non-Python child, a `python -S`/`-E`/`-I` child and a symlink/hardlink planted in the workdir all read past it ([tasks](tasks.md), [coverage audit](../38-fence-coverage-audit-2026-08-13.md)). A resumed pre-2026-08-13 run keeps `off` (`LEGACY_CONFIG_SNAPSHOT_DEFAULTS`) — the fence can FAIL a node, and re-entry never adds an intervention the run's first half did not have |
| `metric_subject` | `LOOPLAB_METRIC_SUBJECT` | `audit` | METRIC PROVENANCE — what a recorded number is a claim ABOUT. `eval.metric.subject` names the workdir-relative artifact (the checkpoint, the predictions file); the engine binds it to that artifact's content identity (`core/atomicio.file_identity` plus a sha256, full under 256 MiB and SAMPLED above it with the mode on the record) at the SCORE stage's start, and folds the result onto `node_evaluated.metric_provenance`. `off` records nothing (the behaviour before 2026-08-13); `audit` records always, bound or not, with no violation, no selection effect and NO stage contract; `require` is the only rung that GATES — it attaches the EXISTING `metric_salvaged` violation to an UNBOUND metric, so the node is counted and visible (budget, UI, lineage) and excluded from `feasible_nodes()` (it can never become champion or be bred from), and it derives a `needs` entry from the declared subject onto the engine-appended `score` stage so a subject the pipeline did not produce refuses before the scorer runs. **The rungs are ORDERED and the middle one must not gate:** that derived `needs` fired for every mode but `off` until 2026-08-14, which made `audit` HARSHER than `require` — driven, a declared-but-missing subject returned `needs_failed` with `metric=None`, i.e. the node lost its number outright where `require` keeps it and records a violation, and `needs_failed` is in the default `inline_repair_reasons`, so it also bought Developer repair calls on a stage no agent may edit. Nothing on disk was affected: the entry is written only for a task that DECLARES a subject and 0 of the 113 task snapshots under `runs/` + `examples/` do. `UNBOUND_REASONS` is closed (`not_declared` · `escapes` · `missing` · `empty` · `stale` · `unreadable` · `ambiguous`) because the fixes differ; the refusal names the one that applies. **A subject may also be declared as a PATTERN** (`eval.metric.subject_glob`), for the measured case where the operator cannot write the path at submit time at all: over `runs/`, all 17 node outputs in the four repo runs that train anything land at `vectorsearch/experiments/<a name the AGENT chose>/final/model.safetensors` with TEN distinct spellings of that segment, so a literal binds 0 of 5 nodes on the live `rubertlite-dr-unified-v8` and every one of its evaluated nodes records `not_declared`. The ENGINE resolves the pattern by walking the real workdir at the score stage's start — nothing the candidate wrote about itself is read — and it binds ONLY on a UNIQUE match: zero is `missing`, two or more is `ambiguous` and is a REFUSAL. That rule is the safety argument, not a convenience: each v8 node holds four `model.safetensors` of 92,174,712 bytes (`final/` plus three `checkpoint-NNNN/`), so a careless `experiments/*/*/model.safetensors` matches all four and binding one would record `subject_bound: true` about bytes nobody chose — strictly worse than the `not_declared` it replaced. mtime never breaks the tie (a value the candidate can set may refuse a binding, never elect one) and the agent's own `expect.files` never breaks it either (docs/36). A pattern derives NO `needs` under `require`, because that check stats literal paths and a second resolution at a second instant can disagree with the binding — what is given up is latency only. A SUBJECT has a freshness rule that a stage `needs` deliberately does not — an input may legitimately predate its stage, but this attempt's number cannot be about an earlier attempt's checkpoint. That rule is SCOPED to attempts that produce their own artifact: on a stage-scoped re-run (`start_stage`, the engine's own decision to skip a `train` the repair did not touch) the earlier attempt's checkpoint IS this attempt's subject, so the floor is dropped for that attempt only — one derivation, `runtime/command_eval.py::attempt_freshness_floor`, shared with the same relaxation the constraint/extra/cross-check readers already had; the primary metric read stays strict because it comes from the stage that re-ran. **Why the number that decides this matters:** across the six repo runs with an event log, 82 of 83 recorded metrics carry no provenance AT ALL (the one exception is the salvage path, i.e. it is written only once something has already failed), and 2 of 83 are provably about bytes the node did not produce — the same 0.224975, in two runs three weeks apart, from the same foreign checkpoint. **Why `audit` and not `require` by default:** no shipped task declares a `subject` yet, so `require` today would leave every existing repo run with no champion. **The evidence that would flip it:** one real repo run on a task that declares `eval.metric.subject` in which every evaluated node records `subject_bound: true` — countable directly from the audit rung's own record. Re-derived 2026-08-15 against the three evaluated `rubertlite-dr-unified-v8` workdirs, the pattern `vectorsearch/experiments/*/final/model.safetensors` binds 3 of 3 uniquely, with three DISTINCT digests (two of those nodes chose the same experiment name and produced different bytes). That is the shape of the evidence; the default has NOT been flipped on it, because it is a re-derivation over preserved workdirs and not a run that recorded it live. A resumed pre-2026-08-13 run keeps `off` (`LEGACY_CONFIG_SNAPSHOT_DEFAULTS`): it must not acquire a provenance record — nor, at `require`, a violation and a stage contract — that its own first half never had ([tasks](tasks.md)) |
| `auto_extra_metrics` | `LOOPLAB_AUTO_EXTRA_METRICS` | `true` | MAY AN UNDECLARED NUMBER BECOME A METRIC? `extra_metrics` is filled from two channels and only one was ever guarded: the operator's own `eval.metrics` reader specs (which refuse an agent-authored `adapter` reader, the same rule `cross_check` follows) and AUTO-CAPTURE — every OTHER numeric key on the candidate's own stdout JSON line, with no declaration, no reader spec and no gate. **Re-measured 2026-08-14 across the whole preserved corpus (238 logs under `runs/`): 1,642 recorded values over 10 distinct keys, and declared produced 0 of them.** They are two populations: 1,636 values are `speculation_cuda_probe_v=1.0` (a schema VERSION number), `device_count`, `alloc_bytes` and `device_ordinal` — diagnostics the ENGINE's own spliced CUDA-probe source printed — and 6 are genuine measurements an agent-authored script printed (`train_auc`, `test_auc`, `cv_mean_auc`, `cv_std_auc`, `std`, `overfitting_gap`) — and all of them reached the operator's metrics table, the Pareto front, the MLflow export and the reviewer view in the same visual place as the protected primary metric. `true` (default) captures them and TAGS each value with its channel in `node_evaluated.extra_metrics_provenance` (`{name: "declared"|"auto"|"engine"}`); `false` records only the values the candidate did NOT author — an operator-declared reader's, and the engine probe's. **The tag is what closes the defect and it is written either way** — the gate is for a run that wants no agent-authored number in its record at all, and it DELETES information that cannot be recovered later. `engine` is the third channel (2026-08-14): the engine splices its own probe source into some artifacts, so the keys that source declares are authenticated rather than taken on trust, and `false` no longer strips the CUDA proof `search/speculation_quality.py::_validate_cuda_probe_artifact` re-derives from that exact key schema. **What authenticates it is the ENGINE's own assertion that it authored the artifact** (`engine/speculation_gate.py::engine_authored_artifacts`, read from the run's role wiring), NOT a property of the artifact: the first implementation granted `engine` on a byte-exact match against the code about to run, which is `solution.py` — code the candidate writes, over a prefix that is a public constant in the shipped tree — so a pasted prefix earned the tag and passed this very switch (fixed 2026-08-14). It is a diagnostic, not a measurement of the experiment, and every surface labels it that way. A value carrying NO tag is a row written before 2026-08-14 and reads back `unknown` at every consumer — deliberately not `declared`, since every such preserved value was in fact auto-captured. Write-side only: the fold never reads this field, so it cannot change how an already-recorded run replays, which is why it is not pinned in `run_started` and needs no `LEGACY_CONFIG_SNAPSHOT_DEFAULTS` row (its default IS the pre-field behaviour) ([tasks](tasks.md)) |
| `landlock` | `LOOPLAB_LANDLOCK` | `off` | KERNEL read allow-list for the eval process and everything it spawns (`runtime/landlock.py`). `enforce` derives the allow-list from what the OPERATOR declared — the node workdir, the run dir, every `data:`/`references:` mount SOURCE, the interpreter, the model cache and the documented machine tiers (`runtime/read_allowlist.py`) — and applies it in a fresh single-threaded launcher between fork and exec, so it is inherited across `exec` by the kernel and covers a native reader, a child process and a `torchrun` rank alike. It does NOT supersede `read_fence`, which stays as the MESSAGE rung: the audit hook's refusal is a plain non-`OSError` naming the fix, and Landlock's is an `EACCES` → `PermissionError` → `OSError`, exactly the shape `except OSError: <fall back>` turns into a silent skip. It exists because the hook cannot see a read that never reaches Python — measured on the incident's own 92 MB checkpoint, `safetensors.torch.load_file` loads 55 tensors and raises ZERO `open` audit events, so the fence's coverage of that incident is a property of `transformers` reading `config.json` first, not of the fence. **Why `off` by default:** enforcement and cost are verified on this box (ABI 2; Python `open`, a child `cat` and raw `libc.fopen` all refused; +2.1 %/open, 0.15 ms setup) but only on `open`/`read` microbenchmarks — nobody has run an allow-list through a real GPU eval, and torch, CUDA, NCCL, `/dev/nvidia*`, `/dev/shm`, `/sys/class` and the geesefs read surfaces were never exercised. A ruleset missing one does not degrade, it refuses mid-training. **The evidence that would flip it:** one real GPU eval — a full train+score on the box the runs happen on — completed under `landlock="enforce"` with `looplab landlock-check <run_dir>` reporting `skipped: 0`. Validate with that command, which prints the task's DECLARED MOUNTS and the exact rule set, and forks a probe proving an inside read succeeds and an outside one is refused. **Check the mount list before you trust the `skipped: 0`** — until 2026-08-15 that command read the wrong snapshot key, so it derived its list with the operator's `data:`/`references:` mounts silently dropped and reported a pass about it; it now reads the spec from `TaskAdapter.repo_spec()`, the same place the engine does, and REFUSES (exit 2) rather than passing when the snapshot is missing or no longer rebuilds |
...
| `cadence_while_evaluating` | `LOOPLAB_CADENCE_WHILE_EVALUATING` | `true` | Whether the node-count cadences may fire at a creation decision point that still has evaluations IN FLIGHT (backlog F1i). `false` restores the historical `if state.pending_nodes(): return False` guard that the Strategist consult (`strategy_decision`), the breadth snapshot (`coverage_snapshot`), the concept classifier pass (`node_concepts` / `concept_consolidation` / `concept_edge` / `hypothesis_concepts` / `concept_coverage_snapshot`), comparative-lesson distillation (`lessons_distilled`) and the run-report refresh (`report_generated`, `trigger=cadence`) all shared. The serial deep research is the ONE member of that family deliberately left on the old predicate — its concurrent half never carried the guard, so its phase is alive in every run, and opening the serial half mid-eval would only race it (backlog F1i-b). Since the eval task group became run-scoped (F1f, 2026-08-13) a node stays `pending` across outer-loop turns, so that guard is false for the whole life of any evaluation: measured over `runs/`, `rubertlite-dr-unified-v7`, `-v9` and `e5small-dr-unified-v2` have **zero** prefixes with nodes and none pending and recorded **zero** events of that whole family between them (measured 2026-08-19: `lessons_distilled` and `report_generated (trigger=cadence)` are 0 in all three, against 19/26 on `rubertlite-dense-retrieval` and 1/1 on `-v6` and `-v8`, on identical settings), while `-v8`'s single quiescent window is the last 8.1 minutes of a 47.6-hour run and holds every cadence firing it ever made. It buys **no extra paid passes**: the pace is unchanged (`cadence_due` plus each consumer's `at_node` idempotence still admit one firing per node count) and the two consumers that record no mark on their "nothing changed" path carry an in-process attempted-at-`n` memo. It reaches **no record**: a classifier row produced beside a live evaluation is stamped `at_pending` and refused by `classifier_verified_node_concepts`, so it never enters the `graded_novelty` admission channel, and an in-flight pass records no `concept_consolidation` rename (the one output that is retroactive and run-wide). Turn it off if you want no paid cadence work at all while a GPU is busy. **A run resumed from a snapshot written before this field keeps the OLD behaviour** — it has a `LEGACY_CONFIG_SNAPSHOT_DEFAULTS` row pinning it `false`, because re-entry must not silently add paid calls to a run whose first half bought none; set it explicitly to opt a resumed run in. Every new run gets it without asking |
...
| Setting | Env | Default | Description |
|---|---|---|---|
| `trust_mode` | `LOOPLAB_TRUST_MODE` | `trusted_local` | Sandbox tier: `trusted_local` (subprocess) · `untrusted` (Docker `--network none`) · `hostile` (Docker `--network none` **+ gVisor** `--runtime runsc`). Selects the tier for the `solution.py` sandbox, the command-eval wrap **and** the operator assistant's shell. |
| `docker_image` | `LOOPLAB_DOCKER_IMAGE` | `python:3.12-slim` | Image for every containerized surface of the untrusted/hostile tier: the `solution.py` sandbox, the command-eval wrap, and the operator assistant's shell (`run_command`/`run_tests`/`git`). |
| `sandbox_memory` | `LOOPLAB_SANDBOX_MEMORY` | `4g` | Memory cap for the untrusted/hostile Docker tier (`docker run --memory`). Raise for model-training evals; `""` = unbounded. Ignored by `trusted_local`. Applies to **all three** containerized surfaces including the operator assistant's shell — before 2026-08-15 that surface got no `--memory` at all. |
| `sandbox_cpus` | `LOOPLAB_SANDBOX_CPUS` | _(unset)_ | CPU cap for the untrusted/hostile Docker tier (`docker run --cpus`, e.g. `2`). `""` = unbounded. Ignored by `trusted_local`. Applies to all three containerized surfaces, the assistant's shell included. |
| `sandbox_readonly_rootfs` | `LOOPLAB_SANDBOX_READONLY_ROOTFS` | _(unset)_ | Container ROOT-FILESYSTEM hardening for the untrusted/hostile Docker tier. The value is the size of the writable tmpfs scratch the container gets in exchange, so `1g` means `docker run --read-only --tmpfs /tmp:rw,exec,nosuid,nodev,size=1g --tmpfs /var/tmp:…`. `""` = off = today's behaviour: the image's own filesystem (site-packages, `/usr/bin`, `/etc`, the interpreter) is writable by root inside the container. The `/work` bind — the node's workdir, where checkpoints, stage logs and the metric file the engine reads back are written — stays **read-write** either way; per-source write protection is the separate `edit: false` mount rule. OFF by default because it can break a legitimate eval: an `eval.setup` running `apt-get`, anything writing under `$HOME` (HF hub lock files, `datasets` caches, a pip cache), and GPU runs, where the NVIDIA container runtime writes into the container rootfs during `--gpus` setup — that last one is **unverified**, no box in this repo has a Docker daemon. An unparseable size is refused at launch rather than silently ignored. Ignored by `trusted_local`. Applies to all three containerized surfaces, the operator assistant's shell included — a read-only rootfs there also stops `pip install` from the chat. |
...
| `redact_output` | `LOOPLAB_REDACT_OUTPUT` | `true` | Adds the HIGH-ENTROPY pass to the bounded event/span/UI stdout/stderr tail redactor. It does **not** switch redaction on: known credential shapes (`sk-…`, `AKIA…`, `ghp_…`, `Authorization:`, `key=value`, URL userinfo) and the values of the operator's own secret-named env vars are masked on every persisted tail regardless of this flag. Turning it on additionally masks a 24+ character high-entropy run **that mixes upper case, lower case and digits** — the composition of a base64 credential, and deliberately not that of a path, a slug or a CamelCase identifier; `/` is treated as a separator for the same reason. **It ships ON since 2026-08-15, and that screen is the whole reason:** measured over every persisted tail in `runs/`, the entropy pass changed 13 of 744 before the screen and all 13 were traceback file paths, and 0 of 744 after — re-measured 2026-08-15 over a corpus twice that size, 0 of 1,652 tails, while the pre-fix rule still changes 18 of the same 1,652. The default had been the odd one out on two counts: the ~30 always-on `redact_persisted_text` boundaries have run this identical pass unconditionally since B3, and the module's own docstring asserted the pass was already on. **What you give up by setting it `false`:** a high-entropy base64 credential an eval prints reaches `node_evaluated.stdout_tail` — and so `events.jsonl`, the trace, the UI and every export — unmasked, unless a known shape or one of your own env values catches it; you get nothing back, because the composition screen that protects tracebacks is unconditional. The residual cost runs the other way: a base64 credential containing a `/` can now escape the pass (75–98 % masked by length). Deliberately **not** in `LEGACY_CONFIG_SNAPSHOT_DEFAULTS` — it adds no paid call and no intervention, it is write-side only (the fold never reads it, so a recorded run replays identically either way), and all 46 preserved snapshots carry the key explicitly, so a `setdefault` row could never fire; the map's own closing section states all three. Raw node-workdir `setup.log`, stage logs, `eval.log`, code and artifacts are outside this redaction boundary and may contain secrets; protect and retain the run root accordingly |
| `reward_hack_detect` | `LOOPLAB_REWARD_HACK_DETECT` | `true` | Flag suspicious wins (grader access, frozen-file writes). **ON by default since 2026-08-23**; whether a flag CHANGES selection is `trust_gate`'s decision, and that still defaults to `audit`, so this buys visibility and nothing else |
| `code_leakage_detect` | `LOOPLAB_CODE_LEAKAGE_DETECT` | `true` | Static code-leakage scan (fit-before-split, fit-on-test). **ON by default since 2026-08-23** — static, execution-free, advisory under the default `trust_gate` |
| `critic_check` | `LOOPLAB_CRITIC_CHECK` | `true` | Execution-free critic of each solution. **ON by default since 2026-08-23**, on the strength of `critic:params_ignored`: with `params_style: none` the Developer realises a proposal by EDITING the repo, 41 of 457 corpus comparisons diverged, and the e5 champion is recorded as batch 8192/accum 2/15 epochs while it ran 512/32/3. Broad critic warnings are advisory; the narrowly detected literal-with-no-computed-assignment `critic:hardcoded_metric` signal is classified as high precision and can gate under `trust_gate=gate|block` |
...
| `literature_search` | `LOOPLAB_LITERATURE_SEARCH` | `true` | arXiv search tool for the Researcher (network-optional). **ON by default since 2026-08-23.** It was off because "egress is unreliable on some boxes"; it still fails gracefully where the network is blocked, which is what makes ON the right default rather than a per-box question |
| `web_search` | `LOOPLAB_WEB_SEARCH` | `true` | Web search/fetch for the DeepResearcher (network-optional). **ON by default since 2026-08-23**, same reasoning as `literature_search`. The SSRF guard, the download bound and the redirect handler are unchanged — this flips whether the stage may LOOK, not what it may reach |
| `memo_verdict_cue` | `LOOPLAB_MEMO_VERDICT_CUE` | `true` | Qualify the one-line deep-research takeaway `roles.py::_state_brief` pushes into every Researcher / crash-triage / repair-critic prompt with the memo's own verifier tally, plus the fact that the verifier only ever checks a memo's CLAIMS and never its summary. Annotates only — the summary is still delivered in full, and nothing here reaches a metric, a champion, selectability or a violation. `false` reproduces the historical prompt byte for byte |
...
| `deep_research_every` | `LOOPLAB_DEEP_RESEARCH_EVERY` | `0` (START IMMEDIATELY) | Run the Deep-Research stage every N created nodes. **This is the one interval setting whose `0` is not the off switch** — it means "no window at all": the stage is due at the FIRST node and at every node-count after it that has not already been researched, so with `concurrent_research` on the first think overlaps the first evaluation instead of waiting. Separately from the window, **any enabled value also buys one run-OPENING think at node-count 0**, before the first idea is proposed — the interval decides how often the run re-thinks, never whether its first proposal is grounded (that pass leaves an `at_node=0` mark, which the window already reads as "never fired", so it does not re-phase the interval). **Off is `-1`** (any negative), where manual `deep_research` and Strategist-requested research still run — and where the run-opening think is skipped too — exactly what `0` used to mean before 2026-08-07. `N > 0` is a window N nodes wide, as before. Why the default moved: the cadence counts NODES while the feature is phrased around TIME, and on the three flagship GPU runs (1.5–4 h per node, `deep_research_every=3`, `concurrent_research=true`) it fired **zero** times — the first think was 5–12 hours away, so the one feature built to use the idle reasoning agents during a long training never ran on the workload it exists for. A cadence with no Deep-Research model wired (`backend=toy`) is a no-op rather than a recorded stub memo; a manual or Strategist request still records one, because its gate needs the answer |
