...
| `train_monitor_kill` | `LOOPLAB_TRAIN_MONITOR_KILL` | `true` | INTERVENTION: let the monitor tree-kill a training it judges `broken` (diverged / silent CPU fallback / not learning) early. **What a stop COSTS depends on whose fault the judge says it is** (since 2026-08-18): a verdict blaming the IDEA terminalizes (`reason=monitor_broken`, no repair and no retry), while one blaming the IMPLEMENTATION — `fault="implementation"`, the judge naming the specific thing to change — stops the attempt FOR REPAIR instead, as `reason=not_learning`, which is in the default `inline_repair_reasons`, so the node goes back to its own Developer with the diagnosis attached and the experiment is retried. That is the whole reason the role gate below can open at all: being wrong about a repair-stop costs one restart of a run the judge has just called wasted, where being wrong about a kill costs a multi-hour training with no refunded slot — so a repair-stop admits every JUDGED role and only the terminal kill is gated as narrowly as the rest of this row describes. **Which log may KILL:** only one the eval plan can PROVE is the run's own training — the classic single-command eval (`eval.log`, one command that trains *and* scores), a one-stage pipeline (that same shape wearing a stage name), or the ONE stage a manifest DECLARES as the training loop (`role: "training"`, at most one per pipeline, never the positional scorer — see [tasks](tasks.md)). A DECLARATION is admissible where a stage NAME is not, because it spends in exactly one direction: omit it and the stage keeps today's advisory role, write it and the only thing bought is the power to have your own stage stopped. That authority is then SPENT the moment the declared stage's own `expect.files` artifact exists — `e5small-dr-unified-v2`'s `train` stage also runs its retrieval evaluation in-process, so `RECALL@100: 0.793344` is a line in `train.log`, and a stage that does not distinguish its own phases cannot be taken at its word about them; after the artifact appears the verdict is advisory again. In a MULTI-stage pipeline that declares NOTHING the last stage is the scorer (that is where `command_eval` reads the metric) and every earlier stage is a work stage the engine cannot prove is the training step, so its verdicts stay **advisory** — a `data_prep` stage printing framework warnings and a flat loss draws a confident `broken` even when the prompt names the stage. That was measured to make the kill path UNREACHABLE for every pipeline this engine actually runs: v2 node 2 drew 31 `broken` verdicts at 0.85-0.95 over 7.3 h, ran all 57,600 steps and scored RECALL@100 = 0.000000, and every span of a pipeline that declares no training stage now carries `kill_reachable: false` from its first tick so that is readable before the hours are spent. `setup.log` (dep install), the scorer, and any filename with two possible writers (a stage named `setup`) are never judged at all; a log the monitor cannot attribute is advisory. **On top of that**, a kill must be CONFIRMED by a second consecutive `broken` verdict about the same log — a prompt re-look, not another full cadence — where a tick that produced no parseable verdict (endpoint failure, an answer outside the schema) re-arms the gate from zero, and the armed re-look is bounded to one identical re-ask and a five-minute window. **And on top of THAT**, the engine's own measured loss trajectory must not contradict the verdict: the judge sees the digested tail, which on a tqdm-style log is about ten loss values over ~30 seconds of a multi-hour run, so the engine accumulates one reduction of that tail per check and decides "is it still descending" deterministically over the whole eval. A `broken` verdict about a run whose measured loss has fallen by more than its own step-to-step noise floor since the first check neither kills nor arms — it stays an advisory row, stamped with the measurement (`trajectory`, `trajectory_veto`). A non-finite loss/`grad_norm` or an explosion withdraws that protection, and a log with no parseable loss at all is measured as `unknown` and keeps exactly the authority it had. Off = observe only |
...
| `inline_repair_reasons` | `LOOPLAB_INLINE_REPAIR_REASONS` | `["crash","timeout","oom","setup","no_metric","drift","expect_failed","check_failed","diverged","stalled","needs_failed","not_learning"]` | Which failure reasons are eligible for inline repair. `diverged` and `stalled` are the two WATCHDOG verdicts and are separate members on purpose: both are tree-kills the engine itself issued, so both exit `-9` with no traceback — the exact signature the KERNEL half of the `oom` heuristic recognises — and "reduce memory" is the wrong instruction for both (v6 node 5 spent three repair rounds halving a batch size while its loss was going NaN). `oom` itself covers TWO signatures that look nothing alike: that kernel kill, and the GPU allocator RAISING (`torch.OutOfMemoryError`), which exits 1 WITH a full traceback and so matches none of the kernel conjuncts. It was the kernel one only until 2026-08-20, which meant every GPU exhaustion classified as `crash` and never reached the memory-reduction directive — measured on `e5small-dr-unified-v3`, where all three nodes OOMed, all three recorded `reason: crash`, and the run stopped systemic having produced no metric. `not_learning` is the LIVE judge's member and joined on 2026-08-19: a FINITE loss that stopped descending needs "make the objective able to descend", which is neither `oom`'s "cut memory" nor `diverged`'s "stabilise the numerics" — and the deterministic diverge watchdog cannot see it at all, because 8.8534 forever is a perfectly finite number. Being in this list is exactly what buys that node a Developer look at its own code instead of a terminal verdict against a hypothesis nothing ever tested. **All of them by default since 2026-08-12** — none of these reasons is evidence the HYPOTHESIS is wrong, which is the only thing that should end a node without a repair. The spend is bounded by `inline_repair_attempts`, `inline_repair_retrain_cap` and the per-attempt triage judge, not by this list. Every member is validated against the registry at construction: a mistyped reason used to be accepted and then matched nothing, i.e. it silently switched inline repair OFF for that class |
