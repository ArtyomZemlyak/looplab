...
| `select_verifier` | `LOOPLAB_SELECT_VERIFIER` | `false` | R1-c / Part IV. Break an EXACT metric tie in best-selection with a calibrated §12-verifier soundness score: when a node's robust metric ties an already-eligible node, the engine verifies the tied nodes (grounded on their realized result, `selection_criteria`) and records a per-node score (`node_verified`, generation-scoped). The tie-break applies across the whole selection path — the mean pick, and (when `holdout_select` is on) both the holdout-slot ranking and the final holdout override — always preferring the more trustworthy node. Strictly a TIE-BREAK — never moves a node ahead of a strictly-better metric (§21.7). Best-effort: verification is lazy (only fires on a real MEAN-metric tie; the holdout keys reuse those scores rather than triggering their own) and bounded per cadence, so an unscored node in a large/late tie group falls back to a neutral midpoint. Opt-in, needs an LLM client; enable only after validating calibration offline with `verifier.calibrate`. Recorded in `run_started` so replay applies the same rule |
| `verifier_ci_tie` | `LOOPLAB_VERIFIER_CI_TIE` | `false` | R1-d / Part IV (§21.19). Widen the `select_verifier` tie-break from an EXACT metric tie to a STATISTICAL one: two confirmed means are "tied" when the loser is within the leader's confirm-phase confidence interval (`|Δ| ≤ 1.96·SE`, `SE = confirmed_std / sqrt(confirmed_seeds)`), and the tie is broken by soundness. The band is anchored on the LEADER's precision only, so a noisy candidate's own variance cannot widen it; with no confirm-noise data it falls back to an exact-metric tie. NEVER widens past the measured noise, so a SIGNIFICANT difference is never a tie and the verifier can never override a genuinely-better metric (§21.7). Requires `select_verifier`; off ⇒ exact-tie only. Recorded in `run_started` so replay applies the same rule |
| `select_verifier_samples` | `LOOPLAB_SELECT_VERIFIER_SAMPLES` | `3` | Verifier sample count for `select_verifier` (the §12 repeated-sampling expectation). `3` tames single-shot variance; `1` = cheaper/noisier. Validation currently enforces only the lower bound; add a safe maximum before exposing this input in UI |
...
| `concept_pivot` | `LOOPLAB_CONCEPT_PIVOT` | `false` | PART IV Phase 2a live steering. Record a concept-graph coverage + uncovered-region snapshot (`concept_coverage_snapshot`) at the strategist cadence, and on an `explore` stance make the Researcher's novelty hint name the exact uncovered regions ("0 coverage in {negatives/external-mining, distillation} — go there") instead of the vague "broaden". The snapshot is built by the LLM agent when a reflect client is wired (universal — works on ANY task, no curated skeleton needed, with per-task LLM-derived importance), falling back to the deterministic heuristic over a curated skeleton otherwise; recorded once per cadence so replay stays deterministic. Audit + prompt-cue only, never affects selection |
| `graded_novelty` | `LOOPLAB_GRADED_NOVELTY` | `false` | PART IV Phase 2b (D3). Grade a fresh proposal over the concept graph in the LIVE novelty gate: a level-4 "same direction, DIFFERENT implementation" or level-5 "re-opens a wrongly-abandoned FAILED direction" proposal may pass the flat dedup gate and is recorded as `novelty_graded`. It uses the agentic tagger when cached agentic node tags + a reflect client are available; otherwise it falls back to the curated deterministic graph/heuristic tagger and no-ops when no vocabulary exists. It changes proposal admission (never best-metric ranking), so validate its cost/quality behavior before enabling |
| `fingerprint_universal` | `LOOPLAB_FINGERPRINT_UNIVERSAL` | `false` | PART IV cross-run Step 0 (§21.20). Universal task-fingerprint tokenization: drop the ASCII-only `[a-z0-9]` allowlist on goal keywords (`[^\W_]+`/`.casefold()`, any script) so a non-Latin goal (Russian, CJK, …) is not silently dropped from its cross-run fingerprint and can reach SIMILAR-task priors/lessons/cases. OPT-IN (default off) because it changes which stored fingerprints a LIVE run matches — a running portfolio must not silently re-key mid-flight. Legacy default is byte-identical |
| `cross_run_concepts` | `LOOPLAB_CROSS_RUN_CONCEPTS` | `false` | PART IV cross-run Step 2 (§21.20). At run end write a per-run concept capsule **and upsert extracted D8 research-claim rows** to shared `memory_dir`; during `_graded_novelty_precheck`, separately surface overlapping earlier concepts as a `cross_run_prior` audit event. The prior is not fed into the gating grade and never rejects. D8 persistence currently loses the memo verifier verdict/scope projection, so this flag widens shared untrusted data as well as concept recall. Effective prior surfacing requires a shared `memory_dir`, `graded_novelty` and concept tags produced through `concept_pivot`; use `fingerprint_universal` consistently for non-Latin portfolios. The current similarity/scope/evidence contract is lean and experimental; keep opt-in |
| `cross_run_read_tools` | `LOOPLAB_CROSS_RUN_READ_TOOLS` | `false` | PART V §22. Adds read-only `cross_run_prior_attempts` / `cross_run_claims` / `cross_run_atlas` / `cross_run_search` to in-house Researcher, Strategist, deep-research, Genesis and the in-house `LLMRepoDeveloper` lesson-role variant; external coding-agent Developer backends do not receive this provider. Agents have no cross-run mutation function, and rejected claims are filtered from active projections. Consumers are still **not fully scope-safe**: one shared goal word passes the bound heuristic, Repo Developer is not state-bound, persisted D8 claims bypass the scoped lesson input, search reloads portfolio-wide capsules, and automatic paths can be portfolio-wide. Stored text is untrusted prompt data and no persisted derivation receipt is recorded. Keep off until §22.7 scope/redaction/receipt gates pass |
| `cross_run_advisory` | `LOOPLAB_CROSS_RUN_ADVISORY` | `false` | PART IV cross-run Step 5 (§21.20.5). Inject a **claim-count-bounded** context pack plus lean portfolio line into Researcher/Strategist prompts. Rejected claims are now excluded, but this remains live prompt influence: it scans shared stores without the full scope/comparison contract, D8 verification/provenance is incomplete, untrusted text is not redacted and no derivation receipt is persisted. It never directly changes best-metric selection. Keep opt-in until false-transfer/injection/verification and frozen A/B gates pass. Empty when stores are empty |
| `cross_run_structured_claims` | `LOOPLAB_CROSS_RUN_STRUCTURED_CLAIMS` | `false` | PART IV cross-run §21.20.13 (full CR of the lean fuzzy claim merge). Switch the claim read-model to the SCOPE+POLARITY-safe **structured claim key** (`engine/claim_key.py`): claims from different tasks never merge, opposite polarity ("X helps" vs "X never helps") is surfaced as a CONTRADICTION rather than collapsed, paraphrase/inflection variants group by exact structured key (no transitive over-merge), and operator governance is scope-precise (a decision in task A cannot reach a same-worded claim in task B). Affects the `cross_run_advisory` context pack; OPT-IN (default off). Lean stemming/negation — a full subject/comparator parse is a further TODO — so treat cross-task recall as scope-safe, not semantically complete |
| `cross_run_curation` | `LOOPLAB_CROSS_RUN_CURATION` | `false` | PART IV cross-run §22.4 (AGENTIC taxonomy steward). At finalize, when an LLM client is available, the concept steward (`engine/concept_steward.py`) reviews the freshly-updated portfolio concept graph and PROPOSES a curation — merge duplicate slugs / split conflated ones / purge noise — LOGGED to `concept_curation_log.jsonl` for operator ratification. The LLM only PROPOSES; every write goes through the same reversible append-only governance path (`record_concept_alias`/`record_concept_split`), so `fold()` stays pure and replay-safe. Portfolio-scoped, decoupled from the run's terminal state (never blocks/retries finalize). OPT-IN (default off); needs `memory_dir` + an LLM backend |
| `cross_run_curation_auto` | `LOOPLAB_CROSS_RUN_CURATION_AUTO` | `false` | Companion to `cross_run_curation`: also APPLY the steward's proposals automatically (through the same reversible append-only governance writes), instead of only logging them for ratification — the "fully agentic" mode, with append-only reversibility as the safety net. OPT-IN (default off); inert unless `cross_run_curation` is also on |
