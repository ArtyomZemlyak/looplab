...
```
width = min( open selection-ready cards,           # what the research asked to run at once
             gpu_pool // widest declared gpus,     # what the box can serve at that footprint
...
devices for its whole lifecycle. Five cards each declaring `{"gpus": 1}` on that box settle it to
**2**, and the other three **queue**.
