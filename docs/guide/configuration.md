...
<!-- REVIEW(2026-07-16): this row contradicts itself — the default column says `true`
(flipped in ce4a379) but the description still ends "Keep off until §22.8 scope/redaction/
retrieval gates pass". 93d2ec5 reconciled the NEIGHBORING rows' "OPT-IN (default off)" endings
and missed this one. An operator cannot tell whether the §22.8 gates actually passed (making the
sentence stale) or the flip was premature (making the default wrong) — state which, in the row.
Same sweep: docs/guide/memory.md ~line 38 and docs/guide/concepts.md ~line 550 still call the
whole Part-IV slice "opt-in experimental" while it now ships ON by default. -->
