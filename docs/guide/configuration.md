...
server-owned curated catalogue with **173 of the 204 direct `Settings` fields in 10 groups**. The default
...
| `eval_deadline_grace_s` | `LOOPLAB_EVAL_DEADLINE_GRACE_S` | `0.0` | DEADLINE GRACE (seconds): the most extra wall clock a live-log judge may buy for a stage that has reached its time budget — asked ONCE per command, never on an operator cancel, and clamped to this value inside the runtime (`sandbox.py::_granted_grace`). `0` (the default) keeps the unconditional tree-kill. It exists because a deadline cannot see a progress bar: all four `timeout` stages in the shipped corpus land exactly on their 4h/6h/8h wall and discarded 22.0 GPU-hours, and the entire captured record of `rubertlite-dense-retrieval` node 72 ends `100%|##########| 664/664`. OPT-IN because the judge reads the candidate's OWN live log, so a solution that prints a convincing progress bar can buy exactly this many seconds — once, and nothing else: the metric still comes from the operator's reader over the protected `score` stage. Seconds actually granted are stamped on the stage row as `deadline_grace_s` |
