...
| `cross_run_curation` | `LOOPLAB_CROSS_RUN_CURATION` | `false` | PART IV cross-run §22.4. At finalize, when an LLM client is available, the concept, claim and task-facet stewards review the portfolio and **propose only**: outcomes are durably queued in curation logs for operator review. Finalize never applies an agent proposal and never changes taxonomy, claim maturity or retrieval scope. Local steward CLI `--apply` remains an explicit operator action over locked append-only overlays. The typed owner HTTP actions are stronger: every mutation requires `expected_revision` and `action_id`, derives actor/time on the server, returns stable 409 conflicts, requires a currently projected claim plus its observed evidence digest, supports claim-decision `clear`, keeps merge separate from confirmed purge, and has explicit alias/split clear actions. Steward HTTP endpoints reject `apply` and only persist proposals. Validated concept targets/backfill, versioned taxonomy/assignment and evidence-family releases, impact preview, ACL/workbench and queryable history are still absent. Portfolio-scoped, best-effort and decoupled from terminal run state. OPT-IN (default off); needs `memory_dir` + an LLM backend |
| `cross_run_curation_auto` | `LOOPLAB_CROSS_RUN_CURATION_AUTO` | `false` | **Deprecated compatibility input; it does not auto-apply.** Retained so old environment/config snapshots still load. When `cross_run_curation` is enabled, finalize records the request as `auto_requested` in the proposal audit row but remains fail-closed and performs no governance write. An operator must review and explicitly apply through CLI/API. Default off; otherwise inert |
