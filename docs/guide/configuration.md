...
| `parallel_build` | `LOOPLAB_PARALLEL_BUILD` | `1` | Concurrent node BUILDS (research + code) for independent seed/explore drafts, each on its own pooled (researcher, developer) pair. `1` = serial + fully deterministic (today). `>1` fans draft builds out across worker threads: the event-log write order (seq/interleave of `node_building`/`node_created`) becomes nondeterministic across re-runs, though the fold stays deterministic and order-tolerant so replay is unaffected. Needs a `role_factory` wired into the engine; without it, clamps to `1` |
