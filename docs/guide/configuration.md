...
| `parallel_build` | `LOOPLAB_PARALLEL_BUILD` | `1` | Concurrent node BUILDS (research + code) for independent seed/explore drafts, each on its own pooled (researcher, developer) pair. `1` = serial (today). Needs a `role_factory` wired into the engine; without it, clamps to `1` |
