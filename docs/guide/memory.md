...
| **Governance policy** (`claim_decisions.jsonl`, `concept_aliases.jsonl`, `concept_splits.jsonl`) | *Your* corrections to the portfolio — ratify/reject/pin a claim; merge, purge or split a concept slug. Applied at READ time, so nothing is destroyed. | one action row each, with `action_id` + revision CAS | cross-run | **you only** — `looplab claim-decide` / `concept-merge` / `concept-split`, or the `/api/cross-run/*` routes | overlays every claim and concept projection an agent sees |
