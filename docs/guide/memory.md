...
|---|---|---|---|---|
| **Authoring** | Lab → Authoring | `prompts`, `skills`, `knowledge` — three directories of Markdown you write — plus `memory_skills`, a read-only fourth tab over `<memory_dir>/skills/` | **You** (and, for `knowledge`, the assistant's `remember` tool; `memory_skills` is written by the runs) | yes, except `memory_skills` |
...
|---|---|---|---|---|---|
| **Cases** (`cases.jsonl`) | *The winning run's exact configuration* — the parameter dict that produced the best metric on this task, kept machine-readable so the next run can start from it instead of re-deriving it from prose. One elected `active` row per `(task_id, direction, comparability)` — a case only ever competes with cases measured against the same evaluation (`engine/comparability.py`; a row that recorded no key groups with the other unkeyed rows): a leaderboard, not a history (a legacy run with no `run_uid` keeps its own one-row slot, which must beat the standing champion and never erases the per-run rows beside it; an over-long `goal`/`rationale` is stored truncated with its receipt, never dropped). | `{task_id, goal, direction, params, metric, rationale}` | cross-run | run-end | **exact-task warm start** (one line in the Researcher prior, beside the meta-note) + `kb_search` (see [What are cases for?](#what-are-cases-for)) |
...
| **Steward proposals** (`*_curation_log.jsonl`) | *What a paid steward suggested* — a review queue plus the idempotency ledger that stops the same paid call being charged twice. | `{v:2, curation_key, input_digest, model, outcome, proposals, …}` | cross-run | run-end concept/claim stewards (`cross_run_curation`), optional run-end facet steward (`task_facets_finalize`), or the CLI/HTTP steward commands | **no prompt reads these.** Three decisions do: the at-most-once gate (a logged `curation_key` is never paid for again); the ratification stage, which with `concept_tidy` on APPLIES the concept steward's proposed merges itself (`by=concept-ratifier/v1`; splits and purges stay your work and are counted as pending); and a run deletion's cascade, which keeps a claim whose pool another run's claim curation was computed over and a capsule carrying a concept a steward proposed to merge. Beyond those, a proposal changes nothing until you issue the governance write above |
...
bounded-window scan (`core/memory_window.py`) every other cross-run reader uses. There is one case
store: the vector/Memora-capable `CaseLibrary` that used to sit beside it in `engine/memory.py` was
never constructed by anything and was deleted (review 2026-09-22); `tests/test_case_store_wiring.py`
holds that line.
...
3. **A claim only becomes a skill if it could transfer.** The promotion gate used to ask three
   ...
   **There is a rung BEFORE both of those, and since 2026-08-19 it leaves a receipt too.** A card is
   ...
   `core/fitness.py::counts_toward_best` — so an aborted or trust-flagged node sets none; and since
   2026-09-26 a node whose Developer reported building something else — `Card.substituted_nodes` —
   counts in no verdict of its card, and a skill never quotes its code as the card's technique).
   So a
...
the paid-call idempotency gate, the `concept_tidy` ratification stage (which applies proposed concept
merges) and the run-deletion cascade's keep rules (see the table above).
...
| **Consolidation / hygiene** (D2) | Merges duplicate lessons into an `evidence_count`, preserves bounded cross-run `evidence_refs` plus traceable/untraceable source counts, retires contradicted verdicts, and bounds the store size. Dedup identity is `(statement, task, role)` — a Researcher and a Developer lesson with the same statement never collapse. On top of exact-normalized dedup, a **hybrid-retrieval (grep+BM25+vector, RRF) → agentic paraphrase-merge** pass (per `(task, role)`) folds re-worded duplicates. A cluster the model **declined** is remembered in `lesson_merge_verdicts.jsonl`, keyed on the exact question (every member as presented, plus the merge prompt), and not bought again until it changes — a later lesson that joins the cluster, a reworded member or a new `merge_system.md` makes it a new question. Before this, every finalize re-sent every declined cluster in the store. | lessons |
...
| `lesson_merge_verdicts.jsonl` | never | a content-addressed cache of declined lesson merges: a row is a digest, names no run, and removing one costs one re-asked question |
