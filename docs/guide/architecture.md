...
| Canonical eval/LLM concurrency + named-lane broker | `engine/orchestrator.py`, `engine/width_settling.py`, `core/llm_broker.py`, `engine/strategy.py` |
...
| Cross-run STORE REGISTRY: one row per store under `memory_dir` — its row key, its writer, and whether a run's deletion may take it (cascaded) or never touches it (preserved, with the reason). The deletion tiers, the strict governance-ledger table and the governed-source trio are views of it, and `tests/test_memory_stores.py` holds it to the tree both ways | `engine/memory_stores.py` |
| Cross-run memory CASCADE: what a run's deletion may remove from the shared stores the registry marks cascaded — one attribution predicate per store, and an irreversible purge keyed on `run_uid` (never the reused directory NAME) | `serve/memory_cascade.py` |
