...
nothing owed is left (`drain-only resume: every reset or interrupted evaluation reached its terminal`
— evaluated or failed, see each node); the run's eval budget is spent or this invocation's time
budget ran out — which would otherwise FINALIZE the run — naming the nodes left unevaluated (for the
...
|---|---|
| **AUTO** (`0` for the two widths, `-1` for `speculation_depth` — each of them that axis's default) | **Adopts** the width the log pinned — including on a *smaller* box. One treatment spans the whole log; a width above what the hardware can serve is bounded by the resource scheduler, not by silently rewriting the treatment. For `speculation_depth` the value adopted is the **effective** one — the run-start pin capped by every `speculation_depth_settled` row — not `run_started`'s alone: AUTO may ratchet the depth down mid-run once the run has measured its own evaluations (see [Adaptive AUTO depth](configuration.md#adaptive-auto-depth)). The two are separate folded facts, which is what lets AUTO's own adaptation be adopted while an operator's disagreement is still refused; a *spelled* depth never adapts at all. |
| An **explicit** width equal to the pin | Proceeds unchanged. For `speculation_depth` that means equal to what `run_started` pinned — a run that has since ratcheted itself down is accepted and still executes at its settled depth, because the ratchet is durable and no resume flag lifts it. |
| An **explicit** width that disagrees with the pin | **Fails closed**, naming the axis and both values, and writes nothing to the log it declined to trust. For `speculation_depth` the message names the launch pin and, when they differ, the depth the run settled to as well. |
...
standing. That covers three cases:

- A later `resume` lifted it.
- A resume request is pending. A LoopLab server whose root is the run's parent directory serves it by
  starting an engine, provided it can read the run's task snapshot. It does so at its startup, when
  it lists the runs, or once the current engine exits; `looplab resume` serves it too. A pending
  *finalize* request is named as one: it wraps the run up rather than lifting the stop.
- A server command that starts an engine is still unsettled in the run's `.commands/`, with a worker
...
- **A stale command.** An unsettled command no worker has driven for 30 s (a server killed
  mid-command) starts nothing now, but nothing cancels it before its deadline. A LoopLab server that
  reads it again re-drives it, starts an engine and lifts the stop. That happens on a GET of the
...
The pulse is advisory: a server killed in the last 30 s still reads as coming.
...
PART IV D4 (§21.5) read-only analytic. Tags canonical Card work items (several may share one `belief_id`) and surfaces the dominant **within-concept**
