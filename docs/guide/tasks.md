...
| `eval.metric.path` | **`file_json` / `file_regex` only** — the metrics file the candidate/framework writes, relative to the eval workdir. Required by those two readers; ignored by the `stdout_*` readers. The same rule applies to **every** reader slot below (`eval.metrics`, `eval.constraints`, `eval.cross_check`): a `file_*` reader without a string `path` is refused at submit, because it can never read anything and each slot then fails a different silent way. An existing run whose snapshot predates that check still resumes — `looplab resume` prints the refusal as a warning instead. The same walk (since 2026-09-22) refuses, in every slot, a `stdout_regex` / `file_regex` reader with no usable pattern — none (`key` is still accepted in its place), one that does not compile, or one without the capture group it reads — and, in the three gate slots, a `kind` that names no reader or a key nothing reads: each reader's own keys, `direction` on an `eval.metrics` entry and `name`/`min`/`max` on a constraint are known, a key starting with `_` is a comment. A constraint bound typed `mx` used to validate and never be enforced. Same resume warnings. |
