# AutoRND — Decision Records (added requirements)
...
**Key finding.** The strong agent CLIs are *already* event-sourced, LiteLLM-backed, sandboxed loops — the same architecture as AutoRND — so this is **nesting a compatible sub-engine, not bolting on an alien.** They all support headless, unattended, structured-output invocation:
