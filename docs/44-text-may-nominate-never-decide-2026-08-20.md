...
## Where the rule was not applied
...
`OPEN[setup-reason-decided-from-forgeable-text]` — the engine holds the setup outcome out-of-band and
launders it through stderr; promote it to a `RunResult` flag and decide from that.
proof:present:startswith("setup@looplab/engine/triage.py
...
`OPEN[failure-reason-has-no-downstream-recheck]` — a classification that routes repair, gates salvage
and can end a node is accepted with nothing able to contradict it.
proof:missing:looplab/engine/failure_diagnosis.py
